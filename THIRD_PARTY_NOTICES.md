...
Optional desktop GUI/editor, asset-importer, native physics adapter, and network adapter dependencies are declared in `vcpkg.json`, installed through vcpkg manifest mode, and excluded from source control. The `desktop-runtime` feature uses first-party or host SDK paths and declares no package dependencies. Optional dependencies are used only by explicit optional build lanes such as `tools/bootstrap-deps.ps1`, `tools/build-asset-importers.ps1`, `tools/validate-physics-jolt.ps1`, and `tools/validate-network-enet.ps1`.
...
| Dear ImGui | https://github.com/ocornut/imgui | 2026-05-30 | 1.92.8 via vcpkg baseline `3909e67a639d426ea939d9bff77bfe1d10443476` | Omar Cornut and Dear ImGui contributors | MIT | No first-party modifications; vcpkg applies packaging patches | Optional `desktop-gui` `MK_editor` developer/editor shell build output |
