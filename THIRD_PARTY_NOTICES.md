...
Optional asset-importer, native physics adapter, and network adapter dependencies are declared in `vcpkg.json`, installed through vcpkg manifest mode, and excluded from source control. They are used only by explicit optional build lanes such as `tools/build-asset-importers.ps1`, `tools/validate-physics-jolt.ps1`, and `tools/validate-network-enet.ps1`.
