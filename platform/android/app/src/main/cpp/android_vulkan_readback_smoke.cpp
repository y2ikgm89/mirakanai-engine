...
AndroidVulkanReadbackSmokeResult run_android_vulkan_readback_smoke_impl() {
    ...
    void* mapped = nullptr;
    if (const auto result = vkMapMemory(context.device, context.memory, 0, k_readback_size, 0, &mapped);
    ...
    if (!host_coherent) {
        ...
        const auto atom_size = std::max<VkDeviceSize>(properties.limits.nonCoherentAtomSize, 1U);
        const auto aligned_size = std::min(requirements.size, ((k_readback_size + atom_size - 1U) / atom_size) * atom_size);
