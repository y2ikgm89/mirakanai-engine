...
- Gameplay systems package proof: deterministic `mirakana::PhysicsWorld2D` contact/trigger rows, `mirakana::plan_navigation_grid_agent_path`, `mirakana::update_navigation_agent`, AI behavior authoring validation, quest/dialogue state transitions plus save-state validation, inventory item pickup/state validation/recipe completion, perception blackboard projection, and behavior-tree evaluation are required with `--require-gameplay-systems` and status counters such as `gameplay_systems_physics_contacts`, `gameplay_systems_navigation_plan_status`, `gameplay_systems_behavior_authoring_trace_nodes`, `gameplay_systems_quest_dialogue_transition_rows`, `gameplay_systems_quest_dialogue_state_rows`, `gameplay_systems_inventory_items_transition_rows`, `gameplay_systems_inventory_items_final_workbench_quantity`, and `gameplay_systems_behavior_status`
