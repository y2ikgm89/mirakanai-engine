...
- Gameplay systems package proof: deterministic `mirakana::PhysicsWorld2D` contact/trigger rows, `mirakana::plan_navigation_grid_agent_path`, `mirakana::update_navigation_agent`, AI behavior authoring validation, perception blackboard projection, and behavior-tree evaluation are required with `--require-gameplay-systems` and status counters such as `gameplay_systems_physics_contacts`, `gameplay_systems_navigation_plan_status`, `gameplay_systems_behavior_authoring_trace_nodes`, and `gameplay_systems_behavior_status`
