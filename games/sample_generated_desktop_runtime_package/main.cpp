...
class sample_generated_desktop_runtime_package_Game final : public mirakana::GameApp {
  public:
    sample_generated_desktop_runtime_package_Game(mirakana::VirtualInput& input, mirakana::IRenderer& renderer,
                                                  bool throttle)
...
void print_usage() {
    std::cout << "sample_generated_desktop_runtime_package [--smoke] [--max-frames N] [--video-driver NAME] "
                 "[--require-config PATH]\n";
}
...
int main(int argc, char** argv) {
    ...
    if (options.smoke && (result.status != mirakana::DesktopRunStatus::completed ||
