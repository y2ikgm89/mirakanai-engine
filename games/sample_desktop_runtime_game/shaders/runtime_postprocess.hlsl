...
float4 ps_postprocess(VsOut input) : SV_Target {
    ...
    float scene_depth = scene_depth_texture.Sample(scene_depth_sampler, input.uv);
    float near_depth_grade = saturate(1.0 - scene_depth) * 0.08;
    float3 graded = saturate(scene.rgb * float3(1.04, 1.02, 0.98) +
                             float3(0.012 + near_depth_grade, 0.008 + near_depth_grade * 0.55,
                                    near_depth_grade * 0.25));
