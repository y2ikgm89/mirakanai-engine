...
MK_TEST("runtime resident package replacement commit preserves mount slot and refreshes catalog cache") {
    ...
    const auto result = mirakana::runtime::commit_runtime_resident_package_replace(
    ...
    MK_REQUIRE(result.status == mirakana::runtime::RuntimeResidentPackageReplaceCommitStatus::replaced);
...
MK_TEST("runtime resident package replacement commit rejects invalid and missing ids before mutation") {
    ...
    const auto invalid = mirakana::runtime::commit_runtime_resident_package_replace(
    ...
    MK_REQUIRE(invalid.status == mirakana::runtime::RuntimeResidentPackageReplaceCommitStatus::invalid_mount_id);
    MK_REQUIRE(!invalid.invoked_candidate_catalog_build);
    MK_REQUIRE(mount_set.generation() == previous_mount_generation);
    MK_REQUIRE(catalog_cache.catalog().generation() == previous_catalog_generation);

    const auto missing = mirakana::runtime::commit_runtime_resident_package_replace(
    ...
    MK_REQUIRE(missing.status == mirakana::runtime::RuntimeResidentPackageReplaceCommitStatus::missing_mount_id);
...
MK_TEST("runtime resident package replacement commit rejects duplicate candidate records before mutation") {
    ...
    MK_REQUIRE(!result.succeeded());
    MK_REQUIRE(result.status == mirakana::runtime::RuntimeResidentPackageReplaceCommitStatus::catalog_build_failed);
...
MK_TEST("runtime resident package replacement commit preserves state on projected budget failure") {
    ...
    const mirakana::runtime::RuntimeResourceResidencyBudget tight_budget{
    ...
    MK_REQUIRE(!result.succeeded());
    MK_REQUIRE(result.status == mirakana::runtime::RuntimeResidentPackageReplaceCommitStatus::budget_failed);
    MK_REQUIRE(result.invoked_candidate_catalog_build);
