...
[[nodiscard]] std::string expected_index_data_hex() {
    ...
    append_le_u32(bytes, 0U);
    append_le_u32(bytes, 1U);
    append_le_u32(bytes, 2U);
    return hex_encode(bytes);
}

[[nodiscard]] bool has_failure_diagnostic(const std::vector<mirakana::MavgClusterCookFailure>& failures,
                                          std::string_view needle) {
    for (const auto& failure : failures) {
        if (failure.diagnostic.find(needle) != std::string::npos) {
            return true;
        }
    }
    return false;
...
MK_TEST("mavg cluster cook planner produces deterministic graph payload and package rows") {
    ...
    MK_REQUIRE(result.graph.clusters[0].vertex_count == 6U);
    ...
    MK_REQUIRE(result.graph.clusters[1].vertex_count == 6U);
    ...
    MK_REQUIRE(result.graph.clusters[2].vertex_base == 6);
    MK_REQUIRE(result.graph.clusters[2].vertex_count == 3U);
    ...
    MK_REQUIRE(result.graph.clusters[3].vertex_base == 6);
    MK_REQUIRE(result.graph.clusters[3].vertex_count == 3U);
    ...
    MK_REQUIRE(result.payload_content.find(
                   "index.data_hex=000000000100000002000000030000000400000005000000000000000100000002000000\n") !=
               std::string::npos);
    ...
    std::ranges::reverse(reordered.material_partitions);
    const auto reordered_result = mirakana::plan_mavg_cluster_graph_cook_package(reordered);
    MK_REQUIRE(reordered_result.succeeded());
    MK_REQUIRE(reordered_result.graph_content == result.graph_content);
    MK_REQUIRE(reordered_result.payload_content == result.payload_content);
}
...
MK_TEST("mavg cluster cook planner rejects invalid static payload inputs") {
    ...
    MK_REQUIRE(has_failure_diagnostic(invalid_index_result.failures, "mavg triangle index is invalid"));
    ...
    auto gap = make_request();
    gap.material_partitions[0].triangle_count = 1U;
    const auto gap_result = mirakana::plan_mavg_cluster_graph_cook_package(gap);
    MK_REQUIRE(!gap_result.succeeded());
    MK_REQUIRE(gap_result.changed_files.empty());
    MK_REQUIRE(has_failure_diagnostic(gap_result.failures, "mavg material partitions must cover each triangle once"));

    auto overlap = make_request();
    overlap.material_partitions[1].first_triangle = 1U;
    overlap.material_partitions[1].triangle_count = 2U;
    const auto overlap_result = mirakana::plan_mavg_cluster_graph_cook_package(overlap);
    MK_REQUIRE(!overlap_result.succeeded());
    MK_REQUIRE(overlap_result.changed_files.empty());
    MK_REQUIRE(
        has_failure_diagnostic(overlap_result.failures, "mavg material partitions must cover each triangle once"));
}
