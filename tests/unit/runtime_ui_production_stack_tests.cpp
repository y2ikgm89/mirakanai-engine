...
[[nodiscard]] RuntimeUiProductionEvidenceRow make_ime_row() {
    return RuntimeUiProductionEvidenceRow{
        ...
        .ime_platform_adapter_proof_rows = true,
...
MK_TEST("runtime ui production stack rejects incomplete ime and accessibility evidence") {
    ...
    request.rows[4].ime_platform_adapter_proof_rows = false;
    ...
    MK_REQUIRE(
        has_diagnostic(plan.diagnostics, RuntimeUiProductionDiagnosticCode::missing_ime_platform_adapter_proof_rows));
    MK_REQUIRE(
...
MK_TEST("runtime ui production stack keeps selected platform ime proof host gated") {
    const auto plan = mirakana::ui::plan_runtime_ui_production_stack(make_package_request());
    ...
    MK_REQUIRE(ime_row->ime_platform_adapter_proof_rows);
