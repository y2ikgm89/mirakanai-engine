...
[[nodiscard]] mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsDesc
make_desc(mirakana::runtime::RuntimeResidentPackageMountId mount_id, std::string selected_package_index_path,
    ...
    return mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsDesc{
        .discovery =
...
MK_TEST("runtime package discovery resident mount with reviewed evictions commits selected candidate after eviction") {
    ...
    const auto result = mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2},
    ...
    MK_REQUIRE(result.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::committed);
    ...
    MK_REQUIRE(result.candidate_resident_mount.status ==
               mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsStatus::mounted);
...
MK_TEST("runtime package discovery resident mount with reviewed evictions rejects descriptors before scans") {
    ...
    mirakana::runtime::RuntimeResidentPackageMountSet mount_set;
    mount_package(mount_set, mirakana::runtime::RuntimeResidentPackageMountId{.value = 4}, "base", base, "base");
    auto catalog_cache = make_refreshed_cache(mount_set);
    const auto previous_mount_generation = mount_set.generation();
    ...
    const auto invalid_path =
        mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
            filesystem, mount_set, catalog_cache,
            make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 5}, "runtime/../escape.geindex"));
    const auto invalid_mount =
        mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
            filesystem, mount_set, catalog_cache,
            make_desc(mirakana::runtime::RuntimeResidentPackageMountId{},
                      "runtime/packages/characters/incoming.geindex"));
    const auto duplicate_mount =
        mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
            filesystem, mount_set, catalog_cache,
            make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 4},
                      "runtime/packages/characters/incoming.geindex"));

    MK_REQUIRE(invalid_path.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::invalid_descriptor);
    MK_REQUIRE(invalid_mount.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::invalid_mount_id);
    MK_REQUIRE(duplicate_mount.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::duplicate_mount_id);
...
MK_TEST(
    ...
    const auto result = mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 6}, "runtime/packages/missing.geindex"));

    MK_REQUIRE(result.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::candidate_not_found);
    MK_REQUIRE(result.discovery.status == mirakana::runtime::RuntimePackageIndexDiscoveryStatus::discovered);
...
MK_TEST("runtime package discovery resident mount with reviewed evictions preserves state on discovery failures") {
    ...
    const auto base = mirakana::AssetId::from_name("textures/base");
    mirakana::runtime::RuntimeResidentPackageMountSet mount_set;
    mount_package(mount_set, mirakana::runtime::RuntimeResidentPackageMountId{.value = 2}, "base", base, "base");
    auto catalog_cache = make_refreshed_cache(mount_set);
    const auto previous_mount_generation = mount_set.generation();
    const auto previous_catalog_generation = catalog_cache.catalog().generation();

    auto invalid_desc = make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 8},
                                  "runtime/packages/characters/incoming.geindex");
    invalid_desc.discovery.root = "";
    const auto invalid = mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache, invalid_desc);

    filesystem.fail_list_files(true);
    const auto scan = mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 9},
                  "runtime/packages/characters/incoming.geindex"));

    MK_REQUIRE(invalid.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::invalid_descriptor);
    MK_REQUIRE(invalid.discovery.status == mirakana::runtime::RuntimePackageIndexDiscoveryStatus::invalid_descriptor);
    MK_REQUIRE(invalid.diagnostics.size() == 1);
    MK_REQUIRE(invalid.diagnostics[0].code == "invalid-root");
    MK_REQUIRE(scan.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::discovery_failed);
    MK_REQUIRE(scan.discovery.status == mirakana::runtime::RuntimePackageIndexDiscoveryStatus::scan_failed);
...
MK_TEST("runtime package discovery resident mount with reviewed evictions preserves state on delegated load failure") {
    ...
    const auto result = mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2}, "runtime/packages/broken.geindex"));

    MK_REQUIRE(result.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::candidate_load_failed);
    MK_REQUIRE(result.candidate_resident_mount.status ==
               mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsStatus::candidate_load_failed);
    MK_REQUIRE(result.diagnostics.size() == 1);
...
MK_TEST(
    ...
    const auto duplicate = mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2},
                  "runtime/packages/characters/incoming.geindex", 3,
                  {mirakana::runtime::RuntimeResidentPackageMountId{.value = 1},
    ...
    MK_REQUIRE(invalid.status == mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::
                                     invalid_eviction_candidate_mount_id);
    MK_REQUIRE(invalid.candidate_resident_mount.status ==
               mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsStatus::
                   invalid_eviction_candidate_mount_id);
    MK_REQUIRE(invalid.diagnostics.size() == 1);
    MK_REQUIRE(invalid.diagnostics[0].phase ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsDiagnosticPhase::eviction_plan);
    MK_REQUIRE(invalid.diagnostics[0].code == "invalid-candidate-mount-id");
    MK_REQUIRE(duplicate.status == mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::
...
MK_TEST("runtime package discovery resident mount with reviewed evictions maps protected and missing candidates") {
    ...
    const auto protected_mount =
        mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
            ...
            make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2},
                      ...
                      {mirakana::runtime::RuntimeResidentPackageMountId{.value = 1}},
                      {mirakana::runtime::RuntimeResidentPackageMountId{.value = 1}}));
    const auto missing = mirakana::runtime::commit_runtime_package_discovery_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2},
                  "runtime/packages/characters/incoming.geindex", 3,
    ...
    MK_REQUIRE(protected_mount.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::
    ...
    MK_REQUIRE(missing.status == mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::
...
MK_TEST("runtime package discovery resident mount with reviewed evictions preserves state when candidates are "
    ...
    MK_REQUIRE(result.status ==
               mirakana::runtime::RuntimePackageDiscoveryResidentMountReviewedEvictionsStatus::budget_failed);
    MK_REQUIRE(result.candidate_resident_mount.status ==
               mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsStatus::budget_failed);
    MK_REQUIRE(result.eviction_plan.status ==
