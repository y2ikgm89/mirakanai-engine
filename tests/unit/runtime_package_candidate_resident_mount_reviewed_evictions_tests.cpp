...
[[nodiscard]] mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsDesc
make_desc(mirakana::runtime::RuntimeResidentPackageMountId mount_id, std::uint64_t max_bytes,
    ...
    return mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsDesc{
...
MK_TEST("runtime package candidate resident mount with reviewed evictions commits selected candidate after eviction") {
    ...
    const auto result = mirakana::runtime::commit_runtime_package_candidate_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2}, incoming_payload.size(),
...
MK_TEST("runtime package candidate resident mount with reviewed evictions skips eviction when projected view fits") {
    ...
    const auto result = mirakana::runtime::commit_runtime_package_candidate_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2}, 64));
...
MK_TEST("runtime package candidate resident mount with reviewed evictions rejects mount ids before reads") {
    ...
    const auto invalid = mirakana::runtime::commit_runtime_package_candidate_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{}, 64,
                  {mirakana::runtime::RuntimeResidentPackageMountId{.value = 3}}));
    const auto duplicate = mirakana::runtime::commit_runtime_package_candidate_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 3}, 64,
                  {mirakana::runtime::RuntimeResidentPackageMountId{.value = 3}}));

    MK_REQUIRE(invalid.status ==
               mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsStatus::invalid_mount_id);
    MK_REQUIRE(duplicate.status ==
               mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsStatus::duplicate_mount_id);
...
MK_TEST("runtime package candidate resident mount with reviewed evictions preserves state on candidate load failure") {
    ...
    const auto previous_mount_generation = mount_set.generation();
    const auto previous_catalog_generation = catalog_cache.catalog().generation();
    auto desc = make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2}, 64,
                          {mirakana::runtime::RuntimeResidentPackageMountId{.value = 1}});
...
MK_TEST("runtime package candidate resident mount with reviewed evictions rejects protected candidates") {
    ...
    const auto protected_new_mount =
        mirakana::runtime::commit_runtime_package_candidate_resident_mount_with_reviewed_evictions(
            filesystem, mount_set, catalog_cache,
            make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2}, 3,
    ...
    MK_REQUIRE(protected_existing_mount.diagnostics[0].phase ==
               mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsDiagnosticPhase::eviction_plan);
...
MK_TEST("runtime package candidate resident mount with reviewed evictions rejects invalid and duplicate eviction "
    ...
    MK_REQUIRE(invalid.status == mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsStatus::
    ...
    MK_REQUIRE(invalid.diagnostics[0].phase ==
               mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsDiagnosticPhase::eviction_plan);
    MK_REQUIRE(invalid.diagnostics[0].mount == mirakana::runtime::RuntimeResidentPackageMountId{});
    ...
    MK_REQUIRE(duplicate.status == mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsStatus::
    ...
    MK_REQUIRE(duplicate.diagnostics[0].phase ==
               mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsDiagnosticPhase::eviction_plan);
    MK_REQUIRE(duplicate.diagnostics[0].mount == mirakana::runtime::RuntimeResidentPackageMountId{.value = 1});
...
MK_TEST("runtime package candidate resident mount with reviewed evictions rejects missing candidates") {
    ...
    MK_REQUIRE(result.status == mirakana::runtime::RuntimePackageCandidateResidentMountReviewedEvictionsStatus::
...
MK_TEST("runtime package candidate resident mount with reviewed evictions preserves state when candidates are "
    ...
    const auto result = mirakana::runtime::commit_runtime_package_candidate_resident_mount_with_reviewed_evictions(
        filesystem, mount_set, catalog_cache,
        make_desc(mirakana::runtime::RuntimeResidentPackageMountId{.value = 2}, 4,
                  {mirakana::runtime::RuntimeResidentPackageMountId{.value = 1}}));
