...
[[nodiscard]] mirakana::MavgClusterGraphDocument make_scene_lod_graph() {
    ...
    return mirakana::MavgClusterGraphDocument{
        ...
        .cluster_payload_uri = "runtime/hero.mavgpayload",
...
[[nodiscard]] mirakana::SceneGpuBindingPalette make_gpu_bindings(const mirakana::MavgClusterGraphDocument& graph,
                                                                 mirakana::rhi::IRhiDevice& device) {
    mirakana::SceneGpuBindingPalette bindings;
    MK_REQUIRE(bindings.try_add_mesh(graph.asset, make_mesh_binding(device)));
...
MK_TEST("mavg scene lod planning creates range aware mesh commands") {
    ...
    auto gpu_bindings = make_gpu_bindings(graph, device);
    ...
    MK_REQUIRE(has_diagnostic(result.diagnostics, mirakana::MavgSceneLodDiagnosticCode::fallback_substitution));
    ...
    MK_REQUIRE(first.mesh_binding.vertex_buffer.value == 101);
    MK_REQUIRE(first.mesh_binding.index_buffer.value == 102);
    MK_REQUIRE(first.mesh_binding.owner_device == &device);
    MK_REQUIRE(first.material_binding.pipeline_layout.value == 201);
    MK_REQUIRE(first.material_binding.owner_device == &device);
    ...
    MK_REQUIRE(first.indexed_range.first_instance == 0);

    const auto& second = result.mesh_commands[1];
    MK_REQUIRE(second.mesh == graph.asset);
...
MK_TEST("mavg scene lod planning reports missing material binding and uses fallback color") {
    ...
    MK_REQUIRE(gpu_bindings.try_add_mesh(graph.asset, make_mesh_binding(device)));
    MK_REQUIRE(
        gpu_bindings.try_add_material(graph.material_partitions[0].material, make_material_binding(device, 401)));
    ...
    MK_REQUIRE(has_diagnostic(result.diagnostics, mirakana::MavgSceneLodDiagnosticCode::missing_material_binding));
    MK_REQUIRE(result.mesh_commands.size() == 2);
...
MK_TEST("mavg scene lod planning rejects missing mesh binding evidence") {
    const auto graph = make_scene_lod_graph();
    const auto selection = make_scene_lod_selection(graph.asset);
    mirakana::SceneGpuBindingPalette gpu_bindings;

    const auto result = mirakana::plan_mavg_scene_lod_mesh_commands(selection, graph,
                                                                    mirakana::MavgSceneLodSubmitDesc{
                                                                        .gpu_bindings = &gpu_bindings,
                                                                    });
    ...
    MK_REQUIRE(has_diagnostic(result.diagnostics, mirakana::MavgSceneLodDiagnosticCode::missing_mesh_binding));
}

MK_TEST("mavg scene lod planning rejects fatal selector diagnostics") {
    const auto graph = make_scene_lod_graph();
    auto selection = make_scene_lod_selection(graph.asset);
    selection.diagnostics.push_back(mirakana::MavgLodSelectionDiagnostic{
        .code = mirakana::MavgLodSelectionDiagnosticCode::invalid_view,
        .message = "invalid test view",
    });
    mirakana::rhi::NullRhiDevice device;
    auto gpu_bindings = make_gpu_bindings(graph, device);
    ...
    MK_REQUIRE(!result.succeeded());
    MK_REQUIRE(result.mesh_commands.empty());
    MK_REQUIRE(has_diagnostic(result.diagnostics, mirakana::MavgSceneLodDiagnosticCode::invalid_selection));
}
