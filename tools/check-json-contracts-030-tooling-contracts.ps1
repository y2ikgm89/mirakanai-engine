...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-directstorage-request-plan-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-native-directstorage-win32-async-io-dispatch-status-v1") { foreach ($needle in @("MAVG DirectStorage Request Plan v1", "MAVG Runtime LOD Milestone v1", "RuntimeMavgPayloadDirectStorageRequestPlanDesc", "RuntimeMavgPayloadDirectStorageRequestRow", "RuntimeMavgPayloadDirectStorageRequestPlanResult", "RuntimeMavgPayloadDirectStorageFenceWaitPoint", "plan_runtime_mavg_payload_directstorage_requests", "requires_native_directstorage_sdk=true", "used_native_directstorage=false", "no file IO", "native enqueue", "native submit", "native fence signal", "async-overlap/performance", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { if (-not $recommendedText.Contains($needle)) { Write-Error "engine manifest aiOperableProductionLoop recommendedNextPlan must describe MAVG DirectStorage request-plan selection: $needle" } }
