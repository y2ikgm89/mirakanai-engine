...
if ($validationRunnerCommand.Count -ne 1 -or $validationRunnerCommand[0].status -ne "ready") {
    ...
    foreach ($recipe in @(
            ...
            "desktop-runtime-sample-game-environment-profile-package", "desktop-runtime-sample-game-environment-ready-aggregate",
    ...
    if (@($validationRunnerCommand[0].validationRecipes).Count -ne 31) {
        Write-Error "engine manifest run-validation-recipe validationRecipes must be exactly the reviewed allowlist of 31 recipes"
    }
