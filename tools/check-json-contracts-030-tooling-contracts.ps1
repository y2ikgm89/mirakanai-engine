...
if ($validationRunnerCommand.Count -ne 1 -or $validationRunnerCommand[0].status -ne "ready") {
    ...
    foreach ($recipe in @(
            ...
            "desktop-runtime-sample-game-environment-profile-package", "desktop-runtime-sample-game-environment-ready-aggregate", "desktop-runtime-sample-game-environment-vulkan-strict-aggregate", "desktop-runtime-sample-game-environment-backend-parity", "desktop-runtime-sample-game-environment-platform-readiness", "desktop-runtime-sample-game-environment-weather-simulation-vulkan-solver-package", "environment-weather-metal-solver-host-gate",
    ...
    if (@($validationRunnerCommand[0].validationRecipes).Count -ne 37) {
        Write-Error "engine manifest run-validation-recipe validationRecipes must be exactly the reviewed allowlist of 37 recipes"
    }
