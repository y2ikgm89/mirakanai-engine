...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-payload-byte-range-file-io-v1") { foreach ($needle in @("MAVG Payload Byte-Range File IO v1", "MAVG Runtime LOD Milestone v1", "IFileSystem read_bytes", "write_bytes", "read_byte_range", "MemoryFileSystem and RootedFileSystem exact byte-range coverage", "RuntimeMavgPayloadPageFileLoadResult", "RuntimeMavgPayloadPageFileLoadDesc", "RuntimeMavgPayloadPageFileLoadRow", "load_runtime_mavg_payload_file_pages", "invoked_file_io=true", "used_native_directstorage=false", "native DirectStorage queues/fences/status arrays", "Win32 async IO", "async-overlap/performance", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { if (-not $recommendedText.Contains($needle)) { Write-Error "engine manifest aiOperableProductionLoop recommendedNextPlan must describe MAVG byte-range file IO selection: $needle" } }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-autonomous-page-streaming-worker-v1") { foreach ($needle in @("MAVG Runtime LOD Milestone v1", "MAVG Asset Graph v1", "RuntimeMavgPageStreamingDispatchMode::engine_owned_background_worker", "RuntimeMavgPageStreamingWorkerDesc", "RuntimeMavgPageStreamingWorkerResult", "execute_runtime_mavg_page_streaming_worker", "joined private worker thread execution", "max_worker_rows degradation evidence", "executed_background_worker=true", "no DirectStorage file IO", "no automatic eviction policy", "no GPU memory pressure integration", "no renderer/RHI handles", "no async-overlap/performance", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { if (-not $recommendedText.Contains($needle)) { Write-Error "engine manifest aiOperableProductionLoop recommendedNextPlan must describe MAVG autonomous page streaming worker selection: $needle" } }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-directstorage-request-plan-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-native-directstorage-win32-async-io-dispatch-status-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-win32-async-file-io-adapter-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-win32-iocp-file-io-worker-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-directstorage-sdk-dependency-gate-v1") { foreach ($needle in @("MAVG DirectStorage Request Plan v1", "MAVG Runtime LOD Milestone v1", "RuntimeMavgPayloadDirectStorageRequestPlanDesc", "RuntimeMavgPayloadDirectStorageRequestRow", "RuntimeMavgPayloadDirectStorageRequestPlanResult", "RuntimeMavgPayloadDirectStorageFenceWaitPoint", "plan_runtime_mavg_payload_directstorage_requests", "requires_native_directstorage_sdk=true", "used_native_directstorage=false", "no file IO", "native enqueue", "native submit", "native fence signal", "async-overlap/performance", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { if (-not $recommendedText.Contains($needle)) { Write-Error "engine manifest aiOperableProductionLoop recommendedNextPlan must describe MAVG DirectStorage request-plan selection: $needle" } }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-runtime-lod-milestone-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-package-streaming-residency-dispatch-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-page-addressable-payload-schema-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-runtime-inferred-frequency-eviction-policy-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-gpu-memory-pressure-eviction-policy-v1") { foreach ($needle in @("MAVG Runtime LOD Milestone v1", "MAVG Asset Graph v1", "MAVG Phase 0 completed", "hierarchy/error/fallback", "MavgClusterGraphCluster", "resident_fallback_cluster_index", "geometric_error", "first_index", "index_count", "vertex_base", "GameEngine.MavgClusterGraph.v1", "MavgClusterCookVertex", "GameEngine.MavgClusterPayload.v1", "vertex.data_hex", "index.data_hex", "page.data_hex", "per-material root/leaf", "mavg_cluster_payload.hpp", "MavgClusterPayloadDocument", "MavgClusterPayloadPage", "validate_mavg_cluster_payload", "mavg_payload_pages.hpp", "RuntimeMavgPayloadPageSliceResult", "extract_runtime_mavg_payload_page_slices", "mavg_lod_selection.hpp", "MavgLodViewDesc", "MavgLodResidentPageSet", "MavgLodSelectionResult", "CPU LOD selection", "select_mavg_lod_clusters", "MK_mavg_lod_selection_tests", "mavg_lod_residency.hpp", "RuntimeMavgLodResidencyDesc", "RuntimeMavgLodResidencyResult", "build_runtime_mavg_lod_residency", "MK_runtime_mavg_lod_residency_tests", "range-aware conventional indexed draws", "mavg_scene_lod.hpp", "MavgSceneLodSubmitDesc", "MavgSceneLodSubmitResult", "plan_mavg_scene_lod_mesh_commands", "MK_scene_renderer_mavg_lod_tests", "RuntimeMavgPageStreamingDispatchPlan", "plan_runtime_mavg_page_streaming_dispatches", "RuntimeMavgPageStreamingDrainDesc", "RuntimeMavgPageStreamingAutomaticEvictionPlanDesc", "plan_runtime_mavg_page_streaming_automatic_evictions", "runtime_inferred_frequency", "RuntimeMavgPageStreamingFrequencyRow", "runtime_inferred_frequency_eviction_candidate_count", "frequency_counter_overflow", "caller_supplied_gpu_memory_pressure", "RuntimeMavgPageStreamingGpuMemoryPressureRow", "gpu_memory_pressure_eviction_candidate_count", "gpu_memory_pressure_counter_overflow", "GPU culling", "indirect draw execution", "mesh shaders", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { if (-not $recommendedText.Contains($needle)) { Write-Error "engine manifest aiOperableProductionLoop recommendedNextPlan must describe MAVG runtime LOD selection: $needle" } }
