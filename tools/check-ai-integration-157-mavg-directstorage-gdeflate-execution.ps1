...
# Chapter 157 for check-ai-integration.ps1 MAVG DirectStorage GDeflate execution evidence gate.

$headerText = Get-AgentSurfaceText "engine/runtime_rhi/include/mirakana/runtime_rhi/mavg_directstorage_gdeflate_execution.hpp"
$sourceText = Get-AgentSurfaceText "engine/runtime_rhi/src/mavg_directstorage_gdeflate_execution.cpp"
$testsText = Get-AgentSurfaceText "tests/unit/runtime_rhi_mavg_directstorage_gdeflate_execution_tests.cpp"
$schemaText = Get-AgentSurfaceText "schemas/mavg-directstorage-gdeflate-execution.schema.json"
$validatorText = Get-AgentSurfaceText "tools/validate-mavg-directstorage-gdeflate-execution.ps1"
...
$gdeflatePlanText = Get-AgentSurfaceText "docs/superpowers/plans/2026-07-02-mavg-directstorage-gdeflate-execution-evidence-v1.md"

foreach ($needle in @(
        "RuntimeMavgDirectStorageGDeflateExecutionRow",
        "RuntimeMavgDirectStorageGDeflateExecutionResult",
        "RuntimeMavgDirectStorageGDeflateExecutionDiagnosticCode",
        "RuntimeMavgDirectStorageGDeflateDestinationKind",
        "RuntimeMavgDirectStorageGDeflateExecutionSourceKind",
        "d3d12_multiple_subresources_range",
        "evaluate_runtime_mavg_directstorage_gdeflate_execution_evidence",
        "has_runtime_mavg_directstorage_gdeflate_execution_diagnostic",
        "runtime_mavg_directstorage_gdeflate_execution_status_label",
    ...
    Assert-ContainsText $headerText $needle "mavg_directstorage_gdeflate_execution.hpp public contract"
}
...
foreach ($forbiddenNeedle in @(
    ...
    Assert-DoesNotContainText $headerText $forbiddenNeedle "mavg_directstorage_gdeflate_execution.hpp native handle boundary"
    Assert-DoesNotContainText $sourceText $forbiddenNeedle "mavg_directstorage_gdeflate_execution.cpp native handle boundary"
}

foreach ($needle in @(
        "DirectStorage 1.4/Zstd/GACL preview rows are future review input",
        "requires reviewed GetCompressionSupport evidence",
        "cannot treat CPU fallback as GPU execution evidence",
        ...
        "requires package-visible output evidence",
    ...
    Assert-ContainsText $sourceText $needle "mavg_directstorage_gdeflate_execution.cpp fail-closed implementation"
}

foreach ($needle in @(
        "runtime rhi mavg directstorage gdeflate execution accepts reviewed gdeflate range evidence",
        "runtime rhi mavg directstorage gdeflate execution host gates missing request and hash evidence",
        "runtime rhi mavg directstorage gdeflate execution blocks invalid artifacts ranges and byte counts",
        "runtime rhi mavg directstorage gdeflate execution blocks preview and unsupported claims",
        "runtime rhi mavg directstorage gdeflate execution accepts buffer memory evidence without range readiness",
        "microsoft-directstorage-compression-format",
        "missing_get_compression_support",
        "cpu_fallback_not_gpu_execution",
        "missing_gpu_compression_queue",
        "directstorage_preview_selected",
        "external_engine_claim_not_allowed"
    )) {
    Assert-ContainsText $testsText $needle "MK_runtime_rhi_mavg_ds_gdeflate_execution_tests coverage"
}
...
foreach ($needle in @(
    ...
    Assert-ContainsText $schemaText $needle "mavg-directstorage-gdeflate-execution schema"
}

foreach ($needle in @(
        "validation_recipe=mavg-directstorage-gdeflate-execution-evidence",
        "MK_runtime_rhi_mavg_ds_gdeflate_execution_tests",
    ...
    Assert-ContainsText $validatorText $needle "tools/validate-mavg-directstorage-gdeflate-execution.ps1"
}

Assert-ContainsText $runtimeRhiCMakeText "src/mavg_directstorage_gdeflate_execution.cpp" "engine/runtime_rhi/CMakeLists.txt MAVG DirectStorage GDeflate execution source registration"
Assert-ContainsText $rootCMakeText "MK_runtime_rhi_mavg_ds_gdeflate_execution_tests" "root CMake MAVG DirectStorage GDeflate execution test target"
Assert-ContainsText $commandsFragmentText "mavgDirectStorageGDeflateExecutionCheck" "manifest commands MAVG DirectStorage GDeflate execution command"
Assert-ContainsText $productionLoopFragmentText "mavgDirectStorageGDeflateExecutionEvidence" "production loop retained MAVG DirectStorage GDeflate execution evidence"
...
foreach ($surface in @(
        ...
        @{ Text = $gdeflatePlanText; Label = "MAVG DirectStorage GDeflate execution plan" },
    ...
    foreach ($needle in @(
        ...
        Assert-ContainsText $surface.Text $needle "$($surface.Label) MAVG DirectStorage GDeflate execution evidence"
    }
    foreach ($forbiddenNeedle in @(
        ...
        Assert-DoesNotContainText $surface.Text $forbiddenNeedle "$($surface.Label) forbidden MAVG DirectStorage GDeflate ready claim"
    }
}
...
foreach ($needle in @(
        "mavg_directstorage_gdeflate_execution.hpp",
        "RuntimeMavgDirectStorageGDeflateExecutionResult",
        "GameEngine.MavgDirectStorageGDeflateExecutionEvidence.v1",
        "mavg_directstorage_gdeflate_execution_ready=0",
        "mavg_directstorage_gpu_destination_execution_ready=0",
    ...
    Assert-ContainsText $runtimeRhiManifestText $needle "engine/agent/manifest.json MK_runtime_rhi MAVG DirectStorage GDeflate execution evidence"
}
