...
if ($validationRunnerCommand.Count -ne 1 -or $validationRunnerCommand[0].status -ne "ready") {
    ...
    if (@($validationRunnerCommand[0].validationRecipes).Count -ne 32) {
        Write-Error "engine/agent/manifest.json run-validation-recipe validationRecipes must be exactly the reviewed allowlist of 32 recipes"
    }
    ...
    foreach ($needle in @("Test-ValidationRecipeRequest", "New-ValidationRecipeDryRunResult", "New-ValidationRecipeRejectedResult", "Get-SampleDesktopRuntimeGameEnvironmentReadyAggregateSmokeArgs", "Get-SampleDesktopRuntimeGameEnvironmentVulkanStrictAggregateSmokeArgs")) {
    ...
    Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-vulkan-environment-ibl-renderer-execution" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-ready-aggregate" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-vulkan-strict-aggregate" "tools/run-validation-recipe-plans.ps1"
