...
if ($validationRunnerCommand.Count -ne 1 -or $validationRunnerCommand[0].status -ne "ready") {
    ...
    if (@($validationRunnerCommand[0].validationRecipes).Count -ne 31) {
        Write-Error "engine/agent/manifest.json run-validation-recipe validationRecipes must be exactly the reviewed allowlist of 31 recipes"
    }
    ...
    Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-vulkan-environment-ibl-renderer-execution" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-ready-aggregate" "tools/run-validation-recipe-plans.ps1"
