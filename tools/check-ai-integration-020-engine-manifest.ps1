...
if ($validationRunnerCommand.Count -ne 1 -or $validationRunnerCommand[0].status -ne "ready") {
    ...
    if (@($validationRunnerCommand[0].validationRecipes).Count -ne 23) {
        Write-Error "engine/agent/manifest.json run-validation-recipe validationRecipes must be exactly the reviewed allowlist of 23 recipes"
    }
