...
if ($validationRunnerCommand.Count -ne 1 -or $validationRunnerCommand[0].status -ne "ready") {
    ...
    foreach ($recipe in @(
            ...
            "desktop-runtime-sample-game-environment-backend-parity", "desktop-runtime-sample-game-environment-platform-readiness",
    ...
    if (@($validationRunnerCommand[0].validationRecipes).Count -ne 40) {
        Write-Error "engine/agent/manifest.json run-validation-recipe validationRecipes must be exactly the reviewed allowlist of 40 recipes"
    }
    ...
    Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-vulkan-environment-ibl-renderer-execution" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-ready-aggregate" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-vulkan-strict-aggregate" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-platform-readiness" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-artist-workflow-package" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-commercial-readiness" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-commercial-vulkan-evidence" "tools/run-validation-recipe-plans.ps1"
