...
$rhiPublicHeaderText = Get-AgentSurfaceText "engine/rhi/include/mirakana/rhi/rhi.hpp"; $rhiAsyncOverlapSourceText = Get-AgentSurfaceText "engine/rhi/src/async_overlap.cpp"
...
$sceneRendererMavgLodHeaderText = Get-AgentSurfaceText "engine/scene_renderer/include/mirakana/scene_renderer/mavg_scene_lod.hpp"; $sceneRendererMavgLodSourceText = Get-AgentSurfaceText "engine/scene_renderer/src/mavg_scene_lod.cpp"; $sceneRendererMavgLodTestsText = Get-AgentSurfaceText "tests/unit/scene_renderer_mavg_lod_tests.cpp"
$runtimeHostWin32HeaderText = Get-AgentSurfaceText "engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp"; $runtimeHostWin32SourceText = Get-AgentSurfaceText "engine/runtime_host/win32/src/win32_desktop_presentation.cpp"; $runtimeHostWin32SceneGpuInjectingRendererText = Get-AgentSurfaceText "engine/runtime_host/win32/src/scene_gpu_binding_injecting_renderer.hpp"; $runtimeHostWin32TestsText = Get-AgentSurfaceText "tests/unit/runtime_host_win32_tests.cpp"; $runtimeHostWin32PublicApiText = Get-AgentSurfaceText "tests/unit/runtime_host_win32_public_api_compile.cpp"
...
$vulkanComputeMorphShaderText = Get-AgentSurfaceText "tests/shaders/vulkan_compute_morph_position.hlsl"; $vulkanComputeMorphRendererShaderText = Get-AgentSurfaceText "tests/shaders/vulkan_compute_morph_renderer_position.hlsl"; $vulkanComputeMorphTangentFrameShaderText = Get-AgentSurfaceText "tests/shaders/vulkan_compute_morph_tangent_frame.hlsl"
...
Assert-ContainsText $masterPlanText "recommendedNextPlan.id = next-production-gap-selection" "production master plan MAVG async overlap performance proof closeout pointer"
...
Assert-ContainsText $masterPlanText "Renderer Backend Parity Apple Evidence Closeout v1 completed through PR #710" "production master plan renderer backend parity Apple evidence closeout pointer"
...
foreach ($needle in @("last_indexed_draw_first_index", "last_indexed_draw_vertex_offset", "draw_indexed(std::uint32_t index_count, std::uint32_t instance_count, std::uint32_t first_index")) { Assert-ContainsText $rhiPublicHeaderText $needle "engine/rhi/include/mirakana/rhi/rhi.hpp MAVG indexed range API" }
...
Assert-ContainsText $d3d12RhiSourceText "DrawIndexedInstanced(index_count, instance_count, first_index, vertex_offset, first_instance)" "engine/rhi/d3d12/src/d3d12_backend.cpp MAVG indexed range API"
...
Assert-ContainsText $vulkanRhiSourceText "cmd_draw_indexed(command_buffer, desc.index_count, desc.instance_count, desc.first_index" "engine/rhi/vulkan/src/vulkan_backend.cpp MAVG indexed range API"
foreach ($needle in @("struct MeshIndexedDrawRange", "MeshIndexedDrawRange indexed_range")) { Assert-ContainsText $rendererHeaderText $needle "engine/renderer/include/mirakana/renderer/renderer.hpp MAVG indexed range" }
foreach ($entry in @(@($rendererSourceText, "make_indexed_draw_args(command.indexed_range", "engine/renderer/src/rhi_frame_renderer.cpp MAVG indexed range"), @($sceneRendererMavgLodHeaderText, "plan_mavg_scene_lod_mesh_commands", "engine/scene_renderer/include/mirakana/scene_renderer/mavg_scene_lod.hpp"), @($sceneRendererMavgLodHeaderText, "const SceneGpuBindingPalette* gpu_bindings", "engine/scene_renderer/include/mirakana/scene_renderer/mavg_scene_lod.hpp"), @($sceneRendererMavgLodSourceText, "MavgSceneLodDiagnosticCode::fallback_substitution", "engine/scene_renderer/src/mavg_scene_lod.cpp"), @($sceneRendererMavgLodSourceText, "MeshIndexedDrawRange", "engine/scene_renderer/src/mavg_scene_lod.cpp"), @($sceneRendererMavgLodTestsText, "mavg scene lod planning creates range aware mesh commands", "tests/unit/scene_renderer_mavg_lod_tests.cpp"))) { Assert-ContainsText $entry[0] $entry[1] $entry[2] }
...
if ($validationRunnerCommand.Count -ne 1 -or $validationRunnerCommand[0].status -ne "ready") {
    ...
    foreach ($recipe in @(
            ...
            "desktop-runtime-sample-game-environment-snow-package", "desktop-runtime-sample-game-environment-snow-renderer-execution", "desktop-runtime-sample-game-environment-material-weathering", "desktop-runtime-sample-game-environment-ibl-renderer-execution", "desktop-runtime-sample-game-vulkan-environment-ibl-renderer-execution", "desktop-runtime-sample-game-environment-profile-package", "desktop-runtime-sample-game-environment-ready-aggregate", "desktop-runtime-sample-game-environment-vulkan-strict-aggregate",
            "desktop-runtime-sample-game-environment-backend-parity", "desktop-runtime-sample-game-environment-backend-parity-ready", "desktop-runtime-sample-game-environment-platform-readiness",
    ...
    $runnerScript = Resolve-RequiredAgentPath "tools/run-validation-recipe.ps1"; $runnerText = Get-Content -LiteralPath $runnerScript -Raw
    ...
    foreach ($needle in @("Test-ValidationRecipeRequest", "New-ValidationRecipeDryRunResult", "New-ValidationRecipeRejectedResult", "Get-SampleDesktopRuntimeGameEnvironmentReadyAggregateSmokeArgs", "Get-SampleDesktopRuntimeGameEnvironmentVulkanStrictAggregateSmokeArgs", "Get-SampleDesktopRuntimeGameEnvironmentBackendParityReadySmokeArgs", "Get-SampleDesktopRuntimeGameEnvironmentPlatformReadinessSmokeArgs", "Get-SampleDesktopRuntimeGameEnvironmentOptimizationMeasurementSmokeArgs", "Get-SampleDesktopRuntimeGameEnvironmentArtistWorkflowPackageSmokeArgs", "Get-SampleDesktopRuntimeGameEnvironmentCommercialReadinessSmokeArgs")) {
        Assert-ContainsText $runnerCoreText $needle "tools/validation-recipe-core.ps1"
    }
    Assert-ContainsText $runnerCoreText "Get-SampleDesktopRuntimeGameEnvironmentLightingRendererExecutionSmokeArgs" "tools/validation-recipe-core.ps1"; Assert-ContainsText $runnerCoreText "Get-SampleDesktopRuntimeGameVulkanEnvironmentIblRendererExecutionSmokeArgs" "tools/validation-recipe-core.ps1"
    ...
    Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-vulkan-environment-ibl-renderer-execution" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-ready-aggregate" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-vulkan-strict-aggregate" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-backend-parity-ready" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-platform-readiness" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-artist-workflow-package" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-commercial-readiness" "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-Content -LiteralPath (Resolve-RequiredAgentPath "tools/run-validation-recipe-plans.ps1") -Raw) "desktop-runtime-sample-game-environment-commercial-vulkan-evidence" "tools/run-validation-recipe-plans.ps1"
...
$cpuProfilingHostGate = @($productionLoop.hostGates | Where-Object { $_.id -eq "cpu-profiling-matrix-host" }); if ($cpuProfilingHostGate.Count -ne 1 -or $cpuProfilingHostGate[0].status -ne "ready" -or $cpuProfilingHostGate[0].residualClass -ne "ready") { Write-Error "engine/agent/manifest.json aiOperableProductionLoop must expose one ready cpu-profiling-matrix-host gate after hosted Windows WPR/WPA evidence" }; foreach ($needle in @("CPU profiling matrix host gate closeout", "cpu_profiling_matrix_ready=1", "cpu_profiling_matrix_missing_host_class_rows=0", "cpu_profiling_matrix_broad_optimization=0", "broad CPU/GPU/memory optimization")) { Assert-ContainsText ([string]$cpuProfilingHostGate[0].notes) $needle "engine/agent/manifest.json aiOperableProductionLoop cpu-profiling-matrix-host notes" }
$cpuProfilingRecipe = @($manifest.validationRecipes | Where-Object { $_.name -eq "host-cpu-profiling-matrix" }); if ($cpuProfilingRecipe.Count -ne 1) { Write-Error "engine/agent/manifest.json validationRecipes must include host-cpu-profiling-matrix" }
if (-not (Test-Path (Join-Path $root "tools/check-cpu-profiling-host-evidence.ps1"))) { Write-Error "tools/check-cpu-profiling-host-evidence.ps1 must exist for host-cpu-profiling-matrix evidence validation" }
foreach ($scriptName in @("collect-cpu-profiling-host-evidence.ps1", "check-cpu-profiling-host-evidence-collector.ps1")) { if (-not (Test-Path (Join-Path $root "tools/$scriptName"))) { Write-Error "tools/$scriptName must exist for host-cpu-profiling-matrix collector validation" } }
...
foreach ($needle in @("Intel/AMD CPU Profiling Matrix v1", "host-gated evidence selection", "tools/collect-cpu-profiling-host-evidence.ps1", "before/after trace pairs")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentCpuProfilingMatrix) $needle "engine/agent/manifest.json gameCodeGuidance.currentCpuProfilingMatrix" }
...
$optionalGpuComputeHostGate = @($productionLoop.hostGates | Where-Object { $_.id -eq "optional-gpu-compute-review-host" }); if ($optionalGpuComputeHostGate.Count -ne 1 -or $optionalGpuComputeHostGate[0].status -ne "ready" -or $optionalGpuComputeHostGate[0].residualClass -ne "ready") { Write-Error "engine/agent/manifest.json aiOperableProductionLoop must expose one ready optional-gpu-compute-review-host gate" }
$optionalGpuComputeRecipe = @($manifest.validationRecipes | Where-Object { $_.name -eq "host-optional-gpu-compute-review" }); if ($optionalGpuComputeRecipe.Count -ne 1) { Write-Error "engine/agent/manifest.json validationRecipes must include host-optional-gpu-compute-review" }; foreach ($scriptFile in @("tools/collect-optional-gpu-compute-review-evidence.ps1", "tools/check-optional-gpu-compute-review-evidence.ps1", "tools/validate-optional-gpu-compute-review-host-gate.ps1")) { if (-not (Test-Path (Join-Path $root $scriptFile))) { Write-Error "$scriptFile must exist for host-optional-gpu-compute-review evidence handling" } }; foreach ($needle in @("tools/collect-optional-gpu-compute-review-evidence.ps1", "-Mode Import", "tools/check-optional-gpu-compute-review-evidence.ps1", "-EvidenceRoot", "-RequiredCandidateIds", "-RequireReady", "tools/validate-optional-gpu-compute-review-host-gate.ps1")) { Assert-ContainsText ([string]$optionalGpuComputeRecipe[0].command) $needle "engine/agent/manifest.json validationRecipes host-optional-gpu-compute-review command" }
foreach ($needle in @("Optional GPU Compute Review v1", "review-only evidence selection", "tools/collect-optional-gpu-compute-review-evidence.ps1", "CMake linkage")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentOptionalGpuComputeReview) $needle "engine/agent/manifest.json gameCodeGuidance.currentOptionalGpuComputeReview" }
$recommendedPlanUsesLegacyCloseoutContext = $recommendedPlanId -notin @("general-purpose-game-production-v1", "generated-game-studio-v1", "engine-1-0-gap-matrix-v1", "next-production-gap-selection", "native-win32-editor-shell-v1", "first-party-editor-shell-v1", "first-party-ui-editor-production-stack-v1", "physics-navigation-commercial-coverage-v1", "renderer-backend-parity-metal-apple-evidence-v1", "renderer-backend-parity-apple-evidence-closeout-v1", "renderer-postprocess-tone-mapping-evidence-v1", "renderer-commercial-quality-closeout-v1", "renderer-commercial-readiness-evidence-promotion-v1", "sandbox-world-network-modding-gate-v1", "sandbox-world-package-validation-performance-budgets-v1", "ai-operable-performance-budget-and-evidence-v1", "performance-baseline-v1", "long-running-performance-readiness-v1-phase-1", "long-running-performance-readiness-v1-phase-2", "long-running-performance-readiness-v1-phase-7", "memory-lifetime-taxonomy-v1", "memory-diagnostics-v1", "frame-thread-scratch-v1", "job-scheduling-evidence-v1", "job-execution-worker-pool-v1", "job-execution-topology-policy-v1", "job-execution-work-stealing-v1", "job-execution-placement-policy-v1", "windows-cpu-set-worker-placement-v1", "windows-cpu-set-smt-worker-placement-v1", "simd-dispatch-policy-and-evidence-v1", "avx2-reviewed-target-execution-v1", "environment-system-v1", "environment-rendering-readiness-v1", "environment-production-excellence-v1", "environment-highest-commercial-readiness-v1", "mavg-research-legal-benchmark-baseline-v1", "mavg-asset-graph-v1", "mavg-runtime-lod-milestone-v1", "mavg-page-streaming-eviction-review-v1", "mavg-win32-directstorage-sdk-adapter-v1", "mavg-async-overlap-performance-proof-v1", "mavg-mesh-shader-capability-gate-v1", "mavg-deformation-backend-execution-evidence-v1", "2d-production-engine-capability-gap-cluster-v1", "2d-commercial-production-excellence-v1")
...
} elseif ($recommendedPlanId -eq "mavg-async-overlap-performance-proof-v1") { $mavgAsyncOverlapRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8; foreach ($needle in @("MAVG Async Overlap Performance Proof v1", "selected measured-sample async-overlap performance proof", "MAVG Win32 DirectStorage SDK Adapter v1", "RuntimeMavgStreamingUploadOverlapEvidenceResult", "RuntimeMavgAsyncOverlapPerformanceProofResult", "p95 serial/overlapped ticks", "speedup basis points", "mavg_async_overlap_performance_proof_ready=1", "GPU DirectStorage destinations", "GDeflate", "DirectStorage 1.4 preview", "mesh shaders", "Metal readiness", "Nanite", "native handles", "broad optimization", "broad MAVG backend readiness", "autonomous streaming-service performance", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgAsyncOverlapRecommendedText $needle "engine/agent/manifest.json aiOperableProductionLoop recommendedNextPlan MAVG async overlap performance proof selection" }
...
} elseif ($recommendedPlanId -eq "renderer-backend-parity-apple-evidence-closeout-v1") { foreach ($needle in @("Renderer Backend Parity Apple Evidence Closeout v1", "renderer-backend-parity-v1", "renderer-metal-apple-host-evidence", "Apple/Metal host evidence", "Windows/Vulkan proof must not promote Metal readiness", "native handles remain hidden", "unsupportedProductionGaps = []")) { Assert-ContainsText $recommendedText $needle "engine/agent/manifest.json aiOperableProductionLoop recommendedNextPlan renderer backend parity Apple evidence closeout selection" }
} elseif ($recommendedPlanId -eq "renderer-postprocess-tone-mapping-evidence-v1") { foreach ($needle in @("Renderer Postprocess Tone Mapping Evidence v1", "renderer-postprocess-v1", "PostprocessToneMappingEvidencePlan", "plan_postprocess_tone_mapping_evidence", "D3D12/Vulkan", "Metal host-gated", "no SDL3", "native handles", "subjective visual quality", "unsupportedProductionGaps = []")) { Assert-ContainsText $recommendedText $needle "engine/agent/manifest.json aiOperableProductionLoop recommendedNextPlan renderer postprocess tone mapping selection" }
