...
$cpuProfilingHostGate = @($productionLoop.hostGates | Where-Object { $_.id -eq "cpu-profiling-matrix-host" }); if ($cpuProfilingHostGate.Count -ne 1 -or $cpuProfilingHostGate[0].status -ne "host-gated") { Write-Error "engine/agent/manifest.json aiOperableProductionLoop must expose one host-gated cpu-profiling-matrix-host gate" }
