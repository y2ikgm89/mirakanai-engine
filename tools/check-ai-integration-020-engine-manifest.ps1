...
} elseif ($recommendedPlanId -eq "mavg-mesh-shader-capability-gate-v1") { foreach ($needle in @("MAVG Mesh Shader Capability Gate v1", "RuntimeMavgMeshShaderCapabilityGateResult", "evaluate_runtime_mavg_mesh_shader_capability_gate", "D3D12_FEATURE_D3D12_OPTIONS7", "MeshShaderTier", "VK_EXT_mesh_shader", "VkPhysicalDeviceMeshShaderFeaturesEXT", "VkPhysicalDeviceMeshShaderPropertiesEXT", "mavg_mesh_shader_capability_gate_ready=1", "two backend rows", "two feature-query rows", "two pipeline-statistics rows", "compute/indirect fallback", "mesh shader execution remains 0", "native handles", "Metal readiness", "Nanite", "broad MAVG backend readiness", "broad optimization", "unsupportedProductionGaps = []")) { Assert-ContainsText $recommendedText $needle "engine/agent/manifest.json aiOperableProductionLoop recommendedNextPlan MAVG mesh shader capability gate selection" }
