...
foreach ($needle in @("tools/check-cpu-profiling-host-evidence.ps1", "-EvidenceRoot", "-RequireReady")) { Assert-ContainsText ([string]$cpuProfilingRecipe[0].command) $needle "engine/agent/manifest.json validationRecipes host-cpu-profiling-matrix command" }
