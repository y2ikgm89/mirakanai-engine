...
Assert-ContainsText $masterPlanText "recommendedNextPlan.id = next-production-gap-selection" "production master plan active pointer"
...
} elseif ($recommendedPlanId -eq "sandbox-world-network-modding-gate-v1") { foreach ($needle in @("Selected focused child plan", "sandbox-world-specific mutation replication", "reviewed modding policy gates", "unsupportedProductionGaps = []", "Broad online multiplayer", "SDL3", "native handle exposure")) { Assert-ContainsText $recommendedText $needle "engine/agent/manifest.json aiOperableProductionLoop recommendedNextPlan sandbox world network/modding selection" } } elseif ($recommendedPlanId -eq "sandbox-world-package-validation-performance-budgets-v1") { foreach ($needle in @("Selected focused child plan", "sample package smoke flags", "installed validation", "package-visible counters", "--require-sandbox-package-budgets", "sandbox_package_budget_*", "unsupportedProductionGaps = []", "broad renderer quality", "package mutation", "SDL3", "native handle exposure")) { Assert-ContainsText $recommendedText $needle "engine/agent/manifest.json aiOperableProductionLoop recommendedNextPlan sandbox world package validation and performance budget selection" } } elseif ($recommendedPlanId -eq "native-win32-editor-shell-v1") { foreach ($needle in @("Native Win32 Editor Shell v1", "MK_editor", "Dear ImGui", "Direct3D 12", "desktop-gui", "PR #316", "unsupportedProductionGaps = []", "SDL3", "native handles", "editor/developer-shell")) { Assert-ContainsText $recommendedText $needle "engine/agent/manifest.json aiOperableProductionLoop recommendedNextPlan native editor shell selection" } } elseif ($recommendedPlanId -eq "first-party-editor-shell-v1") { foreach ($needle in @("First-Party Editor Shell v1", "first-party retained MK_editor shell", "desktop-editor", "dock graph", "rich-text rows", "EditorAiOperationSnapshot", "EditorAiCommandCatalog", "EditorAiCommandRequest", "EditorAiCommandDryRunResult", "EditorAiCommandApplyResult", "adapter-boundary diagnostics", "DirectWrite", "Text Services Framework", "UI Automation", "unsupportedProductionGaps = []", "SDL3", "native handles", "Dear ImGui")) { Assert-ContainsText $recommendedText $needle "engine/agent/manifest.json aiOperableProductionLoop recommendedNextPlan first-party editor shell selection" } } else {
