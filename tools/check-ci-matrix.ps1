...
Assert-ContainsAll $windowsCpp23Job @(
    ...
    "Collect renderer package commercial quality host evidence",
    "artifacts/renderer/package-commercial-quality-host-evidence/current-run",
    ...
    "./tools/collect-renderer-package-commercial-quality-host-evidence.ps1",
    ...
    "Upload renderer package commercial quality host evidence",
    "name: renderer-package-commercial-quality-host-evidence",
    "artifacts/renderer/package-commercial-quality-host-evidence/current-run/**",
    "Collect renderer D3D12 commercial quality host evidence",
    ...
    "Upload renderer D3D12 commercial quality host evidence",
    "name: renderer-d3d12-commercial-quality-host-evidence",
    "artifacts/renderer/d3d12-commercial-quality-host-evidence/current-run/**",
    "Save vcpkg package cache",
    "Save vcpkg installed cache",
    "steps.restore-vcpkg-package.outputs.cache-hit != 'true'",
...
Assert-ContainsAll $macosJob @(
    ...
    "./tools/validate-renderer-metal-apple.ps1 -Jobs `$jobs",
    ...
    "name: renderer-apple-metal-commercial-quality-host-evidence",
    "artifacts/renderer/apple-metal-commercial-quality-host-evidence/current-run/**",
...
Assert-ContainsAll $rendererCommercialArtifactIntakeJob @(
    ...
    "- linux-vulkan",
    "- macos",
    "if: `${{ always() && !cancelled() && needs.changes.outputs.windows_msvc == 'true' && needs.changes.outputs.windows_cpp23_release == 'true' && needs.changes.outputs.linux_vulkan_host == 'true' && needs.changes.outputs.macos_metal_cmake == 'true' && needs.changes.outputs.metal_host_evidence == 'true' }}",
    "runs-on: ubuntu-latest",
    "timeout-minutes: 15",
    "permissions:",
    "contents: read",
    ...
    "renderer_quality_vfx_commercial_host_evidence_status=host_evidence_required",
    "renderer_quality_vfx_commercial_host_evidence_writes_evidence=1",
    "Upload renderer quality/VFX commercial artifacts",
    "name: renderer-quality-vfx-commercial-artifacts",
    "artifacts/renderer/quality-vfx-commercial-host-evidence/current-run/**",
    "Capture current workflow artifact list",
    'GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}',
    'gh api -H "Accept: application/vnd.github+json" "/repos/${{ github.repository }}/actions/runs/${{ github.run_id }}/artifacts?per_page=100"',
