...
# Dot-sourced from new-game.ps1 after common.ps1 and new-game-helpers.ps1.
...
$materialGraphEvidenceLocal
    ...
    std::cout << "$TargetName status=" << status_name(result.status)
              ...
              << modern_material_evidence.d3d12_shader_evidence_ready
...
"@, @"
              ...
              << renderer_quality.expected_framegraph_render_passes
              << " renderer_quality_expected_framegraph_barrier_steps="
              << renderer_quality.expected_framegraph_barrier_steps
              ...
              << " gltf_scene_import_review_parser_validation_rows="
              << gltf_scene_import_review_probe.parser_validation_rows
              << " gltf_scene_import_review_geometry_payload_rows="
              << gltf_scene_import_review_probe.geometry_payload_rows
              << " gltf_scene_import_review_material_payload_rows="
              << gltf_scene_import_review_probe.material_payload_rows
              << " gltf_scene_import_review_animation_payload_rows="
              << gltf_scene_import_review_probe.animation_payload_rows
              << " gltf_scene_import_review_external_resource_policy_rows="
              << gltf_scene_import_review_probe.external_resource_policy_rows
              << " gltf_scene_import_review_source_provenance_rows="
              << gltf_scene_import_review_probe.source_provenance_rows
              ...
              << " gltf_scene_import_review_mutated_packages="
              << (gltf_scene_import_review_probe.mutated_packages ? 1 : 0)
...
"@, @"
        ...
        if (options.require_gltf_scene_import_review &&
            (!gltf_scene_import_review_probe.reviewed || !gltf_scene_import_review_probe.ready ||
             ...
             gltf_scene_import_review_probe.leaked_parser_type || gltf_scene_import_review_probe.exposed_native_handle ||
             gltf_scene_import_review_probe.mutated_packages)) {
            return 24;
        }
...
"@, @"
        if (options.require_postprocess) {
            ...
            const auto expected_framegraph_barrier_step_count =
                expected_framegraph_barrier_steps(options.require_directional_shadow,
...
- `mirakana::runtime::plan_runtime_entity_scale_culling`
...
- selected glTF scene import review package smoke with `--require-gltf-scene-import-review`, `gltf_scene_import_review_status=ready`, `gltf_scene_import_review_ready=1`, eight review rows, eight ready rows, zero dependency-gated rows, zero unsupported-claim rows, selected package evidence ready, zero external network/runtime source parsing/parser type/native handle/package mutation counters, no broad scene import claim, and a positive `gltf_scene_import_review_replay_hash`
...
- selected generated 3D gameplay systems package smoke with `gameplay_systems_status=ready`, `gameplay_systems_ready=1`, physics authored collision/controller counters, navmesh dynamic-obstacle/readiness counters, deterministic navmesh/crowd readiness counters, local avoidance counters, character/dynamic physics policy counters, character dynamics readiness counters, kinematic/simple vehicle policy counters, navigation, AI perception/behavior, audio stream, and animation/lifecycle counters
- selected generated 3D scene collision package smoke with `--require-scene-collision-package`, `collision_package_status=ready`, `collision_package_bodies=3`, `collision_package_trigger_overlaps=1`, `collision_query_readiness_status=ready`, `collision_query_readiness_diagnostic=none`, `collision_query_readiness_diagnostics=0`, and `gameplay_systems_collision_package_ready=1`
...
function New-AiValidationRemediationRecipe {
    param(
        [string]$Id,
        [string]$FailureClassificationId,
        [string]$RemediationActionId,
        [string]$ReviewedCommandSurfaceId,
        [string]$Mode,
        [string[]]$ValidationRecipeIds,
    ...
    return [ordered]@{
        id = $Id
        failureClassificationId = $FailureClassificationId
        remediationActionId = $RemediationActionId
        reviewedCommandSurfaceId = $ReviewedCommandSurfaceId
        mode = $Mode
        validationRecipeIds = @($ValidationRecipeIds)
        requiredEvidence = @($RequiredEvidence)
        stopConditions = @($StopConditions)
        rerunPolicy = "rerun-selected-validation-recipe"
    }
}

function New-AiValidationRemediationRecipes {
    param(
        [string]$GameName,
        [string[]]$ValidationRecipeIds
    )
    ...
    return [ordered]@{
        schemaVersion = 1
        capabilityId = "ai-validation-remediation-recipes-v1"
        recipeSetId = "$gameId-validation-remediation-recipes"
        playtestLoopId = "$gameId-playtest-loop"
        mutationLedgerId = "$gameId-ai-mutation-ledger"
        recipes = @(
            New-AiValidationRemediationRecipe `
                -Id "missing-package-file-remediation" `
                -FailureClassificationId "missing-package-file" `
                -RemediationActionId "missing-package-file" `
                -ReviewedCommandSurfaceId "register-runtime-package-files" `
                -Mode "repair-through-reviewed-surface" `
                -ValidationRecipeIds $recipeIds `
                -RequiredEvidence @("missing runtimePackageFiles row or package artifact path", "reviewed package registration dry-run") `
                -StopConditions @("target path leaves the game root", "repair requires cooked package mutation")
            New-AiValidationRemediationRecipe `
                -Id "invalid-reference-remediation" `
                -FailureClassificationId "invalid-reference" `
                -RemediationActionId "validation-failure" `
                -ReviewedCommandSurfaceId "create-game-recipe" `
                -Mode "repair-through-reviewed-surface" `
                -ValidationRecipeIds $recipeIds `
                -RequiredEvidence @("invalid manifest, package, scene, asset, or recipe reference", "reviewed game-local dry-run") `
                -StopConditions @("reference targets shared repository paths", "repair would delete validation evidence")
            New-AiValidationRemediationRecipe `
                -Id "host-gated-remediation" `
                -FailureClassificationId "host-gated" `
                -RemediationActionId "validation-failure" `
                -ReviewedCommandSurfaceId "create-game-recipe" `
                -Mode "record-host-gate" `
                -ValidationRecipeIds $recipeIds `
                -RequiredEvidence @("host gate id and unavailable host tool or device", "blocked recipe summary") `
                -StopConditions @("host gate would be bypassed", "an unrelated ready claim would replace host evidence")
            New-AiValidationRemediationRecipe `
                -Id "shader-tool-gap-remediation" `
                -FailureClassificationId "shader-tool-gap" `
                -RemediationActionId "unsupported-engine-capability" `
                -ReviewedCommandSurfaceId "engine-capability-handoff" `
                -Mode "developer-handoff" `
                -ValidationRecipeIds $recipeIds `
                -RequiredEvidence @("missing shader toolchain or backend evidence", "canonical developer-owned capability id") `
                -StopConditions @("shader compilation would be faked", "engine internals or toolchain policy must change")
            New-AiValidationRemediationRecipe `
                -Id "counter-mismatch-remediation" `
                -FailureClassificationId "counter-mismatch" `
                -RemediationActionId "validation-failure" `
                -ReviewedCommandSurfaceId "create-game-recipe" `
                -Mode "repair-through-reviewed-surface" `
                -ValidationRecipeIds $recipeIds `
                -RequiredEvidence @("expected gameplay, renderer, UI, audio, physics, navigation, or AI counter", "actual package smoke output") `
                -StopConditions @("repair weakens expected counters", "repair broadens quality claims")
            New-AiValidationRemediationRecipe `
                -Id "runtime-package-load-remediation" `
                -FailureClassificationId "runtime-package-load" `
                -RemediationActionId "missing-package-file" `
                -ReviewedCommandSurfaceId "register-runtime-package-files" `
                -Mode "repair-through-reviewed-surface" `
                -ValidationRecipeIds $recipeIds `
                -RequiredEvidence @("package load, index, asset payload, or runtime scene validation diagnostic", "reviewed package file registration dry-run") `
                -StopConditions @("package repair needs unreviewed cooked artifact mutation", "runtime source parsing would be introduced")
            New-AiValidationRemediationRecipe `
                -Id "unsafe-mutation-request-remediation" `
                -FailureClassificationId "unsafe-mutation-request" `
                -RemediationActionId "unsafe-shared-path-request" `
                -ReviewedCommandSurfaceId "engine-capability-handoff" `
                -Mode "developer-handoff" `
                -ValidationRecipeIds $recipeIds `
                -RequiredEvidence @("forbidden shared path or unsafe mutation request", "developer-owned handoff target") `
                -StopConditions @("shared repository files would be edited from the game lane", "raw shell or native handles would be exposed")
        )
        unsupportedClaims = @(
            "validation-weakening",
            "evidence-deletion",
...
function New-DesktopRuntime3DGameDesignSpec {
    ...
    return [ordered]@{
        ...
        assetRequests = @(
            ...
            [ordered]@{
                id = "gltf-scene-import-review"
                kind = "mesh"
                purpose = "Selected glTF scene import review package smoke evidence for source-root policy, parser validation, geometry/material/animation payload evidence, external resource policy, source provenance, package output, and dependency/legal records without runtime source parsing, parser type leakage, native handles, package mutation, or broad scene import readiness."
                delivery = "reviewed-package-evidence"
            }
        )
        ...
        qualityGates = @(
            ...
            [ordered]@{
                id = "gltf-scene-import-review"
                evidence = "Installed package validation reports glTF scene import review rows, dependency/legal records, selected package evidence, zero dependency-gated or unsupported rows, zero external network/runtime source parsing/parser type/native handle/package mutation counters, no broad scene import claim, and a positive replay hash."
                recipeIds = @("installed-d3d12-3d-package-smoke")
            }
        )
        unsupportedClaims = @("engine-internal-edits", "native-handles", "middleware-contracts", "unreviewed-external-assets", "arbitrary-shell", "broad-commercial-quality", "runtime-source-parsing", "renderer-rhi-residency", "metal-readiness", "runtime-ktx2-basis-transcoding", "ktx2-basis-gpu-upload", "ktx2-basis-compression-execution", "broad-texture-codec-readiness", "gltf-external-network-fetch", "gltf-runtime-source-parsing", "gltf-parser-type-leakage", "gltf-native-handle-access", "gltf-package-mutation", "broad-gltf-scene-import-readiness")
    }
}
...
function New-DesktopRuntime2DManifest {
    ...
    return [ordered]@{
        ...
        backendReadiness = [ordered]@{
            ...
            ai = "MK_ai behavior authoring, perception-blackboard, behavior-tree proof through --require-gameplay-systems, and MK_runtime entity scale/culling proof through --require-entity-scale-culling"
        }
        importerRequirements = [ordered]@{
            sourceFormats = @(
        ...
        spriteAtlasSourceAuthoringTargets = @(
            [ordered]@{
                ...
                maxSide = 1024
                pagePolicy = [ordered]@{
                    mode = "single-page-tight-rgba8-texture-source"
                    pageId = "page-0"
                    pageCount = 1
                    paddingPixels = 0
                }
        ...
        validationRecipes = @(
            ...
            [ordered]@{
                name = "installed-2d-entity-scale-culling-smoke"
