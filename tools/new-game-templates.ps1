...
function New-Win32DesktopRuntimeCookedSceneMainCpp {
    param(
        [string]$GameName,
        [string]$TargetName,
        [string]$Title,
...
[[nodiscard]] mirakana::AssetId packaged_scene_asset_id() {
    return asset_id_from_game_asset_key("__ESCAPED_SCENE_ASSET_NAME__");
}
...
void print_cooked_scene_usage() {
    std::cout
        << "__TARGET_NAME__ [--smoke] [--max-frames N] "
...
int main(int argc, char** argv) {
    ...
    std::cout << "__TARGET_NAME__ status=" << cooked_scene_status_name(result.status)
              << " renderer=" << mirakana::win32_desktop_presentation_backend_name(report.selected_backend)
              << " presentation_requested=" << mirakana::win32_desktop_presentation_backend_name(report.requested_backend)
              << " presentation_selected=" << mirakana::win32_desktop_presentation_backend_name(report.selected_backend)
              ...
              << " scene_meshes=" << game.scene_meshes_submitted()
              << " scene_materials=" << game.scene_materials_resolved() << '\n';
    print_presentation_report("__TARGET_NAME__", host);
    for (const auto& diagnostic : host.presentation_diagnostics()) {
        std::cout << "__TARGET_NAME__ presentation_diagnostic="
...
'@
    ...
    return $template.Replace("__TARGET_NAME__", $TargetName).
        Replace("__ESCAPED_TITLE__", $escapedTitle).
...
function New-Win32DesktopRuntime3DMainCpp {
    param(
        [string]$GameName,
        [string]$TargetName,
        [string]$Title,
    ...
    $text = New-Win32DesktopRuntimeMaterialShaderMainCpp `
        -GameName $GameName `
        -TargetName $TargetName `
        -Title $Title `
        -SceneAssetName $SceneAssetName
    ...
    $text = $text.Replace("generated desktop runtime cooked scene", "generated desktop runtime 3D package")
...
function New-DesktopRuntime3DMainCpp {
    ...
    return New-Win32DesktopRuntime3DMainCpp `
        -GameName $GameName `
        -TargetName $TargetName `
        -Title $Title `
        -SceneAssetName $SceneAssetName
...
The generated package proves Win32 host startup, config loading, cooked scene loading, renderer-neutral scene submission counters, and deterministic NullRenderer fallback only. It does not generate D3D12/Vulkan/Metal shader artifacts, execute package streaming, or claim scene GPU binding.
...
- `mirakana::instantiate_runtime_scene_render_data`
- `mirakana::submit_scene_render_packet`
- static mesh, material, animation, morph, quaternion animation, UI atlas, collision, and scene package payloads
- host-built D3D12 scene shader artifacts when the selected package target is validated
- deterministic `NullRenderer` fallback
- `game.agent.json.runtimePackageFiles`
- `game.agent.json.runtimeSceneValidationTargets`
- `PACKAGE_FILES_FROM_MANIFEST`

The generated package proves manifest-derived file packaging, cooked scene loading, and selected Win32 D3D12 scene GPU binding counters. It does not claim runtime source parsing, broad dependency cooking, background package streaming, scene/physics gameplay execution, native UI overlay execution, postprocess, directional shadows, graphics or compute morph rendering, Vulkan renderer execution, Metal readiness, native/RHI handle exposure, general renderer quality, or broad generated 3D production readiness.
...
```powershell
out\install\desktop-runtime-release\bin\__TARGET_NAME__.exe --smoke --require-config runtime/__GAME_NAME__.config --require-scene-package runtime/__GAME_NAME__.geindex --require-d3d12-scene-shaders --video-driver windows --require-d3d12-renderer --require-scene-gpu-bindings
```
'@
    return $template.Replace("__TITLE__", $Title).Replace("__TARGET_NAME__", $TargetName).Replace("__GAME_NAME__", $GameName)
}
...
function New-DesktopRuntimeCookedSceneManifest {
    ...
    return [ordered]@{
        ...
        gameplayContract = [ordered]@{
            appType = "mirakana::GameApp"
            runner = "mirakana::Win32DesktopGameHost"
            currentRuntime = "desktop-windowed-win32-host-with-null-renderer-packaged-config-cooked-scene-smoke"
        }
        backendReadiness = [ordered]@{
            platform = "win32-desktop"
            graphics = "native-null-renderer; generated scaffold does not create D3D12/Vulkan/Metal shader artifacts or scene GPU bindings"
            audio = "device-independent"
            ui = "MK_ui-headless"
            physics = "not-required"
        }
...
function New-DesktopRuntime3DGameDesignSpec {
    ...
    return [ordered]@{
        ...
        systems = @("win32-desktop-host", "scene-rendering", "runtime-ui", "physics-3d", "navigation-navmesh", "behavior-ai", "animation", "package-streaming")
        packageTargets = @("desktop-game-runtime", "desktop-runtime-release")
        validationRecipeIds = @("desktop-game-runtime", "desktop-runtime-release-target", "installed-d3d12-3d-package-smoke")
        qualityGates = @(
            [ordered]@{
                id = "package-loads"
            ...
            [ordered]@{
                ...
                evidence = "Scene loading and host-gated Win32 D3D12 scene GPU binding counters are selected package evidence."
                recipeIds = @("installed-d3d12-3d-package-smoke")
            },
            [ordered]@{
                id = "collision-gameplay"
                evidence = "Collision gameplay execution remains unsupported on the Win32 3D scaffold surface."
                recipeIds = @("installed-d3d12-3d-package-smoke")
            }
        )
        unsupportedClaims = @("engine-internal-edits", "native-handles", "middleware-contracts", "unreviewed-external-assets", "arbitrary-shell", "broad-commercial-quality", "runtime-source-parsing", "renderer-rhi-residency", "metal-readiness")
    ...
    return [ordered]@{
        ...
        aiWorkflow = [ordered]@{
            ...
            contentMutationLedger = New-AiContentMutationLedger `
                ...
                -ValidationRecipeIds $desktop3dValidationRecipeIds
            placeholderAssetPipeline = New-AiPlaceholderAssetPipeline `
                -GameName $GameName `
                -PipelineKind "3d" `
                -ValidationRecipeIds $desktop3dValidationRecipeIds
        ...
        gameplayContract = [ordered]@{
            ...
            appType = "mirakana::GameApp"
            runner = "mirakana::Win32DesktopGameHost"
            input = "mirakana::VirtualInput deterministic smoke input over the cooked package scene"
            ...
            renderer = "mirakana::IRenderer from the Win32 desktop host with deterministic NullRenderer fallback and host-gated D3D12 scene GPU binding when selected"
            currentRuntime = "generated host-gated Win32 desktop runtime package proof for 3D package loading and D3D12 scene GPU binding. The scaffold emits cooked 3D package files, loads the packaged scene through first-party runtime package contracts, and can run the selected installed D3D12 scene binding smoke with Win32DesktopGameHost. Postprocess, native UI overlay, visible production proof, directional shadow, graphics/compute morph rendering, Vulkan renderer execution, and broad generated 3D production readiness are unsupported on this Win32 scaffold surface until first-party Win32 presentation contracts add those capabilities."
        }
        backendReadiness = [ordered]@{
            platform = "win32-desktop-host-gated"
            graphics = "Win32 D3D12 package scene GPU proof with host-built D3D12 DXIL shader artifacts; Vulkan SPIR-V artifacts are shader-artifact evidence only, postprocess and Vulkan renderer execution are unsupported on this Win32 package path, and NullRenderer fallback remains available"
            audio = "device-independent package metadata only"
            ui = "runtime UI package files may be present, but native UI overlay execution remains unsupported on the Win32 3D scaffold surface"
            physics = "first-party deterministic authored collision package metadata only; scene collision execution remains unsupported on the Win32 3D scaffold surface"
        }
        importerRequirements = [ordered]@{
            sourceFormats = @(
                "GameEngine.TextureSource.v1",
        ...
        packageStreamingResidencyTargets = @(
            [ordered]@{
                ...
                preflightRecipeIds = $desktop3dValidationRecipeIds
            }
        )
        ...
        prefabScenePackageAuthoringTargets = @(
            [ordered]@{
                ...
                hostGatedSmokeRecipeIds = $desktop3dValidationRecipeIds
        ...
        validationRecipes = @(
            ...
            [ordered]@{
                ...
                command = "out\install\desktop-runtime-release\bin\$TargetName.exe --smoke --require-config runtime/$GameName.config --require-scene-package runtime/$GameName.geindex --require-d3d12-scene-shaders --video-driver windows --require-d3d12-renderer --require-scene-gpu-bindings"
            }
        )
    }
}
