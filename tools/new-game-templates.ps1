...
function New-DesktopRuntime3DManifest {
    ...
    return [ordered]@{
        ...
        physics = "first-party deterministic authored-collision/controller package smoke, character/dynamic interaction policy counters, selected distance/fixed/linear-axis positional constraint counters, and selected GameEngine.PhysicsCollisionScene3D.v1 package collision smoke only; no middleware, native physics backend, dynamic-vs-dynamic TOI, rotational CCD, rotational rigid-body constraints, 2D CCD, persistent joint assets, or scene/physics perception integration"
        }
        importerRequirements = [ordered]@{
            sourceFormats = @(
                "GameEngine.TextureSource.v1",
