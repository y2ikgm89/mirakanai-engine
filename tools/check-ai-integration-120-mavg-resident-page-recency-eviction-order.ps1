...
# Chapter 120 for check-ai-integration.ps1 MAVG resident page recency eviction order evidence.
...
$mavgRecencyPlanText = Get-AgentSurfaceText "docs/superpowers/plans/2026-06-07-mavg-resident-page-recency-eviction-order-v1.md"
...
foreach ($surface in @(
        ...
        @{ Text = $mavgRecencyPlanText; Label = "docs/superpowers/plans/2026-06-07-mavg-resident-page-recency-eviction-order-v1.md" },
...
if ([string]$manifest.aiOperableProductionLoop.recommendedNextPlan.id -notin @(
        ...
        "2d-production-engine-capability-gap-cluster-v1"
    )) {
    foreach ($needle in @(
            "MAVG Resident Page Recency Eviction Order v1",
            "RuntimeMavgPageStreamingRecencyRow",
...
foreach ($needle in @(
        ...
        "duplicate_recency_row",
        "missing_recency_row",
        "no runtime-inferred LRU/frequency policy"
    )) {
