...
foreach ($cpuSimdGuidanceNeedle in @(
        ...
        "simd_dispatch_policy_selected_lane=scalar, sse2, or avx2",
        "simd_dispatch_policy_dot_product_result=120",
        "std::span<const float>",
        "raw pointers",
        "simd_dispatch_policy_reviewed_avx2_target_available",
        "gated simd_dispatch_policy_avx2_selected",
        "target-local reviewed AVX2 object",
        "CUDA",
        "HIP",
        "SYCL",
...
foreach ($jobSchedulingDiagnosticsGuidanceNeedle in @(
        ...
        "JobSchedulingExecutionOptions",
        "JobSchedulingExecutionOrderRow",
...
foreach ($jobExecutionWorkerPoolGuidanceNeedle in @(
        ...
        "dependency cycles",
        "task exceptions",
        "sample_desktop_runtime_game --require-job-execution-foundation",
...
foreach ($needle in @(
    ...
    "selected text atlas handoff evidence",
    "ITextShapingAdapter",
    "IFontRasterizerAdapter",
    "EditorDockCommandPlan",
    "plan_editor_dock_command",
    "apply_editor_dock_command",
    "EditorRichTextViewport",
    "make_editor_ai_command_panel_rich_text_document",
    "make_editor_inspector_rich_text_document",
    "make_editor_rich_text_ai_snapshot",
    "rich_text_rows",
    "Console diagnostics as read-only rich-text spans",
    "AI Commands panel status/command/evidence rows",
    "Inspector property rows as read-only rich-text spans",
    "editor.dock.panel.<id>.split",
    "source_stack_id",
    "new_stack_id",
    "split_axis",
    "split_ratio",
    ...
    "runtime_ui_editor is a visible native shell workspace panel",
    "Runtime UI Editor retained hierarchy/inspector/style-token/preview/platform-readiness/clean-room rows",
    "editor_runtime_ui_editor_panel_visible=1",
    "editor_runtime_ui_editor_hierarchy_rows",
    "editor_runtime_ui_editor_inspector_rows",
    "editor_runtime_ui_editor_style_rows",
    "editor_runtime_ui_editor_preview_rows",
    "editor_runtime_ui_editor_external_engine_parity_claim=0",
    "editor_runtime_ui_editor_native_handles_exposed=0",
    "editor_shell_sdl3=0",
    ...
    "editor_shell_text_atlas_handoff_status=glyphs_ready_atlas_handoff_host_gated",
    "editor_shell_text_font_adapter_invoked=1",
    "editor_shell_text_font_glyphs_ready=1",
    "editor_shell_text_font_fallback_used=0",
    "editor_shell_text_atlas_handoff_host_gated_rows=1",
    "editor_shell_text_atlas_handoff_unsupported_rows=1",
    "editor_shell_text_font_native_handles_exposed=0",
    "private Windows TSF text-input/IME session selection",
    "editor_shell_ime_status=win32_tsf_selected",
    "editor_shell_ime_caret_rect_rows=1",
    "editor_shell_ime_surrounding_text_rows=1",
    "editor_shell_ime_native_handles_exposed=0",
...
Assert-Properties $engine.commands @("validate", "prepareWorktree", "postMergeTaskCleanup", "removeMergedWorktree", "toolchainCheck", "directToolchainCheck", "bootstrapDeps", "build", "buildEditor", "buildAssetImporters", "validatePhysicsJolt", "validateNetworkEnet", "test", "dependencyCheck", "cppStandardCheck", "evaluateCpp23", "shaderToolchainCheck", "agentContext", "agentCheck", "newGame", "ciMatrixCheck", "classifyPrValidationTier") "engine manifest commands"
$postMergeTaskCleanupCmd = "pwsh -NoProfile -ExecutionPolicy Bypass -File tools/post-merge-task-cleanup.ps1 -WorktreePath <path> [-HeadRefOid <headRefOid>] [-BaseRef origin/main] [-BaseBranch main] [-Remote origin] [-LocalCheckoutPath <path>]"
if ($engine.commands.postMergeTaskCleanup -ne $postMergeTaskCleanupCmd) {
    Write-Error "engine manifest commands.postMergeTaskCleanup must expose the preferred post-merge cleanup wrapper command"
}
...
if ($optionalGpuComputeHostGate.Count -ne 1 -or $optionalGpuComputeHostGate[0].status -ne "ready" -or $optionalGpuComputeHostGate[0].residualClass -ne "ready") { Write-Error "engine manifest aiOperableProductionLoop must expose one ready optional-gpu-compute-review-host gate" }
...
foreach ($needle in @("tools/collect-optional-gpu-compute-review-evidence.ps1", "-Mode Import", "tools/check-optional-gpu-compute-review-evidence.ps1", "-EvidenceRoot", "-RequiredCandidateIds", "-RequireReady", "tools/validate-optional-gpu-compute-review-host-gate.ps1")) { Assert-ContainsText ([string]$optionalGpuComputeRecipe[0].command) $needle "engine manifest validationRecipes host-optional-gpu-compute-review command" }
...
Assert-ContainsText ([string]$engine.gameCodeGuidance.currentOptionalGpuComputeReview) "tools/collect-optional-gpu-compute-review-evidence.ps1" "engine manifest gameCodeGuidance.currentOptionalGpuComputeReview"
