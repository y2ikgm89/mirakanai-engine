...
if ([string]$productionLoop.recommendedNextPlan.id -ne "general-purpose-game-production-v1") {
    if ([string]$productionLoop.recommendedNextPlan.id -eq "generated-game-studio-v1") {
        ...
        foreach ($needle in @("First-Party UI Editor Excellence v1", "MK_editor", "MK_editor_core", "mirakana::ui", "MK_ui_renderer", "evidence-first editor/UI optimization", "EditorUiPerformanceBudget", "RetainedUiSnapshot", "diff_retained_ui_snapshots", "actual RendererSubmission payload row keys", "renderer_submission_order_key", "glyph_atlas_binding_reuse_rows", "image_binding_reuse_rows", "ui_retained_diff_status=ready", "ui_retained_dirty_rows=0", "ui_retained_layout_cache_hits", "ui_retained_text_cache_hits", "ui_retained_submission_reused_rows", "ui_retained_submission_rebuilt_rows=0", "ui_retained_cache_native_handle_access=0", "editor_ui_performance_budget_status=ready", "editor_ui_performance_diagnostics=0", "editor_ui_performance_broad_optimization_claimed=0", "multi-window docking", "GameEngine.Workspace.v3", "EditorDockMultiWindowLayout", "editor_shell_multi_window_docking_status=ready", "editor.ai.dock.workspace_v3", "EditorRichTextEditCommandKind", "apply_editor_rich_text_edit_command", "editor_rich_text_edit_status=ready", "<rich_text_document_id>.insert_text", "<rich_text_document_id>.paste_rich_text", "first-party document/AI command/shell-control level", "TextInputParityEvidenceRequest", "NativeEditorTsfTextStoreEvidence", "ITextStoreACP", "editor_ime_parity_status=ready", "editor_ime_windows_tsf_status=ready", "editor_ime_macos_status=host_gated", "editor.ai.ime.parity", "NativeEditorUiaProviderState", "editor_accessibility_parity_status=ready", "editor_accessibility_windows_uia_patterns_ready=1", "editor_accessibility_windows_uia_events_ready=1", "editor_accessibility_linux_at_spi_status=host_gated", "editor.ai.accessibility.parity", "visible OS-level multi-window drag/drop shell restoration", "text shaping", "font fallback", "glyph atlas", "IME parity", "accessibility parity", "external OS accessibility-tool", "Phase 8 host/toolchain-gated Vulkan editor texture-display counters", "Phase 9 Apple-host-gated Metal editor texture-display counters", "default visible-shell Vulkan/Metal backend selection", "cross-platform editor shell adapters", "AI-operable command expansion", "editor_ai_operation_excellence_status=ready", "editor.ai.operation.excellence", "editor.text_font.diagnostics.copy", "missing_expected_revision", "package_script_execution_unsupported", "file_mutation_unsupported", "Official docs and Context7", "license-audit", "THIRD_PARTY_NOTICES.md", "unsupportedProductionGaps = []", "no Dear ImGui", "no SDL3", "no UI middleware", "no native handle exposure", "no broad optimization claim", "no broad first_party_editor_excellence claim")) { Assert-ContainsText $recommendedText $needle "engine manifest aiOperableProductionLoop recommendedNextPlan first-party UI editor excellence selection" }
