...
if ([string]$productionLoop.recommendedNextPlan.id -ne "general-purpose-game-production-v1") {
    ...
    } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "environment-production-excellence-v1") { foreach ($needle in @("Environment Production Excellence v1", "Phase 2 is complete", "Phase 4 is complete", "Phase 5 precipitation candidate", "EnvironmentProfile.v2", "local environment volumes", "runtime cubemap capture", "renderer IBL upload", "material wetness", "snow accumulation", "weather audio playback", "desktop-runtime-sample-game-environment-audio-playback", "environment_audio_playback_status=ready", "environment_audio_trigger_rows=4", "environment_precipitation_audio_playback=0", "desktop-runtime-sample-game-vulkan-environment-precipitation-renderer-execution", "environment_precipitation_vulkan_status=ready", "physical WASAPI endpoint playback", "strict Vulkan", "Apple-host Metal", "quality budgets", "exact environment_ready", "official docs/Context7", "OpenEXR/KTX/Basis asset pipeline", "broad renderer quality", "broad CPU/GPU/memory optimization", "all-platform unconditional parity", "unsupportedProductionGaps = []", "Dear ImGui", "SDL3", "public native handles", "no inferred Vulkan/Metal readiness", "no broad optimization", "no broad environment_ready")) { Assert-ContainsText $recommendedText $needle "engine manifest aiOperableProductionLoop recommendedNextPlan environment production excellence selection" }
