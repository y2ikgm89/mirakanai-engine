...
if ([string]$productionLoop.recommendedNextPlan.id -ne "general-purpose-game-production-v1") {
    ...
    } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-runtime-lod-milestone-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-package-streaming-residency-dispatch-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-page-addressable-payload-schema-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-directstorage-request-plan-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-native-directstorage-win32-async-io-dispatch-status-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-win32-async-file-io-adapter-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-win32-iocp-file-io-worker-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-directstorage-sdk-dependency-gate-v1") { foreach ($needle in @("MAVG Runtime LOD Milestone v1", "MAVG Asset Graph v1", "MAVG Phase 0 completed", "GameEngine.MavgClusterGraph.v1", "GameEngine.MavgClusterPayload.v1", "vertex.data_hex", "index.data_hex", "page.data_hex", "mavg_cluster_payload.hpp", "MavgClusterPayloadDocument", "MavgClusterPayloadPage", "validate_mavg_cluster_payload", "mavg_payload_pages.hpp", "RuntimeMavgPayloadPageSliceResult", "extract_runtime_mavg_payload_page_slices", "RuntimeMavgPageStreamingDispatchPlan", "plan_runtime_mavg_page_streaming_dispatches", "RuntimeMavgPageStreamingDrainDesc", "GPU culling", "indirect draw execution", "mesh shaders", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $recommendedText $needle "engine manifest aiOperableProductionLoop recommendedNextPlan MAVG runtime LOD selection" } } else {
        foreach ($needle in @(
