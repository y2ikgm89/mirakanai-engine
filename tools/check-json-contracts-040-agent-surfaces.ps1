...
if ([string]$productionLoop.recommendedNextPlan.id -ne "general-purpose-game-production-v1") {
    ...
    } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "environment-settings-productization-v1") { foreach ($needle in @("Environment Settings Productization v1", "Environment System v1", "Environment Production Excellence", "EnvironmentProfile.v2", "dedicated Environment Settings panel", "workflow rows", "reviewed preview/package handoff", "environment_settings_productized", "official docs/Context7", "broad environment_ready", "backend parity", "broad optimization", "Dear ImGui", "SDL3", "native-handle exposure", "unreviewed package/script execution")) { Assert-ContainsText $recommendedText $needle "engine manifest aiOperableProductionLoop recommendedNextPlan environment settings productization selection" }
