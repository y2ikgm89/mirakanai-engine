...
if ([string]$productionLoop.recommendedNextPlan.id -ne "general-purpose-game-production-v1") {
    ...
    } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-runtime-lod-milestone-v1") { foreach ($needle in @("MAVG Runtime LOD Milestone v1", "MAVG Asset Graph v1", "MAVG Phase 0 completed", "hierarchy/error/fallback", "MavgClusterGraphCluster", "resident_fallback_cluster_index", "geometric_error", "first_index", "index_count", "vertex_base", "GameEngine.MavgClusterGraph.v1", "CPU LOD selection", "select_mavg_lod_clusters", "range-aware conventional indexed draws", "GPU culling", "indirect draw execution", "mesh shaders", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $recommendedText $needle "engine manifest aiOperableProductionLoop recommendedNextPlan MAVG runtime LOD selection" }
