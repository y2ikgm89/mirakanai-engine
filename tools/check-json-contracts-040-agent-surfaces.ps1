...
$developerBacklogText = Get-JsonContractSurfaceText "docs/superpowers/master-plans/production-completion-v1/04-developer-owned-engine-capability-backlog.md"; if ([regex]::IsMatch($developerBacklogText, '(?m)^\| `[^`]+` \| `[^`]+` \| `active-selected-slice` \|')) { Write-Error "production-completion capability backlog must not use stale active-selected-slice row status" }
if ($productionLoop.currentActivePlan -eq "docs/superpowers/master-plans/2026-05-03-production-completion-master-plan-v1.md" -and [regex]::IsMatch($developerBacklogText, '(?m)^\| `[^`]+` \| `[^`]+` \| `selected-production-slice` \|')) { Write-Error "production-completion capability backlog must not keep selected-production-slice rows at the master selection gate" }
...
if ([string]$productionLoop.recommendedNextPlan.id -ne "general-purpose-game-production-v1") {
    ...
    } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "renderer-backend-parity-apple-evidence-closeout-v1") { foreach ($needle in @("Renderer Backend Parity Apple Evidence Closeout v1", "renderer-backend-parity-v1", "renderer-metal-apple-host-evidence", "Apple/Metal host evidence", "Windows/Vulkan proof must not promote Metal readiness", "native handles remain hidden", "unsupportedProductionGaps = []")) { Assert-ContainsText $recommendedText $needle "engine manifest aiOperableProductionLoop recommendedNextPlan renderer backend parity Apple evidence closeout selection" }
    ...
    } else { foreach ($needle in @("Frame Graph Transient Texture Alias Planning v1", "FrameGraphTransientTextureAliasPlan", "plan_frame_graph_transient_texture_aliases", "Frame Graph Backend-Neutral Distinct Alias-Group Lease Binding v1", "FrameGraphTransientTextureLeaseBindingResult", "IRhiDevice::acquire_transient_texture_alias_group", "acquire_frame_graph_transient_texture_lease_bindings", "Frame Graph Shared Texture State Handoff v1", "conflicting initial shared-handle states", "Frame Graph RHI Pass Target Access Validation v1", "FrameGraphTexturePassTargetAccess", "build_frame_graph_texture_pass_target_accesses", "RhiPostprocessFrameRenderer scene pass command recording", "Frame Graph Shadow Scratch Color Target-State Ownership v1", "shadow_color", "6 pass callbacks/15 barrier steps", "Frame Graph Viewport Surface Color State Executor v1", "RhiViewportSurface", "viewport_color", "Frame Graph Texture Aliasing Barrier Command v1", "Frame Graph D3D12 Texture Aliasing Barrier Evidence v1", "null_resource_aliasing_barriers", "record_frame_graph_texture_aliasing_barriers", "Frame Graph Automatic Aliasing Barrier Insertion v1", "Package Streaming Frame Graph Texture Binding Handoff v1", "make_runtime_package_streaming_frame_graph_texture_bindings", "Runtime Package Streaming RHI Upload Binding Transaction v1", "upload_runtime_package_streaming_frame_graph_texture_bindings", "Package Static Mesh Upload Binding Transaction v1", "upload_runtime_package_streaming_mesh_gpu_bindings", "Runtime Upload Queue Wait v1", "wait_for_runtime_uploads_on_queue", "upload_queue_waits_recorded", "Frame Graph Render Pass Envelope v1", "render_passes_recorded", "Frame Graph RHI Queue Dependency Plan v1", "plan_frame_graph_rhi_queue_waits", "IRhiDevice::wait_for_queue", "Frame Graph RHI Multi-Queue Executor v1", "execute_frame_graph_rhi_multi_queue_schedule", "Frame Graph RHI Multi-Queue Texture Barrier Execution v1", "FrameGraphRhiMultiQueueExecutionResult::barriers_recorded", "Frame Graph Remaining Render Pass Envelopes v1", "Frame Graph v1 1.0 Scope Closeout v1 closes frame-graph-v1", "upload-staging-v1", "scene-component-prefab-schema-v2", "refresh-prefab-instance", "2d-playable-vertical-slice", "3d-playable-vertical-slice")) { if (-not $recommendedText.Contains($needle)) { Write-Error "engine manifest aiOperableProductionLoop recommendedNextPlan must describe frame-graph closeout and upload-staging next gap: $needle" } }
    }
}
