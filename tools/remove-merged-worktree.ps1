...
if ($record.Branch -in @("main", "master")) {
...
if ($mergeBaseResult.ExitCode -eq 1) {
    Write-Error "Refusing to remove worktree because branch '$($record.Branch)' is not fully merged into '$BaseRef'. Run git fetch --prune origin or use the correct -BaseRef after the PR is merged."
}
