...
if ($null -eq $sample3dManifestEntry) {
    ...
    foreach ($needle in @(
        ...
        "--require-simd-dispatch-policy", "simd_dispatch_policy_status=ready", "simd_dispatch_policy_requested_lane=auto_select", "simd_dispatch_policy_dot_product_result=120", "simd_dispatch_policy_avx2_selected=0",
    ...
    foreach ($needle in @('$requiresSimdDispatchPolicy', '"simd_dispatch_policy_status" = "ready"', '"simd_dispatch_policy_ready" = "1"', '"simd_dispatch_policy_requested_lane" = "auto_select"', '"simd_dispatch_policy_input_count" = "8"', '"simd_dispatch_policy_dot_product_result" = "120"', '"simd_dispatch_policy_avx2_selected" = "0"', '"simd_dispatch_policy_span_inputs_used" = "1"', '"simd_dispatch_policy_raw_pointers_retained" = "0"', '"simd_dispatch_policy_native_handles_exposed" = "0"', '"simd_dispatch_policy_numa_allocation_applied" = "0"', '"simd_dispatch_policy_gpu_async_overlap_applied" = "0"', '"simd_dispatch_policy_cuda_path_used" = "0"', '"simd_dispatch_policy_hip_path_used" = "0"', '"simd_dispatch_policy_sycl_path_used" = "0"')) { if (-not $sample3dInstalledValidatorText.Contains($needle)) { Write-Error "tools/validate-installed-desktop-runtime.ps1 missing SIMD dispatch policy assertion: $needle" } }
    $sceneRendererHeaderText = Get-Content -LiteralPath (Join-Path $root "engine/scene_renderer/include/mirakana/scene_renderer/scene_renderer.hpp") -Raw
    foreach ($needle in @(
        "SceneMeshDrawPlan",
