...
$gamesCMakeText = Get-Content -LiteralPath (Join-Path $root "games/CMakeLists.txt") -Raw
foreach ($retiredDesktopRuntimeSample in @(
    "sample_desktop_runtime_game",
    "sample_generated_desktop_runtime_3d_package"
)) {
    $retiredDesktopRuntimeSamplePath = Join-Path $root "games/$retiredDesktopRuntimeSample"
    if (Test-Path -LiteralPath $retiredDesktopRuntimeSamplePath) {
        Write-Error "retired SDL3 desktop runtime sample must be deleted: games/$retiredDesktopRuntimeSample"
    }
    if ($desktopRuntimeGameRegistrations.ContainsKey($retiredDesktopRuntimeSample)) {
        Write-Error "retired SDL3 desktop runtime sample must not be registered with MK_add_desktop_runtime_game: $retiredDesktopRuntimeSample"
    }
    if ($gamesCMakeText.Contains($retiredDesktopRuntimeSample)) {
        Write-Error "games/CMakeLists.txt must not retain retired SDL3 desktop runtime sample wiring: $retiredDesktopRuntimeSample"
    }
}
