...
if ($null -eq $sample3dManifestEntry) {
    ...
    foreach ($needle in @(
        ...
        "--require-job-scheduling-evidence",
        "--require-job-execution-foundation", "--require-job-execution-topology-policy",
    ...
    foreach ($needle in @("--require-job-scheduling-evidence", "--require-job-execution-foundation", "--require-job-execution-topology-policy", "sample_desktop_runtime_game")) { if (-not $sample3dCMakeText.Contains($needle)) { Write-Error "games/CMakeLists.txt missing sample_desktop_runtime_game job scheduling/execution smoke evidence: $needle" } }
