...
foreach ($vulkanGuidanceNeedleSet in @(@($vulkanHeightFogArtifactNeedles + @("height-fog runtime readback")), @($vulkanPhysicalSkyArtifactNeedles + @("physical-sky runtime readback")))) { foreach ($needle in $vulkanGuidanceNeedleSet) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentVulkan) $needle "Vulkan game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentVulkanRuntimeOwners) $needle "Vulkan runtime owner guidance" } }
foreach ($needle in @("Vulkan env-gated height-fog runtime readback proof", "Vulkan env-gated physical-sky runtime readback proof")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRendering) $needle "rendering game guidance" }; foreach ($needle in @("strict env-gated Vulkan height-fog runtime readback evidence", "strict env-gated Vulkan physical-sky runtime readback evidence")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; foreach ($needle in @("environment.readiness.physical_sky.package_status", "environment.readiness.backend.metal_status", "environment.readiness.unsupported.environment_ready")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentEditorAuthoringTask9) $needle "environment editor authoring task9 game guidance" }; foreach ($needle in @("environment_quality_budget_status=ready", "environment_quality_budget_transient_gpu_byte_estimate", "environment_quality_budget_framegraph_barrier_steps_executed", "environment_quality_budget_broad_environment_ready_claimed=0")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentQualityBudgetTask10) $needle "environment quality budget task10 game guidance" }
