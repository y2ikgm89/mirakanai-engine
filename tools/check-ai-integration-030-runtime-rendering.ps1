...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-runtime-lod-milestone-v1") { $mavgRuntimeLodRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG Runtime LOD Milestone v1", "static conventional MAVG LOD readiness", "MavgClusterGraphCluster", "resident_fallback_cluster_index", "geometric_error", "first_index", "index_count", "vertex_base", "GameEngine.MavgClusterGraph.v1", "MavgClusterCookVertex", "GameEngine.MavgClusterPayload.v1", "vertex.data_hex", "index.data_hex", "per-material root/leaf", "CPU select_mavg_lod_clusters", "CPU LOD selection", "range-aware conventional indexed draws", "MeshCommand submission", "GPU culling", "indirect draw execution", "mesh shaders", "Metal readiness", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgRuntimeLodRecommendedText $needle "recommended next plan MAVG runtime LOD selection reason" }
} else {
