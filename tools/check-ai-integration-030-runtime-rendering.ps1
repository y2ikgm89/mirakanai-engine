...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-async-overlap-performance-proof-v1") {
    $mavgAsyncOverlapRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8
    foreach ($needle in @("MAVG Async Overlap Performance Proof v1", "RuntimeMavgStreamingUploadOverlapEvidenceResult", "RuntimeMavgAsyncOverlapPerformanceProofResult", "mavg_async_overlap_performance_proof_ready=1", "GPU DirectStorage destinations", "GDeflate", "DirectStorage 1.4 preview", "mesh shaders", "Metal readiness", "Nanite", "native handles", "broad optimization", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgAsyncOverlapRecommendedText $needle "recommended next plan MAVG async overlap performance proof reason" }
