...
Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEditorProfilerTraceExport) "retired file-dialog bridge" "editor profiler native trace open dialog guidance"
...
Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEditorPrefabVariantFileDialogs) "retired file-dialog bridge" "editor prefab variant native dialog guidance"
...
Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEditorSceneFileDialogs) "retired file-dialog bridge" "editor scene native dialog guidance"
...
Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEditorProjectFileDialogs) "retired file-dialog bridge" "editor project native dialog guidance"
...
$retiredSdl3DesktopRuntimePackageProofs = Test-RetiredSdl3DesktopRuntimeSamplePath "games/sample_desktop_runtime_game/main.cpp"
...
foreach ($needle in $postprocessPolicyNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRendering) $needle "rendering game guidance"; if (-not $retiredSdl3DesktopRuntimePackageProofs) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRuntime) $needle "runtime game guidance" } }
...
foreach ($needle in $d3d12PostprocessExecutionGuidanceNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRendering) $needle "rendering game guidance"; if (-not $retiredSdl3DesktopRuntimePackageProofs) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRuntime) $needle "runtime game guidance" } }
...
if (-not $retiredSdl3DesktopRuntimePackageProofs) {
...
foreach ($lightingShadowPolicyFlagGuidance in @("tools/validate-installed-desktop-runtime.ps1")) { Assert-ContainsText (Get-AgentSurfaceText $lightingShadowPolicyFlagGuidance) "--require-lighting-shadow-policy" $lightingShadowPolicyFlagGuidance }
