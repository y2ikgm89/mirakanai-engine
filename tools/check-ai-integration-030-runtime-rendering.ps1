...
Assert-ContainsText ([string]$geRendererModule[0].purpose) "RhiPostprocessFrameRenderer scheduled inter-pass plus post-chain target-state and scene-depth final-state transition adoption" "MK_renderer module purpose"
Assert-ContainsText ([string]$geRendererModule[0].purpose) "RhiDirectionalShadowSmokeFrameRenderer scheduled shadow-depth/scene-color/scene-depth inter-pass plus writer target-state and scene-depth/shadow-depth final-state transition adoption" "MK_renderer module purpose"
