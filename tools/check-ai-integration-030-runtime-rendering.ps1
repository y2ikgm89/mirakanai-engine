...
foreach ($needle in $d3d12PostprocessExecutionNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRendering) $needle "rendering game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRuntime) $needle "runtime game guidance" }
...
foreach ($needle in @("evaluate_sdl_desktop_presentation_d3d12_postprocess_execution", "postprocess_d3d12_execution_status")) { Assert-ContainsText $sampleDesktopRuntimePostprocessPolicyText $needle "games/sample_desktop_runtime_game/main.cpp" }
...
foreach ($sceneScaleNeedle in @("Scene Scale Policy v1", "SceneScalePolicyDesc", "SceneScalePolicyPlan", "SceneScaleBatchingMode", "plan_scene_scale_policy", "backend instancing-evidence", "performance-measurement", "MeshCommand::instance_count", "SceneRenderSubmitDesc::mesh_instance_count", "RhiStats", "instanced_draw_calls", "instanced_indexed_draw_calls", "instanced_instances_submitted")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRendering) $sceneScaleNeedle "rendering game guidance" }
