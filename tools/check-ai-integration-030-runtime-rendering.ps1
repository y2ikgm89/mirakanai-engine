...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-rhi-indirect-draw-v1") { $mavgRhiIndirectRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG RHI Indirect Draw v1", "MAVG Runtime LOD Milestone v1", "MAVG GPU Culling Indirect", "indirect_draw.hpp", "IndexedIndirectDrawCommand", "IndexedIndirectDrawDesc", "BufferUsage::indirect", "IRhiCommandList::draw_indexed_indirect", "RhiStats", "Null RHI", "Vulkan VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT", "D3D12 ExecuteIndirect", "Vulkan indirect draw execution", "actual GPU culling dispatch", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgRhiIndirectRecommendedText $needle "recommended next plan MAVG RHI indirect draw selection reason" }
} else {
