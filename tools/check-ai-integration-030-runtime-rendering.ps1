...
foreach ($needle in @("RHI Depth Attachment Contract v0", "GPU Memory Policy v1", "GpuMemoryPolicyPlan", "GpuMemoryResidencyClass", "plan_gpu_memory_policy", "Scene Scale Policy v1", "SceneScalePolicyPlan", "SceneScaleBatchingMode", "plan_scene_scale_policy", "Postprocess Chain Policy v1", "PostprocessChainPolicyPlan", "plan_postprocess_chain_policy", "Lighting Shadow Policy v1", "LightingShadowPolicyPlan", "plan_lighting_shadow_policy", "Stable Directional Light-Space Policy v0", "DirectionalShadowLightSpacePlan")) {
    Assert-ContainsText ([string]$geRendererModule[0].purpose) $needle "MK_renderer module purpose"
}
foreach ($rendererPolicyHeader in @("postprocess_policy.hpp", "gpu_memory_policy.hpp", "scene_scale_policy.hpp")) { if (@($geRendererModule[0].publicHeaders) -notcontains "engine/renderer/include/mirakana/renderer/$rendererPolicyHeader") { Write-Error "engine/agent/manifest.json MK_renderer publicHeaders must include $rendererPolicyHeader" } }
