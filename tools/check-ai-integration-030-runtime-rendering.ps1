...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "environment-rendering-readiness-v1") { $environmentRenderingRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("Environment Rendering Readiness v1", "Environment System v1", "official docs/Context7", "snow package readiness", "physical-sky package/Vulkan proof", "cloud/precipitation renderer execution", "volumetric-cloud execution/package readiness", "height/volumetric-fog package proof", "environment lighting/IBL package proof", "later renderer upload/runtime-capture proof", "D3D12", "strict Vulkan", "Metal host-gated", "MetalEnvironmentFeatureHostEvidencePlan", "renderer-metal-apple-host-evidence", "environment.readiness.physical_sky.package_status", "environment.readiness.backend.metal_status", "environment.readiness.unsupported.environment_ready", "broad optimization", "broad environment_ready", "native handles", "Dear ImGui", "SDL3", "unsupportedProductionGaps = []")) { Assert-ContainsText $environmentRenderingRecommendedText $needle "recommended next plan environment rendering readiness selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "environment-production-excellence-v1") { $environmentProductionRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("Environment Production Excellence v1", "Phase 2 is complete", "Phase 5 physical-sky package candidate", "desktop-runtime-sample-game-vulkan-physical-sky-package", "environment_physical_sky_vulkan_status=ready", "EnvironmentProfile.v2", "local environment volumes", "runtime cubemap capture", "renderer IBL upload", "material wetness", "snow accumulation", "weather audio playback", "strict Vulkan", "Apple-host Metal", "quality budgets", "exact environment_ready", "official docs/Context7", "OpenEXR/KTX/Basis asset pipeline", "broad renderer quality", "broad CPU/GPU/memory optimization", "all-platform unconditional parity", "unsupportedProductionGaps = []", "Dear ImGui", "SDL3", "public native handles", "no inferred Vulkan/Metal readiness", "no broad optimization", "no broad environment_ready")) { Assert-ContainsText $environmentProductionRecommendedText $needle "recommended next plan environment production excellence selection reason" }
...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-runtime-lod-milestone-v1") { $mavgRuntimeLodRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG Runtime LOD Milestone v1", "static conventional MAVG LOD readiness", "MavgClusterGraphCluster", "resident_fallback_cluster_index", "geometric_error", "first_index", "index_count", "vertex_base", "GameEngine.MavgClusterGraph.v1", "MavgClusterCookVertex", "GameEngine.MavgClusterPayload.v1", "vertex.data_hex", "index.data_hex", "per-material root/leaf", "CPU select_mavg_lod_clusters", "CPU LOD selection", "mavg_lod_selection.hpp", "MavgLodViewDesc", "MavgLodResidentPageSet", "MavgLodSelectionResult", "MK_mavg_lod_selection_tests", "MavgLodPageRequest", "mavg_lod_residency.hpp", "RuntimeMavgLodResidencyDesc", "RuntimeMavgLodResidencyResult", "build_runtime_mavg_lod_residency", "MK_runtime_mavg_lod_residency_tests", "range-aware conventional indexed draws", "MeshCommand submission", "GPU culling", "indirect draw execution", "mesh shaders", "Metal readiness", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgRuntimeLodRecommendedText $needle "recommended next plan MAVG runtime LOD selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-page-streaming-eviction-review-v1") { $mavgEvictionReviewRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG Page Streaming Eviction Review v1", "RuntimeMavgResidentPageMountRow", "RuntimeMavgPageStreamingSelectedClusterRow", "RuntimeMavgPageStreamingEvictionReviewResult", "review_runtime_mavg_page_streaming_evictions", "plan_runtime_resident_package_evictions_v2", "selected/fallback page protection", "automatic eviction policy", "autonomous/background package streaming execution", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgEvictionReviewRecommendedText $needle "recommended next plan MAVG page streaming eviction review selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-rhi-indirect-draw-v1") { $mavgRhiIndirectRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG RHI Indirect Draw v1", "MAVG Runtime LOD Milestone v1", "MAVG GPU Culling Indirect", "indirect_draw.hpp", "IndexedIndirectDrawCommand", "IndexedIndirectDrawDesc", "BufferUsage::indirect", "IRhiCommandList::draw_indexed_indirect", "RhiStats", "Null RHI", "Vulkan VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT", "D3D12 ExecuteIndirect", "Vulkan indirect execution", "actual GPU dispatch", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgRhiIndirectRecommendedText $needle "recommended next plan MAVG RHI indirect draw selection reason" }
} else {
...
if ([string]$productionLoop.recommendedNextPlan.id -eq "environment-production-excellence-v1") { $environmentProductionMetalRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("Phase 6 Metal candidate", "renderer-metal-apple-host-evidence", "environment_feature_evidence.metallib", "metal_environment_status=ready", "metal_environment_render_pipeline_ready=1", "metal_environment_compute_pipeline_ready=1", "metal_environment_synchronization_evidence_ready=1", "seven ready feature rows", "zero native-handle access", "broad Metal readiness outside the selected Apple-host recipe")) { Assert-ContainsText $environmentProductionMetalRecommendedText $needle "recommended next plan environment production Metal Phase 6 evidence" } }; $environmentProductionMetalGuidanceText = [string]$manifest.gameCodeGuidance.currentEnvironmentProductionMetalPhase6; foreach ($needle in @("Environment Production Excellence v1 Phase 6", "renderer-metal-apple-host-evidence", "metal_environment_status=ready", "metal_environment_selected_backend=metal", "metal_environment_render_pipeline_ready=1", "metal_environment_compute_pipeline_ready=1", "metal_environment_depth_texture_ready=1", "metal_environment_particle_buffer_ready=1", "metal_environment_synchronization_evidence_ready=1", "metal_environment_render_readback_nonzero=1", "metal_environment_compute_readback_nonzero=1", "metal_environment_ready_rows=7", "metal_environment_native_handle_access=0", "metal_environment_broad_environment_ready_claimed=0")) { Assert-ContainsText $environmentProductionMetalGuidanceText $needle "game guidance currentEnvironmentProductionMetalPhase6" } $metalModule = @($manifest.modules | Where-Object { $_.name -eq "MK_rhi_metal" }); if ($metalModule.Count -ne 1) { Write-Error "engine/agent/manifest.json must expose exactly one MK_rhi_metal module" } foreach ($needle in @("synchronization_evidence_required", "metal_environment_feature_host_evidence_status_line", "MetalNativeEnvironmentFeatureHostEvidenceDesc/Result", "create_native_environment_feature_host_evidence", "environment_feature_evidence.metallib", "metal_environment_* counters")) { Assert-ContainsText ([string]$metalModule[0].purpose) $needle "MK_rhi_metal module purpose" } foreach ($metalEnvironmentContractCheck in @(@("engine/rhi/metal/include/mirakana/rhi/metal/metal_backend.hpp", @("synchronization_evidence_required", "synchronization_evidence_ready", "metal_environment_feature_host_evidence_status_line", "MetalNativeEnvironmentFeatureHostEvidenceDesc", "MetalNativeEnvironmentFeatureHostEvidenceResult", "create_native_environment_feature_host_evidence")), @("engine/rhi/metal/src/metal_backend.cpp", @("metal_environment_status=", "metal_environment_selected_backend=metal", "metal_environment_feature_host_evidence_status_line", "synchronization_evidence_ready", "metal_environment_broad_environment_ready_claimed=0", "feature-local pipeline")), @("engine/rhi/metal/src/metal_native.mm", @("newLibraryWithURL", "newRenderPipelineStateWithDescriptor", "newComputePipelineStateWithFunction", "textureCubeDescriptorWithPixelFormat", "MTLPixelFormatDepth32Float", "dispatchThreads", "copyFromTexture", "waitUntilCompleted")), @("engine/rhi/metal/shaders/environment_feature_evidence.metal", @("vertex VertexOut", "fragment float4", "texturecube<float>", "kernel void", "device uint")), @("engine/rhi/metal/CMakeLists.txt", @("environment_feature_evidence.metal", "environment_feature_evidence.metallib", "MK_metal_environment_evidence_metallib", "xcrun", "metallib")), @("CMakeLists.txt", @("MK_metal_environment_evidence_metallib", "MK_METAL_ENVIRONMENT_EVIDENCE_METALLIB")), @("tools/validate-renderer-metal-apple.ps1", @("MK_metal_environment_evidence_metallib", "metal_environment_status=ready", "metal_environment_render_pipeline_ready=1", "metal_environment_compute_pipeline_ready=1", "metal_environment_synchronization_evidence_ready=1", "metal_environment_native_handle_access=0")), @("tools/run-validation-recipe-plans.ps1", @("environment_feature_evidence.metallib", "metal_environment_* counters", "synchronization/readback")))) { $metalEnvironmentContractText = Get-AgentSurfaceText ([string]$metalEnvironmentContractCheck[0]); foreach ($needle in @($metalEnvironmentContractCheck[1])) { Assert-ContainsText $metalEnvironmentContractText $needle ([string]$metalEnvironmentContractCheck[0]) } } foreach ($metalEnvironmentGuidance in @("docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md", "docs/superpowers/plans/README.md", "docs/superpowers/plans/2026-06-06-environment-production-excellence-v1.md", ".agents/skills/rendering-change/references/full-guidance.md", ".claude/skills/gameengine-rendering/references/full-guidance.md")) { $metalEnvironmentGuidanceText = Get-AgentSurfaceText $metalEnvironmentGuidance; foreach ($needle in @("renderer-metal-apple-host-evidence", "environment_feature_evidence.metallib", "metal_environment_*", "native-handle", "environment_ready")) { Assert-ContainsText $metalEnvironmentGuidanceText $needle $metalEnvironmentGuidance } }
foreach ($needle in @("RHI Depth Attachment Contract v0", "GPU Memory Policy v1", "GpuMemoryPolicyPlan", "GpuMemoryResidencyClass", "plan_gpu_memory_policy", "Debug Profiling Policy v1", "DebugProfilingPolicyPlan", "plan_debug_profiling_policy", "Scene Scale Policy v1", "SceneScalePolicyPlan", "SceneScaleBatchingMode", "plan_scene_scale_policy", "Postprocess Chain Policy v1", "PostprocessChainPolicyPlan", "plan_postprocess_chain_policy", "Postprocess Tone Mapping Evidence v1", "PostprocessToneMappingEvidencePlan", "plan_postprocess_tone_mapping_evidence", "Lighting Shadow Policy v1", "LightingShadowPolicyPlan", "plan_lighting_shadow_policy", "Stable Directional Light-Space Policy v0", "DirectionalShadowLightSpacePlan")) { Assert-ContainsText ([string]$geRendererModule[0].purpose) $needle "MK_renderer module purpose" }
foreach ($needle in @("environment-gated height-fog runtime readback proof", "MK_VULKAN_TEST_HEIGHT_FOG", "environment-gated physical-sky runtime readback proof", "MK_VULKAN_TEST_PHYSICAL_SKY", "VK_LAYER_KHRONOS_validation")) { Assert-ContainsText ([string]$geRhiModule[0].purpose) $needle "MK_rhi module purpose" }
...
Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRuntime) "--require-postprocess-depth-input" "runtime game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRuntime) "generated color-postprocess scaffold" "runtime game guidance"
foreach ($needle in @("MK_VULKAN_TEST_POSTPROCESS_DEPTH_VERTEX_SPV", "MK_VULKAN_TEST_POSTPROCESS_DEPTH_FRAGMENT_SPV", "postprocess-depth readback")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentVulkan) $needle "Vulkan game guidance" }
$vulkanHeightFogArtifactNeedles = @("MK_VULKAN_TEST_HEIGHT_FOG_DEPTH_VERTEX_SPV", "MK_VULKAN_TEST_HEIGHT_FOG_DEPTH_FRAGMENT_SPV", "MK_VULKAN_TEST_HEIGHT_FOG_VERTEX_SPV", "MK_VULKAN_TEST_HEIGHT_FOG_FRAGMENT_SPV", "VK_LAYER_KHRONOS_validation"); $vulkanPhysicalSkyArtifactNeedles = @("MK_VULKAN_TEST_PHYSICAL_SKY_VERTEX_SPV", "MK_VULKAN_TEST_PHYSICAL_SKY_FRAGMENT_SPV", "VK_LAYER_KHRONOS_validation")
foreach ($vulkanGuidanceNeedleSet in @(@($vulkanHeightFogArtifactNeedles + @("height-fog runtime readback")), @($vulkanPhysicalSkyArtifactNeedles + @("physical-sky runtime readback")))) { foreach ($needle in $vulkanGuidanceNeedleSet) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentVulkan) $needle "Vulkan game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentVulkanRuntimeOwners) $needle "Vulkan runtime owner guidance" } }; foreach ($needle in @("Vulkan env-gated height-fog runtime readback proof", "Vulkan env-gated physical-sky runtime readback proof")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRendering) $needle "rendering game guidance" }; foreach ($needle in @("strict env-gated Vulkan height-fog runtime readback evidence", "strict env-gated Vulkan physical-sky runtime readback evidence", "environment.readiness.physical_sky.package_status", "environment.readiness.backend.metal_status", "environment.readiness.unsupported.environment_ready")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; $environmentEditorAuthoringNeedles = @("GameEngine.EnvironmentProfile.v2", "EnvironmentAuthoringDocument", "load_environment_authoring_document", "save_environment_authoring_document", "environment.profile_v2.volume_count", "environment.profile_v2.weather_keyframes", "environment.volume.<index>.id", "environment.volume.<index>.shape", "environment.volume.<index>.priority", "environment.volume.<index>.blend_weight", "environment.volume.<index>.fade_distance_m", "environment.weather_keyframe.<index>.time_of_day_hours", "environment.weather_keyframe.<index>.weather", "environment.weather_keyframe.<index>.precipitation", "environment.weather_keyframe.<index>.quality_preset", "environment.quality.tier", "environment.capture.cubemap.request_status", "environment.command.volume.add", "environment.command.volume.remove", "environment.command.volume.reorder", "environment.command.weather_keyframe.edit", "environment.command.quality_preset.select", "environment.command.capture.cubemap.request", "invokes_backend=false", "executes_package_scripts=false", "exposes_native_handles=false", "Dear ImGui", "SDL3", "broad environment_ready"); foreach ($needle in $environmentEditorAuthoringNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentEditorAuthoringTask9) $needle "environment editor authoring task9 game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentProductionEditorAuthoringPhase7) $needle "environment editor authoring phase7 game guidance" }; $environmentEditorAuthoringDocNeedles = @("GameEngine.EnvironmentProfile.v2", "environment.profile_v2.volume_count", "environment.profile_v2.weather_keyframes", "environment.volume.<index>.id", "environment.volume.<index>.shape", "environment.volume.<index>.priority", "environment.volume.<index>.blend_weight", "environment.volume.<index>.fade_distance_m", "environment.weather_keyframe.<index>.time_of_day_hours", "environment.weather_keyframe.<index>.weather", "environment.weather_keyframe.<index>.precipitation", "environment.weather_keyframe.<index>.quality_preset", "environment.quality.tier", "environment.capture.cubemap.request_status", "environment.command.volume.add", "environment.command.volume.remove", "environment.command.volume.reorder", "environment.command.weather_keyframe.edit", "environment.command.quality_preset.select", "environment.command.capture.cubemap.request"); foreach ($environmentEditorAuthoringGuidance in @("docs/ai-game-development.md", "docs/current-capabilities.md", "docs/editor.md", "docs/roadmap.md", "docs/testing.md", "docs/superpowers/plans/README.md", "docs/superpowers/plans/2026-06-06-environment-production-excellence-v1.md", ".agents/skills/editor-change/SKILL.md", ".claude/skills/gameengine-editor/SKILL.md")) { $environmentEditorAuthoringText = Get-AgentSurfaceText $environmentEditorAuthoringGuidance; foreach ($needle in $environmentEditorAuthoringDocNeedles) { Assert-ContainsText $environmentEditorAuthoringText $needle $environmentEditorAuthoringGuidance } }; $environmentAuthoringHeaderText = Get-AgentSurfaceText "editor/core/include/mirakana/editor/environment_authoring.hpp"; foreach ($needle in @("EnvironmentAuthoringCommandKind", "EnvironmentAuthoringCommandRequest", "EnvironmentAuthoringCommandPlan", "plan_environment_authoring_command", "make_environment_authoring_command_action", "from_profile_document_v2", "profile_document_v2")) { Assert-ContainsText $environmentAuthoringHeaderText $needle "editor/core/include/mirakana/editor/environment_authoring.hpp" }; $environmentAuthoringSourceText = Get-AgentSurfaceText "editor/core/src/environment_authoring.cpp"; foreach ($needle in @("deserialize_environment_profile_v2", "serialize_environment_profile_v2", "validate_environment_profile_v2", "environment.command.volume.add", "environment.command.volume.remove", "environment.command.volume.reorder", "environment.command.weather_keyframe.edit", "environment.command.quality_preset.select", "environment.command.capture.cubemap.request")) { Assert-ContainsText $environmentAuthoringSourceText $needle "editor/core/src/environment_authoring.cpp" }; $environmentAuthoringTestText = Get-AgentSurfaceText "tests/unit/editor_environment_tests.cpp"; foreach ($needle in @("GameEngine.EnvironmentProfile.v2", "environment.profile_v2.volume_count", "environment.weather_keyframe.1.quality_preset", "environment.command.capture.cubemap.request", "cubemap capture command is reviewed but never executed")) { Assert-ContainsText $environmentAuthoringTestText $needle "tests/unit/editor_environment_tests.cpp" }; foreach ($needle in @("environment_quality_budget_status=ready", "environment_quality_budget_transient_gpu_byte_estimate", "environment_quality_budget_framegraph_barrier_steps_executed", "environment_quality_budget_broad_environment_ready_claimed=0")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentQualityBudgetTask10) $needle "environment quality budget task10 game guidance" }
$environmentQualityBudgetPhase8Needles = @("EnvironmentQualityBudgetRequest", "EnvironmentQualityBudgetUsageDesc", "EnvironmentQualityBudgetLimitsDesc", "EnvironmentQualityBudgetPlan", "environment_quality_budget_preset_limits", "evaluate_environment_quality_budget", "has_environment_quality_budget_diagnostic", "environment_quality_preset=high", "environment_quality_budget_status=ready", "environment_quality_budget_ready=1", "environment_quality_budget_requested=1", "environment_quality_budget_violations=0", "environment_quality_budget_broad_optimization_claimed=0", "environment_quality_budget_broad_environment_ready_claimed=0", "broad optimization", "backend parity", "broad environment_ready"); foreach ($needle in $environmentQualityBudgetPhase8Needles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentProductionQualityBudgetPhase8) $needle "environment quality budget phase8 game guidance" }; foreach ($environmentQualityBudgetGuidance in @("engine/agent/manifest.json", "engine/agent/manifest.fragments/006-runtimeBackendReadiness.json", "engine/agent/manifest.fragments/008-packagingTargets.json", "engine/agent/manifest.fragments/010-aiOperableProductionLoop.json", "engine/agent/manifest.fragments/014-gameCodeGuidance.json", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md", "docs/superpowers/plans/README.md", "docs/superpowers/plans/2026-06-06-environment-production-excellence-v1.md", "games/sample_desktop_runtime_game/README.md")) { $environmentQualityBudgetText = Get-AgentSurfaceText $environmentQualityBudgetGuidance; foreach ($needle in @("environment_quality_preset", "environment_quality_budget_violations", "environment_quality_budget_broad_optimization_claimed")) { Assert-ContainsText $environmentQualityBudgetText $needle $environmentQualityBudgetGuidance } }; foreach ($needle in @("environment-quality-budget-package-evidence", "EnvironmentQualityBudgetRequest", "environment_quality_budget_broad_optimization_claimed=0")) { Assert-ContainsText (Get-AgentSurfaceText "engine/agent/manifest.fragments/006-runtimeBackendReadiness.json") $needle "engine/agent/manifest.fragments/006-runtimeBackendReadiness.json" }; foreach ($needle in @("desktop-runtime-environment-quality-budget-evidence", "environment_quality_budget_violations=0", "environment_quality_budget_broad_optimization_claimed=0")) { Assert-ContainsText (Get-AgentSurfaceText "engine/agent/manifest.fragments/008-packagingTargets.json") $needle "engine/agent/manifest.fragments/008-packagingTargets.json" }; foreach ($needle in @("EnvironmentQualityBudgetRequest", "EnvironmentQualityBudgetUsageDesc", "EnvironmentQualityBudgetLimitsDesc", "EnvironmentQualityBudgetPlan", "environment_quality_budget_preset_limits", "evaluate_environment_quality_budget", "has_environment_quality_budget_diagnostic")) { Assert-ContainsText (Get-AgentSurfaceText "engine/environment/include/mirakana/environment/environment_quality_budget.hpp") $needle "engine/environment/include/mirakana/environment/environment_quality_budget.hpp" }; foreach ($needle in @("unsupported_quality_preset", "broad_optimization_claimed", "native_handle_access", "budget_exceeded")) { Assert-ContainsText (Get-AgentSurfaceText "engine/environment/src/environment_quality_budget.cpp") $needle "engine/environment/src/environment_quality_budget.cpp" }; foreach ($needle in @("environment quality budgets expand presets and pass selected high usage", "environment quality budgets reject budget violations and broad optimization claims", "environment quality budgets require explicit custom limits")) { Assert-ContainsText (Get-AgentSurfaceText "tests/unit/environment_tests.cpp") $needle "tests/unit/environment_tests.cpp" }; foreach ($needle in @("environment_quality_preset", "environment_quality_budget_violations", "environment_quality_budget_broad_optimization_claimed", "evaluate_environment_quality_budget", "environment_profile.quality_preset", "require_environment_audio_playback")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($needle in @('"environment_quality_preset" = "high"', '"environment_quality_budget_violations" = "0"', '"environment_quality_budget_broad_optimization_claimed" = "0"', '$requiresEnvironmentAudioPlayback', '++$expectedEnvironmentQualityBudgetRows')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $needle "tools/validate-installed-desktop-runtime.ps1" }
$environmentFogVulkanPackageNeedles = @("--require-environment-fog-vulkan-package-evidence", "environment_fog_vulkan_package_status=ready", "environment_fog_vulkan_selected_backend=vulkan"); foreach ($needle in $environmentFogVulkanPackageNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; foreach ($environmentFogVulkanCodeGuidance in @("engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp", "engine/runtime_host/win32/src/win32_desktop_presentation.cpp")) { $environmentFogVulkanCodeText = Get-AgentSurfaceText $environmentFogVulkanCodeGuidance; foreach ($needle in @("Win32DesktopPresentationVulkanEnvironmentFogPackageReport", "evaluate_win32_desktop_presentation_vulkan_environment_fog_package", "Win32DesktopPresentationVulkanEnvironmentFogPackageStatus", "environment_fog_vulkan_package_evidence_ready")) { Assert-ContainsText $environmentFogVulkanCodeText $needle $environmentFogVulkanCodeGuidance } }; foreach ($environmentFogVulkanPackageGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md")) { $environmentFogVulkanPackageText = Get-AgentSurfaceText $environmentFogVulkanPackageGuidance; foreach ($needle in $environmentFogVulkanPackageNeedles) { Assert-ContainsText $environmentFogVulkanPackageText $needle $environmentFogVulkanPackageGuidance } }; foreach ($environmentFogVulkanValidateNeedle in @('$requiresEnvironmentFogVulkanPackageEvidence', '"environment_fog_vulkan_package_status" = "ready"', '"environment_fog_vulkan_package_ready" = "1"', '"environment_fog_vulkan_selected_backend" = "vulkan"', '"environment_fog_vulkan_package_evidence_ready" = "1"', '"environment_fog_vulkan_native_handle_access" = "0"')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentFogVulkanValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }
$environmentVolumetricFogPackageNeedles = @("--require-environment-volumetric-fog-package-evidence", "environment_volumetric_fog_status=ready", "environment_volumetric_fog_selected_backend=d3d12", "environment_volumetric_fog_compute_dispatches"); foreach ($needle in $environmentVolumetricFogPackageNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; foreach ($environmentVolumetricFogCodeGuidance in @("engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp", "engine/runtime_host/win32/src/win32_desktop_presentation.cpp")) { $environmentVolumetricFogCodeText = Get-AgentSurfaceText $environmentVolumetricFogCodeGuidance; foreach ($needle in @("Win32DesktopPresentationEnvironmentVolumetricFogReport", "evaluate_win32_desktop_presentation_environment_volumetric_fog", "Win32DesktopPresentationEnvironmentVolumetricFogStatus", "environment_volumetric_fog_compute_dispatches")) { Assert-ContainsText $environmentVolumetricFogCodeText $needle $environmentVolumetricFogCodeGuidance } }; foreach ($environmentVolumetricFogPackageGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md")) { $environmentVolumetricFogPackageText = Get-AgentSurfaceText $environmentVolumetricFogPackageGuidance; foreach ($needle in $environmentVolumetricFogPackageNeedles) { Assert-ContainsText $environmentVolumetricFogPackageText $needle $environmentVolumetricFogPackageGuidance } }; foreach ($environmentVolumetricFogValidateNeedle in @('$requiresEnvironmentVolumetricFogPackageEvidence', '"environment_volumetric_fog_status" = "ready"', '"environment_volumetric_fog_ready" = "1"', '"environment_volumetric_fog_selected_backend" = "d3d12"', '"environment_volumetric_fog_froxel_output_ready" = "1"', '"environment_volumetric_fog_scene_depth_ready" = "1"', '"environment_volumetric_fog_native_handle_access" = "0"', 'environment_volumetric_fog_compute_dispatches=([1-9][0-9]*)')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentVolumetricFogValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }
$environmentLightingRendererExecutionNeedles = @("--require-environment-lighting-renderer-execution", "desktop-runtime-sample-game-environment-ibl-renderer-execution", "environment_lighting_renderer_upload_status=ready", "environment_lighting_renderer_execution_status=ready", "environment_lighting_texture_cube_uploads", "environment_lighting_texture_cube_faces=6", "environment_lighting_texture_cube_edge_size=16", "environment_lighting_radiance_mips=5", "environment_lighting_renderer_irradiance_rows=9", "environment_lighting_shader_sampling_proven=1", "environment_lighting_shader_sample_readback_nonzero=1", "environment_lighting_runtime_capture_faces=6", "environment_lighting_runtime_capture_readback_nonzero=1", "environment_lighting_runtime_capture_readback_checksum"); foreach ($needle in @("--require-environment-lighting-renderer-execution", "environment_lighting_texture_cube_uploads", "environment_lighting_runtime_capture_readback_nonzero")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; foreach ($needle in @("D3d12EnvironmentIblRendererUploadDesc", "execute_environment_ibl_renderer_upload", "D3d12EnvironmentIblRendererUploadResult", "sampling_vertex_shader", "sampling_fragment_shader")) { Assert-ContainsText (Get-AgentSurfaceText "engine/rhi/d3d12/include/mirakana/rhi/d3d12/d3d12_backend.hpp") $needle "engine/rhi/d3d12/include/mirakana/rhi/d3d12/d3d12_backend.hpp" }; foreach ($needle in @("--require-environment-lighting-renderer-execution", "environment_lighting_renderer_upload_status", "environment_lighting_renderer_execution_status", "environment_lighting_texture_cube_uploads", "environment_lighting_texture_cube_faces", "environment_lighting_texture_cube_edge_size", "environment_lighting_radiance_mips", "environment_lighting_renderer_irradiance_rows", "environment_lighting_shader_sampling_proven", "environment_lighting_shader_sample_readback_nonzero", "environment_lighting_runtime_capture_readback_nonzero")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($environmentLightingRendererExecutionGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md")) { $environmentLightingRendererExecutionText = Get-AgentSurfaceText $environmentLightingRendererExecutionGuidance; foreach ($needle in $environmentLightingRendererExecutionNeedles) { Assert-ContainsText $environmentLightingRendererExecutionText $needle $environmentLightingRendererExecutionGuidance } }; foreach ($environmentLightingRendererValidateNeedle in @('$requiresEnvironmentLightingRendererExecution', '"environment_lighting_renderer_upload_status" = $expectedEnvironmentLightingRendererUploadStatus', '"environment_lighting_renderer_execution_status" = $expectedEnvironmentLightingRendererExecutionStatus', '"environment_lighting_texture_cube_faces" = $expectedEnvironmentLightingTextureCubeFaces', '"environment_lighting_texture_cube_edge_size" = $expectedEnvironmentLightingTextureCubeEdgeSize', '"environment_lighting_radiance_mips" = $expectedEnvironmentLightingRadianceMips', '"environment_lighting_renderer_irradiance_rows" = $expectedEnvironmentLightingRendererIrradianceRows', '"environment_lighting_shader_sampling_proven" = $expectedEnvironmentLightingShaderSampling', '"environment_lighting_shader_sample_readback_nonzero" = $expectedEnvironmentLightingShaderReadback', '"environment_lighting_runtime_capture_readback_nonzero" = $expectedEnvironmentLightingRuntimeCaptureReadback', 'environment_lighting_runtime_capture_readback_checksum=([1-9][0-9]*)')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentLightingRendererValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }
$environmentVulkanIblRendererExecutionNeedles = @("--require-environment-lighting-vulkan-renderer-execution", "desktop-runtime-sample-game-vulkan-environment-ibl-renderer-execution", "environment_lighting_vulkan_status=ready", "environment_lighting_vulkan_ready=1", "environment_lighting_vulkan_selected_backend=vulkan", "environment_lighting_vulkan_cube_compatible_image_created=1", "environment_lighting_vulkan_cube_image_view_created=1", "environment_lighting_vulkan_sampler_created=1", "environment_lighting_vulkan_descriptor_set_bindings=2", "environment_lighting_vulkan_synchronization2_barriers", "environment_lighting_vulkan_texture_cube_uploads=1", "environment_lighting_vulkan_texture_cube_faces=6", "environment_lighting_vulkan_texture_cube_edge_size=16", "environment_lighting_vulkan_radiance_mips=5", "environment_lighting_vulkan_renderer_irradiance_rows=9", "environment_lighting_vulkan_shader_sampling_proven=1", "environment_lighting_vulkan_shader_sample_readback_nonzero=1", "environment_lighting_vulkan_runtime_capture_readback_checksum", "environment_lighting_vulkan_native_handle_access=0", "environment_lighting_vulkan_diagnostics=0"); foreach ($needle in $environmentVulkanIblRendererExecutionNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentProductionVulkanIblPhase5) $needle "environment Vulkan IBL phase5 game guidance" }; foreach ($needle in @("--require-environment-lighting-vulkan-renderer-execution", "environment_lighting_vulkan_status=ready", "environment_lighting_vulkan_selected_backend=vulkan")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; foreach ($environmentVulkanIblCodeGuidance in @("engine/rhi/vulkan/include/mirakana/rhi/vulkan/vulkan_backend.hpp", "engine/rhi/vulkan/src/vulkan_backend.cpp")) { $environmentVulkanIblCodeText = Get-AgentSurfaceText $environmentVulkanIblCodeGuidance; foreach ($needle in @("VulkanEnvironmentIblRendererUploadDesc", "VulkanEnvironmentIblRendererUploadResult", "execute_environment_ibl_renderer_upload", "cube_compatible_image_created", "synchronization2_barriers", "sampling_vertex_entry_point", "sampling_fragment_entry_point")) { Assert-ContainsText $environmentVulkanIblCodeText $needle $environmentVulkanIblCodeGuidance } }; $environmentVulkanIblSourceText = Get-AgentSurfaceText "engine/rhi/vulkan/src/vulkan_backend.cpp"; foreach ($needle in @("shader_sample_readback_offset", "shader_sample_image", "cmd_begin_rendering", "cmd_bind_descriptor_sets", "cmd_draw")) { Assert-ContainsText $environmentVulkanIblSourceText $needle "engine/rhi/vulkan/src/vulkan_backend.cpp" }; foreach ($environmentVulkanIblCodeGuidance in @("engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp", "engine/runtime_host/win32/src/win32_desktop_presentation.cpp")) { $environmentVulkanIblCodeText = Get-AgentSurfaceText $environmentVulkanIblCodeGuidance; foreach ($needle in @("Win32DesktopPresentationVulkanEnvironmentIblRendererExecutionReport", "evaluate_win32_desktop_presentation_vulkan_environment_ibl_renderer_execution", "Win32DesktopPresentationVulkanEnvironmentIblRendererExecutionStatus")) { Assert-ContainsText $environmentVulkanIblCodeText $needle $environmentVulkanIblCodeGuidance } }; foreach ($needle in @("--require-environment-lighting-vulkan-renderer-execution", "sample_desktop_runtime_game_environment_ibl_sample.vs.spv", "sample_desktop_runtime_game_environment_ibl_sample.ps.spv", "environment_lighting_vulkan_status")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($environmentVulkanIblGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md", "docs/superpowers/plans/2026-06-06-environment-production-excellence-v1.md")) { $environmentVulkanIblText = Get-AgentSurfaceText $environmentVulkanIblGuidance; foreach ($needle in $environmentVulkanIblRendererExecutionNeedles) { Assert-ContainsText $environmentVulkanIblText $needle $environmentVulkanIblGuidance } }; foreach ($environmentVulkanIblValidateNeedle in @('$requiresEnvironmentLightingVulkanRendererExecution', '"environment_lighting_vulkan_status" = "ready"', '"environment_lighting_vulkan_ready" = "1"', '"environment_lighting_vulkan_selected_backend" = "vulkan"', '"environment_lighting_vulkan_cube_compatible_image_created" = "1"', '"environment_lighting_vulkan_cube_image_view_created" = "1"', '"environment_lighting_vulkan_sampler_created" = "1"', '"environment_lighting_vulkan_descriptor_set_bindings" = "2"', '"environment_lighting_vulkan_texture_cube_faces" = "6"', '"environment_lighting_vulkan_texture_cube_edge_size" = "16"', '"environment_lighting_vulkan_radiance_mips" = "5"', '"environment_lighting_vulkan_renderer_irradiance_rows" = "9"', '"environment_lighting_vulkan_shader_sampling_proven" = "1"', '"environment_lighting_vulkan_shader_sample_readback_nonzero" = "1"', '"environment_lighting_vulkan_runtime_capture_readback_nonzero" = "1"', '"environment_lighting_vulkan_native_handle_access" = "0"', '"environment_lighting_vulkan_diagnostics" = "0"', 'environment_lighting_vulkan_synchronization2_barriers=([1-9][0-9]*)', 'environment_lighting_vulkan_runtime_capture_readback_checksum=([1-9][0-9]*)')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentVulkanIblValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }; foreach ($needle in @("desktop-runtime-sample-game-vulkan-environment-ibl-renderer-execution", "vulkan-strict")) { Assert-ContainsText (Get-AgentSurfaceText "tools/run-validation-recipe-plans.ps1") $needle "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-AgentSurfaceText "tools/check-validation-recipe-runner.ps1") $needle "tools/check-validation-recipe-runner.ps1" }; Assert-ContainsText (Get-AgentSurfaceText "tools/validation-recipe-core.ps1") "--require-environment-lighting-vulkan-renderer-execution" "tools/validation-recipe-core.ps1"; foreach ($needle in @("sample_desktop_runtime_game_environment_ibl_sample.vs.spv", "sample_desktop_runtime_game_environment_ibl_sample.ps.spv", "-fspv-target-env=vulkan1.3")) { Assert-ContainsText (Get-AgentSurfaceText "games/CMakeLists.txt") $needle "games/CMakeLists.txt" }
$cloudLayerRendererExecutionNeedles = @("--require-cloud-layer-renderer-execution", "desktop-runtime-sample-game-cloud-layer-renderer-execution", "cloud_layer_texture_uploads", "cloud_layer_backend_invocations", "cloud_layer_renderer_draws"); foreach ($needle in @("--require-cloud-layer-renderer-execution", "cloud_layer_renderer_draws")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; foreach ($cloudLayerCodeGuidance in @("engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp", "engine/runtime_host/win32/src/win32_desktop_presentation.cpp")) { Assert-ContainsText (Get-AgentSurfaceText $cloudLayerCodeGuidance) "cloud_layer_renderer_draws" $cloudLayerCodeGuidance }; foreach ($needle in @("--require-cloud-layer-renderer-execution", "cloud_layer_renderer_draws")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($cloudLayerPackageGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md")) { $cloudLayerPackageText = Get-AgentSurfaceText $cloudLayerPackageGuidance; foreach ($needle in $cloudLayerRendererExecutionNeedles) { Assert-ContainsText $cloudLayerPackageText $needle $cloudLayerPackageGuidance } }; foreach ($cloudLayerValidateNeedle in @('$requiresCloudLayerRendererExecution', '"cloud_layer_status" = "ready"', '"cloud_layer_selected_backend" = "d3d12"', '"cloud_layer_cloud_map_binding" = "6"', '"cloud_layer_flow_map_binding" = "7"', '"cloud_layer_sampler_binding" = "6"', '"cloud_layer_constants_binding" = "6"', 'cloud_layer_texture_uploads=([1-9][0-9]*)', 'cloud_layer_backend_invocations=([1-9][0-9]*)', 'cloud_layer_renderer_draws=([1-9][0-9]*)')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $cloudLayerValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }
...
$environmentSnowPackageNeedles = @("--require-environment-snow-package-evidence", "desktop-runtime-sample-game-environment-snow-package", "weather=snow", "kind=snow", "zero wetness rows"); foreach ($needle in $environmentSnowPackageNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; foreach ($environmentSnowCodeGuidance in @("engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp", "engine/runtime_host/win32/src/win32_desktop_presentation.cpp", "tests/unit/runtime_host_win32_public_api_compile.cpp")) { Assert-ContainsText (Get-AgentSurfaceText $environmentSnowCodeGuidance) "Win32DesktopPresentationEnvironmentPrecipitationExpectation" $environmentSnowCodeGuidance }; foreach ($needle in @("--require-environment-snow-package-evidence", "make_sample_environment_snow_policy_desc", "environment_precipitation_expectation")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($environmentSnowPackageGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md")) { $environmentSnowPackageText = Get-AgentSurfaceText $environmentSnowPackageGuidance; foreach ($needle in $environmentSnowPackageNeedles) { Assert-ContainsText $environmentSnowPackageText $needle $environmentSnowPackageGuidance } }; foreach ($environmentSnowValidateNeedle in @('$requiresEnvironmentSnowPackageEvidence', '"environment_precipitation_status" = "ready"', '"environment_precipitation_selected_backend" = "d3d12"', '"environment_precipitation_weather" = "snow"', '"environment_precipitation_kind" = "snow"', '"environment_precipitation_wetness_rows" = "0"', '"environment_precipitation_audio_handoff_rows" = "1"', '"environment_precipitation_particle_buffer_uploads" = "0"', '"environment_precipitation_backend_invocations" = "0"', '"environment_precipitation_material_mutations" = "0"', '"environment_precipitation_audio_playback" = "0"')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentSnowValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }
$environmentPrecipitationRendererExecutionNeedles = @("--require-environment-precipitation-renderer-execution", "desktop-runtime-sample-game-environment-precipitation-renderer-execution", "environment_precipitation_renderer_draws", "environment_precipitation_depth_occlusion_readback=1"); foreach ($needle in $environmentPrecipitationRendererExecutionNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; foreach ($environmentPrecipitationRendererExecutionCodeGuidance in @("engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp", "engine/runtime_host/win32/src/win32_desktop_presentation.cpp")) { $environmentPrecipitationRendererExecutionCodeText = Get-AgentSurfaceText $environmentPrecipitationRendererExecutionCodeGuidance; foreach ($needle in @("environment_precipitation_renderer_draws", "environment_precipitation_depth_occlusion_readback")) { Assert-ContainsText $environmentPrecipitationRendererExecutionCodeText $needle $environmentPrecipitationRendererExecutionCodeGuidance } }; foreach ($needle in @("execute_precipitation_runtime_probe", "occluded_alpha")) { Assert-ContainsText (Get-AgentSurfaceText "engine/runtime_host/win32/src/win32_desktop_presentation.cpp") $needle "engine/runtime_host/win32/src/win32_desktop_presentation.cpp" }; foreach ($needle in @("--require-environment-precipitation-renderer-execution", "sample_desktop_runtime_game_precipitation.vs.dxil", "sample_desktop_runtime_game_precipitation.ps.dxil", "environment_precipitation_renderer_draws", "environment_precipitation_depth_occlusion_readback")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($environmentPrecipitationRendererExecutionGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md")) { $environmentPrecipitationRendererExecutionText = Get-AgentSurfaceText $environmentPrecipitationRendererExecutionGuidance; foreach ($needle in $environmentPrecipitationRendererExecutionNeedles) { Assert-ContainsText $environmentPrecipitationRendererExecutionText $needle $environmentPrecipitationRendererExecutionGuidance } }; foreach ($environmentPrecipitationRendererExecutionValidateNeedle in @('$requiresEnvironmentPrecipitationRendererExecution', '"environment_precipitation_particle_buffer_uploads"', '"environment_precipitation_backend_invocations"', '"environment_precipitation_renderer_draws"', 'environment_precipitation_depth_occlusion_readback=1')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentPrecipitationRendererExecutionValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }
$environmentSnowRendererExecutionNeedles = @("--require-environment-snow-renderer-execution", "desktop-runtime-sample-game-environment-snow-renderer-execution", "environment_precipitation_renderer_draws", "environment_precipitation_depth_occlusion_readback=1"); foreach ($needle in $environmentSnowRendererExecutionNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentSystem) $needle "environment system game guidance" }; foreach ($needle in @("--require-environment-snow-renderer-execution", "require_environment_snow_renderer_execution")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($environmentSnowRendererExecutionGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md")) { $environmentSnowRendererExecutionText = Get-AgentSurfaceText $environmentSnowRendererExecutionGuidance; foreach ($needle in $environmentSnowRendererExecutionNeedles) { Assert-ContainsText $environmentSnowRendererExecutionText $needle $environmentSnowRendererExecutionGuidance } }; foreach ($environmentSnowRendererExecutionValidateNeedle in @('$requiresEnvironmentSnowRendererExecution', '"environment_precipitation_weather" = "snow"', '"environment_precipitation_kind" = "snow"', '"environment_precipitation_particle_buffer_uploads"', '"environment_precipitation_backend_invocations"', '"environment_precipitation_renderer_draws"', 'environment_precipitation_depth_occlusion_readback=1')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentSnowRendererExecutionValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }
$environmentMaterialWeatheringNeedles = @("--require-environment-material-weathering", "desktop-runtime-sample-game-environment-material-weathering", "environment_material_weathering_status=ready"); foreach ($needle in @("desktop-runtime-sample-game-environment-material-weathering", "environment_material_weathering_status=ready", "environment_material_weathering_source_material_mutations=0", "environment_material_weathering_native_handle_access=0")) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentProductionMaterialWeatheringPhase3) $needle "environment material weathering phase3 game guidance" }; foreach ($environmentMaterialWeatheringCodeGuidance in @("engine/environment/include/mirakana/environment/material_weathering.hpp", "engine/environment/src/material_weathering.cpp", "tests/unit/environment_material_weathering_tests.cpp")) { $environmentMaterialWeatheringCodeText = Get-AgentSurfaceText $environmentMaterialWeatheringCodeGuidance; foreach ($needle in @("EnvironmentMaterialWeatheringPlan", "plan_environment_material_weathering")) { Assert-ContainsText $environmentMaterialWeatheringCodeText $needle $environmentMaterialWeatheringCodeGuidance } }; foreach ($environmentMaterialWeatheringPolicyGuidance in @("engine/renderer/include/mirakana/renderer/material_weathering_policy.hpp", "engine/renderer/src/material_weathering_policy.cpp", "tests/unit/renderer_material_weathering_policy_tests.cpp")) { $environmentMaterialWeatheringPolicyText = Get-AgentSurfaceText $environmentMaterialWeatheringPolicyGuidance; foreach ($needle in @("MaterialWeatheringPolicyPlan", "plan_material_weathering_policy")) { Assert-ContainsText $environmentMaterialWeatheringPolicyText $needle $environmentMaterialWeatheringPolicyGuidance } }; foreach ($needle in @("--require-environment-material-weathering", "require_environment_material_weathering", "make_sample_environment_material_weathering_policy_desc", "environment_material_weathering_status", "environment_material_weathering_source_material_mutations", "environment_material_weathering_native_handle_access")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($environmentMaterialWeatheringGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md", "docs/superpowers/plans/2026-06-06-environment-production-excellence-v1.md")) { $environmentMaterialWeatheringText = Get-AgentSurfaceText $environmentMaterialWeatheringGuidance; foreach ($needle in $environmentMaterialWeatheringNeedles) { Assert-ContainsText $environmentMaterialWeatheringText $needle $environmentMaterialWeatheringGuidance } }; foreach ($environmentMaterialWeatheringValidateNeedle in @('$requiresEnvironmentMaterialWeathering', '"environment_material_weathering_status" = "ready"', '"environment_material_weathering_ready" = "1"', '"environment_material_weathering_state" = "mixed"', '"environment_material_weathering_constants_binding" = "14"', '"environment_material_weathering_constants_byte_size" = "256"', '"environment_material_weathering_wet_rows" = "1"', '"environment_material_weathering_snow_rows" = "1"', '"environment_material_weathering_ice_rows" = "1"', '"environment_material_weathering_source_material_mutations" = "0"', '"environment_material_weathering_native_handle_access" = "0"', '"environment_material_weathering_diagnostics" = "0"', '"environment_material_weathering_material_parameter_bindings"', '"environment_material_weathering_material_constant_bytes"', '"environment_material_weathering_backend_invocations"')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentMaterialWeatheringValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }
foreach ($vulkanHeightFogArtifactGuidance in @("docs/testing.md", "docs/rhi.md", "docs/current-capabilities.md", "docs/superpowers/plans/2026-05-26-environment-system-v1.md", "engine/agent/manifest.json")) { $vulkanHeightFogArtifactText = Get-AgentSurfaceText $vulkanHeightFogArtifactGuidance; foreach ($needle in $vulkanHeightFogArtifactNeedles) { Assert-ContainsText $vulkanHeightFogArtifactText $needle $vulkanHeightFogArtifactGuidance } }; foreach ($vulkanHeightFogClaimGuidance in @("docs/current-capabilities.md", "docs/roadmap.md", "docs/architecture.md", "docs/rhi.md", "docs/testing.md")) { $vulkanHeightFogClaimText = Get-AgentSurfaceText $vulkanHeightFogClaimGuidance; foreach ($needle in @("environment_fog_vulkan_package_status=ready", "broad")) { Assert-ContainsText $vulkanHeightFogClaimText $needle $vulkanHeightFogClaimGuidance } }
Assert-ContainsText (Get-AgentSurfaceText "docs/superpowers/plans/2026-05-26-environment-system-v1.md") "MK_backend_scaffold_tests" "environment system plan"; foreach ($postprocessDepthReadyGuidance in @("docs/testing.md", "docs/rhi.md", "docs/architecture.md", "docs/roadmap.md", "docs/specs/2026-04-27-engine-essential-gap-analysis.md", ".agents/skills/rendering-change/SKILL.md", ".claude/skills/gameengine-rendering/SKILL.md", "games/sample_desktop_runtime_game/README.md")) { Assert-ContainsText (Get-AgentSurfaceText $postprocessDepthReadyGuidance) "postprocess_depth_input_ready" $postprocessDepthReadyGuidance }
...
$environmentAudioPlaybackNeedles = @("--require-environment-audio-playback", "desktop-runtime-sample-game-environment-audio-playback", "environment_audio_playback_status=ready", "environment_audio_playback_ready=1", "environment_audio_playback_requested=1", "environment_audio_runtime_lane_ready=1", "environment_audio_trigger_rows=4", "environment_audio_runtime_cues_started", "environment_audio_runtime_render_commands", "environment_audio_runtime_render_frames", "environment_audio_device_host_evidence=0", "environment_audio_device_io_invoked=0", "environment_audio_device_owned_by_environment=0", "environment_audio_native_handle_access=0", "environment_audio_diagnostics=0", "environment_precipitation_audio_playback=0"); foreach ($needle in $environmentAudioPlaybackNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentProductionWeatherAudioPhase4) $needle "environment weather audio phase4 game guidance" }; foreach ($environmentAudioPlaybackGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md", "docs/superpowers/plans/README.md", "docs/superpowers/plans/2026-06-06-environment-production-excellence-v1.md")) { $environmentAudioPlaybackText = Get-AgentSurfaceText $environmentAudioPlaybackGuidance; foreach ($needle in $environmentAudioPlaybackNeedles) { Assert-ContainsText $environmentAudioPlaybackText $needle $environmentAudioPlaybackGuidance } }; foreach ($needle in @("EnvironmentWeatherAudioPlaybackPlan", "EnvironmentWeatherAudioTriggerRow", "default_environment_weather_audio_cue_bindings", "plan_environment_weather_audio_playback", "has_environment_weather_audio_trigger", "unsupported_audio_device_ownership", "unsupported_native_handle_access", "wind_loop")) { Assert-ContainsText (Get-AgentSurfaceText "engine/environment/include/mirakana/environment/weather.hpp") $needle "engine/environment/include/mirakana/environment/weather.hpp"; Assert-ContainsText (Get-AgentSurfaceText "engine/environment/src/weather.cpp") $needle "engine/environment/src/weather.cpp" }; foreach ($needle in @("environment weather audio planning emits value-only runtime trigger rows", "environment weather audio planning supports snow ambience without device ownership", "environment weather audio planning rejects backend device and native handle side effects", "wind_loop")) { Assert-ContainsText (Get-AgentSurfaceText "tests/unit/environment_weather_tests.cpp") $needle "tests/unit/environment_weather_tests.cpp" }; foreach ($needle in @("--require-environment-audio-playback", "require_environment_audio_playback", "evaluate_environment_audio_playback", "environment_audio_playback_status", "environment_audio_runtime_cues_started", "environment_audio_runtime_render_commands", "environment_audio_device_io_invoked", "environment_audio_native_handle_access")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($environmentAudioPlaybackValidateNeedle in @('$requiresEnvironmentAudioPlayback', '"environment_audio_playback_status" = "ready"', '"environment_audio_playback_ready" = "1"', '"environment_audio_playback_requested" = "1"', '"environment_audio_runtime_lane_ready" = "1"', '"environment_audio_trigger_rows" = "4"', '"environment_audio_device_host_evidence" = "0"', '"environment_audio_device_io_invoked" = "0"', '"environment_audio_device_owned_by_environment" = "0"', '"environment_audio_native_handle_access" = "0"', '"environment_audio_diagnostics" = "0"', '"environment_precipitation_audio_playback" = "0"', "\b`$field=([1-9][0-9]*)\b")) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentAudioPlaybackValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }; foreach ($needle in @("desktop-runtime-sample-game-environment-audio-playback", "--require-environment-audio-playback", "d3d12-windows-primary")) { Assert-ContainsText (Get-AgentSurfaceText "tools/run-validation-recipe-plans.ps1") $needle "tools/run-validation-recipe-plans.ps1" }; $environmentVulkanPrecipitationRendererExecutionNeedles = @("--require-environment-precipitation-vulkan-renderer-execution", "desktop-runtime-sample-game-vulkan-environment-precipitation-renderer-execution", "environment_precipitation_vulkan_status=ready", "environment_precipitation_vulkan_ready=1", "environment_precipitation_vulkan_selected_backend=vulkan", "8/9/28/47", "descriptor_set_bindings=4", "synchronization2", "native-handle"); foreach ($needle in $environmentVulkanPrecipitationRendererExecutionNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentProductionVulkanPrecipitationPhase5) $needle "environment Vulkan precipitation phase5 game guidance" }; foreach ($environmentVulkanPrecipitationGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md", "docs/superpowers/plans/README.md", "docs/superpowers/plans/2026-06-06-environment-production-excellence-v1.md")) { $environmentVulkanPrecipitationText = Get-AgentSurfaceText $environmentVulkanPrecipitationGuidance; foreach ($needle in $environmentVulkanPrecipitationRendererExecutionNeedles) { Assert-ContainsText $environmentVulkanPrecipitationText $needle $environmentVulkanPrecipitationGuidance } }; foreach ($environmentVulkanPrecipitationCodeGuidance in @("engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp", "engine/runtime_host/win32/src/win32_desktop_presentation.cpp")) { $environmentVulkanPrecipitationCodeText = Get-AgentSurfaceText $environmentVulkanPrecipitationCodeGuidance; foreach ($needle in @("Win32DesktopPresentationVulkanEnvironmentPrecipitationReport", "evaluate_win32_desktop_presentation_vulkan_environment_precipitation", "Win32DesktopPresentationVulkanEnvironmentPrecipitationStatus", "environment_precipitation_vulkan_synchronization2_barriers")) { Assert-ContainsText $environmentVulkanPrecipitationCodeText $needle $environmentVulkanPrecipitationCodeGuidance } }; foreach ($environmentVulkanPrecipitationValidateNeedle in @('$requiresEnvironmentPrecipitationVulkanRendererExecution', '"environment_precipitation_vulkan_status" = "ready"', '"environment_precipitation_vulkan_ready" = "1"', '"environment_precipitation_vulkan_selected_backend" = "vulkan"', '"environment_precipitation_vulkan_sampler_binding" = "28"', '"environment_precipitation_vulkan_constants_binding" = "47"', '"environment_precipitation_vulkan_descriptor_set_bindings" = "4"', '"environment_precipitation_vulkan_synchronization2_barriers"')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentVulkanPrecipitationValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }; foreach ($needle in @("desktop-runtime-sample-game-vulkan-environment-precipitation-renderer-execution", "vulkan-strict")) { Assert-ContainsText (Get-AgentSurfaceText "tools/run-validation-recipe-plans.ps1") $needle "tools/run-validation-recipe-plans.ps1" }; foreach ($needle in @("desktop-runtime-sample-game-vulkan-environment-precipitation-renderer-execution", "--require-environment-precipitation-vulkan-renderer-execution", "vulkan-strict")) { Assert-ContainsText (Get-AgentSurfaceText "tools/check-validation-recipe-runner.ps1") $needle "tools/check-validation-recipe-runner.ps1" }; Assert-ContainsText (Get-AgentSurfaceText "tools/validation-recipe-core.ps1") "--require-environment-precipitation-vulkan-renderer-execution" "tools/validation-recipe-core.ps1"; foreach ($needle in @("sample_desktop_runtime_game_precipitation.vs.spv", "sample_desktop_runtime_game_precipitation.ps.spv", "-fvk-s-shift", "-fvk-b-shift")) { Assert-ContainsText (Get-AgentSurfaceText "games/CMakeLists.txt") $needle "games/CMakeLists.txt" }
$environmentVulkanPhysicalSkyPackageNeedles = @("--require-physical-sky-vulkan-package-evidence", "desktop-runtime-sample-game-vulkan-physical-sky-package", "environment_physical_sky_vulkan_status=ready", "environment_physical_sky_vulkan_ready=1", "environment_physical_sky_vulkan_selected_backend=vulkan"); foreach ($needle in $environmentVulkanPhysicalSkyPackageNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentProductionVulkanPhysicalSkyPhase5) $needle "environment Vulkan physical sky phase5 game guidance" }; foreach ($environmentVulkanPhysicalSkyGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md", "docs/superpowers/plans/README.md", "docs/superpowers/plans/2026-06-06-environment-production-excellence-v1.md")) { $environmentVulkanPhysicalSkyText = Get-AgentSurfaceText $environmentVulkanPhysicalSkyGuidance; foreach ($needle in $environmentVulkanPhysicalSkyPackageNeedles) { Assert-ContainsText $environmentVulkanPhysicalSkyText $needle $environmentVulkanPhysicalSkyGuidance } }; foreach ($environmentVulkanPhysicalSkyCodeGuidance in @("engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp", "engine/runtime_host/win32/src/win32_desktop_presentation.cpp")) { $environmentVulkanPhysicalSkyCodeText = Get-AgentSurfaceText $environmentVulkanPhysicalSkyCodeGuidance; foreach ($needle in @("Win32DesktopPresentationVulkanPhysicalSkyPackageReport", "evaluate_win32_desktop_presentation_vulkan_physical_sky_package", "Win32DesktopPresentationVulkanPhysicalSkyPackageStatus", "physical_sky_vulkan_package_constant_buffer_ready")) { Assert-ContainsText $environmentVulkanPhysicalSkyCodeText $needle $environmentVulkanPhysicalSkyCodeGuidance } }; foreach ($needle in @("--require-physical-sky-vulkan-package-evidence", "require_physical_sky_vulkan_package_evidence", "environment_physical_sky_vulkan_status", "environment_physical_sky_vulkan_constants_byte_size", "environment_physical_sky_vulkan_native_handle_access")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($environmentVulkanPhysicalSkyValidateNeedle in @('$requiresPhysicalSkyVulkanPackageEvidence', '"environment_physical_sky_vulkan_status" = "ready"', '"environment_physical_sky_vulkan_ready" = "1"', '"environment_physical_sky_vulkan_selected_backend" = "vulkan"', '"environment_physical_sky_vulkan_constants_binding" = "0"', '"environment_physical_sky_vulkan_constants_byte_size" = "256"', '"environment_physical_sky_vulkan_native_handle_access" = "0"')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentVulkanPhysicalSkyValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }; foreach ($needle in @("desktop-runtime-sample-game-vulkan-physical-sky-package", "--require-physical-sky-vulkan-package-evidence", "vulkan-strict")) { Assert-ContainsText (Get-AgentSurfaceText "tools/run-validation-recipe-plans.ps1") $needle "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-AgentSurfaceText "tools/check-validation-recipe-runner.ps1") $needle "tools/check-validation-recipe-runner.ps1" }; Assert-ContainsText (Get-AgentSurfaceText "tools/validation-recipe-core.ps1") "--require-physical-sky-vulkan-package-evidence" "tools/validation-recipe-core.ps1"
$environmentVulkanVolumetricFogRendererExecutionNeedles = @("--require-environment-volumetric-fog-vulkan-renderer-execution", "desktop-runtime-sample-game-vulkan-volumetric-fog-renderer-execution", "environment_volumetric_fog_vulkan_status=ready", "environment_volumetric_fog_vulkan_ready=1", "environment_volumetric_fog_vulkan_selected_backend=vulkan", "2/3/5/13", "descriptor_set_bindings=4", "synchronization2", "froxel_readback_nonzero=1", "native-handle"); foreach ($needle in $environmentVulkanVolumetricFogRendererExecutionNeedles) { Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentEnvironmentProductionVulkanVolumetricFogPhase5) $needle "environment Vulkan volumetric fog phase5 game guidance" }; foreach ($environmentVulkanVolumetricFogGuidance in @("engine/agent/manifest.json", "games/sample_desktop_runtime_game/game.agent.json", "games/sample_desktop_runtime_game/README.md", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/roadmap.md", "docs/testing.md", "docs/superpowers/plans/README.md", "docs/superpowers/plans/2026-06-06-environment-production-excellence-v1.md")) { $environmentVulkanVolumetricFogText = Get-AgentSurfaceText $environmentVulkanVolumetricFogGuidance; foreach ($needle in $environmentVulkanVolumetricFogRendererExecutionNeedles) { Assert-ContainsText $environmentVulkanVolumetricFogText $needle $environmentVulkanVolumetricFogGuidance } }; foreach ($environmentVulkanVolumetricFogCodeGuidance in @("engine/runtime_host/win32/include/mirakana/runtime_host/win32/win32_desktop_presentation.hpp", "engine/runtime_host/win32/src/win32_desktop_presentation.cpp")) { $environmentVulkanVolumetricFogCodeText = Get-AgentSurfaceText $environmentVulkanVolumetricFogCodeGuidance; foreach ($needle in @("Win32DesktopPresentationVulkanEnvironmentVolumetricFogReport", "evaluate_win32_desktop_presentation_vulkan_environment_volumetric_fog", "Win32DesktopPresentationVulkanEnvironmentVolumetricFogStatus", "environment_volumetric_fog_vulkan_synchronization2_barriers")) { Assert-ContainsText $environmentVulkanVolumetricFogCodeText $needle $environmentVulkanVolumetricFogCodeGuidance } }; foreach ($needle in @("--require-environment-volumetric-fog-vulkan-renderer-execution", "require_environment_volumetric_fog_vulkan_renderer_execution", "environment_volumetric_fog_vulkan_status", "environment_volumetric_fog_vulkan_descriptor_set_bindings", "environment_volumetric_fog_vulkan_froxel_output_storage_buffer_binding")) { Assert-ContainsText (Get-AgentSurfaceText "games/sample_desktop_runtime_game/main.cpp") $needle "games/sample_desktop_runtime_game/main.cpp" }; foreach ($environmentVulkanVolumetricFogValidateNeedle in @('$requiresEnvironmentVolumetricFogVulkanRendererExecution', '"environment_volumetric_fog_vulkan_status" = "ready"', '"environment_volumetric_fog_vulkan_ready" = "1"', '"environment_volumetric_fog_vulkan_selected_backend" = "vulkan"', '"environment_volumetric_fog_vulkan_descriptor_set_bindings" = "4"', '"environment_volumetric_fog_vulkan_scene_depth_texture_binding" = "2"', '"environment_volumetric_fog_vulkan_scene_depth_sampler_binding" = "3"', '"environment_volumetric_fog_vulkan_constants_binding" = "5"', '"environment_volumetric_fog_vulkan_constants_byte_size" = "256"', '"environment_volumetric_fog_vulkan_froxel_output_storage_buffer_binding" = "13"', '"environment_volumetric_fog_vulkan_native_handle_access" = "0"', '"environment_volumetric_fog_vulkan_diagnostics" = "0"', 'environment_volumetric_fog_vulkan_compute_dispatches', 'environment_volumetric_fog_vulkan_synchronization2_barriers')) { Assert-ContainsText (Get-AgentSurfaceText "tools/validate-installed-desktop-runtime.ps1") $environmentVulkanVolumetricFogValidateNeedle "tools/validate-installed-desktop-runtime.ps1" }; foreach ($needle in @("desktop-runtime-sample-game-vulkan-volumetric-fog-renderer-execution", "--require-environment-volumetric-fog-vulkan-renderer-execution", "vulkan-strict")) { Assert-ContainsText (Get-AgentSurfaceText "tools/run-validation-recipe-plans.ps1") $needle "tools/run-validation-recipe-plans.ps1"; Assert-ContainsText (Get-AgentSurfaceText "tools/check-validation-recipe-runner.ps1") $needle "tools/check-validation-recipe-runner.ps1" }; Assert-ContainsText (Get-AgentSurfaceText "tools/validation-recipe-core.ps1") "--require-environment-volumetric-fog-vulkan-renderer-execution" "tools/validation-recipe-core.ps1"; foreach ($needle in @("sample_desktop_runtime_game_environment_volumetric_fog.cs.spv", "-DMK_VOLUMETRIC_FOG_COMPUTE=1", "-fspv-target-env=vulkan1.3")) { Assert-ContainsText (Get-AgentSurfaceText "games/CMakeLists.txt") $needle "games/CMakeLists.txt" }
...
Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRuntime) "--require-directional-shadow" "runtime game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRuntime) "--require-directional-shadow-filtering" "runtime game guidance"; Assert-ContainsText ([string]$manifest.gameCodeGuidance.currentRuntime) "--require-d3d12-shadow-cascade-policy" "runtime game guidance"
