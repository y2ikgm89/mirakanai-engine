...
Assert-ContainsText ([string]$productionLoop.currentActivePlan) "2026-05-18-frame-graph-v1-production-ownership-milestone-v1.md" "recommended next plan active milestone"
Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.completedContext) "Completed evidence is indexed in docs/superpowers/plans/README.md" "recommended next plan completed context"
Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.completedContext) "production graph ownership beyond selected renderer/runtime-upload paths" "recommended next plan completed context"
Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.completedContext) "production multi-queue adoption" "recommended next plan completed context"
Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.completedContext) "broad renderer quality remain unsupported" "recommended next plan completed context"
Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.reason) "avoid repeated one-function child plans" "recommended next plan reason"
Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.reason) "production graph ownership boundary selection and RED tests" "recommended next plan reason"
Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.reason) "one reviewable PR checkpoint" "recommended next plan reason"
