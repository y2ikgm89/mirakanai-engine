...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-runtime-lod-milestone-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-package-streaming-residency-dispatch-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-page-addressable-payload-schema-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-payload-byte-range-file-io-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-directstorage-request-plan-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-native-directstorage-win32-async-io-dispatch-status-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-win32-async-file-io-adapter-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-win32-iocp-file-io-worker-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-directstorage-sdk-dependency-gate-v1" -or [string]$productionLoop.recommendedNextPlan.id -eq "mavg-autonomous-page-streaming-worker-v1") { $mavgRuntimeRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG Runtime LOD Milestone v1", "MAVG Asset Graph v1", "MAVG Phase 0 completed", "hierarchy/error/fallback", "MavgClusterGraphCluster", "resident_fallback_cluster_index", "geometric_error", "first_index", "index_count", "vertex_base", "GameEngine.MavgClusterGraph.v1", "MavgClusterCookVertex", "GameEngine.MavgClusterPayload.v1", "vertex.data_hex", "index.data_hex", "page.data_hex", "per-material root/leaf", "mavg_cluster_payload.hpp", "MavgClusterPayloadDocument", "MavgClusterPayloadPage", "validate_mavg_cluster_payload", "mavg_payload_pages.hpp", "RuntimeMavgPayloadPageSliceResult", "extract_runtime_mavg_payload_page_slices", "mavg_lod_selection.hpp", "MavgLodViewDesc", "MavgLodResidentPageSet", "MavgLodSelectionResult", "CPU LOD selection", "select_mavg_lod_clusters", "MK_mavg_lod_selection_tests", "mavg_lod_residency.hpp", "RuntimeMavgLodResidencyDesc", "RuntimeMavgLodResidencyResult", "build_runtime_mavg_lod_residency", "MK_runtime_mavg_lod_residency_tests", "range-aware conventional indexed draws", "mavg_scene_lod.hpp", "MavgSceneLodSubmitDesc", "MavgSceneLodSubmitResult", "plan_mavg_scene_lod_mesh_commands", "MK_scene_renderer_mavg_lod_tests", "RuntimeMavgPageStreamingDispatchPlan", "plan_runtime_mavg_page_streaming_dispatches", "RuntimeMavgPageStreamingDrainDesc", "GPU culling", "indirect draw execution", "mesh shaders", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgRuntimeRecommendedText $needle "recommended next plan MAVG runtime LOD selection reason" }
...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-d3d12-gpu-culling-execution-v1") { $mavgD3d12GpuCullingRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG D3D12 GPU Culling Execution v1", "mavg-d3d12-compute-generated-indirect-execution-v1", "MavgGpuCullingIndirectPlan", "D3D12 WARP-backed MAVG GPU culling dispatch proof", "selected-cluster command rows", "visibility rows", "same-buffer indexed indirect argument/count layout", "ExecuteIndirect", "compute dispatch", "queue wait", "GPU-generated indirect", "UAV barrier", "indirect transition", "generic GPU culling frameworks", "generic GPU-generated count-buffer systems", "generic storage-buffer state management", "Vulkan indirect draw execution", "Metal indirect command buffers", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgD3d12GpuCullingRecommendedText $needle "recommended next plan MAVG D3D12 GPU culling selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-d3d12-compute-generated-indirect-execution-v1") { $mavgD3d12ComputeRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG D3D12 Compute-Generated Indirect Execution v1", "mavg-d3d12-indexed-indirect-count-buffer-execution-v1", "D3D12 compute-generated indexed indirect argument/count buffer execution", "BufferUsage::storage | BufferUsage::indirect", "compute-to-graphics queue wait", "storage UAV", "D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT", "same-buffer argument/count", "visible WARP-backed proof", "no CPU decode/stat overclaim", "generic GPU-generated count-buffer systems", "generic storage-buffer state management", "Vulkan indirect draw execution", "actual GPU culling dispatch", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgD3d12ComputeRecommendedText $needle "recommended next plan MAVG D3D12 compute-generated indirect selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-d3d12-indexed-indirect-count-buffer-execution-v1") { $mavgD3d12CountRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG D3D12 Indexed Indirect Count Buffer Execution v1", "mavg-d3d12-indexed-indirect-draw-execution-v1", "D3D12 ExecuteIndirect count-buffer execution", "CPU-generated upload", "indexed indirect argument/count buffers", "visible WARP-backed proof", "zero-count execution", "min(count, MaxCommandCount)", "fail-closed count-buffer", "GPU-generated count buffers", "compute-generated indirect buffers", "Vulkan indirect draw execution", "actual GPU culling dispatch", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgD3d12CountRecommendedText $needle "recommended next plan MAVG D3D12 indexed indirect count-buffer selection reason" }
