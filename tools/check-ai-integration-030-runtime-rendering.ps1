...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "environment-system-v1") { $environmentRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("Environment System v1", "MK_environment", "EnvironmentProfileDesc", "validate_environment_profile", "official docs/Context7", "sky", "sun/moon", "fog", "clouds", "rain/snow/storm", "time-of-day", "quality tiers", "D3D12", "strict Vulkan", "Metal host-gated", "unsupportedProductionGaps = []", "broad environment_ready", "native handles", "Dear ImGui", "SDL3", "OpenEXR/KTX")) { Assert-ContainsText $environmentRecommendedText $needle "recommended next plan environment system selection reason" }
...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "first-party-ui-editor-excellence-v1") { $firstPartyUiEditorExcellenceRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("First-Party UI Editor Excellence v1", "Official docs and Context7", "EditorUiPerformanceBudget", "editor_ui_performance_budget_status=ready", "editor_ui_performance_diagnostics=0", "editor_ui_performance_broad_optimization_claimed=0", "retained UI diff/cache/submission rows", "multi-window docking", "GameEngine.Workspace.v3", "EditorRichTextEditCommandKind", "apply_editor_rich_text_edit_command", "editor_rich_text_edit_status=ready", "<rich_text_document_id>.insert_text", "<rich_text_document_id>.paste_rich_text", "first-party document/AI command/shell-control level", "text shaping", "font fallback", "glyph atlas", "IME parity", "accessibility parity", "Phase 8 host/toolchain-gated Vulkan editor texture-display counters", "Phase 9 Apple-host-gated Metal editor texture-display counters", "default visible-shell Vulkan/Metal backend selection", "cross-platform editor shell adapters", "AI-operable command expansion", "editor_ai_operation_excellence_status=ready", "editor.ai.operation.excellence", "editor.text_font.diagnostics.copy", "missing_expected_revision", "package_script_execution_unsupported", "file_mutation_unsupported", "unsupportedProductionGaps = []", "Dear ImGui", "SDL3", "native handles", "no inferred backend parity", "no inferred cross-platform parity", "no broad optimization claim", "no broad first_party_editor_excellence claim")) { Assert-ContainsText $firstPartyUiEditorExcellenceRecommendedText $needle "recommended next plan first-party UI editor excellence selection reason" }
...
foreach ($requiredNeedle in @("Windows-native editor/developer shell", "first-party retained", "MK_ui_renderer", "EditorDockLayout", "editor_dock_panel_catalog", "EditorDockCommandPlan", "plan_editor_dock_command", "apply_editor_dock_command", "EditorRichTextDocument", "EditorRichTextViewport", "make_editor_ai_command_panel_rich_text_document", "make_editor_inspector_rich_text_document", "make_editor_rich_text_ai_snapshot", "EditorRichTextEditCommandKind", "EditorRichTextEditRequest", "EditorRichTextEditResult", "EditorRichTextClipboardPayload", "editor_rich_text_revision", "normalize_editor_rich_text_selection", "apply_editor_rich_text_edit_command", "<rich_text_document_id>.insert_text", "<rich_text_document_id>.paste_rich_text", "private Windows SDK DirectWrite text-layout/glyph-raster adapters", "selected text atlas handoff evidence", "private Windows TSF text-input/IME session selection", "ITextShapingAdapter", "IFontRasterizerAdapter", "TextInputParityEvidenceRequest", "NativeEditorTsfTextStoreEvidence", "ITextStoreACP", "Console diagnostics", "AI Commands panel status/command/evidence rows", "Inspector property rows with visible edit command controls only for editable rich-text documents", "input_rebinding remains a workspace panel outside the current native shell panel set", "editor_shell_ui=first_party", "editor_shell_backend=d3d12", "editor_shell_imgui=0", "editor_shell_panels=11", "editor_shell_docking_status=single_window_ready", "editor_shell_dock_tab_headers=11", "editor_shell_dock_split_gutters=3", "editor_shell_dock_active_panels=4", "editor_shell_dock_focusable_controls=11", "editor_rich_text_edit_status=ready", "editor_rich_text_clipboard_plain_ready=1", "editor_rich_text_clipboard_rich_ready=1", "editor_rich_text_native_handles_exposed=0", "editor_shell_viewport_status=d3d12_texture_ready", "editor_shell_viewport_visible_texture_composites", "editor_shell_viewport_native_handles_exposed=0", "editor_shell_material_preview_status=d3d12_texture_ready", "editor_shell_material_preview_visible_texture_composites", "editor_shell_material_preview_native_handles_exposed=0", "editor_shell_text_atlas_handoff_status=glyphs_ready_atlas_handoff_host_gated", "editor_shell_text_font_adapter_invoked=1", "editor_shell_text_font_glyphs_ready=1", "editor_shell_text_font_fallback_used=0", "editor_shell_text_atlas_handoff_host_gated_rows=1", "editor_shell_text_atlas_handoff_unsupported_rows=1", "editor_shell_text_font_native_handles_exposed=0", "editor_shell_ime_status=win32_tsf_selected", "editor_shell_ime_caret_rect_rows=1", "editor_shell_ime_surrounding_text_rows=1", "editor_shell_ime_native_handles_exposed=0", "editor_ime_parity_status=ready", "editor_ime_windows_tsf_status=ready", "editor_ime_macos_status=host_gated", "editor_ime_linux_ibus_status=host_gated", "editor_ime_linux_fcitx_status=host_gated", "editor_ime_android_status=host_gated", "editor_ime_ios_status=host_gated", "editor_ime_native_handles_exposed=0", "editor_shell_renderer_boxes_submitted", "editor_shell_renderer_text_runs_available", "--no-user-config", "Resources panel refreshes D3D12 host availability", "private D3D12 texture adapters", "private visible compositor", "broader material-preview GPU parity", "runtime game UI Dear ImGui usage")) { Assert-ContainsText $currentEditorGuidance $requiredNeedle "gameCodeGuidance.currentEditor native shell" }
...
foreach ($requiredNeedle in @("native_editor_launch.hpp", "native_editor_app.hpp", "win32_first_party_editor_host.hpp", "parse_native_editor_launch", "validate_native_editor_launch", "native_editor_launch_usage_error_exit_code", "editor_shell_status=ready", "editor_shell_ui=first_party", "editor_shell_backend=", "editor_shell_imgui=0", "editor_shell_panels=", "editor_shell_docking_status=", "editor_shell_dock_tab_headers=", "editor_shell_dock_split_gutters=", "editor_shell_dock_active_panels=", "editor_shell_dock_focusable_controls=", "editor_rich_text_edit_status=", "editor_rich_text_clipboard_plain_ready=", "editor_rich_text_clipboard_rich_ready=", "editor_rich_text_native_handles_exposed=", "editor_ai_operation_excellence_status=", "editor_ai_operation_snapshot_rows=", "editor_ai_operation_command_rows=", "editor_ai_operation_mutating_commands_revision_checked=", "editor_ai_operation_native_handles_exposed=", "first_party_editor_excellence_status=", "first_party_editor_excellence=", "first_party_editor_excellence_windows_d3d12=", "first_party_editor_excellence_vulkan=", "first_party_editor_excellence_metal=", "first_party_editor_excellence_cross_platform=", "first_party_editor_excellence_text_parity=", "first_party_editor_excellence_accessibility_parity=", "first_party_editor_excellence_broad_optimization_claimed=", "first_party_editor_excellence_native_handles_exposed=", "editor_shell_sdl3=0", "editor_shell_material_preview_status=", "editor_shell_material_preview_native_handles_exposed=", "editor_shell_text_atlas_handoff_status=", "editor_shell_text_font_adapter_invoked=", "editor_shell_text_font_glyphs_ready=", "editor_shell_text_font_fallback_used=", "editor_shell_text_atlas_handoff_host_gated_rows=", "editor_shell_text_atlas_handoff_unsupported_rows=", "editor_shell_text_font_native_handles_exposed=", "editor_shell_ime_status=", "editor_shell_ime_caret_rect_rows=", "editor_shell_ime_surrounding_text_rows=", "editor_shell_ime_native_handles_exposed=", "editor_ime_parity_status=", "editor_ime_windows_tsf_status=", "editor_ime_macos_status=", "editor_ime_linux_ibus_status=", "editor_ime_linux_fcitx_status=", "editor_ime_android_status=", "editor_ime_ios_status=", "editor_ime_native_handles_exposed=", "editor_shell_renderer_boxes_submitted=", "editor_shell_renderer_text_runs_available=")) { Assert-ContainsText $editorMainText $requiredNeedle "native editor shell main" }
