...
foreach ($requiredNeedle in @("Windows-native editor/developer shell", "editor_shell_backend=d3d12", "editor_shell_panels=11", "editor_shell_viewport_status=diagnostic_only", "editor_shell_viewport_native_handles_exposed=0", "--no-user-config", "Resources panel refreshes D3D12 host availability", "private D3D12 texture adapter", "runtime game UI Dear ImGui usage")) { Assert-ContainsText $currentEditorGuidance $requiredNeedle "gameCodeGuidance.currentEditor native shell" }
