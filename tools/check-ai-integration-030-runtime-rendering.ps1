...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-runtime-lod-milestone-v1") { $mavgRuntimeLodRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG Runtime LOD Milestone v1", "static conventional MAVG LOD readiness", "CPU select_mavg_lod_clusters", "range-aware conventional indexed draws", "MeshCommand submission", "GPU culling", "indirect draw execution", "mesh shaders", "Metal readiness", "Nanite compatibility/equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgRuntimeLodRecommendedText $needle "recommended next plan MAVG runtime LOD selection reason" }
} else {
