...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "environment-highest-commercial-readiness-v1") { $environmentHighestRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.retainedCompletedEvidence), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("Environment Highest Commercial Readiness v1", "official docs/Context7", "Context7 gate partially verified", "environment_highest_readiness_code_edit_allowed=0", "strict Vulkan", "Apple Metal", "backend parity v2", "all-platform readiness", "broad measured optimization", "AAA preset asset library", "full OpenEXR/KTX2/Basis asset pipeline", "physical weather simulation", "production artist workflow", "commercial aggregate", "broad environment_ready", "unsupportedProductionGaps = []")) { Assert-ContainsText $environmentHighestRecommendedText $needle "recommended next plan environment highest commercial readiness selection reason" }
