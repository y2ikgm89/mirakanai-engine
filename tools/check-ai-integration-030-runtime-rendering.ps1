...
foreach ($requiredNeedle in @("Windows-native editor/developer shell", "editor_shell_backend=d3d12", "editor_shell_panels=10", "--no-user-config", "Resources panel refreshes D3D12 host availability", "runtime game UI Dear ImGui usage")) { Assert-ContainsText $currentEditorGuidance $requiredNeedle "gameCodeGuidance.currentEditor native shell" }
...
foreach ($requiredNeedle in @("native_editor_launch.hpp", "native_editor_app.hpp", "win32_imgui_d3d12_host.hpp", "parse_native_editor_launch", "validate_native_editor_launch", "native_editor_launch_usage_error_exit_code", "editor_shell_status=ready", "editor_shell_backend=d3d12", "editor_shell_panels=", "editor_shell_sdl3=0")) { Assert-ContainsText $editorMainText $requiredNeedle "native editor shell main" }
...
$editorHostText = Get-AgentSurfaceText "editor/src/win32_imgui_d3d12_host.cpp"
foreach ($requiredNeedle in @("make_native_editor_imgui_user_config_policy", "IniFilename = nullptr", "LogFilename = nullptr", "record_native_resource_device_ready", "render_native_editor_panels")) { Assert-ContainsText $editorHostText $requiredNeedle "native editor shell host" }
