...
if ($metalGate.Count -ne 1 -or $metalGate[0].status -ne "ready" -or $metalGate[0].residualClass -ne "ready") { Write-Error "engine/agent/manifest.json aiOperableProductionLoop must keep metal-apple ready after hosted Apple Metal closeout evidence" }
foreach ($recipe in @("renderer-metal-environment-aggregate-apple-host-evidence", "environment-platform-ios-metal-package", "environment-weather-metal-solver-host-gate")) { if (@($metalGate[0].validationRecipes) -notcontains $recipe) { Write-Error "engine/agent/manifest.json aiOperableProductionLoop metal-apple host gate must reference $recipe" } }
foreach ($needle in @("Apple Metal host gate closeout", "environment_metal_host_aggregate_ready=1", "environment_platform_macos_metal_ready=1", "environment_platform_ios_metal_ready=1", "environment_weather_simulation_metal_gpu_solver_ready=1", "environment_weather_simulation_metal_gpu_solver_native_handle_access=0", "environment_ready=0")) { Assert-ContainsText ([string]$metalGate[0].notes) $needle "engine/agent/manifest.json aiOperableProductionLoop metal-apple host gate notes" }
...
if ([string]$productionLoop.recommendedNextPlan.id -notin @("general-purpose-game-production-v1", "generated-game-studio-v1", "engine-1-0-gap-matrix-v1", "next-production-gap-selection", "native-win32-editor-shell-v1", "first-party-editor-shell-v1", "first-party-ui-editor-production-stack-v1", "physics-navigation-commercial-coverage-v1", "renderer-backend-parity-metal-apple-evidence-v1", "renderer-backend-parity-apple-evidence-closeout-v1", "renderer-postprocess-tone-mapping-evidence-v1", "renderer-commercial-quality-closeout-v1", "renderer-commercial-readiness-evidence-promotion-v1", "sandbox-world-network-modding-gate-v1", "sandbox-world-package-validation-performance-budgets-v1", "ai-operable-performance-budget-and-evidence-v1", "performance-baseline-v1", "long-running-performance-readiness-v1-phase-1", "long-running-performance-readiness-v1-phase-2", "long-running-performance-readiness-v1-phase-7", "memory-lifetime-taxonomy-v1", "memory-diagnostics-v1", "frame-thread-scratch-v1", "job-scheduling-evidence-v1", "job-execution-worker-pool-v1", "job-execution-topology-policy-v1", "job-execution-work-stealing-v1", "job-execution-placement-policy-v1", "windows-cpu-set-worker-placement-v1", "windows-cpu-set-smt-worker-placement-v1", "simd-dispatch-policy-and-evidence-v1", "avx2-reviewed-target-execution-v1", "environment-system-v1", "environment-commercial-excellence-v1", "environment-highest-commercial-readiness-v1", "mavg-research-legal-benchmark-baseline-v1", "mavg-runtime-lod-milestone-v1", "mavg-win32-directstorage-sdk-adapter-v1", "mavg-async-overlap-performance-proof-v1", "mavg-mesh-shader-capability-gate-v1", "mavg-deformation-backend-execution-evidence-v1", "2d-production-engine-capability-gap-cluster-v1", "2d-commercial-production-excellence-v1")) { foreach ($needle in @("Frame Graph Remaining Render Pass Envelopes v1", "Frame Graph Primary Pass Target-State Evidence v1", "Frame Graph RHI Queue Dependency Plan v1", "record_frame_graph_rhi_queue_waits", "Frame Graph RHI Multi-Queue Executor v1", "execute_frame_graph_rhi_multi_queue_schedule", "Frame Graph RHI Multi-Queue Texture Barrier Execution v1", "FrameGraphRhiMultiQueueExecutionResult::barriers_recorded", "Runtime Material Factor Frame Graph Command Evidence v1", "RuntimeMaterialGpuBinding", "Frame Graph v1 1.0 Scope Closeout v1 closes frame-graph-v1")) { Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.completedContext) $needle "recommended next plan completed context" } }
if ([string]$productionLoop.recommendedNextPlan.id -eq "general-purpose-game-production-v1") {
    ...
    foreach ($needle in @("First-Party Desktop Platform And SDL3 Removal v1", "MK_platform_win32", "MK_runtime_host_win32_presentation", "MK_audio_wasapi", "Job Execution Placement Policy v1", "job_execution_placement_policy_status=ready", "host-independent CPU placement policy evidence", "Windows CPU Set Worker Placement v1", "windows_cpu_set_worker_placement_status=ready", "windows_cpu_set_worker_placement_native_thread_handles_exposed=0", "selection gate", "MAVG Vulkan GPU Culling Dispatch v1", "mavg-vulkan-gpu-culling-dispatch-v1", "PR #563", "dispatch_mavg_gpu_culling_indirect", "MK_mavg_vulkan_gpu_culling_dispatch_tests", "MAVG GPU Culling Dispatch v1", "mavg-gpu-culling-dispatch-v1", "PR #556", "MK_mavg_gpu_culling_dispatch_tests", "unsupportedProductionGaps = []")) { Assert-ContainsText $selectionGateRecommendedText $needle "recommended next plan selection gate reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-async-overlap-performance-proof-v1") { $mavgAsyncOverlapRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8; foreach ($needle in @("MAVG Async Overlap Performance Proof v1", "RuntimeMavgStreamingUploadOverlapEvidenceResult", "RuntimeMavgAsyncOverlapPerformanceProofResult", "mavg_async_overlap_performance_proof_ready=1", "GPU DirectStorage destinations", "GDeflate", "DirectStorage 1.4 preview", "mesh shaders", "Metal readiness", "Nanite", "native handles", "broad optimization", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgAsyncOverlapRecommendedText $needle "recommended next plan MAVG async overlap performance proof reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "2d-production-engine-capability-gap-cluster-v1") { $twoDRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8; foreach ($needle in @("2D Production Engine Capability Gap Cluster v1", "RuntimeGameplayExecutionLoop2D", "execute_runtime_gameplay_loop_2d_step", "--require-2d-gameplay-execution-loop", "2d_gameplay_execution_loop_status=ready", "positive replay hash", "legal clean-room constraints", "official docs category-only", "unsupportedProductionGaps = []", "broad 2D production readiness")) { Assert-ContainsText $twoDRecommendedText $needle "recommended next plan 2D production engine capability selection reason" } } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "2d-commercial-production-excellence-v1") { $twoDCommercialRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8; foreach ($needle in @("2D Commercial Production Excellence", "2d-commercial-runtime-ui-closeout-v1", "TwoDCommercialRuntimeUiCloseoutResult", "evaluate_2d_commercial_runtime_ui_closeout", "RuntimeUiPlatformProductionResult", "RuntimeUiPackageSmokeSceneReview", "DirectWrite", "TSF", "UIA", "D3D12 atlas upload", "Context7/official-source", "public native handles", "UI middleware", "cross-platform parity", "legal approval", "Unity", "Unreal Engine", "Godot", "unsupportedProductionGaps = []", "Phase 6 renderer/RHI 2D commercial quality")) { Assert-ContainsText $twoDCommercialRecommendedText $needle "recommended next plan 2D commercial production excellence selection reason" }
...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "renderer-backend-parity-apple-evidence-closeout-v1") { foreach ($needle in @("Renderer Backend Parity Apple Evidence Closeout v1", "renderer-backend-parity-v1", "renderer-metal-apple-host-evidence", "Apple/Metal host evidence", "BackendRendererParityAppleMetalEnvironmentEvidenceDesc", "make_backend_renderer_parity_apple_metal_environment_proofs", "synchronization", "shader_validation", "package_evidence", "memory_residency", "profiling_capture", "Windows/Vulkan proof must not promote Metal readiness", "native handles remain hidden", "unsupportedProductionGaps = []")) { Assert-ContainsText $recommendedNextPlanText $needle "recommended next plan renderer backend parity Apple evidence closeout reason" }
...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "environment-commercial-excellence-v1") { $environmentCommercialRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.activeLocalSliceEvidence), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("Environment Commercial Excellence v1", "strict Vulkan aggregate readiness", "Metal host aggregate readiness", "backend parity", "all-platform readiness", "broad optimization", "OpenEXR/KTX/Basis asset-pipeline readiness", "AAA preset-library readiness", "physical weather simulation readiness", "artist-workflow readiness", "public native handles", "unsupportedProductionGaps = []", "Phase 0")) { Assert-ContainsText $environmentCommercialRecommendedText $needle "recommended next plan environment commercial excellence selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "environment-highest-commercial-readiness-v1") { $environmentHighestRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.retainedCompletedEvidence), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("Environment Highest Commercial Readiness v1", "official docs/Context7", "Source gate ready", "environment_highest_readiness_context7_verified_rows=9", "environment_highest_readiness_context7_partial_rows=2", "environment_highest_readiness_official_fallback_rows=2", "environment_highest_readiness_authoritative_doc_ready_rows=11", "environment_highest_readiness_source_gate_status=ready", "environment_highest_readiness_code_edit_allowed=1", "official_fallback.apple_metal_framework_api=ready", "official_fallback.khronos_gltf_spec=ready", "strict Vulkan", "Apple Metal", "backend parity v2", "all-platform readiness", "broad measured optimization", "AAA preset asset library", "full OpenEXR/KTX2/Basis asset pipeline", "physical weather simulation", "production artist workflow", "commercial aggregate", "broad environment_ready", "unsupportedProductionGaps = []")) { Assert-ContainsText $environmentHighestRecommendedText $needle "recommended next plan environment highest commercial readiness selection reason" }
...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-d3d12-indexed-indirect-draw-execution-v1") { $mavgD3d12IndirectRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG D3D12 Indexed Indirect Draw Execution v1", "mavg-rhi-indirect-draw-v1", "D3D12 ExecuteIndirect", "CPU-generated upload", "indexed indirect argument buffers", "D3D12_INDIRECT_ARGUMENT_TYPE_DRAW_INDEXED", "visible WARP-backed texture readback proof", "count-buffer execution", "compute-generated argument buffers", "Vulkan indirect draw execution", "actual GPU dispatch", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgD3d12IndirectRecommendedText $needle "recommended next plan MAVG D3D12 indexed indirect draw selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-vulkan-indexed-indirect-draw-execution-v1") { $mavgVulkanIndirectRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG Vulkan Indexed Indirect Draw Execution v1", "mavg-d3d12-indexed-indirect-draw-execution-v1", "vkCmdDrawIndexedIndirect", "CPU-generated upload", "indexed indirect argument buffers", "SPIR-V environment-gated visible texture readback proof", "count-buffer execution", "compute-generated argument buffers", "D3D12 changes", "actual GPU culling dispatch", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgVulkanIndirectRecommendedText $needle "recommended next plan MAVG Vulkan indexed indirect draw selection reason" } } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-d3d12-count-buffer-indirect-execution-v1") { $mavgD3d12CountBufferRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG D3D12 Count-Buffer Indirect Execution v1", "mavg-vulkan-indexed-indirect-draw-execution-v1", "ExecuteIndirect", "CountBufferOffset", "CPU-generated upload", "count buffer", "count-buffer Vulkan execution", "compute-generated count/argument buffers", "actual GPU culling dispatch", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgD3d12CountBufferRecommendedText $needle "recommended next plan MAVG D3D12 count-buffer indirect execution selection reason" } } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-vulkan-count-buffer-indirect-execution-v1") { $mavgVulkanCountBufferRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG Vulkan Count-Buffer Indirect Execution v1", "mavg-d3d12-count-buffer-indirect-execution-v1", "vkCmdDrawIndexedIndirectCount", "CPU-generated upload", "count buffer", "count-buffer Vulkan execution", "decode_indexed_indirect_count_buffer_value", "effective_indexed_indirect_draw_count", "compute-generated count/argument buffers", "D3D12 changes", "actual GPU culling dispatch", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgVulkanCountBufferRecommendedText $needle "recommended next plan MAVG Vulkan count-buffer indirect execution selection reason" } } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-gpu-culling-dispatch-v1") { $mavgGpuCullingDispatchRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG GPU Culling Dispatch v1", "mavg-vulkan-count-buffer-indirect-execution-v1", "MavgGpuCullingIndirectPlan", "MavgGpuCullingSyncRequirement", "dispatch_mavg_gpu_culling_indirect", "MK_mavg_gpu_culling_dispatch_tests", "D3D12 compute dispatch", "compute-write-to-indirect-read", "PR #552", "PR #553", "Vulkan compute dispatch", "compute-generated indirect consumption", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgGpuCullingDispatchRecommendedText $needle "recommended next plan MAVG GPU culling dispatch selection reason" } } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-d3d12-compute-generated-indirect-consumption-v1") { $mavgComputeGeneratedRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG D3D12 Compute-Generated Indirect Consumption v1", "mavg-gpu-culling-dispatch-v1", "dispatch_mavg_gpu_culling_indirect", "draw_indexed_indirect", "copy_source", "fail-closed", "PR #556", "PR #557", "PR #558", "MK_mavg_d3d12_compute_generated_indirect_consumption_tests", "Vulkan compute dispatch", "Vulkan compute-generated indirect consumption", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgComputeGeneratedRecommendedText $needle "recommended next plan MAVG D3D12 compute-generated indirect consumption selection reason" } } elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-vulkan-gpu-culling-dispatch-v1") { $mavgVulkanGpuCullingDispatchRecommendedText = (([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence), ([string]$productionLoop.recommendedNextPlan.completedContext), ([string]$productionLoop.recommendedNextPlan.reason)) -join " "; foreach ($needle in @("MAVG Vulkan GPU Culling Dispatch v1", "mavg-d3d12-compute-generated-indirect-consumption-v1", "dispatch_mavg_gpu_culling_indirect", "VK_KHR_synchronization2", "copy_source", "fail-closed", "PR #560", "PR #561", "MK_mavg_vulkan_gpu_culling_dispatch_tests", "Vulkan compute-generated indirect consumption", "D3D12 changes", "mesh shaders", "Nanite equivalence/superiority", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgVulkanGpuCullingDispatchRecommendedText $needle "recommended next plan MAVG Vulkan GPU culling dispatch selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-win32-directstorage-sdk-adapter-v1") { $mavgWin32DirectStorageRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8; foreach ($needle in @("MAVG Win32 DirectStorage SDK Adapter v1", "MAVG DirectStorage Page IO Execution v1", "Microsoft DirectStorage SDK 1.3.0", "vcpkg port dstorage 1.3.0", "find_package(dstorage CONFIG REQUIRED)", "Microsoft::DirectStorage", "dstorage.h", "DStorageGetFactory", "IDStorageFactory", "IDStorageQueue", "IDStorageStatusArray", "IDStorageQueue::EnqueueRequest", "IDStorageQueue::EnqueueStatus", "IDStorageQueue::Submit", "IDStorageStatusArray::GetHResult", "MK_platform_win32", "IByteRangeIoExecutor", "ByteRangeIoBackendKind::direct_storage", "first-party Win32 DirectStorage SDK adapter", "public native handles", "GPU destinations", "GDeflate", "DirectStorage 1.4 preview APIs", "async-overlap/performance proof", "mesh shaders", "Metal readiness", "Nanite", "broad optimization", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgWin32DirectStorageRecommendedText $needle "recommended next plan MAVG Win32 DirectStorage SDK adapter selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-mesh-shader-capability-gate-v1") { $mavgMeshShaderGateRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8; foreach ($needle in @("MAVG Mesh Shader Capability Gate v1", "RuntimeMavgMeshShaderCapabilityGateResult", "evaluate_runtime_mavg_mesh_shader_capability_gate", "D3D12_FEATURE_D3D12_OPTIONS7", "MeshShaderTier", "VK_EXT_mesh_shader", "VkPhysicalDeviceMeshShaderFeaturesEXT", "VkPhysicalDeviceMeshShaderPropertiesEXT", "mavg_mesh_shader_capability_gate_ready=1", "compute/indirect fallback", "mesh shader execution remains 0", "native handles", "Metal readiness", "Nanite", "broad MAVG backend readiness", "broad optimization", "unsupportedProductionGaps = []")) { Assert-ContainsText $mavgMeshShaderGateRecommendedText $needle "recommended next plan MAVG mesh shader capability gate selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "mavg-deformation-backend-execution-evidence-v1") { $mavgDeformationRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8; foreach ($needle in @("MAVG Deformation Backend Execution Evidence v1", "mavg-deformation-backend-execution-evidence-v1", "runtime scene RHI compute morph/skinned", "D3D12/Vulkan deformation integration", "explicit retained backend execution evidence rows", "native handles", "Metal readiness", "Nanite", "compatibility", "equivalence", "superiority", "unsupportedProductionGaps remains []")) { Assert-ContainsText $mavgDeformationRecommendedText $needle "recommended next plan MAVG deformation backend execution evidence selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "renderer-commercial-readiness-evidence-promotion-v1") { $rendererCommercialPromotionRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8; foreach ($needle in @("Renderer Commercial Readiness Evidence Promotion v1", "renderer-commercial-readiness-evidence-promotion-v1", "GameEngine.RendererCommercialReadinessEvidence.v1", "renderer_backend_parity_ready=0", "renderer_metal_broad_readiness=0", "renderer_broad_quality_ready=0", "renderer_commercial_readiness=0", "D3D12", "strict Vulkan", "Apple-host Metal", "renderer quality matrix", "production VFX/profiling", "Metal memory/profiling", "selected 3D/UI/environment/generated-game package evidence", "third-party notice", "JSON Schema", "Context7", "Unity", "Unreal Engine", "Godot", "external engine source", "sample code", "shader code", "UI expression", "assets", "trademarks", "compatibility claims", "equivalence claims", "parity claims", "unsupportedProductionGaps = []")) { Assert-ContainsText $rendererCommercialPromotionRecommendedText $needle "recommended next plan renderer commercial readiness evidence promotion selection reason" }
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "renderer-commercial-quality-closeout-v1") { $rendererCommercialCloseoutRecommendedText = $productionLoop.recommendedNextPlan | ConvertTo-Json -Depth 8; foreach ($needle in @("Renderer Commercial Quality Closeout v1", "renderer-commercial-quality-closeout-v1", "renderer_backend_parity_ready=0", "renderer_metal_broad_readiness=0", "renderer_broad_quality_ready=0", "renderer_commercial_readiness=0", "D3D12", "strict Vulkan", "Apple-host Metal", "renderer quality matrix", "production VFX/profiling", "Metal memory/profiling", "selected 3D/UI/environment/generated-game package evidence", "complete third-party notices", "Unity", "Unreal Engine", "Godot", "external engine source", "sample code", "shader code", "UI expression", "assets", "trademarks", "compatibility claims", "parity claims", "unsupportedProductionGaps = []")) { Assert-ContainsText $rendererCommercialCloseoutRecommendedText $needle "recommended next plan renderer commercial quality closeout selection reason" }
} else { foreach ($needle in @("Frame Graph v1", "upload-staging-v1", "scene-component-prefab-schema-v2", "2d-playable-vertical-slice", "3d-playable-vertical-slice")) { Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.reason) $needle "recommended next plan reason" } }
...
foreach ($postprocessPolicySamplePath in @("games/sample_desktop_runtime_game/main.cpp", "games/sample_generated_desktop_runtime_3d_package/main.cpp", "games/sample_generated_desktop_runtime_material_shader_package/main.cpp")) { $postprocessPolicySampleText = Get-AgentSurfaceText $postprocessPolicySamplePath; foreach ($postprocessPolicySampleNeedle in @("evaluate_win32_desktop_presentation_postprocess_policy", "postprocess_policy_status", "postprocess_policy_fog_effect")) { Assert-ContainsText $postprocessPolicySampleText $postprocessPolicySampleNeedle $postprocessPolicySamplePath } }
