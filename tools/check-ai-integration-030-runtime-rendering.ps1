...
$backendParityHostRecipeText = [string]$manifest.gameCodeGuidance.currentRenderingBackendParityHostRecipeProof; foreach ($backendParityHostRecipeNeedle in @("Backend Renderer Parity Host Recipe Proof v1", "host_validation_recipe_id", "missing_host_validation_recipe", "shader-toolchain", "mobile-packaging", "ios-simulator-smoke", "Metal", "host-gated", "D3D12", "Vulkan")) { Assert-ContainsText $backendParityHostRecipeText $backendParityHostRecipeNeedle "rendering game guidance backend parity host recipe proof" }; foreach ($backendParityHostRecipeGuidance in @("engine/agent/manifest.json", "docs/ai-game-development.md", "docs/current-capabilities.md", "docs/specs/generated-game-validation-scenarios.md", "docs/roadmap.md", "docs/superpowers/plans/README.md", "docs/superpowers/plans/2026-05-29-renderer-backend-parity-host-recipe-proof-v1.md", "docs/superpowers/master-plans/production-completion-v1/04-developer-owned-engine-capability-backlog.md", "docs/superpowers/master-plans/production-completion-v1/05-projections-and-scenarios.md", ".agents/skills/rendering-change/references/full-guidance.md", ".claude/skills/gameengine-rendering/references/full-guidance.md")) { $backendParityHostRecipeGuidanceText = Get-AgentSurfaceText $backendParityHostRecipeGuidance; foreach ($backendParityHostRecipeNeedle in @("host_validation_recipe_id", "missing_host_validation_recipe", "ios-simulator-smoke")) { Assert-ContainsText $backendParityHostRecipeGuidanceText $backendParityHostRecipeNeedle $backendParityHostRecipeGuidance } }; foreach ($needle in @("host_validation_recipe_id", "missing_host_validation_recipe", "BackendRendererParityProofRow", "BackendRendererParityDiagnosticCode")) { Assert-ContainsText (Get-AgentSurfaceText "engine/renderer/include/mirakana/renderer/backend_renderer_parity_policy.hpp") $needle "engine/renderer/include/mirakana/renderer/backend_renderer_parity_policy.hpp" }; foreach ($needle in @("host_validation_recipe_ready", "missing_host_validation_recipe", "host_validation_recipe_id")) { Assert-ContainsText (Get-AgentSurfaceText "engine/renderer/src/backend_renderer_parity_policy.cpp") $needle "engine/renderer/src/backend_renderer_parity_policy.cpp" }; foreach ($needle in @("requires host validation recipe for Metal host gates", "missing_host_validation_recipe", "ios-simulator-smoke")) { Assert-ContainsText (Get-AgentSurfaceText "tests/unit/renderer_rhi_tests.cpp") $needle "tests/unit/renderer_rhi_tests.cpp" }; if ([string]$productionLoop.recommendedNextPlan.id -notin @("sandbox-world-package-validation-performance-budgets-v1", "native-win32-editor-shell-v1", "first-party-editor-shell-v1")) { foreach ($needle in @("Renderer Backend Parity Host Recipe Proof v1", "unsupportedProductionGaps = []", "SDL3 stays absent")) { Assert-ContainsText ([string]$productionLoop.recommendedNextPlan.latestCloseoutEvidence) $needle "recommended next plan latest closeout evidence" } }; $geRendererRecentEvidenceText = @($geRendererModule[0].recentEvidence | ForEach-Object { [string]$_ }) -join " "; Assert-ContainsText $geRendererRecentEvidenceText "Renderer Backend Parity Host Recipe Proof v1" "MK_renderer module recent evidence"; if (@($geRendererModule[0].publicHeaders) -notcontains "engine/renderer/include/mirakana/renderer/backend_renderer_parity_policy.hpp") { Write-Error "engine/agent/manifest.json MK_renderer publicHeaders must include backend_renderer_parity_policy.hpp" }
...
foreach ($requiredNeedle in @("Windows-native editor/developer shell", "first-party retained", "MK_ui_renderer", "EditorDockLayout", "editor_dock_panel_catalog", "EditorDockCommandPlan", "plan_editor_dock_command", "apply_editor_dock_command", "EditorRichTextDocument", "input_rebinding remains a workspace panel outside the current native shell panel set", "editor_shell_ui=first_party", "editor_shell_backend=d3d12", "editor_shell_imgui=0", "editor_shell_panels=11", "editor_shell_viewport_status=diagnostic_only", "editor_shell_viewport_native_handles_exposed=0", "editor_shell_material_preview_status=diagnostic_only", "editor_shell_material_preview_native_handles_exposed=0", "editor_shell_renderer_boxes_submitted", "editor_shell_renderer_text_runs_available", "--no-user-config", "Resources panel refreshes D3D12 host availability", "private D3D12 texture adapters", "material-preview GPU parity remains unsupported", "runtime game UI Dear ImGui usage")) { Assert-ContainsText $currentEditorGuidance $requiredNeedle "gameCodeGuidance.currentEditor native shell" }
foreach ($requiredNeedle in @("First-Party Editor Shell v1", "EditorAiOperationSnapshot", "EditorAiCommandCatalog", "EditorAiCommandRequest", "EditorAiCommandDryRunResult", "EditorAiCommandApplyResult", "dry-run-before-apply AI panel visibility command rows", "core dock command planning", "not yet exposed through EditorAiOperationSnapshot", "editor.panel.resources.show", "editor.panel.ai_commands.show", "editor.panel.profiler.hide", "visual scraping")) { Assert-ContainsText $currentEditorGuidance $requiredNeedle "gameCodeGuidance.currentEditor AI operation surface" }
...
foreach ($nativeEditorNeedle in @("add_library(MK_editor_shell_common", "core/src/editor_dock_layout.cpp", "core/src/editor_rich_text.cpp", "src/first_party_editor_document.cpp", "src/native_editor_launch.cpp", "add_library(MK_editor_shell_win32", "src/win32_first_party_editor_host.cpp", "MK_renderer", "MK_ui_renderer", "add_executable(MK_editor", "MK_platform_win32", "MK_ENABLE_DESKTOP_EDITOR is Windows-only", "MK_ENABLE_DESKTOP_EDITOR requires MK_platform_win32", "Native MK_editor shell is SDL3-free")) { Assert-ContainsText $editorCmake $nativeEditorNeedle "native editor shell contract" }
