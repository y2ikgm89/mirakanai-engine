...
$expectedEnvironmentPlatformHostRecipes = @(
    @{
        ...
        Needles = @("host=linux", "vulkaninfo_ready=1", "VK_LAYER_KHRONOS_validation_ready=1", "dxc_spirv_codegen_ready=1", "spirv_val_ready=1", "linux_icd_runtime_ready=1", "first_party_linux_runtime_host_ready=1", "linux_package_script_ready=1", "linux_installed_validator_ready=1", "linux_vulkan_runtime_probe_ready=1", "linux_vulkan_runtime_readback_ready=1", "linux_vulkan_runtime_probe_surface_family=offscreen_compute", "environment_platform_linux_vulkan_ready=1", "environment_platform_requires_linux_vulkan_host_evidence=0")
    },
    @{
...
$expectedEnvironmentPlatformReadinessRows = @(
    ...
    @{
        ...
        state = "ready"
        needles = @("Linux Vulkan", "Windows Vulkan evidence", "Vulkan SDK", "validation layers", "first-party Linux runtime host", "Win32/x64-windows", "tools/package-linux-runtime.ps1", "tools/validate-installed-linux-runtime.ps1", "tools/validate-linux-vulkan-runtime-host.ps1", "environment-platform-linux-vulkan-package", "linux-vulkan-runtime-host", "linux_vulkan_runtime_probe", "vulkaninfo_ready=1", "VK_LAYER_KHRONOS_validation_ready=1", "dxc_spirv_codegen_ready=1", "spirv_val_ready=1", "linux_icd_runtime_ready=1", "first_party_linux_runtime_host_ready=1", "linux_package_script_ready=1", "linux_installed_validator_ready=1", "linux_vulkan_runtime_probe_ready=1", "linux_vulkan_runtime_readback_ready=1", "linux_vulkan_runtime_probe_surface_family=offscreen_compute", "environment_platform_linux_vulkan_ready=1", "environment_platform_requires_linux_vulkan_host_evidence=0")
    },
    @{
...
foreach ($sourceSurface in @(
        ...
        @{ Path = "tools/validate-linux-vulkan-runtime-host.ps1"; Needles = @("Find-VulkanInfoCommand", "--summary", "VK_LAYER_KHRONOS_validation", "ExpectedEvidenceCounters", "actualCounterLine", '$missingExpectedCounters', "environment-platform-linux-vulkan-package is missing expected actual counters", "dxc_spirv_codegen_ready", "spirv_val_ready", "linux_icd_runtime_ready", "first_party_linux_runtime_host_ready", "linux_package_script_ready", "linux_installed_validator_ready", "linux_vulkan_runtime_probe_ready", "linux_vulkan_runtime_readback_ready", "linux_vulkan_runtime_probe_surface_family", "environment_platform_linux_vulkan_ready", "windows_vulkan_inferred=0") },
        @{ Path = "tools/check-shader-toolchain.ps1"; Needles = @("tools/vulkan-tools", "vulkan-tools", "VCPKG_DEFAULT_TRIPLET", "VCPKG_TARGET_TRIPLET", "x64-linux", "x64-osx", "arm64-osx", 'Join-IfEnvSet "VULKAN_SDK" "bin"', 'Join-IfEnvSet "VK_SDK_PATH" "bin"') },
        ...
        @{ Path = "tools/smoke-android-package.ps1"; Needles = @("logcat", "Mirakanai:I", "VALIDATION:W", "android_vulkan_readback_ready=1", "android_vulkan_validation_layer_enumerated=1", "android_vulkan_validation_log_clean=1", "android-smoke-log") },
        @{ Path = "platform/android/app/src/main/cpp/android_vulkan_readback_smoke.cpp"; Needles = @("vkEnumerateInstanceLayerProperties", "VK_LAYER_KHRONOS_validation", "vkCmdFillBuffer", "vkCmdPipelineBarrier", "vkDeviceWaitIdle", "vkWaitForFences", "vkMapMemory", "android_vulkan_readback_ready=%d", "android_vulkan_readback_backend=offscreen_transfer", "android_vulkan_validation_layer_enumerated=%d") },
        ...
        @{ Path = ".github/workflows/validate.yml"; Needles = @("Environment Metal aggregate host evidence recipe", "validate-environment-metal-host-aggregate.ps1", "Linux Vulkan Evidence", "ubuntu-24.04", "Install LunarG Vulkan SDK and Lavapipe ICD", "vulkaninfo --summary", "Build Linux Vulkan runtime package", "tools/package-linux-runtime.ps1", "validate-linux-vulkan-runtime-host.ps1 -RequireReady", "linux_vulkan_runtime_probe_ready=1", "linux_vulkan_runtime_readback_ready=1", "environment_platform_linux_vulkan_ready=1", "linux-vulkan-evidence") },
        ...
        @{ Path = "tests/unit/runtime_rhi_environment_platform_evidence_v2_tests.cpp"; Needles = @("windows_vulkan_evidence_does_not_promote_linux_or_android_vulkan", "linux_vulkan_ready_requires_exact_linux_host_gate_and_tool_rows", "android_vulkan_ready_requires_android_device_gpu_debug_layer_settings_and_readback", "native_handle_or_inferred_platform_rows_keep_ready_false") },
