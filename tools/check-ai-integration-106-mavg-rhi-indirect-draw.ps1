...
if ($manifest.aiOperableProductionLoop.currentActivePlan -ne "docs/superpowers/plans/2026-06-08-mavg-d3d12-count-buffer-indirect-execution-v1.md") {
    Write-Error "engine/agent/manifest.json currentActivePlan must point at the active MAVG D3D12 count-buffer indirect execution follow-up"
}
...
if ($manifest.aiOperableProductionLoop.recommendedNextPlan.completedContext -notlike "*mavg-vulkan-indexed-indirect-draw-execution-v1*") {
    Write-Error "engine/agent/manifest.json recommendedNextPlan.completedContext must retain mavg-vulkan-indexed-indirect-draw-execution-v1 prerequisite evidence"
}
