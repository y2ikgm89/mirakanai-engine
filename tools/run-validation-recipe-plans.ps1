...
function Get-ValidationRecipeCommandPlan {
    ...
    elseif ($RecipeName -eq 'desktop-runtime-sample-game-environment-optimization-measurement') {
        ...
        $pwEntry = Get-DesktopRuntimePackageCommandPlan -ScriptPath $packageScript -GameTarget $target -RequireD3d12Shaders -SmokeArgs $smokeTail
        $diagEnvironmentOptimization = New-RunnerDiagnostic -Severity 'info' -Code 'host-evidence-required' -Message 'Environment optimization measurement package validation is restricted to the reviewed sample_desktop_runtime_game D3D12 package lane. It emits preset_pack_flythrough, storm_precipitation, dense_volumetric_fog, and volumetric_cloud_sunset before/after measurement rows, regression-budget counters, and zero diagnostics/native-handle/GPU-command side effects while keeping environment_broad_optimization_ready=0 until all seven workloads, backend parity, retained host profiler artifacts, and regression guards are complete.' -ValidationRecipe $RecipeName -HostGate 'd3d12-windows-primary'
