...
function Get-ValidationRecipeCommandPlan {
    ...
    elseif ($RecipeName -eq 'renderer-metal-apple-host-evidence') {
        ...
        $diagMetal = New-RunnerDiagnostic -Severity 'info' -Code 'host-gate-acknowledged' -Message 'Renderer Metal Apple host evidence requires macOS with full Xcode/Metal tools, then runs the ci-macos-appleclang configure/build/CTest path for MK_backend_scaffold_tests and MK_renderer_quality_matrix_tests. The recipe builds generated environment_feature_evidence.metallib and visible_renderer_package_evidence.metallib, requires feature-local render/compute pipeline, cube/HDR texture, synchronization/readback proof, selected visible 3D material/lighting/postprocess readback, UI atlas upload/readback, environment package row consumption, generated-game package output row consumption, and emits metal_environment_* plus renderer_metal_visible_package_* counters. The environment counters may feed make_backend_renderer_parity_apple_metal_environment_proofs only for backend-local synchronization, shader_validation, and package_evidence rows. memory_residency and profiling_capture stay separate through make_backend_renderer_parity_apple_metal_memory_profiling_proofs and remain host-gated unless exact heap/residency-set/capture evidence is supplied; the recipe does not mark backend parity, broad Metal readiness, broad renderer quality, commercial renderer readiness, or broad environment_ready ready by inference.' -ValidationRecipe $RecipeName -HostGate 'metal-apple'
    ...
    elseif ($RecipeName -eq 'renderer-metal-memory-profiling-capable-host-runner-preflight') {
        $cmdPlanEntry = Get-PwshScriptCommandPlan -ScriptPath 'tools/validate-renderer-metal-memory-profiling-capable-host-runner.ps1'
        $diagMetalRunnerPreflight = New-RunnerDiagnostic -Severity 'info' -Code 'host-evidence-required' -Message 'Renderer Metal memory/profiling capable-host runner preflight uses the GitHub REST repository self-hosted runner endpoint through gh api or a fixture JSON to count idle online runners labelled self-hosted, macOS, ARM64, and metal-residency-set before dispatching the workflow_dispatch capable-host lane. Dry-run does not require RepoFullName, does not dispatch a workflow, does not run Metal validation, and does not promote renderer commercial readiness.' -ValidationRecipe $RecipeName -HostGate 'metal-apple'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('metal-apple') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagMetalRunnerPreflight)
    }
    elseif ($RecipeName -eq 'renderer-public-runner-security-review') {
        $cmdPlanEntry = Get-PwshScriptCommandPlan -ScriptPath 'tools/generate-renderer-public-runner-security-review.ps1' -ScriptArguments @('-Mode', 'Plan')
        $diagPublicRunnerReview = New-RunnerDiagnostic -Severity 'info' -Code 'public-runner-security-review-required' -Message 'Renderer public runner security review generates a retained GameEngine.RendererPublicSelfHostedRunnerSecurityReview.v1 artifact only after the operator explicitly confirms public-repository self-hosted runner risk review, public fork PR risk, runner isolation, secret exposure, exact runner labels, Metal probe truth, and a workflow audit for workflow_dispatch-only execution, read-only contents permission, pinned checkout, disabled checkout credential persistence, and no untrusted PR triggers. Dry-run uses Plan mode, does not fetch or print registration tokens, does not register a runner, does not dispatch workflows, does not run GPU workloads, and does not promote renderer commercial readiness.' -ValidationRecipe $RecipeName -HostGate 'metal-apple'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('metal-apple', 'renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagPublicRunnerReview)
    }
    elseif ($RecipeName -eq 'renderer-commercial-readiness-final-handoff') {
        $cmdPlanEntry = Get-PwshScriptCommandPlan -ScriptPath 'tools/validate-renderer-commercial-readiness-final-handoff.ps1'
        $diagRendererCommercialFinalHandoff = New-RunnerDiagnostic -Severity 'info' -Code 'final-handoff-plan-only' -Message 'Renderer commercial readiness final handoff dry-run reads an optional retained artifact intake manifest, optional capable-host runner preflight fixture, and optional -UseRunDiscovery GitHub Actions run/artifact metadata through the final run discovery planner, then emits the next operator command: runner preflight, public runner security review, capable-host Metal memory/profiling workflow, final-from-runs workflow, final retained-root assembler, or final preflight. Dry-run does not call gh workflow run, does not download artifacts, does not run GPU workloads, does not assemble evidence, and does not promote renderer commercial readiness.' -ValidationRecipe $RecipeName -HostGate 'renderer-commercial-closeout'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('github-actions-artifact-import', 'metal-apple', 'renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagRendererCommercialFinalHandoff)
    }
    elseif ($RecipeName -eq 'renderer-commercial-readiness-final-run-discovery') {
        $cmdPlanEntry = Get-PwshScriptCommandPlan -ScriptPath 'tools/plan-renderer-commercial-readiness-final-run-discovery.ps1' -ScriptArguments @('-Mode', 'Plan')
        $diagRendererCommercialFinalRunDiscovery = New-RunnerDiagnostic -Severity 'info' -Code 'final-run-discovery-plan-only' -Message 'Renderer commercial readiness final run discovery dry-run reads GitHub Actions workflow run and artifact-list metadata through fixture JSON or gh api against the official actions/runs and actions/runs/{run_id}/artifacts REST endpoints. It emits exact source_artifact_run_id and metal_memory_profiling_run_id handoff rows for renderer-commercial-readiness-final-from-runs.yml without dispatching workflows, downloading artifacts, registering runners, running GPU workloads, assembling evidence, or promoting renderer commercial readiness.' -ValidationRecipe $RecipeName -HostGate 'renderer-commercial-closeout'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('github-actions-artifact-import', 'metal-apple', 'renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagRendererCommercialFinalRunDiscovery)
    }
    elseif ($RecipeName -eq 'renderer-commercial-quality-closeout') {
        $cmdPlanEntry = Get-RepositoryToolCommandPlan -ToolScriptName 'validate-renderer-commercial-quality-closeout.ps1'
        $diagRendererCommercialCloseout = New-RunnerDiagnostic -Severity 'info' -Code 'final-aggregate-fail-closed' -Message 'Renderer Commercial Quality Closeout v1 default validation runs the local fail-closed aggregate and emits exact non-ready evidence counters without requiring Apple tools. It reports renderer_commercial_quality_closeout_value_api_ready=1, selected evidence row counts, clean-room/legal counters, zero native-handle/cross-backend/external-engine parity counters, and keeps renderer_backend_parity_ready, renderer_metal_broad_readiness, renderer_broad_quality_ready, and renderer_commercial_readiness at 0. The direct -RequireReady invocation may emit the final four ready counters only when selected D3D12, strict Vulkan, Apple-host Metal, renderer quality matrix, production VFX/profiling, Metal memory/profiling, visible 3D/runtime UI/environment/generated-game package rows, static guards, clean-room review, third-party notices, and legal approval for any external material are supplied. iOS Metal evidence is not selected by this recipe unless the operator explicitly consumes iOS package evidence through the validator.' -ValidationRecipe $RecipeName -HostGate 'renderer-commercial-closeout'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('d3d12-windows-primary', 'vulkan-strict', 'metal-apple', 'renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagRendererCommercialCloseout)
    }
    elseif ($RecipeName -eq 'renderer-commercial-readiness-evidence') {
        $cmdPlanEntry = Get-RepositoryToolCommandPlan -ToolScriptName 'validate-renderer-commercial-readiness-evidence.ps1'
        $diagRendererCommercialReadinessEvidence = New-RunnerDiagnostic -Severity 'info' -Code 'retained-artifact-fail-closed' -Message 'Renderer Commercial Readiness Evidence Promotion v1 validates GameEngine.RendererCommercialReadinessEvidence.v1 retained artifact bundles, verifies referenced artifacts and SHA-256 values, consumes renderer-commercial-quality-closeout and renderer-metal-memory-profiling-host-evidence retained output, emits renderer_commercial_readiness_evidence_* counters, keeps fixture_only=1 for test fixtures, keeps renderer_environment_ready=0, and only permits renderer_backend_parity_ready, renderer_metal_broad_readiness, renderer_broad_quality_ready, and renderer_commercial_readiness when exact D3D12, strict Vulkan, Apple-host Metal, Metal memory/profiling, package-visible 3D/UI/environment/generated-game, clean-room/legal, static guard, and third-party notice rows are present without native handles, cross-backend inference, or Unity/Unreal Engine/Godot compatibility/equivalence/parity claims.' -ValidationRecipe $RecipeName -HostGate 'renderer-commercial-closeout'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('d3d12-windows-primary', 'vulkan-strict', 'metal-apple', 'renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagRendererCommercialReadinessEvidence)
    }
    elseif ($RecipeName -eq 'renderer-clean-room-legal-review-input') {
        $cmdPlanEntry = Get-PwshScriptCommandPlan -ScriptPath 'tools/generate-renderer-clean-room-legal-review-input.ps1' -ScriptArguments @('-Mode', 'Plan')
        $diagRendererCleanRoomLegalReviewInput = New-RunnerDiagnostic -Severity 'info' -Code 'clean-room-review-input-plan-only' -Message 'Renderer clean-room legal review input dry-run uses Mode Plan only. It documents the host-independent retained GameEngine.RendererCleanRoomLegalReviewInput.v1 generator and the renderer-clean-room-legal-review-artifacts CI artifact path without writing evidence, importing Unity/Unreal/Godot material, making legal advice claims, or promoting renderer commercial readiness.' -ValidationRecipe $RecipeName -HostGate 'renderer-commercial-closeout'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagRendererCleanRoomLegalReviewInput)
    }
    elseif ($RecipeName -eq 'renderer-apple-metal-commercial-quality-host-evidence') {
        $cmdPlanEntry = Get-PwshScriptCommandPlan -ScriptPath 'tools/generate-renderer-apple-metal-commercial-quality-host-evidence.ps1' -ScriptArguments @('-Mode', 'Plan')
        $diagAppleMetalHostEvidence = New-RunnerDiagnostic -Severity 'info' -Code 'apple-metal-host-evidence-plan-only' -Message 'Renderer Apple Metal commercial quality host evidence dry-run uses Mode Plan only. It documents the retained GameEngine.RendererAppleMetalCommercialQualityHostEvidence.v1 generator and the renderer-apple-metal-commercial-quality-host-evidence CI artifact path without running Metal validation, writing evidence, inferring readiness from D3D12/Vulkan, or promoting renderer commercial readiness.' -ValidationRecipe $RecipeName -HostGate 'metal-apple'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('metal-apple', 'renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagAppleMetalHostEvidence)
    }
    elseif ($RecipeName -eq 'renderer-commercial-readiness-final-promotion-preflight') {
        $cmdPlanEntry = Get-RepositoryToolCommandPlan -ToolScriptName 'validate-renderer-commercial-readiness-final-promotion-preflight.ps1'
        $diagRendererCommercialFinalPreflight = New-RunnerDiagnostic -Severity 'info' -Code 'final-retained-root-fail-closed' -Message 'Renderer commercial readiness final promotion preflight checks the default retained root in diagnostic mode only. It reports the exact 12 required files, copied-fixture blockers, schema/claim/recipe mismatches, missing full Metal memory/profiling evidence, missing clean-room legal evidence, and downstream validator readiness while keeping renderer_backend_parity_ready, renderer_metal_broad_readiness, renderer_broad_quality_ready, renderer_commercial_readiness, and renderer_environment_ready at 0 until a real non-fixture retained root is supplied separately with -RequireReady.' -ValidationRecipe $RecipeName -HostGate 'renderer-commercial-closeout'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('d3d12-windows-primary', 'vulkan-strict', 'metal-apple', 'renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagRendererCommercialFinalPreflight)
    }
    elseif ($RecipeName -eq 'renderer-commercial-readiness-final-retained-root-assembler') {
        $cmdPlanEntry = Get-PwshScriptCommandPlan -ScriptPath 'tools/assemble-renderer-commercial-readiness-final-retained-root.ps1' -ScriptArguments @('-Mode', 'Plan')
        $diagRendererCommercialFinalAssembler = New-RunnerDiagnostic -Severity 'info' -Code 'final-retained-root-plan-only' -Message 'Renderer commercial readiness final retained-root assembler dry-run uses Mode Plan only. It reports that seven explicit D3D12, strict Vulkan, Apple-host Metal, Metal memory/profiling, package, quality/VFX, and clean-room/legal inputs are required, but does not run producers, assemble evidence, download artifacts, execute GPU work, or pass -RequireReady.' -ValidationRecipe $RecipeName -HostGate 'renderer-commercial-closeout'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('d3d12-windows-primary', 'vulkan-strict', 'metal-apple', 'renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagRendererCommercialFinalAssembler)
    }
    elseif ($RecipeName -eq 'renderer-commercial-readiness-final-retained-root-artifact-import') {
        $cmdPlanEntry = Get-PwshScriptCommandPlan -ScriptPath 'tools/import-renderer-commercial-readiness-final-retained-root-artifacts.ps1' -ScriptArguments @('-Mode', 'Plan')
        $diagRendererCommercialFinalArtifactImport = New-RunnerDiagnostic -Severity 'info' -Code 'artifact-intake-plan-only' -Message 'Renderer commercial readiness final retained-root artifact import dry-run uses Mode Plan only. It documents the GitHub Actions artifact intake contract for renderer-commercial-readiness-final-retained-root, related host artifacts, workflow_artifact_list rows, assembler_handoff rows, final_preflight_handoff rows, and host-gate summaries without requiring RunId, invoking gh run download, writing imported artifacts, or promoting renderer commercial readiness.' -ValidationRecipe $RecipeName -HostGate 'renderer-commercial-closeout'
        return New-RecipePlanRow -Recipe $RecipeName -CommandPlan @($cmdPlanEntry) -HostGates @('github-actions-artifact-import', 'renderer-commercial-closeout') -RequiredAcknowledgements @() -AllowedGameTargets @() -AllowedStrictBackend @() -Diagnostics @($diagRendererCommercialFinalArtifactImport)
    }
