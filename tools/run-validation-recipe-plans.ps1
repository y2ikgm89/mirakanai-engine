...
function Get-ValidationRecipeCommandPlan {
    ...
    elseif ($RecipeName -eq 'renderer-public-runner-security-review') {
        ...
        $diagPublicRunnerReview = New-RunnerDiagnostic -Severity 'info' -Code 'public-runner-security-review-required' -Message 'Renderer public runner security review generates a retained GameEngine.RendererPublicSelfHostedRunnerSecurityReview.v1 artifact only after the operator explicitly confirms public-repository self-hosted runner risk review, public fork PR risk, runner isolation, secret exposure, allowed workflow, exact runner labels, and Metal probe truth. Dry-run uses Plan mode, does not fetch or print registration tokens, does not register a runner, does not dispatch workflows, does not run GPU workloads, and does not promote renderer commercial readiness.' -ValidationRecipe $RecipeName -HostGate 'metal-apple'
