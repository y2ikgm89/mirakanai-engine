...
function Get-ValidationRecipeCommandPlan {
    ...
    elseif ($RecipeName -eq 'desktop-runtime-sample-game-environment-weather-simulation-package') {
        ...
        $pwEntry = Get-DesktopRuntimePackageCommandPlan -ScriptPath $packageScript -GameTarget $target -RequireD3d12Shaders -SmokeArgs $smokeTail
        $diagEnvironmentWeatherSimulation = New-RunnerDiagnostic -Severity 'info' -Code 'package-evidence-reviewed' -Message 'Environment weather simulation package validation is restricted to the reviewed sample_desktop_runtime_game package lane. It emits deterministic CPU reference counters for one clamped timestep, four cells, water-conservation/error bounds, CPU fallback use, selected CPU reference solver elapsed/budget counters, solver_budget_status=host_evidence_required, zero diagnostics/native-handle/GPU/backend side effects, GPU solver ready=0, profiler artifacts=0, production solver ready=0, and environment_physical_weather_simulation_ready=0; it does not promote complete physical weather simulation, backend parity, broad optimization, commercial readiness, or broad environment_ready claims.' -ValidationRecipe $RecipeName
