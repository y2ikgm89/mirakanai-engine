...
} elseif ([string]$productionLoop.recommendedNextPlan.id -eq "2d-commercial-production-excellence-v1") { foreach ($needle in @("2D Commercial Production Excellence v1", "Phase 5", "DirectWrite", "TSF", "UIA", "D3D12 atlas upload", "Context7", "official-source", "Phase 6 renderer/RHI 2D commercial quality", "unsupportedProductionGaps = []", "public native handles", "UI middleware", "cross-platform parity", "legal approval", "Unity", "Unreal Engine", "Godot")) { Assert-ContainsText $recommendedNextPlanText $needle "engine/agent/manifest.json aiOperableProductionLoop recommendedNextPlan 2D commercial production excellence selection" }
