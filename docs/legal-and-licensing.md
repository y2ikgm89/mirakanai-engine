...
Asset Import Regression Corpus v1 follows the same fail-closed policy. The source tree may track only first-party generated text fixtures under `tests/fixtures/asset_import_regression/` unless a separate legal review records every external asset. Large real glTF, texture, material, animation, and audio corpora are host-owned by default under `out/host-artifacts/asset-import-regression-corpus/`; they must pass `tools/validate-asset-import-regression-corpus.ps1 -CorpusRoot <path>` before importer execution and `-RequireReady` before commercial corpus promotion. Accepted third-party corpus asset licenses are `CC0-1.0`, `MIT`, `BSD-2-Clause`, `BSD-3-Clause`, `Apache-2.0`, `Zlib`, or `CC-BY-4.0` with complete attribution and notices. Reject missing-license, ambiguous marketplace, `CC-BY-NC-*`, `CC-BY-ND-*`, trademark-heavy, AI-generated asset with unclear rights, and Unity, Unreal Engine, Godot, or other external-engine sample material unless a separate legal approval record exists. Corpus docs and reports may reference Unity, Unreal Engine, and Godot only as legal/trademark/category research sources and must not claim compatibility, equivalence, parity, replacement, marketplace ingestion, or project/schema import.
...
`docs/specs/2026-06-30-2d-commercial-clean-room-source-ledger-v1.md` records the allowed source classes for 2D commercial production work. `tools/check-2d-commercial-clean-room.ps1` enforces public/product-facing forbidden-token and compatibility-claim checks, `tools/check-2d-commercial-source-diagnostics.ps1` enforces explicit copied-material, external-schema, trademark, missing-notice, unapproved-dependency, and external-engine claim diagnostics, and `tools/generate-2d-commercial-clean-room-review-input.ps1` emits `GameEngine.TwoDCommercialCleanRoomReviewInput.v1` and `GameEngine.TwoDCommercialOfficialSourceSummary.v1` records for counsel review. The retained schemas live in `schemas/2d-commercial-clean-room-review-input.schema.json` and `schemas/2d-commercial-official-source-summary.schema.json`. This gate does not add third-party code, assets, shaders, samples, package-manager dependencies, or redistribution obligations, and the generated records are engineering review inputs without legal conclusions or legal approval claims.

`tools/generate-2d-commercial-release-legal-review-input.ps1`, `tools/check-2d-commercial-release-legal-review-input.ps1`, and `tools/validate-2d-commercial-release-legal-gate.ps1` add the 2D Commercial Release Legal Gate v1 handoff for the selected desktop runtime package. The retained schemas are `schemas/2d-commercial-release-legal-review-input.schema.json` and `schemas/2d-commercial-release-official-source-summary.schema.json`. The generated rows are counsel-ready engineering review input for package contents, notices, dependency manifests, source provenance, clean-room/trademark guards, distribution artifact inventory, generated asset review, and official Microsoft MSIX signing (`https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview`), Apple notarization/distribution (`https://developer.apple.com/documentation/security/notarizing-macos-software-before-distribution`), Android app signing (`https://developer.android.com/studio/publish/app-signing`), repository clean-room ledger, and repository legal policy references. This is not legal advice, does not grant legal approval, and does not claim completed signing, notarization, app-store certification, Play upload, store distribution, external-engine compatibility/parity/replacement/endorsement, copied material, or native-handle exposure.
