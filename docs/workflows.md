...
- **Windows MSVC cache refresh:** selected Windows build lanes restore vcpkg package/install caches and the newest compatible build tree with `actions/cache/restore`, then attempt vcpkg cache refreshes and a new `github.sha`-keyed build cache with non-blocking `actions/cache/save` only after validation succeeds. Keep configure/dependency/toolchain inputs in the restore prefix so stale object trees are rebuilt by MSBuild rather than trusted as validation evidence; cache-save transport or archive failures must not make a validated job red.
