...
Apple Metal iOS Host Evidence v1 current Windows evidence is host-gated: `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-apple-host-evidence.ps1` reports `host=windows`, `xcode=blocked`, `ios-simulator=blocked`, `metal-library=blocked`, missing `xcodebuild`, missing `xcrun`, missing iOS SDK/runtime access, missing `metal`, missing `metallib`, and present iOS Simulator/macOS Metal workflow coverage. `tools/validate-apple-metal-platform-host.ps1 -Platform ios -RequireReady` is the stricter iOS Metal platform gate: it runs only on macOS/full-Xcode hosts and requires the iOS smoke evidence-file counters before `environment_platform_ios_metal_ready=1`. Run `tools/check-apple-host-evidence.ps1 -RequireReady` only on a macOS/full-Xcode host when the task requires hard Apple-ready evidence.
