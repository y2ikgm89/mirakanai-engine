...
- For **generated static 3D desktop package** operator sequencing (`DesktopRuntime3DPackage`, recipe `3d-playable-desktop-package`), treat the committed `games/sample_generated_desktop_runtime_3d_package` source as legacy traceability until the target is ported to the Win32 host or deleted. Current readiness comes from `engine/agent/manifest.json`, the plan registry, and validation evidence for currently registered desktop-runtime targets.
