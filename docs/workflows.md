...
- Plan-file width and PR/phase width are different: a plan may cover a capability/gap-cluster/milestone with multiple phase checkpoints, while each production-completion PR defaults to one `unsupportedProductionGaps` row, or one explicitly named phase inside that row when the row is too large for one review.
...
Use GitHub's official GitHub Flow for agent publishing: make a separate topic branch for each unrelated change, commit isolated complete changes, create a pull request for review, merge only after required reviews/checks pass, then delete the merged branch. Cadence is purpose/checkpoint-based, not count-based: commit complete task-owned checkpoints and push validated checkpoints for remote backup/CI/review/handoff. For production-completion work, the default PR unit is exactly one `unsupportedProductionGaps` row. If one row is too large to review or validate safely, split it into explicitly named phase PRs inside that same row. Do not open separate PRs for plan slices, checklist items, validation-only follow-up, or docs/manifest/static-check synchronization that belongs to the same active gap or phase. Split unrelated gaps or non-gap fixes into separate branches and PRs. Publish only task-owned changes and keep branch selection conservative.
...
PR cadence is purpose-based, not commit-based: one production-completion PR should represent one `unsupportedProductionGaps` row, one explicitly named phase inside that row, or one unrelated non-gap fix. Do not open a PR for every commit, plan slice, checklist item, or validation-only follow-up; keep related checkpoint commits in the same draft PR until final validation, and split only when the work crosses gap rows, is a clearly unrelated fix, or the selected gap row has distinct named review/validation phases.

For large or review-sensitive gap rows or named phases, open one draft PR after the first coherent validated push rather than waiting for the final local polish pass. Draft PRs are the preferred early feedback surface; keep them draft while follow-up checkpoint commits are still expected on that same PR, and move to ready-for-review only after final validation, branch preflight, and task-owned PR evidence are in place. In unattended Codex sessions, `gh pr ready` and other PR state changes remain prompt-gated; use GitHub Web/Desktop or an approval-capable session when conversion is required and approvals are unavailable.
...
In unattended Codex sessions, `gh pr create` may run automatically after the branch is pushed and the validation evidence is ready. Use `--draft` for large gap rows, named phases, or early feedback; create a ready PR only when the work is ready for review. The PR body must include actual validation evidence or blockers, and the head branch must be task-owned.
