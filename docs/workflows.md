...
- **Windows MSVC cache refresh:** selected Windows build lanes restore the newest compatible build tree with `actions/cache/restore`, then save a new `github.sha`-keyed build cache with `actions/cache/save` only after validation succeeds. Keep configure/dependency/toolchain inputs in the restore prefix so stale object trees are rebuilt by MSBuild rather than trusted as validation evidence.
...
- Windows PR lane: `tools/validate.ps1 -SkipStaticChecks` after `Agent Static Guards` covers `tools/validate.ps1 -StaticOnly -StaticJobs 1`; local/default full validation remains `tools/validate.ps1`
