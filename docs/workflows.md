...
- `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1` includes `toolchain-check` before CMake configure/build/test work, runs independent static checks in a bounded parallel group, stores complete parallel-check logs under ignored `out/validation-logs/`, runs one `build.ps1` configure/build gate with automatic CMake parallelism, reuses that gate's CMake File API reply for the clang-tidy smoke unless `-SkipTidySmoke` is explicitly passed, and then runs `test.ps1 -SkipBuild` with automatic CMake/CTest parallelism. Pass `-StaticJobs <N>` to `tools/validate.ps1` only when a local host needs a lower or higher static-check throttle. Hosted CI uses `-StaticOnly -StaticJobs 1` for the dedicated static lane and `-SkipStaticChecks -SkipTidySmoke` for the Windows MSVC lane; local slice gates should keep the default full path unless a narrower check is explicitly justified. Pass `-Jobs <N>` to `tools/build.ps1` or `tools/test.ps1` only when a local host needs an explicit lower throttle.
...
- **Windows MSVC cache refresh:** selected Windows build lanes restore the newest compatible build tree with `actions/cache/restore`, then attempt a new `github.sha`-keyed build cache with non-blocking `actions/cache/save` only after validation succeeds. Keep configure/dependency/toolchain inputs in the restore prefix so stale object trees are rebuilt by MSBuild rather than trusted as validation evidence; cache-save transport or archive failures must not make a validated job red.
