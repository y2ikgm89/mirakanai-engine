...
- Run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/evaluate-cpp23.ps1 -Release` for release packaging and `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/evaluate-cpp23.ps1 -Gui` for the optional SDL3/Dear ImGui editor path. Use `-Debug -Release -Gui` only when all C++23 lanes need one local confidence pass.
