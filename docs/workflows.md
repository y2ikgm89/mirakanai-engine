...
`tools/check-publication-preflight.ps1` is the supported guard for the linked-worktree Git admin write probe, `origin` reachability, and host-local `gh auth status`. For linked worktrees, also verify the `git rev-parse --git-path index.lock` parent directory is writable by the current process before `git add`; if it is not, switch session mode or host context instead of changing repository permissions ad hoc.
...
`pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-mobile-packaging.ps1` is included in default validation as a diagnostic-only gate. It verifies Android/iOS template presence and reports Android SDK, Android NDK, JDK, Gradle, `adb`, `apksigner`, Android Emulator, configured Android AVDs, connected device/emulator smoke readiness, macOS, full Xcode selection, `xcrun`, iPhoneOS/iPhone Simulator SDK availability, iOS Simulator runtime availability, Android Release signing, and Apple signing diagnostics without claiming package readiness. Blocker lines include stable `kind=<slug>` fields for agent/CI classification. Its Android Emulator `-list-avds` probe is bounded so a hosted or local static validation lane reports `probe-timeout` instead of hanging on host tool state. The `adb devices` probe is skipped in GitHub Actions static validation unless `-RequireAndroid` or `MK_MOBILE_DEVICE_PROBE=1` explicitly requests a live device probe, reporting `android-device-smoke=probe-skipped-ci`; `tools/validate.ps1` also sets `MK_MOBILE_DEVICE_PROBE=0` for the default diagnostic run so local adb background servers cannot hold redirected validation pipes open. Run `tools/check-mobile-packaging.ps1` directly, or use `-RequireAndroid`, when a live device probe is required. `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-apple-host-evidence.ps1` adds the Apple Metal iOS Host Evidence v1 view over the same boundary plus Xcode-resolved `metal`/`metallib` readiness and repository iOS/macOS workflow coverage; on non-Apple hosts it should report `apple-host-evidence-check: host-gated` without failing default validation.
