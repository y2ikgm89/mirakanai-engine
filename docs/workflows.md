...
- Run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/evaluate-cpp23.ps1 -Release` for release packaging. `-Gui` is currently fail-closed because the visible editor shell is deferred during SDL3 removal; `MK_editor_core` remains covered by the default Debug lane. The script uses automatic CMake/CTest parallelism by default; pass `-Jobs <N>` only to throttle a constrained host.
