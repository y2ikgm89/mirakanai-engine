...
- `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1` includes `toolchain-check` before CMake configure/build/test work, runs one `build.ps1` configure/build gate, reuses that gate's CMake File API reply for the clang-tidy smoke, and then runs `test.ps1 -SkipBuild`.
