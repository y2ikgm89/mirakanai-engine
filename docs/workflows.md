...
7. Run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1`.
...
- Direct `cmake --preset ...` commands assume CMake is available on `PATH` and, on Windows, that the parent process does not expose both `PATH` and `Path`. Prefer repository wrappers or Visual Studio Developer PowerShell/Command Prompt for direct CMake use. Use `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-toolchain.ps1 -RequireDirectCMake` to make those preconditions fail-fast.
...
- Checked-in CMake configure/build presets inherit `normalized-configure-environment` / `normalized-build-environment`, expose one child `PATH` from parent `PATH`/`Path` variants, and unset duplicate `Path` for Windows CMake and MSBuild tool tasks; keep these hidden presets inherited by every visible configure/build preset.
