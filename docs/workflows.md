...
- **Docs/agent/rules/subagent-only:** the hosted `agent-static` job keeps the required gate meaningful with PR diff whitespace and `tools/validate.ps1 -StaticOnly -StaticJobs 1 -StaticCheckTimeoutSeconds 120`, including formatting, agents, JSON contracts, AI integration, CI matrix, public API, package artifact, production-readiness, and host-gate static checks. Do not run `Windows MSVC`, optional Windows lanes, `macOS Metal CMake`, or `Full Repository Static Analysis` just because instructions, skills, rules, settings, subagents, or prose docs changed.
- **CI/build/tooling policy:** run the matching static guard and the affected hosted lane(s). `validate.yml` edits select all hosted lanes including coverage because they can change lane routing, runner setup, and gate semantics. `ios-validate.yml` edits select the iOS Metal evidence lane. Workflow-dispatch-only supplemental proof workflows that are not branch-protection-required, such as `.github/workflows/renderer-metal-memory-profiling-capable-host.yml`, are proven by agent/static plus `tools/check-ci-matrix.ps1` unless the edit also changes required validation routing, hosted lane setup, runtime/build code, or package tooling. Classifier/check-matrix edits use the always-running agent/static lane with `ci-classifier-policy` unless they also change the required workflow YAML or a runtime/build surface. A clang-tidy policy change should prove static analysis; a Windows runner/toolchain change should prove Windows.
...
- **Windows cache refresh:** selected Windows build lanes restore the `external/vcpkg` tool checkout, vcpkg package/install caches only when the lane consumes optional vcpkg features, and the newest compatible build tree with `actions/cache/restore`; restore transport failures are non-blocking and fall back to cloning/bootstrap/build work. After validation succeeds, the lanes attempt vcpkg/tool/build cache refreshes with non-blocking `actions/cache/save`. Keep configure/dependency/toolchain inputs in the restore prefix so stale object trees are rebuilt by MSBuild rather than trusted as validation evidence; cache transport or archive failures must not make validation red.
...
- For first-party diagnostics operations handoff, use `mirakana::build_diagnostics_ops_plan` over a `DiagnosticCapture` to produce a `DiagnosticsOpsPlan`: summary and Chrome Trace Event JSON rows are ready through `summarize_diagnostics` and `export_diagnostics_trace_json`, pasted supported Trace Event JSON can be reviewed through `review_diagnostics_trace_json`, native crash dump review is host-gated until the caller supplies Debugging Tools for Windows, symbol-path, dump-policy, and operator handoff evidence through `DiagnosticsCrashReviewAdapterDesc`, and telemetry upload stays unsupported without a `DiagnosticsTelemetryBackendDesc` or host-gated until backend id, producer, OpenTelemetry `resourceSpans` / `service.name` contract review, local path redaction, secret redaction, and operator handoff evidence are present. `MK_core` still does not upload telemetry or write native dumps.
...
- Run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/bootstrap-deps.ps1 -Feature <feature>` before optional vcpkg-backed dependency lanes when `vcpkg_installed/` is missing. Bootstrap is the only supported dependency acquisition path; vcpkg-backed CMake presets set `VCPKG_MANIFEST_INSTALL=OFF` and consume `${sourceDir}/vcpkg_installed` without configure-time package installation.
...
Apple Metal iOS Host Evidence v1 current Windows evidence is host-gated: `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-apple-host-evidence.ps1` reports `host=windows`, `xcode=blocked`, `ios-simulator=blocked`, `metal-library=blocked`, missing `xcodebuild`, missing `xcrun`, missing iOS SDK/runtime access, missing `metal`, missing `metallib`, and present iOS Simulator/macOS Metal workflow coverage. `tools/validate-apple-metal-platform-host.ps1 -Platform ios -RequireReady` is the strict iOS Metal platform gate: it runs only on macOS/full-Xcode hosts, is required by the `validate.yml` `ios-metal` job, and requires the iOS smoke evidence-file counters before `environment_platform_ios_metal_ready=1`. Hosted CI passes `-SkipIosBuild` after an explicit `tools/build-mobile-apple.ps1` build step so build and simulator failures remain separate in logs. Run `tools/check-apple-host-evidence.ps1 -RequireReady` only on a macOS/full-Xcode host when the task requires hard Apple-ready evidence.
