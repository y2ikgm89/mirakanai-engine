...
Define the clean-room architecture baseline for Mirakana Adaptive Virtual Geometry (MAVG) and the current data-only/runtime-evidence implementation boundary. This document is a specification and audit record, not a broad renderer/runtime ready-claim. The stacked implementation children add reviewed graph, payload, selection, page streaming planning, conventional upload, and selected D3D12 proof rows only; they do not add autonomous streaming workers, broad backend parity, deformation, ray tracing payloads, or benchmark superiority.

## Status

Phase 0 specification completed for `mavg-research-legal-benchmark-baseline-v1`. The parent stacked implementation milestone is `mavg-runtime-lod-milestone-v1` over the `mavg-asset-graph-v1` foundation, and the active pointer now selects `mavg-directstorage-request-plan-v1` as the focused DirectStorage request-plan child after `mavg-payload-byte-range-file-io-v1` completed as the file-backed page loading child. The LoD checkpoints implement deterministic `MK_assets` `GameEngine.MavgClusterGraph.v1` hierarchy/error/fallback/draw-range graph validation, `MK_assets` `mavg_cluster_payload.hpp` / `MavgClusterPayloadDocument` / `MavgClusterPayloadPage` / `MavgClusterPayloadCluster` / `validate_mavg_cluster_payload` page-addressable `GameEngine.MavgClusterPayload.v1` validation with `vertex.data_hex`, `index.data_hex`, and `page.data_hex`, `MK_tools` static payload rows through `MavgClusterCookVertex`, `MK_runtime` no-IO selected page extraction through `mavg_payload_pages.hpp` / `RuntimeMavgPayloadPageSliceResult` / `extract_runtime_mavg_payload_page_slices`, first-party `IFileSystem` binary byte-range APIs through `read_bytes` / `write_bytes` / `read_byte_range`, `MemoryFileSystem` / `RootedFileSystem` exact range coverage, `MK_runtime` file-backed selected page loading through `RuntimeMavgPayloadPageFileLoadResult` / `load_runtime_mavg_payload_file_pages` with `used_native_directstorage=false`, `MK_runtime` DirectStorage request planning through `RuntimeMavgPayloadDirectStorageRequestPlanDesc` / `RuntimeMavgPayloadDirectStorageRequestRow` / `RuntimeMavgPayloadDirectStorageRequestPlanResult` / `RuntimeMavgPayloadDirectStorageFenceWaitPoint` / `plan_runtime_mavg_payload_directstorage_requests` with source byte offsets/sizes, contiguous destination offsets, fence wait-point intent, `requires_native_directstorage_sdk=true`, and zero native enqueue/submit/fence/file-IO side effects, `MK_renderer` CPU reference selection through `mavg_lod_selection.hpp` / `select_mavg_lod_clusters`, `MK_runtime` resident-page and caller-reviewed page request streaming evidence through `mavg_page_streaming.hpp` / `RuntimeMavgPageStreamingPlanResult` / `RuntimeMavgPageStreamingEvictionReviewResult` / `RuntimeMavgPageStreamingDispatchPlan` / `RuntimeMavgPageStreamingDrainDesc` / `review_runtime_mavg_page_streaming_evictions` / `plan_runtime_mavg_page_streaming_dispatches` / `execute_runtime_mavg_page_streaming_request_safe_point`, range-aware conventional indexed draw execution, `MK_scene_renderer` conventional `MeshCommand` planning, and `MK_runtime_rhi` conventional package-visible MAVG mesh binding upload evidence through `mavg_conventional_upload.hpp` / `RuntimeMavgConventionalMeshUploadResult` / `upload_runtime_mavg_conventional_mesh_binding`. The completed prerequisite children `mavg-gpu-culling-indirect-v1`, `mavg-rhi-indirect-draw-v1`, `mavg-d3d12-indexed-indirect-draw-execution-v1`, `mavg-d3d12-indexed-indirect-count-buffer-execution-v1`, `mavg-d3d12-compute-generated-indirect-execution-v1`, and `mavg-d3d12-gpu-culling-execution-v1` add value-only `mavg_gpu_culling.hpp` / `MavgGpuCullingIndirectPlan` / `plan_mavg_gpu_culling_indirect_commands` GPU culling/indirect command planning, packed indexed indirect command rows, reviewed culling bounds, D3D12/Vulkan synchronization requirement rows, backend-neutral `IndexedIndirectDrawDesc` / `IRhiCommandList::draw_indexed_indirect`, Null RHI argument/count-buffer deterministic execution counters, Vulkan indirect-buffer usage mapping, D3D12 `ExecuteIndirect` count-buffer execution for CPU-generated upload indexed indirect argument/count buffers with zero-count plus `min(count, MaxCommandCount)` clamp evidence, and D3D12 compute-generated indexed indirect argument/count buffer execution for default-heap `BufferUsage::storage | BufferUsage::indirect` buffers with explicit compute-to-graphics queue wait evidence, backend-private storage UAV transition/barrier tracking, transition to `D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT`, same-buffer argument/count offset execution, WARP-backed visible readback proof, fail-closed usage validation, and no CPU decode/stat overclaim for GPU-generated command bytes. The completed D3D12 GPU culling child adds a D3D12 WARP-backed MAVG GPU culling dispatch proof: `MavgGpuCullingIndirectPlan` supplies the CPU reference expectation, a compute shader consumes selected-cluster command rows plus visibility rows, compacts only the visible cluster into a same-buffer indexed indirect argument/count layout, executes it through `ExecuteIndirect`, verifies the visible center triangle and culled top-left probe, and records compute dispatch / queue wait / GPU-generated indirect / UAV barrier / indirect transition stats; native DirectStorage queues/fences/status arrays, Win32 async IO, autonomous background workers, async-overlap/performance, automatic eviction policy, GPU memory pressure integration, generic GPU culling frameworks, generic GPU-generated count-buffer systems beyond this D3D12 storage-indirect lane, generic storage-buffer state management, Vulkan indirect draw execution, mesh shaders, deformation, ray tracing, Metal readiness, Nanite equivalence/superiority, and benchmark superiority remain unclaimed until later focused tasks add code and validation evidence.

## Current Repository Baseline

- `engine/agent/manifest.json.aiOperableProductionLoop.currentActivePlan` selects `docs/superpowers/plans/2026-06-05-mavg-directstorage-request-plan-v1.md` as the focused active child over `docs/superpowers/plans/2026-06-05-mavg-runtime-lod-milestone-v1.md` after the stacked byte-range file IO child completed through draft PR #459.
- Completed prerequisite `mavg-package-streaming-residency-dispatch-v1` evidence remains retained through `RuntimeMavgPageStreamingDispatchPlan`, `plan_runtime_mavg_page_streaming_dispatches`, `RuntimeMavgPageStreamingDrainDesc`, caller-owned safe-point/background rows, and explicit non-claims for autonomous background workers, async overlap/performance, partial native byte-range streaming execution, automatic eviction policy, GPU memory pressure integration, and Nanite equivalence/superiority.
- Completed prerequisite `mavg-page-addressable-payload-schema-v1` evidence remains retained through `mavg_cluster_payload.hpp`, `MavgClusterPayloadDocument`, `MavgClusterPayloadPage`, `validate_mavg_cluster_payload`, `page.data_hex`, `mavg_payload_pages.hpp`, `RuntimeMavgPayloadPageSliceResult`, and `extract_runtime_mavg_payload_page_slices` without DirectStorage/Win32 async IO, autonomous background workers, async overlap/performance, automatic eviction policy, GPU memory pressure integration, or Nanite claims.
...
- Expose `RuntimeMavgPageStreamingCandidateRow`, `RuntimeMavgPageStreamingPlanResult`, `RuntimeMavgPageStreamingDrainDesc`, `RuntimeMavgPageStreamingDrainResult`, `RuntimeMavgResidentPageMountRow`, `RuntimeMavgPageStreamingSelectedClusterRow`, `RuntimeMavgPageStreamingEvictionReviewResult`, and `RuntimeMavgPageStreamingDispatchPlan` as the narrow runtime MAVG page streaming evidence rows.
...
- Native DirectStorage/Win32 async queue, fence, status, and worker execution over the validated byte-range payload page rows.
- Autonomous worker execution, engine-owned thread scheduling, native queue/fence/status integration, and async overlap evidence.
