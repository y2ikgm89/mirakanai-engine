...
- Execute D3D12 indexed indirect draws without count buffers through backend-private `D3D12_INDIRECT_ARGUMENT_TYPE_DRAW_INDEXED` command signatures and `ID3D12GraphicsCommandList::ExecuteIndirect` when the argument buffer is a CPU-generated upload buffer with `BufferUsage::indirect | BufferUsage::copy_source`.
- Keep D3D12 count-buffer execution, compute-generated indirect buffers, and public buffer-state tracking fail-closed until explicit follow-up plans add feature gates and synchronization proof.

Future responsibilities:

- Actual compute culling dispatch and command/count buffer writes.
- D3D12 count-buffer execution plus compute-written argument/count buffer synchronization.
