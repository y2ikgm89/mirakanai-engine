...
- Autonomous background/page package streaming workers, async overlap/performance evidence, filesystem byte-range IO, automatic eviction policy, and GPU memory pressure integration.
...
- Filesystem-backed partial `.mavgpayload` byte-range IO after the caller-owned byte-range loader.
