...
- Keep generic GPU-generated count-buffer systems beyond this D3D12 storage-indirect lane, generic storage-buffer state management, public buffer-state tracking, and Vulkan/Metal backend execution fail-closed until explicit follow-up plans add feature gates and synchronization proof.

Future responsibilities:

- Actual compute culling dispatch that derives command/count rows from visibility tests.
