...
- `engine/agent/manifest.json.aiOperableProductionLoop.currentActivePlan` selects `docs/superpowers/plans/2026-06-10-mavg-vulkan-gpu-culling-dispatch-v1.md` with `recommendedNextPlan.id = mavg-vulkan-gpu-culling-dispatch-v1` over completed MAVG D3D12 Compute-Generated Indirect Consumption v1 (`mavg-d3d12-compute-generated-indirect-consumption-v1`) through PR #560 and plan draft PR #561; activation keeps Vulkan compute dispatch fail-closed until the independent implementation PR lands while Vulkan `draw_indexed_indirect` still requires `copy_source` upload usage in v1, completed D3D12 `dispatch_mavg_gpu_culling_indirect` with WARP-backed `MK_mavg_gpu_culling_dispatch_tests` evidence remains authoritative, and D3D12 compute-generated indirect consumption is implemented through `is_compute_generated_indexed_indirect_buffer`, `native_buffer_resource`, `leave_indirect_argument_state_for_consumption`, and WARP-backed `MK_mavg_d3d12_compute_generated_indirect_consumption_tests`; Vulkan compute-generated indirect consumption and D3D12 changes remain unclaimed.
