...
- MAVG DirectStorage Page IO Execution v1 (mavg-directstorage-page-io-execution-v1) adds IByteRangeIoExecutor, ByteRangeIoBackendKind::direct_storage, RuntimeMavgPayloadDirectStoragePageLoadDesc, and load_runtime_mavg_payload_pages_from_direct_storage so reviewed .mavgpayload format prefix and page ranges can execute through a caller-owned DirectStorage byte-range executor. MK_runtime_mavg_payload_page_loader_tests prove requested-order page rows, unavailable-executor fail-closed behavior, invalid-prefix rejection before page reads, read-failure rejection without partial rows, no filesystem fallback, no mount mutation, no background worker, no GPU memory policy, and no renderer/RHI/native handle side effects. A first-party Win32 DirectStorage SDK adapter, dstorage.h / dstorage.lib linkage, GPU destinations, GDeflate, async-overlap/performance proof, and broad streaming readiness remain unclaimed.
...
- Persistent/autonomous background package streaming services, async overlap/performance proof, first-party Win32 DirectStorage SDK adapter execution, backend draw execution, mesh shader execution, and broad GPU memory pressure enforcement beyond value-only resident byte-budget eviction planning.
