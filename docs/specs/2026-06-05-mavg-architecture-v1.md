...
- Follow-up child MAVG DirectStorage Native Execution v1 (`mavg-directstorage-native-execution-v1`) evidence is limited to optional Windows-only `MK_runtime_host_win32_directstorage`, `Win32MavgPayloadDirectStorageDispatcherDesc`, `Win32MavgPayloadDirectStorageDispatcher`, private `DStorageGetFactory`, `IDStorageFactory::OpenFile`, `CreateQueue`, `CreateStatusArray`, `IDStorageQueue::EnqueueRequest`, `EnqueueStatus`, `Submit`, `IDStorageStatusArray` polling, `used_native_directstorage=true`, `enqueued_native_requests`, `submitted_native_queue`, and `enqueued_status_write` for file-to-memory request rows. It keeps default builds and MK_runtime public APIs SDK-free and handle-free, rejects unsafe source paths before factory creation, fails `signal_fence_after_requests` before native side effects, and does not add native fence signaling, D3D12 resource destinations, GPU decompression, resident mount mutation, allocator/GPU budget enforcement, Vulkan/Metal parity, async-overlap/performance proof, or Nanite readiness.
...
- Broader sustained background/page package streaming workers beyond the joined engine-owned proof, async overlap/performance evidence, DirectStorage native fence signaling, D3D12 resource-destination DirectStorage IO, GPU decompression, and GPU memory pressure integration.
...
- Full real GPU residency enforcement/service, allocator enforcement, renderer/RHI memory mutation beyond the narrow `IRhiDevice::execute_residency_action` committed-resource boundary, and backend-native pressure harvesting.
