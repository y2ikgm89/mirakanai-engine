...
Phase 0 specification completed for `mavg-research-legal-benchmark-baseline-v1`. The parent stacked implementation milestone is `mavg-runtime-lod-milestone-v1` over the `mavg-asset-graph-v1` foundation, and the active pointer now selects `mavg-win32-iocp-file-io-worker-v1` after `mavg-win32-async-file-io-adapter-v1` completed through draft PR #463, `mavg-native-directstorage-win32-async-io-dispatch-status-v1` completed through draft PR #462, and `mavg-directstorage-request-plan-v1` completed as draft PR #460. The LoD checkpoints implement deterministic graph/payload/selection/page-streaming/conventional-renderer evidence, DirectStorage-shaped request planning through `RuntimeMavgPayloadDirectStorageRequestPlanResult`, caller-owned handle-free adapter/status APIs through `IRuntimeMavgPayloadNativeIoDispatcher`, `RuntimeMavgPayloadNativeIoDispatchResult`, `RuntimeMavgPayloadNativeIoStatusPollResult`, `dispatch_runtime_mavg_payload_native_io_requests`, and `poll_runtime_mavg_payload_native_io_status`, a caller-polled Win32 overlapped file-read adapter through `Win32MavgPayloadAsyncFileIoDispatcher`, and now an IOCP worker adapter through `Win32MavgPayloadIocpFileIoDispatcher`, `CreateIoCompletionPort`, `GetQueuedCompletionStatus`, `PostQueuedCompletionStatus`, `CreateFileW`, `ReadFile`, `OVERLAPPED`, and `CancelIoEx`. Concrete DirectStorage SDK installation, `dstorage.h`, `IDStorageFactory`, `IDStorageQueue`, `IDStorageStatusArray`, `IDStorageQueue3`, `ID3D12Fence`, autonomous package streaming workers, async-overlap/performance, automatic eviction policy, GPU memory pressure integration, generic GPU culling frameworks beyond the selected D3D12 proof, Vulkan indirect draw execution, mesh shaders, deformation, ray tracing, Metal readiness, Nanite equivalence/superiority, and benchmark superiority remain unclaimed until later focused tasks add code and validation evidence.

## Current Repository Baseline

- `engine/agent/manifest.json.aiOperableProductionLoop.currentActivePlan` selects `docs/superpowers/plans/2026-06-06-mavg-win32-iocp-file-io-worker-v1.md` as the focused active child over `docs/superpowers/plans/2026-06-05-mavg-runtime-lod-milestone-v1.md` after the Win32 adapter child completed through draft PR #463, the native status child completed through draft PR #462, and the request-plan child completed through draft PR #460.
- Active child `mavg-win32-iocp-file-io-worker-v1` evidence is retained through `Win32MavgPayloadIocpFileIoDispatcherDesc`, `Win32MavgPayloadIocpFileIoDispatcher`, `CreateIoCompletionPort`, `GetQueuedCompletionStatus`, `PostQueuedCompletionStatus`, `CreateFileW`, `FILE_FLAG_OVERLAPPED`, `OVERLAPPED`, `ReadFile`, `CancelIoEx`, private worker threads, deterministic `max_inflight_submissions`, and `executed_background_worker=true`; it has no `dstorage.h` compile path, no DirectStorage SDK dependency, no DirectStorage native queue/status/fence execution, no async-overlap/performance claim, and no Nanite claim.
