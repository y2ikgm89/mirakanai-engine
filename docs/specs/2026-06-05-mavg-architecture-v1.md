...
- Broader sustained background/page package streaming workers beyond the joined engine-owned proof, async overlap/performance evidence, DirectStorage native file IO execution, and GPU memory pressure integration.
...
- Support explicit value-only automatic eviction policies over those protected resident-page rows: deterministic page-index ordering, caller-supplied recency ordering, runtime-inferred LRU ordering, runtime-inferred frequency ordering, and caller-supplied GPU memory pressure ordering through `RuntimeMavgPageStreamingGpuMemoryPressureRow`.
...
- Real GPU residency enforcement, allocator enforcement, renderer/RHI memory mutation, and backend-native pressure harvesting.
