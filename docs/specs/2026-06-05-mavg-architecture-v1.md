...
- `engine/agent/manifest.json.aiOperableProductionLoop.currentActivePlan` selects `docs/superpowers/plans/2026-06-10-mavg-d3d12-compute-generated-indirect-consumption-v1.md` with `recommendedNextPlan.id = mavg-d3d12-compute-generated-indirect-consumption-v1` after MAVG GPU Culling Dispatch v1 completed through PR #556, closeout PR #557, and plan draft PR #558; activation keeps D3D12 compute-generated indirect consumption fail-closed until the independent implementation PR lands while `draw_indexed_indirect` still requires `copy_source` upload usage in v1 and completed `dispatch_mavg_gpu_culling_indirect` with WARP-backed `MK_mavg_gpu_culling_dispatch_tests` evidence remains authoritative; Vulkan compute dispatch and Vulkan compute-generated indirect consumption remain unclaimed.
