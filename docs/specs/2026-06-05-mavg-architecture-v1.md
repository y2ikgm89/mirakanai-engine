...
- MAVG Streaming Upload Overlap Evidence v1 (`mavg-streaming-upload-overlap-evidence-v1`) adds `mavg_streaming_upload_overlap_evidence.hpp`, `RuntimeMavgStreamingUploadOverlapEvidenceDesc`, `RuntimeMavgStreamingUploadOverlapEvidenceResult`, `RuntimeMavgStreamingUploadEvidenceWindow`, `background_load_window`, `gpu_upload_window`, and `plan_runtime_mavg_streaming_upload_overlap_evidence` in `MK_runtime_rhi` so callers can record deterministic temporal overlap from caller-owned timing windows between successful closeout background-load evidence, safe-point adoption evidence, and streamed-cluster GPU upload evidence. It reports `recorded_temporal_overlap_evidence` and `overlap_tick_count` while keeping `claimed_speedup=false` and `proved_async_overlap_performance=false`; it does not claim persistent/autonomous streaming services, DirectStorage, backend execution, mesh shader execution, renderer/RHI native handles, measured speedup, broad async-overlap/performance proof, Nanite readiness, or broad optimization.
...
- Autonomous/background dispatch policy and measured async-overlap/performance proof beyond the caller-owned timing-window evidence recorded by MAVG Streaming Upload Overlap Evidence v1.
