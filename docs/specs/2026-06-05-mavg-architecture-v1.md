...
- MAVG Ray Tracing Consistency Diagnostics v1 (`mavg-ray-tracing-consistency-diagnostics-v1`) adds `mavg_ray_tracing_consistency.hpp`, `MavgRasterClusterPayloadRow`, `MavgRayTracingClusterPayloadRow`, `MavgRayTracingConsistencyResult`, and `plan_mavg_ray_tracing_consistency` in `MK_assets` for host-independent raster/RT payload consistency diagnostics over a caller-owned `MavgClusterGraphDocument`. It does not allocate/build/refit acceleration-structure resources, execute backend RT execution or D3D12/Vulkan RT work, expose native handles, claim RT parity, or infer Metal/Nanite readiness.
