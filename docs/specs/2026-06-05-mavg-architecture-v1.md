...
- MAVG Quality Governor Benchmark Harness v1 (`mavg-quality-governor-benchmark-harness-v1`) adds `mavg_quality_governor.hpp`, `MavgQualityGovernorResult`, `MavgQualityGovernorCounterRow`, `MavgQualityGovernorLimits`, `evaluate_mavg_quality_governor`, `has_mavg_quality_governor_diagnostic`, and `tools/benchmark-mavg.ps1` so future measured benchmark rows have a deterministic budget, host-gate, backend-local-evidence, no-hole/fallback, RT-consistency, unsupported-claim, and replay-hash gate. Metal and explicit host-gated rows remain `host_evidence_required`; the dry-run harness rejects control/native-token input and emits `mavg-benchmark-harness:` rows with `executes_benchmark=false`, `writes_artifacts=false`, `invokes_profiler=false`, and `native_handles=false`. It does not claim benchmark execution/results, benchmark superiority, Nanite compatibility/equivalence/superiority, broad CPU/GPU/memory optimization, backend parity, Metal readiness, mesh shader execution, deformation readiness, ray-tracing readiness, DirectStorage execution, persistent/autonomous streaming services, or async-overlap/performance proof.
