...
- Expose `RuntimeMavgPayloadPageSliceResult` and `extract_runtime_mavg_payload_page_slices` so caller-supplied payload text plus a validated graph can return selected decoded `payload_bytes` in request order without file IO.
...
- DirectStorage/Win32 byte-range file loading over validated payload page rows.
