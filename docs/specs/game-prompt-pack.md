...
```text
Goal: Convert authored Scene source data into the current runtime-loadable Scene package update surface through reviewed tools only.
Context: Run pwsh -NoProfile -ExecutionPolicy Bypass -File tools/agent-context.ps1 and inspect register-source-asset, cook-registered-source-assets, Scene/Prefab authoring commands, migrate-scene-runtime-package, validate-runtime-scene-package, and run-validation-recipe. Read mirakana/scene/schema.hpp, mirakana/assets/source_asset_registry.hpp, mirakana/tools/registered_source_asset_cook_package_tool.hpp, mirakana/tools/scene_runtime_package_migration_tool.hpp, mirakana/tools/runtime_scene_package_validation_tool.hpp, mirakana/runtime/asset_runtime.hpp, mirakana/runtime_scene/runtime_scene.hpp, and the target game.agent.json.
Constraints: Use the validated authored-to-runtime workflow: register-source-asset -> cook-registered-source-assets -> migrate-scene-runtime-package -> validate-runtime-scene-package. Author Scene/Prefab rows before migration and preserve `prefab_source` provenance rows when instantiating prefabs. Do not execute external importers, broaden package cooking, add unselected dependent cooked asset rows outside existing package helpers, parse source assets at runtime, claim renderer/RHI residency, package streaming, material/shader graphs, live shader generation, editor productization, Metal readiness, public native/RHI handles, or general production renderer quality.
Done when: The source registry contains required mesh/material/texture keys, explicitly selected rows are cooked into the package, the authored Scene document uses supported camera/light/mesh_renderer/sprite_renderer properties, `validate-runtime-scene-package` returns successful non-mutating runtime scene package validation through `mirakana::plan_runtime_scene_package_validation` and `mirakana::execute_runtime_scene_package_validation`, and unsupported gaps remain explicit.
```

## Scene/Component/Prefab Schema Contract

```text
Goal: Use the stable-id Scene/Component/Prefab Schema contract while designing future AI scene edits.
Context: Read mirakana/scene/schema.hpp and engine/agent/manifest.json.aiOperableProductionLoop.authoringSurfaces. SceneDocument, SceneComponentDocument, PrefabDocument, PrefabVariantDocument, ScenePrefabInstanceRefreshPlan, ScenePrefabInstanceRefreshRow, plan_scene_prefab_instance_refresh, ScenePrefabInstanceRefreshResult, and apply_scene_prefab_instance_refresh are contract-only mirakana_scene value APIs for stable authoring ids, component rows, deterministic validation, GameEngine.Scene text IO, stable prefab override paths, deterministic prefab source provenance, stable-id prefab instance refresh planning by source_node_id / source_component_id, pure value stable-id apply with source-to-result mappings, `duplicate_prefab_source_identity` diagnostics for ambiguous copied source provenance, `unsupported_nested_prefab_instance` diagnostics for nested prefab roots, and `unsupported_local_prefab_child` / `unsupported_local_prefab_component` diagnostics for local selected-subtree rows without selected-prefab provenance.
...
Done when: The design or code uses stable AuthoringId values, schema-driven component rows, and source-id provenance rows where prefab refresh planning is relevant, and any unsupported production claims remain listed as follow-up work.
```

## Asset Identity And Runtime Resource Surface

```text
Goal: Use Asset Identity as the closed identity/reference boundary and Runtime Resource as the closed Engine 1.0 reviewed safe-point/controller surface.
Context: Read mirakana/assets/asset_identity.hpp, mirakana/assets/source_asset_registry.hpp, mirakana/runtime/resource_runtime.hpp, and engine/agent/manifest.json.aiOperableProductionLoop.unsupportedProductionGaps. Asset Identity provides stable AssetKey rows, deterministic validation, AssetId derivation, GameEngine.AssetIdentity text IO, and plan_asset_identity_placements for reviewed placement resolution from keys into AssetId/kind/source rows. Source Asset Registry provides GameEngine.SourceAssetRegistry rows for first-party source asset identity and deterministic import metadata planning. Runtime Resource provides generation-checked handles, explicit resident mount/cache safe points, reviewed eviction planning/commit, selected package-streaming safe points, reviewed package discovery/candidate load, hot-reload recook replacement, and registered asset watch-tick orchestration.
...
Done when: Renderer/RHI planning references use FrameGraphDesc or RhiUploadStagingPlan where this foundation is in scope, and unsupported production claims remain listed as follow-up work.
```
...
Context: Select production recipe ai-navigation-headless. Use mirakana::BehaviorAuthoringDocument, mirakana::BehaviorAuthoringValidationContext, mirakana::validate_behavior_authoring_document, mirakana::BehaviorTreeDesc, mirakana::BehaviorTreeLeafResult, mirakana::BehaviorTreeBlackboard, mirakana::BehaviorTreeBlackboardCondition, mirakana::BehaviorTreeEvaluationContext, mirakana::evaluate_behavior_tree, mirakana::AiPerceptionAgent2D, mirakana::AiPerceptionTarget2D, mirakana::AiPerceptionSnapshot2D, mirakana::build_ai_perception_snapshot_2d, mirakana::write_ai_perception_blackboard, mirakana::evaluate_ai_perception_readiness_2d, mirakana::NavigationGrid, mirakana::NavigationGridAgentPathRequest, mirakana::NavigationGridAgentPathPlan, mirakana::plan_navigation_grid_agent_path, mirakana::NavigationNavmeshPathRequest, mirakana::NavigationNavmeshDynamicObstacle, mirakana::plan_navigation_navmesh_path, mirakana::NavigationHierarchicalWorldPathRequest, mirakana::plan_navigation_hierarchical_world_path, mirakana::runtime::RuntimeWorldRegionNavigationRefReviewRequest, mirakana::runtime::RuntimeWorldRegionNavigationPathCacheReviewRequest, mirakana::runtime::review_runtime_world_region_navigation_refs, mirakana::runtime::review_runtime_world_region_navigation_path_cache, mirakana::runtime::RuntimeWorldStreamingLargeSceneReadinessRequest, mirakana::runtime::RuntimeWorldStreamingLargeSceneReadinessReport, mirakana::runtime::evaluate_runtime_world_streaming_large_scene_readiness, mirakana::NavigationCrowdPlanRequest, mirakana::plan_navigation_navmesh_crowd, mirakana::smooth_navigation_grid_path when manually inspecting planner smoothing behavior, mirakana::validate_navigation_grid_path, mirakana::replan_navigation_grid_path, mirakana::calculate_navigation_local_avoidance, and mirakana::update_navigation_agent. Validate reusable behavior documents against available blackboard keys and supported game-owned action ids, model condition facts in a caller-owned blackboard snapshot, and keep action execution in game code; the engine evaluator does not execute actions or own persistent world state.
Constraints: Keep the game headless unless another validated scenario is selected. Do not claim navmesh asset import, spline/funnel smoothing, automatic nav-data baking, package streaming execution, persistent/full crowd simulation beyond value-only batch planning, scene/physics perception integration, async tasks, decorators/services, scene/physics navigation integration, blackboard persistence/services, middleware, or editor graph tooling.
