...
- 2D commercial clean-room source gate: `docs/specs/2026-06-30-2d-commercial-clean-room-source-ledger-v1.md`, `tools/check-2d-commercial-clean-room.ps1`, `tools/check-2d-commercial-clean-room-contract.ps1`, `tools/check-2d-commercial-source-diagnostics.ps1`, `tools/check-2d-commercial-source-diagnostics-contract.ps1`, `tools/generate-2d-commercial-clean-room-review-input.ps1`, `schemas/2d-commercial-clean-room-review-input.schema.json`, and `schemas/2d-commercial-official-source-summary.schema.json` add provenance, static-token enforcement, copied-material/notice/dependency diagnostics, and counsel-review input generation without adding a dependency. This does not add third-party code, assets, shaders, samples, package-manager dependencies, or redistribution obligations. Official Vulkan, Direct3D 12, Metal, Unity, Unreal Engine, Godot, U.S. Copyright Office, and USPTO documentation references are category research only; Context7 rows are retained separately as reference mirrors and not as dependency or official-source approval. These records do not authorize copying source, samples, shaders, serialized engine formats, assets, trademarks, UI expression, API shapes, compatibility claims, equivalence claims, or parity claims.
- HarfBuzz, FreeType, Fontconfig, ICU, or similar text/font libraries are future dependency-gated adapter candidates only; selecting them requires non-default `runtime-ui-harfbuzz`, `runtime-ui-freetype`, `runtime-ui-fontconfig`, or later explicit runtime UI vcpkg features plus `license-audit`, `vcpkg.json`, this document, `docs/legal-and-licensing.md`, `THIRD_PARTY_NOTICES.md`, and `tools/bootstrap-deps.ps1` updates before use.
