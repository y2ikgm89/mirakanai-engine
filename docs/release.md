...
`pwsh -NoProfile -ExecutionPolicy Bypass -File tools/package-desktop-runtime.ps1` builds the `desktop-runtime-release` preset with `MK_ENABLE_DESKTOP_RUNTIME=ON`, the dependency-free vcpkg `desktop-runtime` feature, and a selected registered desktop runtime game target. It keeps the default SDK package lean while producing a release artifact for an editor-independent Windows-native desktop runtime game path.
...
- legacy desktop runtime DLLs from removed adapters on Windows
