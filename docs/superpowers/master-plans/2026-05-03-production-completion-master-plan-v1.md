...
- Current active plan: `docs/superpowers/plans/2026-05-20-sprite-batching-renderer-v1.md`.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan` points at `sprite-batching-renderer-v1` as the next developer-owned 2D capability, not a reopened Engine 1.0 production gap. `sprite-atlas-authoring-v1` is completed for reviewed source atlas authoring rows plus `DesktopRuntime2DPackage` / `sample_2d_desktop_runtime_package` source authoring adoption evidence.
