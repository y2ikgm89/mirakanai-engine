...
- Current active plan: `docs/superpowers/plans/2026-06-11-mavg-gpu-culling-dispatch-v1.md`. `recommendedNextPlan.id = mavg-gpu-culling-dispatch-v1`; `unsupportedProductionGaps = []`.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = mavg-gpu-culling-dispatch-v1`. MAVG GPU Culling Dispatch v1 (`mavg-gpu-culling-dispatch-v1`) is the active child over completed MAVG Vulkan Count-Buffer Indirect Execution v1 (`mavg-vulkan-count-buffer-indirect-execution-v1`) through PR #552 and closeout PR #553; implementation adds `dispatch_mavg_gpu_culling_indirect` in `engine/rhi/d3d12/src/d3d12_mavg_gpu_culling_dispatch.cpp`: the first D3D12 compute dispatch writes MAVG packed indexed indirect argument and count buffers from `MavgGpuCullingIndirectPlan` inputs with compute-write-to-indirect-read synchronization from existing `MavgGpuCullingSyncRequirement` rows, sets `executed_gpu_culling=true` only after successful dispatch, and proves visible and culled cluster cases through WARP-backed `MK_mavg_gpu_culling_dispatch_tests` while `MK_mavg_gpu_culling_tests` retain value-only planner evidence; Vulkan compute dispatch and compute-generated indirect consumption remain unclaimed. MAVG Vulkan Count-Buffer Indirect Execution v1 (`mavg-vulkan-count-buffer-indirect-execution-v1`) completed through PR #552. MAVG D3D12 Count-Buffer Indirect Execution v1 (`mavg-d3d12-count-buffer-indirect-execution-v1`) completed through PR #547. MAVG Vulkan Indexed Indirect Draw Execution v1 (`mavg-vulkan-indexed-indirect-draw-execution-v1`) completed through PR #541. MAVG D3D12 Indexed Indirect Draw Execution v1 (`mavg-d3d12-indexed-indirect-draw-execution-v1`) completed through PR #537 conventional upload re-land, PR #538 GPU culling plus RHI indirect contract re-land, and PR #539 D3D12 ExecuteIndirect execution re-land. MAVG Resident Page Recency Eviction Order v1 (`mavg-resident-page-recency-eviction-order-v1`) remains retained evidence through caller-supplied recency ordering via `RuntimeMavgPageStreamingRecencyRow`, `RuntimeMavgPageStreamingAutomaticEvictionPolicyKind::caller_supplied_recency`, and `resident_page_last_used_generation` without runtime-inferred LRU/frequency eviction policy, GPU memory pressure integration, or DirectStorage execution. The completed stacked slice adds `mavg_conventional_upload.hpp` / `upload_runtime_mavg_conventional_mesh_binding`, `mavg_gpu_culling.hpp` / `plan_mavg_gpu_culling_indirect_commands`, `indirect_draw.hpp` / `IRhiCommandList::draw_indexed_indirect`, selected D3D12 `ExecuteIndirect` execution for CPU-generated upload indexed indirect argument buffers without count buffers without actual GPU culling dispatch with visible WARP-backed readback proof and count-buffer fail-closed evidence, completed Vulkan `vkCmdDrawIndexedIndirect` execution for the same CPU-generated upload argument-buffer contract with SPIR-V environment-gated visible readback proof and count-buffer fail-closed evidence, and completed D3D12 count-buffer indirect execution through `ID3D12GraphicsCommandList::ExecuteIndirect` with a non-null count buffer and 4-byte-aligned `CountBufferOffset` running `min(count_buffer_value, max_draw_count)` draws for CPU-generated upload argument and count buffers with WARP-backed `MK_d3d12_rhi_tests` count-limited/zero-count/invalid-input coverage, and completed count-buffer Vulkan execution through `vkCmdDrawIndexedIndirectCount` with 4-byte-aligned count-buffer offsets running `min(count_buffer_value, max_draw_count)` draws for CPU-generated upload argument and count buffers with SPIR-V environment-gated `MK_backend_scaffold_tests` count-limited/zero-count/invalid-input coverage. compute-generated count/argument buffer consumption through existing RHI paths, mesh shaders, package streaming execution beyond reviewed safe points, deformation, ray tracing, benchmark runner/results, Vulkan/Metal readiness by inference, Nanite compatibility/equivalence/superiority, and broad CPU/GPU/memory optimization remain unclaimed. Environment System v1 completed through PR #408-#428 / merge commit `a6946235`; snow package readiness, renderer/RHI precipitation execution, volumetric-cloud execution/package readiness, Vulkan/Metal parity, broad optimization, and broad `environment_ready` remain unclaimed until focused plans validate them. First-Party Desktop Platform And SDL3 Removal v1, MK_platform_win32, MK_runtime_host_win32_presentation, MK_audio_wasapi, Renderer RHI Resource Foundation 1.0 Scope Closeout v1, Native Win32 Editor Shell v1, Job Execution Placement Policy v1 (`job_execution_placement_policy_status=ready`, host-independent CPU placement policy evidence), Windows CPU Set Worker Placement v1 (`windows_cpu_set_worker_placement_status=ready`, `windows_cpu_set_worker_placement_native_thread_handles_exposed=0`), Long-Running Performance Readiness v1, SIMD Dispatch Policy And Evidence v1, and AVX2 Reviewed Target Execution v1 remain completed evidence layers.
