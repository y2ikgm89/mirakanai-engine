...
- Current active plan: `docs/superpowers/plans/2026-05-21-renderer-lighting-shadows-v1.md`.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan` points at `renderer-lighting-shadows-v1` as the current developer-owned renderer 1.x capability, not a reopened Engine 1.0 production gap. `renderer-modern-materials-v1` is completed for backend-gated material package evidence, D3D12/Vulkan/selected-backend shader evidence counters, PR #148, hosted checks, and full validation evidence.
