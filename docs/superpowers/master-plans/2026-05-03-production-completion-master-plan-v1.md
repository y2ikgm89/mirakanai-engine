...
- Current active plan: [First-Party UI Editor Production Stack v1](../plans/2026-05-31-first-party-ui-editor-production-stack-v1.md) is selected as the active clean-break milestone after the operator selected editor/UI productization from the `next-production-gap-selection` gate.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = first-party-ui-editor-production-stack-v1`. [First-Party Editor Shell v1](../plans/2026-05-31-first-party-editor-shell-v1.md) completed the clean-break first-party retained `MK_editor` replacement after [Native Win32 Editor Shell v1](../plans/2026-05-30-native-win32-editor-shell-v1.md) closed through PR #322. The active plan targets first-party docking, rich text, Windows DirectWrite/TSF/UIA adapter execution, full D3D12 viewport/material texture display, and AI-operable editor UX expansion without compatibility shims. Full D3D12 viewport/material texture display, full docking productization, broad production text/font/IME/accessibility execution, cross-platform editor shells, SDL3, public native handles, filesystem/package mutation, runtime source parsing, runtime game UI middleware, and broad renderer quality remain unclaimed until focused phases validate them. Historical zero-gap closeout evidence remains discoverable through the retained plan registry and archive. Previous selection-gate state: `recommendedNextPlan.id = next-production-gap-selection`.
