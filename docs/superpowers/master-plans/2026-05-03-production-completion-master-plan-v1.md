...
- Current active plan: this production-completion master plan is the active selection gate after [First-Party UI Editor Production Stack v1](../plans/2026-05-31-first-party-ui-editor-production-stack-v1.md) completed Phases 1-10 and returned from the clean-break editor/UI productization milestone.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = ai-operable-performance-budget-and-evidence-v1`. The active child [AI-Operable Performance Budget And Evidence v1](../plans/2026-06-01-ai-operable-performance-budget-and-evidence-v1.md) adds `game.agent.json.performanceBudgets`, manifest `performanceBudgetEvidenceLoops`, selected sample/template budget rows, and static checks before broader CPU/GPU/memory optimization implementation. [First-Party Editor Shell v1](../plans/2026-05-31-first-party-editor-shell-v1.md) completed the clean-break first-party retained `MK_editor` replacement after [Native Win32 Editor Shell v1](../plans/2026-05-30-native-win32-editor-shell-v1.md) closed through PR #322, and [First-Party UI Editor Production Stack v1](../plans/2026-05-31-first-party-ui-editor-production-stack-v1.md) then completed first-party docking, rich text, Windows DirectWrite/TSF/UIA adapter evidence, D3D12 viewport/material texture display, AI-operable editor UX rows, and cross-platform adapter future gates without compatibility shims. Broad editable rich text, Direct2D GPU text rendering/upload, broad shaping/bidi/fallback, full app-owned `ITextStoreACP` callback coverage, native IME candidate UI, reconversion, full UIA control pattern/event parity, cross-platform editor/accessibility parity, Vulkan/Metal editor texture-display parity, broader material-preview GPU parity, SDL3, public native handles, filesystem/package mutation, runtime source parsing, runtime game UI middleware, and broad renderer quality remain unclaimed until focused phases validate them. Historical zero-gap closeout evidence remains discoverable through the retained plan registry and archive.
