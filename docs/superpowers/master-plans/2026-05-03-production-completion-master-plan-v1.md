...
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = long-running-performance-readiness-v1-phase-7`. Phase 0/1 published the selected `sample_2d_desktop_runtime_package` short-soak evidence through PR #403 / head `dfc467d4`, including `installed-2d-long-run-readiness-smoke` / `--require-long-run-performance-readiness`, deterministic `long_run_readiness_*` frame, memory, diagnostics, clean-shutdown, and zero unsupported side-effect counters, plus opt-in host recipe `host-2d-long-run-readiness-soak` outside default validation. Phase 2 selects `cpuProfilingMatrix` and `host-cpu-profiling-matrix` as host-gated Intel/AMD CPU profiling evidence before any Linux affinity, NUMA placement, broader SIMD, PGO/LTO, data-layout, scheduler-policy, or batch-size tuning follow-up can execute. Phase 7 selects `optionalGpuComputeReview` and `host-optional-gpu-compute-review` as Optional GPU Compute Review v1: candidates classify as `rhi_compute`, `offline_tool_acceleration`, `cuda_hip_private_adapter_candidate`, `sycl_private_adapter_candidate`, or `non_goal`, and need data transfer cost, memory residency, synchronization, stream/event usage, queue/profiler visibility, dependency burden, and scalar/RHI fallback evidence before follow-up implementation. Runtime rendering/simulation candidates prefer backend-neutral RHI compute; CUDA/HIP/SYCL are optional tooling or private-adapter candidates only. Job Execution Placement Policy v1, Windows CPU Set Worker Placement v1, Windows CPU Set SMT Worker Placement v1, SIMD Dispatch Policy And Evidence v1, and AVX2 Reviewed Target Execution v1 remain narrow CPU placement/SIMD evidence. Native Win32 Editor Shell v1, First-Party Editor Shell v1, First-Party UI Editor Production Stack v1, and Renderer Backend Parity Metal Apple Evidence v1 remain completed evidence layers. Linux affinity, NUMA allocation execution, hybrid P-core/E-core pinning, broad SIMD quality, GPU async overlap, CUDA/HIP/SYCL runtime dependency, default validation GPU SDK requirements, cross-vendor/backend parity, allocator enforcement, broad renderer quality, public native or vendor handles, runtime source parsing, broad long-running readiness, and broad performance readiness remain unclaimed until focused phases validate them.
