...
- Current active plan: [SIMD Dispatch Policy And Evidence v1](../plans/2026-06-03-simd-dispatch-policy-and-evidence-v1.md) is the selected post-1.0 CPU optimization slice after [Windows CPU Set SMT Worker Placement v1](../plans/2026-06-03-windows-cpu-set-smt-worker-placement-v1.md) completed SMT-aware worker placement evidence. It keeps `unsupportedProductionGaps = []` and does not reopen Engine 1.0 blockers.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = simd-dispatch-policy-and-evidence-v1`. Job Execution Placement Policy v1 completed the host-independent placement policy foundation, Windows CPU Set Worker Placement v1 completed the first host-owned Windows CPU Sets worker-start placement evidence through a value-only `MK_core` callback boundary, and Windows CPU Set SMT Worker Placement v1 completed the focused `avoid_smt_siblings` follow-up using official CPU Sets `CoreIndex`, distinct-core ordering before SMT sibling CPU Sets, and selected `sample_desktop_runtime_game --require-windows-cpu-set-smt-worker-placement` package counters through PR #393 / merge commit `28ec2a3f99896df1f62639944e54f1376a41eb99`. SIMD Dispatch Policy And Evidence v1 is the active focused follow-up for narrow `MK_core` scalar/SSE2 CPU SIMD dispatch evidence and package-visible `simd_dispatch_policy_*` counters. Renderer RHI Resource Foundation 1.0 Scope Closeout v1, Native Win32 Editor Shell v1, First-Party Editor Shell v1, and First-Party UI Editor Production Stack v1 remain completed evidence layers. Linux affinity, NUMA allocation execution, hybrid P-core/E-core pinning, broad SIMD quality, GPU async overlap, CUDA/HIP/SYCL, cross-vendor/backend parity, allocator enforcement, broad renderer quality, broad editable rich text, Direct2D GPU text rendering/upload, broad shaping/bidi/fallback, full app-owned `ITextStoreACP` callback coverage, native IME candidate UI, reconversion, full UIA control pattern/event parity, cross-platform editor/accessibility parity, Vulkan/Metal editor texture-display parity, broader material-preview GPU parity, SDL3, public native handles, filesystem/package mutation, runtime source parsing, runtime game UI middleware, and broad performance readiness remain unclaimed until focused phases validate them.
