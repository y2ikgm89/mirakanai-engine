...
- Current active plan: `docs/superpowers/plans/2026-06-06-mavg-directstorage-sdk-dependency-gate-v1.md`. `recommendedNextPlan.id = mavg-directstorage-sdk-dependency-gate-v1`; `unsupportedProductionGaps = [].` Parent milestone: `docs/superpowers/plans/2026-06-05-mavg-runtime-lod-milestone-v1.md`.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = mavg-directstorage-sdk-dependency-gate-v1`. Active child MAVG DirectStorage SDK Dependency Gate v1 (`mavg-directstorage-sdk-dependency-gate-v1`) (`docs/superpowers/plans/2026-06-06-mavg-directstorage-sdk-dependency-gate-v1.md`) builds on completed draft PR #466 `mavg-win32-iocp-file-io-worker-v1` and adds the optional `directstorage-sdk` vcpkg feature with `dstorage`, `MK_ENABLE_DIRECTSTORAGE_SDK`, `directstorage-sdk` CMake configure/build/test presets, `tools/validate-directstorage-sdk.ps1`, `MK_runtime_host_win32_directstorage_sdk_tests`, `dstorage.h`, `dstorageerr.h`, `Microsoft::DirectStorage`, and DirectStorage runtime DLL copy wiring. Completed child `mavg-win32-iocp-file-io-worker-v1` retains `Win32MavgPayloadIocpFileIoDispatcherDesc`, `Win32MavgPayloadIocpFileIoDispatcher`, private Win32 `CreateIoCompletionPort`, `GetQueuedCompletionStatus`, `PostQueuedCompletionStatus`, `CreateFileW`, `FILE_FLAG_OVERLAPPED`, `OVERLAPPED`, `ReadFile`, and `CancelIoEx` handling with `executed_background_worker=true` evidence and draft PR #466. Completed child `mavg-win32-async-file-io-adapter-v1` retains `RuntimeMavgPayloadDirectStorageRequestRow::source_file_path`, caller-owned `destination_memory`, `Win32MavgPayloadAsyncFileIoDispatcherDesc`, `Win32MavgPayloadAsyncFileIoDispatcher`, caller-polled overlapped file reads, no DirectStorage SDK execution, no async-overlap/performance claim, no Nanite claim, and draft PR #463 evidence; completed child `mavg-native-directstorage-win32-async-io-dispatch-status-v1` retains handle-free native IO dispatch/status rows and draft PR #462 evidence; completed child `mavg-directstorage-request-plan-v1` retains value-only DirectStorage request planning and draft PR #460 evidence. `DStorageGetFactory` execution, DirectStorage factory/queue/status/fence execution, DirectStorage file IO execution, autonomous package streaming workers, automatic eviction policy, GPU memory pressure integration, generic GPU culling frameworks, Vulkan indirect draw execution, Metal indirect command buffers, mesh shaders, Nanite compatibility/equivalence/superiority, async-overlap/performance claims, and broad CPU/GPU/memory optimization remain unclaimed.
