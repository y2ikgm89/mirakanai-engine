...
- Current active plan: `docs/superpowers/plans/2026-06-05-mavg-rhi-indirect-draw-v1.md`. `recommendedNextPlan.id = mavg-rhi-indirect-draw-v1`; `unsupportedProductionGaps = []`.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = mavg-rhi-indirect-draw-v1`. MAVG RHI Indirect Draw v1 is the active stacked child over MAVG Runtime LOD Milestone v1, after the completed MAVG GPU Culling Indirect v1 prerequisite and the MAVG Asset Graph v1 foundation. The parent checkpoints add clean-break `MavgClusterGraphCluster` hierarchy/error/fallback/draw-range rows, expanded `GameEngine.MavgClusterGraph.v1` validation/serialization, static `GameEngine.MavgClusterPayload.v1` vertex/index rows, CPU reference selection through `mavg_lod_selection.hpp` / `select_mavg_lod_clusters`, runtime resident-page evidence through `mavg_lod_residency.hpp` / `build_runtime_mavg_lod_residency`, caller-reviewed page streaming/eviction protection rows, range-aware conventional indexed draws, conventional scene submission planning, and conventional runtime RHI upload evidence. The completed prerequisite child adds `mavg_gpu_culling.hpp` / `plan_mavg_gpu_culling_indirect_commands` for value-only selected-cluster-to-packed-indexed-indirect command planning, reviewed culling bounds, deterministic visibility-cull counters, and D3D12/Vulkan compute-write-to-indirect-read synchronization requirement rows. The active child adds `indirect_draw.hpp`, `IndexedIndirectDrawCommand`, `IndexedIndirectDrawDesc`, `BufferUsage::indirect`, `IRhiCommandList::draw_indexed_indirect`, Null RHI deterministic argument/count-buffer execution, indirect-draw `RhiStats` rows, and Vulkan indirect-buffer usage mapping. Actual GPU culling dispatch, D3D12 `ExecuteIndirect`, Vulkan indirect draw execution, mesh shaders, package streaming execution beyond reviewed safe points, deformation, ray tracing, benchmark runner/results, Vulkan/Metal readiness by inference, Nanite compatibility/equivalence/superiority, and broad CPU/GPU/memory optimization remain unclaimed. Environment System v1 completed through PR #408-#428 / merge commit `a6946235`; snow package readiness, renderer/RHI precipitation execution, volumetric-cloud execution/package readiness, Vulkan/Metal parity, broad optimization, and broad `environment_ready` remain unclaimed until focused plans validate them. First-Party Desktop Platform And SDL3 Removal v1, MK_platform_win32, MK_runtime_host_win32_presentation, MK_audio_wasapi, Renderer RHI Resource Foundation 1.0 Scope Closeout v1, Native Win32 Editor Shell v1, Job Execution Placement Policy v1 (`job_execution_placement_policy_status=ready`, host-independent CPU placement policy evidence), Windows CPU Set Worker Placement v1 (`windows_cpu_set_worker_placement_status=ready`, `windows_cpu_set_worker_placement_native_thread_handles_exposed=0`), Long-Running Performance Readiness v1, SIMD Dispatch Policy And Evidence v1, and AVX2 Reviewed Target Execution v1 remain completed evidence layers.
