...
- Current active plan: [Long-Running Performance Readiness v1](../plans/2026-06-03-long-running-performance-readiness-v1.md) is the selected post-1.0 milestone after [AVX2 Reviewed Target Execution v1](../plans/2026-06-03-avx2-reviewed-target-execution-v1.md) completed through PR #397 / head `d86900ea`. It keeps `unsupportedProductionGaps = []` and does not reopen Engine 1.0 blockers.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = long-running-performance-readiness-v1-phase-2`. Phase 0/1 published the selected `sample_2d_desktop_runtime_package` short-soak evidence through PR #403 / head `dfc467d4`, including `installed-2d-long-run-readiness-smoke` / `--require-long-run-performance-readiness`, deterministic `long_run_readiness_*` frame, memory, diagnostics, clean-shutdown, and zero unsupported side-effect counters, plus opt-in host recipe `host-2d-long-run-readiness-soak` outside default validation. Phase 2 selects `cpuProfilingMatrix` and `host-cpu-profiling-matrix` as host-gated Intel/AMD CPU profiling evidence before any Linux affinity, NUMA placement, broader SIMD, PGO/LTO, data-layout, scheduler-policy, or batch-size tuning follow-up can execute. SIMD Dispatch Policy And Evidence v1 adds narrow `MK_core` scalar/SSE2 CPU SIMD dispatch evidence, and AVX2 Reviewed Target Execution v1 extends that boundary with target-local `mirakana_core_avx2` OBJECT execution plus selected `sample_desktop_runtime_game --require-simd-dispatch-policy` / `simd_dispatch_policy_*` counters that may report scalar, SSE2, or gated AVX2 while keeping native handles, NUMA allocation execution, GPU async overlap, CUDA/HIP/SYCL, broad SIMD quality, and broad CPU/GPU/memory optimization unclaimed. Job Execution Placement Policy v1 completed the host-independent CPU placement policy foundation, Windows CPU Set Worker Placement v1 completed the first host-owned Windows CPU Sets worker-start placement evidence, and Windows CPU Set SMT Worker Placement v1 completed the previous focused `avoid_smt_siblings` follow-up through PR #393 / merge commit `28ec2a3f99896df1f62639944e54f1376a41eb99`. Renderer RHI Resource Foundation 1.0 Scope Closeout v1, Native Win32 Editor Shell v1, First-Party Editor Shell v1, and First-Party UI Editor Production Stack v1 remain completed evidence layers. Linux affinity, NUMA allocation execution, hybrid P-core/E-core pinning, broad SIMD quality, GPU async overlap, CUDA/HIP/SYCL, cross-vendor/backend parity, allocator enforcement, broad renderer quality, broad editable rich text, Direct2D GPU text rendering/upload, broad shaping/bidi/fallback, full app-owned `ITextStoreACP` callback coverage, native IME candidate UI, reconversion, full UIA control pattern/event parity, cross-platform editor/accessibility parity, Vulkan/Metal editor texture-display parity, broader material-preview GPU parity, SDL3, public native handles, filesystem/package mutation, runtime source parsing, runtime game UI middleware, broad long-running readiness, and broad performance readiness remain unclaimed until focused phases validate them.
