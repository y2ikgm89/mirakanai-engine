...
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = next-production-gap-selection`. Renderer Metal Apple Host Validation Recipe v1 completed through PR #385 / merge commit `b5e1fdac6cde3250ccddfb4d46143928bc98fd06`, adding the reviewed `renderer-metal-apple-host-evidence` recipe while keeping `metal-apple`, broad Metal parity, and broad renderer quality host-gated or unclaimed. Job Execution Topology Policy v1 adds portable `MK_core` worker-count selection over observed logical processors, fallback/cap/reserve rules, derived `JobExecutionPoolDesc`, processor-group and NUMA host-evidence diagnostics, and selected `sample_desktop_runtime_game --require-job-execution-topology-policy` package-visible `job_execution_topology_policy_*` counters proving two selected workers from eight observed logical processors with one reserved logical processor and a two-worker cap. Job Execution Worker Pool v1 completed through PR #381 / merge commit `8c6a56f8`; it adds a first-party C++23 `MK_core` worker-thread pool with bounded queue execution, RAII `std::thread` lifetime, cooperative stop, `execute(batch)` drain semantics, worker-local scratch, diagnostic evidence, and selected `sample_desktop_runtime_game --require-job-execution-foundation` `job_execution_foundation_*` counters proving two worker threads, three executed task bodies, deterministic merges, and worker scratch evidence. Job Scheduling Evidence v1 completed Phase 3 with package-visible `sample_desktop_runtime_game --require-job-scheduling-evidence` counters including `job_scheduling_evidence_status=ready`, deterministic worker topology, bounded queue, dependency, merge, and scratch evidence, while preserving zero native-thread, thread-pool, affinity, NUMA, SIMD, and GPU async-overlap side-effect claims. Frame/Thread Scratch v1 completed through PR #373, PR #374, and PR #375 / merge commit `c37d3ea2`; it added scratch diagnostic counters, first-party `ScratchArena` / `ScratchLease` APIs, safe-point reset evidence, false-sharing diagnostics, and selected package-visible framegraph aliasing evidence. Memory Diagnostics v1 completed class-specific bytes/counts, high-water marks, budget pressure, stale-generation, use-after-safe-point diagnostics, and package-visible `memory_diagnostics_*` evidence. Memory Lifetime Taxonomy v1 completed ownership semantics, raw pointer non-ownership, `std::unique_ptr`, `std::span`, allocator/job/NUMA/GPU memory boundaries, backend handle opacity, and fail-closed broad CPU/GPU/memory optimization wording. Performance Baseline v1 added selected package frame budgets, p95/p99 diagnostics counters, `installed-2d-performance-baseline-smoke`, Trace Event JSON artifact references, docs, manifest rows, and static checks. AI-Operable Performance Budget And Evidence v1 completed through PR #361 / merge commit `e344326a270d39526be7e56d989bf96d86c01808` and added `game.agent.json.performanceBudgets`, manifest `performanceBudgetEvidenceLoops`, selected sample/template budget rows, and static checks. [First-Party Editor Shell v1](../plans/2026-05-31-first-party-editor-shell-v1.md) completed the clean-break first-party retained `MK_editor` replacement after [Native Win32 Editor Shell v1](../plans/2026-05-30-native-win32-editor-shell-v1.md) closed through PR #322, and [First-Party UI Editor Production Stack v1](../plans/2026-05-31-first-party-ui-editor-production-stack-v1.md) then completed first-party docking, rich text, Windows DirectWrite/TSF/UIA adapter evidence, D3D12 viewport/material texture display, AI-operable editor UX rows, and cross-platform adapter future gates without compatibility shims. The current backlog has no pure `candidate` or `production-candidate` rows; remaining non-completed rows are host-gated. Broad all-core CPU scheduling, work stealing execution, affinity pinning, NUMA placement, SMT/hybrid CPU policy, SIMD dispatch, GPU async overlap, CUDA/HIP/SYCL, cross-vendor/backend parity, allocator enforcement, broad renderer quality, broad editable rich text, Direct2D GPU text rendering/upload, broad shaping/bidi/fallback, full app-owned `ITextStoreACP` callback coverage, native IME candidate UI, reconversion, full UIA control pattern/event parity, cross-platform editor/accessibility parity, Vulkan/Metal editor texture-display parity, broader material-preview GPU parity, SDL3, public native handles, filesystem/package mutation, runtime source parsing, runtime game UI middleware, and broad performance readiness remain unclaimed until focused phases validate them.
