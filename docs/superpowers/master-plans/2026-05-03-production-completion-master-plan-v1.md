...
- Current active plan: [Renderer Production VFX Profiling Host Gate v1](../plans/2026-05-26-renderer-production-vfx-profiling-host-gate-v1.md), selected after [Engine 1.0 Gap Matrix v1](../plans/2026-05-26-engine-1-0-gap-matrix-v1.md) completed its docs/manifest governance pass and its selected `renderer-backend-parity-v1` strict Vulkan evidence follow-up completed through PR #248 while Metal remains Apple-host-gated.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = renderer-production-vfx-profiling-host-gate-v1` selects `Renderer Production VFX Profiling Host Gate v1` as the next developer-owned dated plan. Historical zero-gap closeout evidence remains discoverable through the retained plan registry and archive.
