...
- Current active plan: this master index is the selection gate again. `recommendedNextPlan.id = next-production-gap-selection`; `unsupportedProductionGaps = []`. Do not create the next highest-level plan until a fresh manifest/repository audit selects a concrete next slice.
