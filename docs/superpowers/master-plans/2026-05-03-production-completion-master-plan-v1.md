...
- Current active plan: [Native Win32 Editor Shell v1](../plans/2026-05-30-native-win32-editor-shell-v1.md) is the selected developer-owned clean-break milestone to restore a launchable Windows-native `MK_editor` after SDL3 removal. Phase 1 landed through PR #316 and Phase 2 landed through PR #317; the current candidate wires the private Win32/Dear ImGui/Direct3D 12 host and GUI smoke lane.
- Current manifest state: `unsupportedProductionGaps = []`; `recommendedNextPlan.id = native-win32-editor-shell-v1` marks the selected native editor shell milestone while broad online multiplayer/modding, SDL3, public native handles, filesystem/package mutation, runtime source parsing, runtime game UI middleware, and broad renderer quality remain unclaimed. Historical zero-gap closeout evidence remains discoverable through the retained plan registry and archive.
