...
3. Execute `Engine General Production Quality Expansion v1` in phase order: renderer quality matrix, runtime UI text/font/IME/accessibility, broad reviewed import/cook, physics/navigation breadth, networking execution/security, audio production, cross-domain package evidence, and final agent-surface closeout. The broad reviewed import/cook phase starts with value-only `MK_assets` production review evidence before any importer execution, dependency, or codec promotion.
