...
2. [Renderer Commercial Quality Closeout v1](../../plans/2026-06-25-renderer-commercial-quality-closeout-v1.md) is completed retained renderer closeout evidence. Retained Apple/Metal evidence includes [Renderer Backend Parity Apple Evidence Closeout v1](../../plans/2026-06-21-renderer-backend-parity-apple-evidence-closeout-v1.md), [Renderer Backend Parity Apple Memory Profiling Proof Rows v1](../../plans/2026-06-24-renderer-backend-parity-apple-memory-profiling-proof-rows-v1.md), [Renderer Metal Memory Profiling Apple Host Artifacts v1](../../plans/2026-06-24-renderer-metal-memory-profiling-apple-host-artifacts-v1.md), and the selected Metal visible package evidence lane in the completed plan. The closeout validator default path keeps broad renderer counters at `0`; direct `-RequireReady` can promote `renderer_backend_parity_ready`, `renderer_metal_broad_readiness`, `renderer_broad_quality_ready`, and `renderer_commercial_readiness` only when exact backend, D3D12, strict Vulkan, Apple-host Metal, package, static, clean-room, notice, and legal evidence is supplied. Any Metal parity proof row must name `host_validation_recipe_id` with a reviewed validation recipe from `shader-toolchain`, `mobile-packaging`, `renderer-metal-apple-host-evidence`, or `ios-simulator-smoke`; missing recipes fail closed with `missing_host_validation_recipe`, unreviewed valid ids fail closed with `unreviewed_host_validation_recipe`, and Windows/Vulkan proof must not promote Metal readiness by inference.
