...
2. The current selected concrete `renderer-backend-parity-v1` Apple/Metal implementation/evidence step is [Renderer Backend Parity Apple Evidence Closeout v1](../../plans/2026-06-21-renderer-backend-parity-apple-evidence-closeout-v1.md). Any Metal parity proof row must name `host_validation_recipe_id` with a reviewed validation recipe from `shader-toolchain`, `mobile-packaging`, `renderer-metal-apple-host-evidence`, or `ios-simulator-smoke`; missing recipes fail closed with `missing_host_validation_recipe`, unreviewed valid ids fail closed with `unreviewed_host_validation_recipe`, and Windows/Vulkan proof must not promote Metal readiness.
