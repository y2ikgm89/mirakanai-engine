...
- **Asset/import:** keep Khronos glTF/KTX2/Basis validation, optional codec/importer dependencies, shader compiler invocation, and package mutation as reviewed gates. A broad import claim needs explicit source roots, importer ids, declared extensions, package rows, license/provenance/hash evidence, validator/dependency/legal rows, offline shader command review rows, and host gates before promotion.
...
- **Asset/import:** use `mirakana::review_asset_import_production_readiness` as a value-only promotion gate when broad asset/import evidence is being reviewed; do not infer importer execution, KTX2/Basis adapter readiness, broad codec support, runtime source parsing, external downloads, or live shader generation from descriptor rows.
...
The latest completed selection sequence, [Engine 1.0 Gap Matrix v1](../../plans/2026-05-26-engine-1-0-gap-matrix-v1.md), is complete. It kept `unsupportedProductionGaps = []`, selected `renderer-backend-parity-v1` strict Vulkan evidence as the first follow-up, that follow-up landed with package-generated mapping compute SPIR-V proof plus reviewed validation-recipe execution, and [Renderer Production VFX Profiling Host Gate v1](../../plans/2026-05-26-renderer-production-vfx-profiling-host-gate-v1.md) then completed selected D3D12 and strict Vulkan renderer/VFX/profiling host evidence while keeping Metal Apple-host-gated. [Engine General Production Quality Expansion v1](../../plans/2026-05-26-engine-general-production-quality-expansion-v1.md) is also complete as a post-1.0 production breadth milestone; it does not reopen `unsupportedProductionGaps = []`. The next active sequence is [Clean Break Broad Production Readiness Master Plan v1](../2026-05-27-clean-break-broad-production-readiness-master-plan-v1.md), and the manifest-led selection pointer now selects [Renderer Production Quality And Backend Parity v1](../../plans/2026-05-27-renderer-production-quality-backend-parity-v1.md) with `recommendedNextPlan.id = renderer-production-quality-backend-parity-v1`.
...
4. Execute `renderer-production-quality-backend-parity-v1` first, then select the next child plan only after renderer evidence, package counters, docs/manifest/static checks, validation, and PR evidence are closed.
...
| Sandbox production surface | `genre-sandbox-world-pack-v1` | Implemented first-party `MK_runtime` `plan_runtime_sandbox_world_mutation` value contract for chunk/cell ownership, placement/destruction intent, construction costs, mutation rows, persistence review, repairable persistence evidence, unsafe mutation rejection, positive replay hash, and selected 2D/3D `sandbox_world_*` package counters without world mutation, persistence IO, package loading, streaming execution, biome/block-art ownership, renderer/platform/editor APIs, threads, native handles, or game-specific content rules. |
...
| Rendering/VFX/profiling production breadth | `production-rendering-vfx-profiling-v1` | Implemented first-party `MK_renderer` `plan_renderer_production_vfx_profiling` value contract for GPU particle budget rows, postprocess rows, backend timing rows, crash/telemetry handoff rows, backend parity diagnostics, host-gated Metal evidence, and selected generated 3D `rendering_vfx_profiling_*` package counters without GPU command recording, native capture execution, crash upload execution, public native handles, backend performance parity, Metal readiness, or broad renderer quality. |
| Renderer General Quality Matrix v1 production matrix | `renderer-general-quality-matrix-v1` | Implemented first-party `MK_renderer` `plan_renderer_quality_matrix` value contract for backend-local renderer quality rows across materials, lighting/shadows, postprocess, sprite/UI, scene scale, GPU memory/residency, and profiling/capture, with selected generated 3D `renderer_quality_matrix_status=host_evidence_required` package counters, D3D12/Vulkan readiness, Metal host-gated rows, `renderer_quality_matrix_general_renderer_quality_ready=0`, and no native capture/crash upload/public native handle/broad renderer quality claim. |
