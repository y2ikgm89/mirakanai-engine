...
2. Select the next concrete `renderer-backend-parity-v1` Apple/Metal implementation or evidence step only through an explicit dated plan. Any Metal parity proof row must name `host_validation_recipe_id` with a reviewed validation recipe from `shader-toolchain`, `mobile-packaging`, `renderer-metal-apple-host-evidence`, or `ios-simulator-smoke`; missing recipes fail closed with `missing_host_validation_recipe`, unreviewed valid ids fail closed with `unreviewed_host_validation_recipe`, and Windows/Vulkan proof must not promote Metal readiness.
