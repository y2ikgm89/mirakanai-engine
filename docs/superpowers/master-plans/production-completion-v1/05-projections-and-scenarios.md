...
| Lighting, shadows, and postprocess | `covered-foundation` on selected D3D12/Vulkan lanes; tone-mapping evidence rows are selected through `recommendedNextPlan.id = renderer-postprocess-tone-mapping-evidence-v1`; `host-gated` for broader backend claims. | `renderer-lighting-shadows-v1`, `renderer-postprocess-v1`, `renderer-backend-parity-v1` |
