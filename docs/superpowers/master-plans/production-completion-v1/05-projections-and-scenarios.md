...
- **Asset/import:** keep Khronos glTF/KTX2/Basis validation, optional codec/importer dependencies, shader compiler invocation, and package mutation as reviewed gates. A broad import claim needs explicit source roots, importer ids, declared extensions, package rows, license/provenance/hash evidence, validator/dependency/legal rows, offline shader command review rows, host gates, and execution-readiness rows that separate reviewed execution from dependency-gated, host-gated, package-mutation, and unsupported-claim evidence before promotion.
