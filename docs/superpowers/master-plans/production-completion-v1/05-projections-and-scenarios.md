...
4. Execute `physics-navigation-commercial-coverage-v1` one candidate at a time, starting with optional adapter boundary evidence gates for Jolt/Recast/Detour-class rows. Do not switch to audio, networking, or broader package closeout rows until this slice is implemented, host-gated with evidence, or explicitly paused through a manifest/registry update.
