...
4. Execute `first-party-ui-editor-production-stack-v1` before selecting another post-1.0 capability. Do not treat the plan itself as ready evidence for docking, DirectWrite/TSF/UIA execution, full viewport/material texture display, cross-platform editor parity, public native handle access, or broad runtime UI parity until focused phases validate those claims.
