...
4. Before implementing another clean-break broad-production row, explicitly select exactly one new child/candidate through the manifest/registry, then execute it one candidate at a time with official documentation, Context7 where relevant, clean-break no-backward-compatibility design, validation evidence, PR evidence, and fail-closed non-claims.
