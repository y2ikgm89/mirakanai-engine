...
- MAVG GPU Visibility Buffer Layout v1 (`mavg-gpu-visibility-buffer-layout-v1`) / draft PR #510 adds the backend-neutral `MK_renderer` logical visibility-buffer layout preflight through `mavg_gpu_visibility_buffer_layout.hpp`, `MavgGpuVisibilityBufferLayoutDesc`, `MavgGpuVisibilityBufferSlotRow`, `MavgGpuVisibilityBufferByteRangeRow`, `MavgGpuVisibilityBufferLayoutRow`, `MavgGpuVisibilityBufferSyncIntentRow`, `MavgGpuVisibilityBufferLayoutPlan`, and `plan_mavg_gpu_visibility_buffer_layout`. It consumes successful visible packet rows and emits deterministic slot, byte-range, record-layout, and backend-neutral write-to-read sync-intent rows while failing closed for missing/invalid packet plans, count mismatches, duplicate/non-dense packet indexes, meshlet readiness requirements, slot stride/alignment, slot budgets, and byte-range overflow. It narrows the GPU visibility-buffer handoff gap before actual GPU visibility-buffer allocation/write or traversal execution; RHI allocation, GPU work submission, mesh shader execution, indirect draw execution, GPU decompression, Vulkan/Metal parity, Nanite compatibility/equivalence/superiority, and benchmark superiority remain future work.
- MAVG GPU Visibility Buffer RHI Allocation v1 (`mavg-gpu-visibility-buffer-rhi-allocation-v1`) adds the host-independent `MK_runtime_rhi` allocation-only handoff through `mavg_gpu_visibility_buffer_resource.hpp`, `RuntimeMavgGpuVisibilityBufferResourceDesc`, `RuntimeMavgGpuVisibilityBufferResourceRow`, `RuntimeMavgGpuVisibilityBufferResourceResult`, `RuntimeMavgGpuVisibilityBufferResourceDiagnosticCode`, and `create_runtime_mavg_gpu_visibility_buffer_resource`. It consumes successful logical visibility-buffer layout rows, validates storage/copy-source usage and max-buffer constraints, creates one backend-neutral `rhi::BufferHandle` through `IRhiDevice::create_buffer` for non-empty layouts, and reports readback-proof readiness. It narrows the GPU visibility-buffer handoff gap before any actual GPU visibility-buffer write, descriptor binding, barrier recording, traversal execution, mesh shader execution, indirect draw execution, GPU decompression, allocator/GPU budget enforcement, Vulkan/Metal parity, Nanite compatibility/equivalence/superiority, or benchmark superiority claim.
- MAVG GPU Visibility Buffer RHI Write Readback v1 (`mavg-gpu-visibility-buffer-rhi-write-readback-v1`) adds the host-independent `MK_runtime_rhi` staged write/readback proof through `mavg_gpu_visibility_buffer_write_readback.hpp`, `RuntimeMavgGpuVisibilityBufferWriteReadbackDesc`, `RuntimeMavgGpuVisibilityBufferWriteReadbackRow`, `RuntimeMavgGpuVisibilityBufferWriteReadbackResult`, `RuntimeMavgGpuVisibilityBufferWriteReadbackDiagnosticCode`, and `execute_runtime_mavg_gpu_visibility_buffer_write_readback`. It consumes successful logical visibility-buffer layout rows plus an allocated resource row, validates `BufferUsage::storage | BufferUsage::copy_source | BufferUsage::copy_destination`, encodes deterministic little-endian 64-byte records from layout field offsets and slot flags, stages bytes through an upload buffer, copies them into the visibility buffer, copies them into a readback buffer, waits on the submitted copy fence, and compares readback bytes. It narrows the GPU visibility-buffer handoff gap before descriptor binding, compute-shader generation, traversal execution, mesh shader execution, indirect draw execution, GPU decompression, allocator/GPU budget enforcement, backend parity, Nanite compatibility/equivalence/superiority, async-overlap/performance, or benchmark superiority claims.
...
- No GPU cluster traversal or shader-generated visibility buffer beyond current value-only visible cluster packet/layout rows, allocation-only `MK_runtime_rhi` visibility-buffer resource rows, and staged `MK_runtime_rhi` visibility-buffer write/readback proof rows.
