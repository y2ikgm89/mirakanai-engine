...
**Context:** `unsupportedProductionGaps = []` remains the 1.0 truth. The milestone started with 15 post-1.0 / 1.x developer-owned capability foundations whose narrow basis existed but whose stronger productization evidence was not yet promoted. After the `engine-scene-gameplay-binding-v1`, `engine-input-action-contexts-v1`, `engine-asset-placeholder-generation-v1`, `sprite-atlas-authoring-v1`, `sprite-batching-renderer-v1`, `sprite-animation-flipbook-v1`, `material-shader-graph-production-v1`, `physics-character-dynamics-v1`, and `physics-collision-query-v1` promotions, 6 `foundation-ready` rows remain in the canonical backlog.
...
| `physics-collision-query-v1` | pending | pending | pass (`tools/validate.ps1`, 2026-05-25; `out/validation-logs/validate-20260525-013612-49784`; package lane passed) | `PhysicsCollisionQuery3DReadinessConfig` / `PhysicsCollisionQuery3DReadinessReport` and `evaluate_physics_collision_query_readiness_3d` summarize selected 3D raycast/sweep batch evidence for raycast hits, shape-sweep hits, no-hit rows, invalid-request rows, query-budget rejections, source ordering, and row budgets. Focused RED compile failure, `MK_core_tests`, `sample_generated_desktop_runtime_3d_package` source smoke, `tools/package-desktop-runtime.ps1 -GameTarget sample_generated_desktop_runtime_3d_package`, installed validator `collision_query_readiness_*` fields, scaffold/static checks, scoped tidy, public API boundary checks, and full validation have passed; PR publication remains pending. |
