...
**Context:** `unsupportedProductionGaps = []` remains the 1.0 truth. The milestone started with 15 post-1.0 / 1.x developer-owned capability foundations whose narrow basis existed but whose stronger productization evidence was not yet promoted. After the `engine-scene-gameplay-binding-v1`, `engine-input-action-contexts-v1`, `engine-asset-placeholder-generation-v1`, `sprite-atlas-authoring-v1`, `sprite-batching-renderer-v1`, `sprite-animation-flipbook-v1`, `material-shader-graph-production-v1`, `physics-character-dynamics-v1`, `physics-collision-query-v1`, `navigation-navmesh-v1`, and `navigation-crowd-local-avoidance-v1` promotions, 4 `foundation-ready` rows remain in the canonical backlog.
...
| `navigation-crowd-local-avoidance-v1` | pending | pending | pass (`tools/validate.ps1`, 2026-05-25; `out/validation-logs/validate-20260525-025439-47016`; package lane passed) | `NavigationCrowdReadinessConfig` / `NavigationCrowdReadinessReport` and `evaluate_navigation_crowd_readiness` summarize value-only crowd rows, deterministic source ordering, route/local-avoidance success counts, applied-neighbor counts, dynamic-obstacle propagation, planned-agent minimums, and row budgets. Focused RED compile failure, `MK_navigation_tests`, selected `sample_generated_desktop_runtime_3d_package` source CTest, `tools/package-desktop-runtime.ps1 -GameTarget sample_generated_desktop_runtime_3d_package`, installed validator fields, scaffold/static checks, scoped tidy, public API boundary checks, and full validation passed; PR, hosted checks, merge, main sync, and guarded worktree cleanup remain pending. |
