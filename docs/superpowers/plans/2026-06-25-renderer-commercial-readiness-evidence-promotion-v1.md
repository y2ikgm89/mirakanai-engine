...
- `sample_desktop_runtime_game --emit-vulkan-strict-commercial-host-gate` now emits the strict aggregate, GPU memory, debug profiling, timestamp-query, and `renderer_vulkan_timestamp_ready` fields from `LinuxDesktopVulkanPresentationReport` instead of hard-coded zeros. Raw begin/end GPU timestamp values remain backend-private and are not exposed through the Linux runtime-host public API or package-visible output. The hosted Linux package path still leaves those rows host-gated because `execute_linux_desktop_vulkan_presentation_probe` does not yet populate renderer draw/compute/texture, GPU memory budget/transient heap, or framegraph render-pass evidence.
- `tools/check-renderer-vulkan-strict-commercial-quality-host-evidence.ps1` now locks the Linux sample output to report-driven strict counters, exact strict aggregate feature/layout rows, and raw timestamp non-exposure while still requiring fixed `renderer_commercial_readiness=0`. Platform-only Linux Vulkan evidence can still only produce a host-gated summary; final promotion still requires non-fixture strict Vulkan package evidence plus the final retained root `-RequireReady` path.
- Focused validation passed for `MK_runtime_host_tests` in `dev`, `MK_runtime_host_tests` and `sample_desktop_runtime_game` in `desktop-runtime`, `tools/check-renderer-vulkan-strict-commercial-quality-host-evidence.ps1`, and `tools/check-format.ps1 -Files ...` after RED tests first proved the missing timestamp-request contract, the incomplete strict aggregate false-ready path, and the raw timestamp public/package exposure. This task does not set `renderer_backend_parity_ready=1`, `renderer_metal_broad_readiness=1`, `renderer_broad_quality_ready=1`, or `renderer_commercial_readiness=1`.
