...
- `.github/workflows/validate.yml` now runs `renderer-commercial-artifact-intake` after `agent-static`, `windows`, `linux-vulkan`, and `macos`. The job records the current run artifacts API response at `artifacts/renderer/commercial-readiness-evidence/current-run-artifact-intake/workflow-artifacts.json`, runs `tools/import-renderer-commercial-readiness-final-retained-root-artifacts.ps1 -Mode Import -RepoFullName "${{ github.repository }}" -RunId "${{ github.run_id }}"`, asserts the importer still emits `renderer_commercial_readiness=0`, and uploads `renderer-commercial-readiness-current-run-artifact-intake` with `compression-level: 0`, `include-hidden-files: false`, and `if-no-files-found: error`.
...
- `tools/validate.ps1 -StaticOnly -StaticJobs 1 -StaticCheckTimeoutSeconds 120`: passed with 41 static checks in `out/validation-logs/validate-20260626-092025-29580`; Windows-host Apple/Metal tooling remained diagnostic host gates and broad renderer counters stayed non-ready.
