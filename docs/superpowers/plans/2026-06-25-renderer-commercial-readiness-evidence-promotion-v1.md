...
- `.github/workflows/validate.yml` initially retained `renderer-vulkan-strict-commercial-quality-host-evidence` in the selected `windows-cpp23` lane as a fail-closed host-gate summary. Task 10V moves the ready commercial strict Vulkan artifact to the selected `linux-vulkan` lane, where the existing Linux Vulkan host gate runs the sample package smoke with strict aggregate, GPU memory, and debug profiling args under `xvfb-run`, then `tools/generate-renderer-vulkan-strict-commercial-quality-host-evidence.ps1 -RequireReady` shapes the same log into non-fixture `GameEngine.RendererVulkanStrictCommercialQualityHostEvidence.v1`.
...
- `.github/workflows/validate.yml` now uploads `renderer-vulkan-strict-commercial-quality-host-evidence` from the selected `linux-vulkan` lane, using `artifacts/environment/platform/linux-vulkan-host/validate-linux-vulkan-runtime-host.txt` as the producer input and requiring ready retained JSON. The `windows-cpp23` lane no longer uploads a host-gated strict Vulkan artifact with the same name.
- `tools/check-renderer-vulkan-strict-commercial-quality-host-evidence.ps1` and `tools/check-ci-matrix.ps1` lock the Linux-lane contract so platform-only Linux Vulkan evidence still cannot satisfy `vulkan_strict_host_evidence`, while the strict Linux package log can.
