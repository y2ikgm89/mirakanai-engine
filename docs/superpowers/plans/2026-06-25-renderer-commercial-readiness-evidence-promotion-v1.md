...
- Context7 `/websites/github_en_actions`, `/websites/github_en_rest`, and `/websites/cli_github_manual` were checked before this slice. The planner mirrors the official model: self-hosted runner labels are selected through `runs-on`, repository runners are listed through `/repos/{owner}/{repo}/actions/runners`, and manual workflows are triggered by `gh workflow run <workflow>.yml -f key=value`.
- `tools/validate-renderer-commercial-readiness-final-handoff.ps1` is intentionally non-dispatching. It reads an optional `GameEngine.RendererCommercialReadinessFinalRetainedRootArtifactImport.v1` intake manifest plus optional runner preflight source, then reports whether the next action is runner provisioning, `gh workflow run renderer-metal-memory-profiling-capable-host.yml`, `gh workflow run renderer-commercial-readiness-final-from-runs.yml`, final retained-root assembly, or final preflight. It emits `renderer_commercial_readiness_final_handoff_*` counters including `next_action`, `missing_assembler_inputs`, and `quality_vfx_dependency_blockers`, and keeps `renderer_commercial_readiness=0`.
- TDD evidence: `tools/check-renderer-commercial-readiness-final-handoff.ps1` first failed because `tools/validate-renderer-commercial-readiness-final-handoff.ps1` did not exist. It now proves the planner maps a blocked intake manifest plus no idle runner to `provision_capable_host_runner`, a ready runner without a Metal run id to `run_metal_memory_profiling_capable_host_workflow`, a source run plus Metal memory/profiling run id to the exact final-from-runs command, and an existing final retained root to the exact final preflight command.
