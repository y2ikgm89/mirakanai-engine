...
- `.github/workflows/validate.yml` now uploads `renderer-vulkan-strict-commercial-quality-host-evidence` from the selected `linux-vulkan` lane, using `artifacts/environment/platform/linux-vulkan-host/validate-linux-vulkan-runtime-host.txt` as the producer input and retaining a host-gated summary rather than ready retained JSON. The `windows-cpp23` lane no longer uploads a host-gated strict Vulkan artifact with the same name.
