...
**Date:** 2026-06-25
**Status:** Host-gated. Implementation and non-promoting handoff tooling are in place; final real promotion remains blocked on external Apple-host Metal memory/profiling artifacts.
...
- Task 10AF preserves host-gated missing-input dependency chains in the same artifact intake path. The importer now writes `assembler_input_blockers` rows for missing assembler inputs that have related retained `host_gate_summaries`, emits `host_gate_blocked_assembler_inputs`, `host_gate_blocked_assembler_input_names`, and `quality_vfx_dependency_blockers`, and records `quality_vfx_host_evidence` as dependent on missing `metal_memory_profiling_host_evidence` when the quality/VFX host gate reports `metal_memory_profiling_host_evidence_required`. This remains diagnostic intake evidence only and keeps `renderer_commercial_readiness=0`.
- Task 10AH adds direct multi-run artifact intake and opt-in quality/VFX regeneration. The importer now accepts `-SupplementalRunIds`, `-SupplementalArtifactNames`, `-RegenerateQualityVfx`, and `-RegeneratedQualityVfxOutputRootRelative`, records `supplemental_import` and `quality_vfx_regenerate` manifest rows, emits `supplemental_run_count`, `supplemental_artifact_names`, and `quality_vfx_regenerate_*` counters, can download a separate capable-host `renderer-metal-memory-profiling-host-artifacts` workflow artifact into the same intake root, and can regenerate `GameEngine.RendererQualityVfxCommercialHostEvidence.v1` only when package, D3D12, strict Vulkan, Apple Metal, Metal memory/profiling, and generated-3D source rows are all present. This removes manual path stitching for final retained-root assembly while keeping the default and current-run CI path non-promoting with `renderer_commercial_readiness=0`.
- Task 10AI adds `.github/workflows/renderer-commercial-readiness-final-from-runs.yml` as the `workflow_dispatch` final handoff wrapper. The manifest command `rendererCommercialReadinessFinalRetainedRootFromRunsWorkflow` runs `gh workflow run renderer-commercial-readiness-final-from-runs.yml` with `source_artifact_run_id`, `metal_memory_profiling_run_id`, and `confirm_final_retained_root_handoff=renderer-commercial-final-retained-root`, captures the source run GitHub Actions artifacts API JSON, invokes the importer with `SupplementalRunIds`, `SupplementalArtifactNames renderer-metal-memory-profiling-host-artifacts`, `RegenerateQualityVfx`, and `AutoAssemble`, uploads `renderer-commercial-readiness-final-from-runs-artifact-intake` plus any assembled `renderer-commercial-readiness-final-retained-root`, and keeps `renderer_commercial_readiness=0` until real non-fixture final retained-root preflight passes.
- Task 10AJ adds `renderer-metal-memory-profiling-capable-host-runner-preflight`, `tools/validate-renderer-metal-memory-profiling-capable-host-runner.ps1`, and `tools/check-renderer-metal-memory-profiling-capable-host-runner.ps1` as the pre-dispatch runner availability gate for Task 10AG/10AI. Context7 resolved the official GitHub REST docs to `/websites/github_en_rest`; the selected endpoint is repository self-hosted runners, `/repos/{owner}/{repo}/actions/runners`, invoked through GitHub CLI `gh api`. The preflight also supports fixture JSON through `-RunnersJsonPath`, emits `renderer_metal_memory_profiling_capable_host_runner_*` counters for required labels `self-hosted, macOS, ARM64, metal-residency-set`, and fails closed with `-RequireAvailable` when no idle online runner exists. The 2026-06-27 repository audit for `y2ikgm89/mirakanai-engine` emitted `renderer_metal_memory_profiling_capable_host_runner_status=host_evidence_required`, `renderer_metal_memory_profiling_capable_host_runner_available=0`, and `renderer_metal_memory_profiling_capable_host_runner_total=0`, keeping `renderer_commercial_readiness=0`.
- Task 10AK adds `renderer-commercial-readiness-final-handoff`, `rendererCommercialReadinessFinalHandoff`, `tools/validate-renderer-commercial-readiness-final-handoff.ps1`, and `tools/check-renderer-commercial-readiness-final-handoff.ps1` as the non-dispatching final handoff planner. It consumes an optional intake manifest plus optional runner preflight fixture/API source, optional repository metadata through `GET /repos/{owner}/{repo}` or `-RepositoryJsonPath`, optional retained `GameEngine.RendererPublicSelfHostedRunnerSecurityReview.v1` through `-PublicRepoRunnerSecurityReviewRelative`, and optional `-UseRunDiscovery` output from `tools/plan-renderer-commercial-readiness-final-run-discovery.ps1`; resolves `source_artifact_run_id` from `-SourceRunId`, manifest `run_id`, or artifact-backed run discovery; accepts or discovers `metal_memory_profiling_run_id`; emits `next_action`, run-discovery status, `missing_assembler_inputs`, `quality_vfx_dependency_blockers`, assembler/final-preflight readiness, and exact next commands for the capable-host workflow, final-from-runs workflow, assembler, or final preflight. When runner provisioning is required for a public repository, it emits `public_runner_security_review_required` / `complete_public_runner_security_review`, `runner_public_repo_registration_blocked=1`, and withholds the registration-token command plus `config.sh` template until either `-ConfirmPublicRepoSelfHostedRunnerSecurityReview public-repo-self-hosted-runner-risk-reviewed` or a valid retained review artifact is supplied. The retained artifact must have `review_status=approved`, matching `repo_full_name`, `repository_visibility=public`, `reviewed_public_fork_pr_risk=true`, `reviewed_runner_isolation=true`, `reviewed_secret_exposure=true`, `reviewed_metal_probe_truth=true`, `reviewed_allowed_workflows` including `renderer-metal-memory-profiling-capable-host.yml`, and `reviewed_required_labels` exactly `self-hosted,macOS,ARM64,metal-residency-set`; invalid supplied artifacts fail closed. After review it emits the official registration-token endpoint, non-secret `gh api -X POST` handoff, the fact that the registration token expires after one hour, `config.sh --url ... --labels metal-residency-set` template, public repository security review confirmation, `runner_public_repo_security_review_artifact_present`, `runner_public_repo_security_review_artifact_valid`, `runner_public_repo_security_review_artifact_status`, and Metal probe truth requirement while keeping `renderer_commercial_readiness=0`.
- Task 10AL adds `renderer-public-runner-security-review`, `rendererPublicRunnerSecurityReviewGenerator`, `tools/generate-renderer-public-runner-security-review.ps1`, and `tools/check-renderer-public-runner-security-review.ps1` as the retained public repository security review artifact producer for the final handoff planner. Context7 resolved official GitHub Actions docs to `/websites/github_en_actions` and official GitHub REST docs to `/websites/github_en_rest`; the selected repository metadata endpoint is `GET /repos/{owner}/{repo}` and the selected registration token handoff endpoint is `/repos/{owner}/{repo}/actions/runners/registration-token`. The producer writes `GameEngine.RendererPublicSelfHostedRunnerSecurityReview.v1` only when the repository metadata is public and the operator supplies `-ApprovePublicRepoSelfHostedRunnerReview public-repo-self-hosted-runner-risk-reviewed`, `-ReviewedPublicForkPrRisk`, `-ReviewedRunnerIsolation`, `-ReviewedSecretExposure`, and `-ReviewedMetalProbeTruth`. Approved artifacts record `review_status=approved`, `reviewed_public_fork_pr_risk=true`, `reviewed_runner_isolation=true`, `reviewed_secret_exposure=true`, `reviewed_allowed_workflows` including `renderer-metal-memory-profiling-capable-host.yml`, exact `reviewed_required_labels` `self-hosted, macOS, ARM64, metal-residency-set`, `reviewed_metal_probe_truth=true`, `renderer_public_runner_security_review` counters, `runner_public_repo_security_review_artifact_present`, `runner_public_repo_security_review_artifact_valid`, `runner_public_repo_security_review_artifact_status`, the fact that the registration token expires after one hour, and non-actions proving no registration token fetch/print, no runner registration, no workflow dispatch, and no GPU workload. Missing approvals preserve `complete_public_runner_security_review`; approved artifacts feed `-PublicRepoRunnerSecurityReviewRelative` and keep `renderer_commercial_readiness=0`.
- Task 10AM adds `renderer-commercial-readiness-final-run-discovery`, `rendererCommercialReadinessFinalRunDiscovery`, `tools/plan-renderer-commercial-readiness-final-run-discovery.ps1`, and `tools/check-renderer-commercial-readiness-final-run-discovery.ps1` as the non-dispatching run/artifact discovery planner for the final-from-runs handoff. Context7 resolved official GitHub Actions docs to `/websites/github_en_actions` and official GitHub REST docs to `/websites/github_en_rest`; the selected endpoints are `GET /repos/{owner}/{repo}/actions/runs` and `GET /repos/{owner}/{repo}/actions/runs/{run_id}/artifacts`. The planner reads through `gh api` or fixture JSON, verifies a source `Validate` run with `renderer-commercial-readiness-current-run-artifact-intake` and a capable-host run with `renderer-metal-memory-profiling-host-artifacts`, emits `source_run_required`, `metal_memory_profiling_run_required`, or `ready_for_final_from_runs_workflow`, and prints the exact `gh workflow run renderer-commercial-readiness-final-from-runs.yml -f source_artifact_run_id=<id> -f metal_memory_profiling_run_id=<id> -f confirm_final_retained_root_handoff=renderer-commercial-final-retained-root` command only when both artifact-backed IDs are present. It never dispatches workflows, downloads artifacts, fetches registration tokens, registers runners, runs GPU workloads, assembles evidence, or changes `renderer_commercial_readiness=0`.
...
- The plan is closed as host-gated retained evidence. Real promotion still requires a capable Apple self-hosted runner, retained `renderer-metal-memory-profiling-host-artifacts`, final-from-runs intake, final retained-root assembly, and final non-fixture `-RequireReady` promotion before any broad renderer readiness counter can change from `0`.
...
- `tools/generate-environment-metal-optimization-artifacts.ps1`: now bounds hosted `xcrun xctrace record` with the official `--time-limit` option through default `XctraceTimeLimit=10m`, adds a host-level `XctraceRecordTimeoutSeconds=900` watchdog, emits `xctrace_time_limit=10m`, `xctrace_record_timeout_seconds=900`, and `xctrace_record_watchdog_timeout`, and tolerates a watchdog timeout only after `xctrace export --toc` plus `tools/validate-environment-optimization-artifacts.ps1 -RequireReady` prove the retained trace/artifact set.
...
- Task 10AF local TDD evidence: `tools/check-renderer-commercial-readiness-final-retained-root-artifact-import.ps1` first failed because `host_gate_blocked_assembler_inputs`, `host_gate_blocked_assembler_input_names`, `quality_vfx_dependency_blockers`, and manifest `assembler_input_blockers` rows were absent. It now passes after the importer maps retained host-gate summaries to missing assembler inputs, records `metal_memory_profiling_host_evidence` as directly host-gated, records `quality_vfx_host_evidence` as dependent on missing `metal_memory_profiling_host_evidence`, and keeps `renderer_commercial_readiness=0`.
...
Task 10AF final retained-root artifact import host-gate chain summary:

- `tools/import-renderer-commercial-readiness-final-retained-root-artifacts.ps1` now keeps every discovered `host-gate-summary.json` in the intake manifest `host_gate_summaries` array and emits aggregate `renderer_commercial_readiness_final_retained_root_artifact_import_host_gate_summaries`, `_host_gate_summary_statuses`, and `_host_gate_summary_reasons` counters. The legacy Metal-specific `metal_host_gate_summary` remains populated only from the Metal memory/profiling host-gate summary, so a concurrent quality/VFX host gate no longer overwrites or hides the root `mtlresidencyset_unavailable` reason.
- TDD evidence: `tools/check-renderer-commercial-readiness-final-retained-root-artifact-import.ps1` first failed because the importer did not emit `renderer_commercial_readiness_final_retained_root_artifact_import_host_gate_summaries=3` for concurrent root and nested `GameEngine.RendererMetalMemoryProfilingHostGate.v1` plus `GameEngine.RendererQualityVfxCommercialHostGate.v1` summaries. It now proves both unique reasons, `mtlresidencyset_unavailable` and `metal_memory_profiling_host_evidence_required`, are retained without promoting `renderer_commercial_readiness`.

Task 10AH final retained-root artifact import multi-run intake:

- Context7 `/websites/cli_github_manual`, `/websites/github_en_actions`, and `/github/docs` were checked before this slice. The implementation continues to use the official `gh run download <run-id> -n <name> -D <dir>` path for workflow artifacts and the GitHub Actions run artifacts API shape for artifact-list diagnostics; no custom artifact transport or hidden GitHub state is introduced.
- `tools/import-renderer-commercial-readiness-final-retained-root-artifacts.ps1` now accepts `-SupplementalRunIds`, `-SupplementalArtifactNames`, `-RegenerateQualityVfx`, and `-RegeneratedQualityVfxOutputRootRelative`. In `Import` mode it downloads the selected supplemental artifact names from each supplemental run into the same intake root, records `supplemental_import` rows, and emits `renderer_commercial_readiness_final_retained_root_artifact_import_supplemental_*` counters.
- With explicit `-RegenerateQualityVfx`, the importer locates the package, D3D12, strict Vulkan, Apple Metal, Metal memory/profiling, and generated-3D source rows already staged in the intake root, invokes `tools/collect-renderer-quality-vfx-commercial-host-evidence.ps1`, records `quality_vfx_regenerate` rows plus output-log/evidence-path counters, and feeds the regenerated `GameEngine.RendererQualityVfxCommercialHostEvidence.v1` path into `assembler_handoff`. The default current-run CI importer path still omits `-RegenerateQualityVfx`, `-AutoAssemble`, and `-RequireReady`.
- TDD evidence: `tools/check-renderer-commercial-readiness-final-retained-root-artifact-import.ps1` first failed with `A parameter cannot be found that matches parameter name 'SupplementalRunIds'`. It now proves a staged current-run root plus supplemental Metal memory/profiling evidence can regenerate quality/VFX evidence, raise the assembler input count to seven, write manifest `supplemental_import` and `quality_vfx_regenerate` rows, and keep `renderer_commercial_readiness=0`.

Task 10AI final retained-root from-runs workflow handoff:

- Context7 `/websites/github_en_actions`, `/actions/upload-artifact`, and `/websites/cli_github_manual` were checked before this slice. The workflow follows official `workflow_dispatch` typed inputs, scoped `contents: read` / `actions: read` permissions, `GH_TOKEN` for GitHub CLI API calls, `gh run download <run-id> -n <name> -D <dir>` through the importer, and pinned `actions/upload-artifact` behavior for `if-no-files-found`.
- `.github/workflows/renderer-commercial-readiness-final-from-runs.yml` is manual-only and requires `source_artifact_run_id`, `metal_memory_profiling_run_id`, and `confirm_final_retained_root_handoff=renderer-commercial-final-retained-root`. It captures `actions/runs/${{ inputs.source_artifact_run_id }}/artifacts?per_page=100`, invokes `tools/import-renderer-commercial-readiness-final-retained-root-artifacts.ps1 -Mode Import` with `SupplementalRunIds`, `SupplementalArtifactNames renderer-metal-memory-profiling-host-artifacts`, `RegenerateQualityVfx`, and `AutoAssemble`, uploads `renderer-commercial-readiness-final-from-runs-artifact-intake`, and uploads `renderer-commercial-readiness-final-retained-root` only if the assembler produced files.
- TDD evidence: `tools/check-ci-matrix.ps1` first failed with `ci-matrix-check: missing required file: .github/workflows/renderer-commercial-readiness-final-from-runs.yml`. It now guards the workflow name, manual-only trigger, input names, `actions: read`, checkout retry, artifact-list API capture, importer arguments, `renderer_commercial_readiness=0`, upload artifact names, and final-root `if-no-files-found: warn` behavior.

Task 10AK final handoff planner:

- Context7 `/websites/github_en_actions`, `/websites/github_en_rest`, and `/websites/cli_github_manual` were checked before this slice and rechecked for runner provisioning/security. The planner mirrors the official model: self-hosted runner labels are selected through `runs-on`, repository metadata is read through `GET /repos/{owner}/{repo}`, repository runners are listed through `/repos/{owner}/{repo}/actions/runners`, registration tokens are requested through `POST /repos/{owner}/{repo}/actions/runners/registration-token` and expire after one hour, manual workflows are triggered by `gh workflow run <workflow>.yml -f key=value`, and public repository self-hosted runner use requires an explicit security review because public fork PRs can execute code on the runner.
- `tools/validate-renderer-commercial-readiness-final-handoff.ps1` is intentionally non-dispatching. It reads an optional `GameEngine.RendererCommercialReadinessFinalRetainedRootArtifactImport.v1` intake manifest plus optional runner preflight source, optional repository metadata, optional retained `GameEngine.RendererPublicSelfHostedRunnerSecurityReview.v1` review artifact, and optional `-UseRunDiscovery` source/Metal run artifact metadata, then reports whether the next action is runner provisioning, public repository security review, `gh workflow run renderer-metal-memory-profiling-capable-host.yml`, `gh workflow run renderer-commercial-readiness-final-from-runs.yml`, final retained-root assembly, or final preflight. It emits `renderer_commercial_readiness_final_handoff_*` counters including `next_action`, run discovery status, `missing_assembler_inputs`, and `quality_vfx_dependency_blockers`; for known public repository provisioning or known public ready-runner dispatch it emits `public_runner_security_review_required` / `complete_public_runner_security_review`, `runner_public_repo_registration_blocked=1`, and withholds the registration-token command, `config.sh` template, and capable-host workflow command until either `-ConfirmPublicRepoSelfHostedRunnerSecurityReview public-repo-self-hosted-runner-risk-reviewed` or a valid retained review artifact is supplied. The artifact validates `review_status=approved`, matching `repo_full_name`, `repository_visibility=public`, reviewed public fork PR risk, runner isolation, secret exposure, allowed workflow, exact labels, workflow audit, and Metal probe truth; invalid supplied artifacts fail closed. After review it emits required labels, official registration-token endpoint, non-secret `gh api -X POST` handoff, the fact that the registration token expires after one hour, `config.sh --url ... --labels metal-residency-set` template, public repository security review confirmation, `runner_public_repo_security_review_artifact_present`, `runner_public_repo_security_review_artifact_valid`, `runner_public_repo_security_review_artifact_status`, `runner_public_repo_security_review_artifact_workflow_audit_valid`, and Metal probe truth before trusting the custom label or printing the capable-host workflow command. It never fetches or prints registration tokens and keeps `renderer_commercial_readiness=0`.
- TDD evidence: `tools/check-renderer-commercial-readiness-final-handoff.ps1` first failed because `tools/validate-renderer-commercial-readiness-final-handoff.ps1` did not exist, later failed with `A parameter cannot be found that matches parameter name 'RepositoryJsonPath'` before public repository security gating was implemented, then failed with `A parameter cannot be found that matches parameter name 'PublicRepoRunnerSecurityReviewRelative'` before retained review artifact intake was implemented, then failed with `A positional parameter cannot be found that accepts argument '-Mode'` before the run-discovery bridge called the discovery planner through explicit named parameters, and later failed because a known public ready-runner path still emitted the capable-host workflow command without retained review. It now proves the planner maps a blocked intake manifest plus no idle runner to `provision_capable_host_runner`, maps public repository metadata without review to `complete_public_runner_security_review` while withholding the registration-token command, maps confirmed public repository review and valid retained review artifact back to exact provisioning rows, maps a ready runner without public metadata to `run_metal_memory_profiling_capable_host_workflow`, maps run discovery with source-ready/Metal-missing/no public review to `complete_public_runner_security_review`, maps a known public ready runner without review to `complete_public_runner_security_review` while withholding the capable-host workflow command, maps a known public ready runner with retained review to the capable-host workflow command, maps run discovery with both artifact-backed run IDs to the exact final-from-runs command, maps a manifest source run plus Metal memory/profiling run id to the exact final-from-runs command, and maps an existing final retained root to the exact final preflight command.

Task 10AL public runner security review artifact producer:

- Context7 `/websites/github_en_actions` and `/websites/github_en_rest` were checked before this slice. The implementation follows the official public-repository self-hosted runner security warning, the `runs-on` label model for `self-hosted, macOS, ARM64, metal-residency-set`, repository metadata through `GET /repos/{owner}/{repo}`, repository runner listing through `/repos/{owner}/{repo}/actions/runners`, and registration token handoff through `POST /repos/{owner}/{repo}/actions/runners/registration-token`, whose token expires after one hour.
- `tools/generate-renderer-public-runner-security-review.ps1` is intentionally non-dispatching. In `Plan` mode it reports `renderer_public_runner_security_review_status=review_required`, the public repository security review blockers, the official registration-token endpoint, the one-hour expiry fact, and non-actions with `registration_token_fetched=false`, `registration_token_printed=false`, `workflow_dispatched=false`, `runner_registered=false`, and `gpu_workload_executed=false`. In `Generate` mode it writes retained `GameEngine.RendererPublicSelfHostedRunnerSecurityReview.v1` only after public repository metadata and explicit approvals through `-ApprovePublicRepoSelfHostedRunnerReview public-repo-self-hosted-runner-risk-reviewed`, `-ReviewedPublicForkPrRisk`, `-ReviewedRunnerIsolation`, `-ReviewedSecretExposure`, and `-ReviewedMetalProbeTruth`.
- The retained artifact records `review_status=approved`, matching `repo_full_name`, `repository_visibility=public`, `reviewed_public_fork_pr_risk=true`, `reviewed_runner_isolation=true`, `reviewed_secret_exposure=true`, `reviewed_allowed_workflows` including `renderer-metal-memory-profiling-capable-host.yml`, exact `reviewed_required_labels` `self-hosted, macOS, ARM64, metal-residency-set`, `reviewed_metal_probe_truth=true`, official GitHub source rows, `renderer_public_runner_security_review` counters, and the same non-actions. It is consumed later by `tools/validate-renderer-commercial-readiness-final-handoff.ps1 -PublicRepoRunnerSecurityReviewRelative`, which then emits `runner_public_repo_security_review_artifact_present`, `runner_public_repo_security_review_artifact_valid`, and `runner_public_repo_security_review_artifact_status` while preserving `complete_public_runner_security_review` for missing review.
- Public review artifact hardening now requires `-WorkflowRelative .github/workflows/renderer-metal-memory-profiling-capable-host.yml`, retained `reviewed_workflow_file`, and `workflow_audit` rows before final handoff accepts the artifact. The audit must prove `workflow_dispatch_only=true`, `untrusted_pr_triggers_present=false`, `contents_permission_read_only=true`, `checkout_action_pinned=true`, `checkout_persist_credentials_disabled=true`, `confirm_input_required=true`, and exact `reviewed_required_labels`; final handoff emits `runner_public_repo_security_review_artifact_workflow_audit_valid` and fails closed when the audit is absent or false.
- TDD evidence: `tools/check-renderer-public-runner-security-review.ps1` first failed because `renderer_public_runner_security_review_workflow_audit_file=.github/workflows/renderer-metal-memory-profiling-capable-host.yml` was missing from generator output. `tools/check-renderer-commercial-readiness-final-handoff.ps1` first failed because `renderer_commercial_readiness_final_handoff_runner_public_repo_security_review_artifact_workflow_audit_valid=1` was missing, then failed closed until the fixture preserved `workflow_audit.required_labels`. Both checks now pass.
- Live evidence for `y2ikgm89/mirakanai-engine`: `tools/generate-renderer-public-runner-security-review.ps1 -Mode Generate -RepoFullName y2ikgm89/mirakanai-engine -OutputRootRelative artifacts/renderer/public-runner-security-review/renderer-commercial-readiness -ApprovePublicRepoSelfHostedRunnerReview public-repo-self-hosted-runner-risk-reviewed -ReviewedPublicForkPrRisk -ReviewedRunnerIsolation -ReviewedSecretExposure -ReviewedMetalProbeTruth` wrote `artifacts/renderer/public-runner-security-review/renderer-commercial-readiness/public-runner-security-review.json` with `workflow_audit.ready=true` and no token fetch/print, runner registration, workflow dispatch, or GPU workload. Passing that artifact to `tools/validate-renderer-commercial-readiness-final-handoff.ps1 -RepoFullName y2ikgm89/mirakanai-engine -UseRunDiscovery -PublicRepoRunnerSecurityReviewRelative artifacts/renderer/public-runner-security-review/renderer-commercial-readiness/public-runner-security-review.json` moved the live status to `capable_host_runner_required` / `provision_capable_host_runner`, with `runner_public_repo_registration_blocked=0`, `runner_public_repo_security_review_artifact_valid=1`, `runner_public_repo_security_review_artifact_workflow_audit_valid=1`, and `renderer_commercial_readiness=0`.
- TDD evidence: `tools/check-renderer-public-runner-security-review.ps1` first failed because `tools/generate-renderer-public-runner-security-review.ps1` did not exist. It now proves Plan mode writes no approved artifact, missing approvals report `approval_confirmation_required`, `public_fork_pr_risk_review_required`, `runner_isolation_review_required`, `secret_exposure_review_required`, and `metal_probe_truth_review_required`, approved Generate mode writes a retained artifact without any token or `ghs_` secret text, and the final handoff planner accepts the artifact while still keeping `renderer_commercial_readiness=0`.

Task 10AN public ready-runner dispatch review gate:

- Context7 `/websites/github_en_actions` was rechecked for the official public repository self-hosted runner warning before this hardening. The final handoff now treats a known public repository with an already online matching self-hosted runner the same as provisioning: it requires the explicit public runner security review before printing `gh workflow run renderer-metal-memory-profiling-capable-host.yml`.
- `tools/check-renderer-commercial-readiness-final-handoff.ps1` first failed because the ready public runner fixture still reported `metal_memory_profiling_run_required` and emitted `renderer_commercial_readiness_final_handoff_capable_host_workflow_command`. The check now proves the unreviewed public ready-runner path returns `public_runner_security_review_required` / `complete_public_runner_security_review`, keeps `runner_public_repo_registration_blocked=1`, withholds the capable-host workflow command, and only emits the command after a retained review artifact with a valid workflow audit is supplied.

Task 10AM final run discovery planner:
- `tools/plan-renderer-commercial-readiness-final-run-discovery.ps1` is intentionally non-dispatching. It reads GitHub Actions workflow runs through `GET /repos/{owner}/{repo}/actions/runs` and per-run artifact metadata through `GET /repos/{owner}/{repo}/actions/runs/{run_id}/artifacts`, via `gh api` or fixture JSON, then emits exact `renderer_commercial_readiness_final_run_discovery_*` counters for source/current-run intake readiness, Metal memory/profiling artifact readiness, status, next action, and the final-from-runs command.
- The planner verifies `renderer-commercial-readiness-current-run-artifact-intake` for the source `Validate` run and `renderer-metal-memory-profiling-host-artifacts` for the capable-host run. Missing source artifacts map to `source_run_required` / `wait_for_current_run_artifact_intake`; missing Metal artifacts map to `metal_memory_profiling_run_required` / `run_metal_memory_profiling_capable_host_workflow`; both present map to `ready_for_final_from_runs_workflow` and print `gh workflow run renderer-commercial-readiness-final-from-runs.yml`.
- TDD evidence: `tools/check-renderer-commercial-readiness-final-run-discovery.ps1` first failed because `tools/plan-renderer-commercial-readiness-final-run-discovery.ps1` did not exist. It now proves ready, missing Metal artifact, and missing source artifact paths using fixture JSON, while `workflow_dispatched=0`, `artifacts_downloaded=0`, `gpu_workload_executed=0`, `evidence_assembled=0`, and `renderer_commercial_readiness=0` stay fixed.
- 2026-06-28 read-only repository audit evidence: `tools/plan-renderer-commercial-readiness-final-run-discovery.ps1 -Mode Plan -RepoFullName y2ikgm89/mirakanai-engine` returned `source_run_ready=1`, `source_run_id=28297357528`, `source_artifact_present=1`, `source_artifacts_discovered=11`, `metal_memory_profiling_runs_discovered=0`, `metal_memory_profiling_artifact_present=0`, `status=metal_memory_profiling_run_required`, and `next_action=run_metal_memory_profiling_capable_host_workflow`, with `workflow_dispatched=0`, `artifacts_downloaded=0`, `gpu_workload_executed=0`, `evidence_assembled=0`, and `renderer_commercial_readiness=0`.
- 2026-06-30 host-gated closeout evidence after PR #915 / merge commit `38e68e281efa82d99b91fe606c665e4bc7f347be`: `tools/plan-renderer-commercial-readiness-final-run-discovery.ps1 -Mode Plan -RepoFullName y2ikgm89/mirakanai-engine` returned `source_run_ready=1`, `source_run_id=28450893645`, `source_artifact_present=1`, `source_artifacts_discovered=11`, `metal_memory_profiling_runs_discovered=0`, `metal_memory_profiling_artifact_present=0`, `status=metal_memory_profiling_run_required`, `next_action=run_metal_memory_profiling_capable_host_workflow`, and `renderer_commercial_readiness=0`. `tools/validate-renderer-commercial-readiness-final-handoff.ps1 -RepoFullName y2ikgm89/mirakanai-engine -UseRunDiscovery -PublicRepoRunnerSecurityReviewRelative artifacts/renderer/public-runner-security-review/renderer-commercial-readiness/public-runner-security-review.json` returned `status=capable_host_runner_required`, `next_action=provision_capable_host_runner`, `runner_available=0`, `runner_public_repo_security_review_artifact_valid=1`, `runner_public_repo_security_review_artifact_workflow_audit_valid=1`, `runner_public_repo_registration_blocked=0`, and `renderer_commercial_readiness=0`. Context7 GitHub Actions and GitHub REST docs were rechecked for self-hosted runner labels, public repository runner risk, and the one-hour repository registration token endpoint before closeout.
