...
- `.github/workflows/validate.yml` now collects `renderer-vulkan-strict-commercial-quality-host-evidence` in the selected `windows-cpp23` lane after the D3D12 retained input. It does not pass `-RequireReady`; current hosts can therefore retain a fail-closed host-gate summary when strict Vulkan timestamp/profiling rows are not yet real, instead of failing the whole validation run or pretending the route is ready.
