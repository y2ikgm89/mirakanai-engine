...
- Task 10AK adds `renderer-commercial-readiness-final-handoff`, `rendererCommercialReadinessFinalHandoff`, `tools/validate-renderer-commercial-readiness-final-handoff.ps1`, and `tools/check-renderer-commercial-readiness-final-handoff.ps1` as the non-dispatching final handoff planner. It consumes an optional intake manifest plus optional runner preflight fixture/API source, resolves `source_artifact_run_id` from `-SourceRunId` or manifest `run_id`, accepts `metal_memory_profiling_run_id`, emits `next_action`, `missing_assembler_inputs`, `quality_vfx_dependency_blockers`, assembler/final-preflight readiness, and exact next commands for the capable-host workflow, final-from-runs workflow, assembler, or final preflight. When runner provisioning is required, it emits the official registration-token endpoint, non-secret `gh api -X POST` handoff, the fact that the registration token expires after one hour, `config.sh --url ... --labels metal-residency-set` template, public repository security review, and Metal probe truth requirement while keeping `renderer_commercial_readiness=0`.
...
- Context7 `/websites/github_en_actions`, `/websites/github_en_rest`, and `/websites/cli_github_manual` were checked before this slice and rechecked for runner provisioning. The planner mirrors the official model: self-hosted runner labels are selected through `runs-on`, repository runners are listed through `/repos/{owner}/{repo}/actions/runners`, registration tokens are requested through `POST /repos/{owner}/{repo}/actions/runners/registration-token` and expire after one hour, manual workflows are triggered by `gh workflow run <workflow>.yml -f key=value`, and public repository runner use requires an explicit security review.
- `tools/validate-renderer-commercial-readiness-final-handoff.ps1` is intentionally non-dispatching. It reads an optional `GameEngine.RendererCommercialReadinessFinalRetainedRootArtifactImport.v1` intake manifest plus optional runner preflight source, then reports whether the next action is runner provisioning, `gh workflow run renderer-metal-memory-profiling-capable-host.yml`, `gh workflow run renderer-commercial-readiness-final-from-runs.yml`, final retained-root assembly, or final preflight. It emits `renderer_commercial_readiness_final_handoff_*` counters including `next_action`, `missing_assembler_inputs`, and `quality_vfx_dependency_blockers`; for `provision_capable_host_runner` it also emits required labels, official registration-token endpoint, non-secret `gh api -X POST` handoff, the fact that the registration token expires after one hour, `config.sh --url ... --labels metal-residency-set` template, public repository security review, and Metal probe truth before trusting the custom label. It never fetches or prints registration tokens and keeps `renderer_commercial_readiness=0`.
- TDD evidence: `tools/check-renderer-commercial-readiness-final-handoff.ps1` first failed because `tools/validate-renderer-commercial-readiness-final-handoff.ps1` did not exist. It now proves the planner maps a blocked intake manifest plus no idle runner to `provision_capable_host_runner` with exact provisioning rows, a ready runner without a Metal run id to `run_metal_memory_profiling_capable_host_workflow`, a source run plus Metal memory/profiling run id to the exact final-from-runs command, and an existing final retained root to the exact final preflight command.
