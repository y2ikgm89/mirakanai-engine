...
- `sample_desktop_runtime_game --emit-vulkan-strict-commercial-host-gate` now emits the strict aggregate, GPU memory, debug profiling, timestamp-query, and `renderer_vulkan_timestamp_ready` fields from `LinuxDesktopVulkanPresentationReport` instead of hard-coded zeros. Raw begin/end GPU timestamp values remain backend-private and are not exposed through the Linux runtime-host public API or package-visible output. With `collect_strict_commercial_evidence` selected, the Linux package probe runs a private RHI copy/marker/timestamp path, feeds GPU memory diagnostics and RHI stats back into the report, and can satisfy the exact strict aggregate/memory/profiling rows required by the retained host-evidence producer.
- `tools/check-renderer-vulkan-strict-commercial-quality-host-evidence.ps1` now locks the Linux sample output to report-driven strict counters, exact strict aggregate feature/layout rows, GPU memory/profiling ready rows, raw timestamp non-exposure, and fixed `renderer_commercial_readiness=0`. Platform-only Linux Vulkan evidence can still only produce a host-gated summary; ready strict Vulkan source evidence requires the selected strict package smoke plus the final retained root `-RequireReady` path for commercial promotion.
- Focused validation passed for `MK_runtime_host_tests`, `tools/check-renderer-vulkan-strict-commercial-quality-host-evidence.ps1`, `tools/check-ci-matrix.ps1`, `tools/check-json-contracts.ps1`, `tools/check-agents.ps1`, `tools/check-ai-integration.ps1`, `tools/check-format.ps1`, and full `tools/validate.ps1` with 46 static checks plus 159/159 CTest cases in `out/validation-logs/validate-20260627-081602-42828`. This task does not set `renderer_backend_parity_ready=1`, `renderer_metal_broad_readiness=1`, `renderer_broad_quality_ready=1`, or `renderer_commercial_readiness=1`.
