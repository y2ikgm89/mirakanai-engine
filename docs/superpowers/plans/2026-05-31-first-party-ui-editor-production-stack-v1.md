...
- `editor/src/native_viewport_surface.*`, `native_material_preview_cache.*`, and `native_texture_display_adapter.*` still report `diagnostic_only` in the visible shell for D3D12 texture display. They preserve private-handle policy and now include value-only readiness gates plus RHI-backed private adapter evidence for requested host-private texture display, offscreen targets, descriptor leases, resource barriers, fences, viewport resize-safe lifecycle during recreation, and visible-compositor consumption gates (`visible_composite_pending` / `visible_panel_unavailable`) before `d3d12_texture_ready`, but do not yet bind actual viewport/material textures into visible UI presentation or promote smoke counters.
- The active manifest and docs keep full D3D12 viewport/material texture display, full docking productization, native IME candidate UI, runtime UI OS accessibility publication, full UIA pattern/event parity, broad production font fallback, cross-platform editor shells, public native handles, SDL3, and UI middleware automation unsupported.
