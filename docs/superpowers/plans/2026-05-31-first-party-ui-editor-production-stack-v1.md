...
- `editor/src/native_viewport_surface.*` and `native_material_preview_cache.*` still report `diagnostic_only` in the visible shell for D3D12 texture display. They preserve private-handle policy and now include value-only readiness gates for requested host-private texture display, private D3D12 texture adapter, offscreen target, descriptor lease, resource-barrier, and fence evidence, plus viewport-only resize-safe lifecycle evidence during recreation, but do not yet display actual viewport/material textures or promote smoke counters.
