...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | No child plan is active after `Generic 2D Sandbox Tile Simulation Primitives v1` closeout. The composed manifest returns to `recommendedNextPlan.id = next-production-gap-selection` while keeping `unsupportedProductionGaps = []`. |
| Completed slice | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Completed Phase 3 tile simulation primitives through `RuntimeSandboxTileMaterialRow`, `RuntimeSandboxTileSimulationDesc`, `RuntimeSandboxTileSimulationPlan`, `RuntimeSandboxTileCollisionSpanRow`, `RuntimeSandboxTileCellRow`, `RuntimeSandboxLightPropagationRow`, `RuntimeSandboxLiquidFlowRow`, `RuntimeSandboxScheduledTileUpdateRow`, and `plan_runtime_sandbox_tile_simulation`, with deterministic collision/platform/liquid/trigger/light/liquid-flow/scheduled-update rows, replay hashes, fail-closed diagnostics, focused runtime tests, docs/manifest/static-check sync, and no physics upload, renderer lightmap upload, full fluid simulation, persistence IO, package loading, streaming execution, renderer/platform/editor APIs, threads, native handles, legacy desktop middleware, or game-specific content rules. |
...
| Proposed future milestone | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Planning milestone for a clean-break, first-party, large mutable 2D sandbox production lane. Its Phase 1 runtime foundation, Phase 2 mutation execution, and Phase 3 tile simulation primitives have landed as reviewable slices. It depends on the completed Win32 desktop host stack (`MK_platform_win32`, `MK_runtime_host_win32`, `MK_runtime_host_win32_presentation`, `MK_audio_wasapi`) and keeps persistence, streaming, production tile renderer, authoring/cook, optional network/mod gates, package evidence, and closeout as unimplemented until selected. It does not replace `currentActivePlan`, reopen `unsupportedProductionGaps`, or mark planned future phases ready. |
