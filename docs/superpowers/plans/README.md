...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | No child plan is active after `Generic 2D Sandbox Streaming Residency v1` closeout. The composed manifest returns to `recommendedNextPlan.id = next-production-gap-selection` while keeping `unsupportedProductionGaps = []`. |
| Completed slice | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Completed Phase 5 streaming/residency planning through `RuntimeSandboxWorldStreamingPlan`, `RuntimeSandboxWorldStreamingSourceRow`, `RuntimeSandboxWorldAddressableDependencyRow`, `RuntimeSandboxWorldStreamingSafePointDesc`, `RuntimeSandboxWorldStreamingSafePointResult`, `plan_runtime_sandbox_world_streaming`, and `execute_runtime_sandbox_world_streaming_safe_point`, with source-range chunk selection, dirty resident chunk pinning, addressable dependency rows, resident budget diagnostics, reviewed world-region safe-point adoption, focused runtime/world-region/addressable tests, docs/manifest/static-check sync, and no background streaming, async addressable execution, automatic LRU eviction, renderer/RHI residency, native handles, legacy desktop middleware, or game-specific biome/block-art rules. Retained Phase 4 evidence includes `RuntimeSandboxWorldPersistenceDocumentPlan` and `plan_runtime_sandbox_world_atomic_save`; retained Phase 3 evidence includes `RuntimeSandboxTileSimulationPlan` and `plan_runtime_sandbox_tile_simulation`. |
...
| Proposed future milestone | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Planning milestone for a clean-break, first-party, large mutable 2D sandbox production lane. Its Phase 1 runtime foundation, Phase 2 mutation execution, Phase 3 tile simulation primitives, Phase 4 persistence planning, and Phase 5 streaming/residency planning have landed as reviewable slices. It depends on the completed Win32 desktop host stack (`MK_platform_win32`, `MK_runtime_host_win32`, `MK_runtime_host_win32_presentation`, `MK_audio_wasapi`) and keeps persistence execution beyond value-only plans, production tile renderer, authoring/cook, optional network/mod gates, package smoke evidence, and closeout as unimplemented until selected. It does not replace `currentActivePlan`, reopen `unsupportedProductionGaps`, or mark planned future phases ready. |
