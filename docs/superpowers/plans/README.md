...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [General Purpose Game Production v1](2026-05-25-general-purpose-game-production-v1.md) | Selected post-foundation production track; Phases 1-8 implement reusable gameplay runtime scheduling, world/entity ownership, addressable content streaming, production authoring workflow review, runtime UI workbench review, `genre-rpg-systems-pack-v1` through `plan_runtime_rpg_systems`, `genre-sandbox-world-pack-v1` through `plan_runtime_sandbox_world_mutation`, and `genre-simulation-management-pack-v1` through `plan_runtime_simulation_management`; the remaining candidates cover network replication and rendering/VFX/profiling surfaces. |
