...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [First-Party Desktop Platform And SDL3 Removal v1](2026-05-27-first-party-desktop-platform-sdl3-removal-v1.md) | Operator-selected long-running clean-break milestone to replace SDL3 desktop runtime/editor/audio surfaces with first-party Windows desktop backends before deleting SDL3 from build, package, generated-game, docs, legal, and agent contract surfaces. Phases 0-6 are merged; Phase 7 has migrated the config-only `DesktopRuntimePackage` / `sample_generated_desktop_runtime_package` slice and is extending that Win32 host proof to `DesktopRuntimeCookedScenePackage` / `sample_generated_desktop_runtime_cooked_scene_package`, while material/2D/3D generated-game, editor, and final deletion lanes remain separate PRs. |
