...
| Status | Plan | Notes |
| --- | --- | --- |
| Active slice (`currentActivePlan`) | [Renderer Backend Parity Metal Apple Evidence v1](2026-05-29-renderer-backend-parity-metal-apple-evidence-v1.md) | Selects `renderer-backend-parity-v1` Apple/Metal host evidence without promoting Metal readiness: `metal-apple` remains host-gated through `shader-toolchain`, `mobile-packaging`, and `ios-simulator-smoke`, while `unsupportedProductionGaps = []` stays the 1.0 ready-surface truth. |
...
| Parent milestone | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | Parent production-completion roadmap. It remains the governing milestone while the selected renderer backend parity Apple/Metal evidence slice executes. |
