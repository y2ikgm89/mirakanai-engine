...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | No child plan is active after `Generic 2D Sandbox Persistence Snapshot v1` closeout. The composed manifest returns to `recommendedNextPlan.id = next-production-gap-selection` while keeping `unsupportedProductionGaps = []`. |
| Completed slice | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Completed Phase 4 persistence planning through `RuntimeSandboxWorldPersistenceDocumentPlan`, `RuntimeSandboxWorldMigrationReviewPlan`, `RuntimeSandboxWorldAtomicSavePlan`, `plan_runtime_sandbox_world_persistence_document`, `plan_runtime_sandbox_world_snapshot_diff`, `review_runtime_sandbox_world_migration`, and `plan_runtime_sandbox_world_atomic_save`, with canonical snapshot rows, dirty-chunk diffs, schema migration/recovery review, safe project-relative atomic-save operation intent, focused runtime tests, docs/manifest/static-check sync, and no filesystem writes, Win32/`IFileSystem` execution, package mutation, migration execution, streaming execution, native handles, legacy desktop middleware, or game-specific save serialization. Retained Phase 3 evidence includes `RuntimeSandboxTileSimulationPlan` and `plan_runtime_sandbox_tile_simulation` for deterministic tile simulation primitives. |
...
| Proposed future milestone | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Planning milestone for a clean-break, first-party, large mutable 2D sandbox production lane. Its Phase 1 runtime foundation, Phase 2 mutation execution, Phase 3 tile simulation primitives, and Phase 4 persistence planning have landed as reviewable slices. It depends on the completed Win32 desktop host stack (`MK_platform_win32`, `MK_runtime_host_win32`, `MK_runtime_host_win32_presentation`, `MK_audio_wasapi`) and keeps persistence execution, streaming, production tile renderer, authoring/cook, optional network/mod gates, package evidence, and closeout as unimplemented until selected. It does not replace `currentActivePlan`, reopen `unsupportedProductionGaps`, or mark planned future phases ready. |
