...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Engine General Production Quality Expansion v1](2026-05-26-engine-general-production-quality-expansion-v1.md) | Selected post-1.0 production breadth plan for renderer quality, runtime UI text/font/IME/accessibility, broad reviewed import, physics/navigation breadth, networking execution/security, and audio production evidence. It keeps `unsupportedProductionGaps = []` because this is not a reopened Engine 1.0 blocker. Phases 1-6 are implemented through Renderer General Quality Matrix v1, Runtime UI Production Stack Evidence v1, Asset Import Production Review v1, Physics/Navigation Production Breadth v1, Runtime Network Production Security Gate v1 through `plan_runtime_network_production_security_gate` with threat-modeled selected evidence and without NAT traversal or broad online claims, and Audio Production Playback/Streaming/DSP/Spatialization Evidence v1. None claims broad production readiness beyond selected evidence and host-gated rows. |
| Active phase checkpoint | [Engine General Production Quality Expansion v1 - Phase 7](2026-05-26-engine-general-production-quality-expansion-v1.md#phase-7---cross-domain-package-generated-game-and-editor-evidence) | Cross-domain package evidence extends landed production proofs into selected generated package recipes. The current slice adds generated 3D `--require-audio-production` package evidence over the cooked audio payload through `review_audio_production_readiness` with `audio_production_*` counters, while keeping device/HRTF host gates and broad codec, middleware, background streaming, native handle, and general production audio quality non-claims. |
