...
Retained sandbox-world evidence for the active slice includes `genre-sandbox-world-pack-v1`, `plan_runtime_sandbox_world_mutation`, `sandbox_world_tile_drop_rows`, `RuntimeSandboxTileSimulationPlan`, `plan_runtime_sandbox_tile_simulation`, `RuntimeSandboxWorldPersistenceDocumentPlan`, `plan_runtime_sandbox_world_atomic_save`, `RuntimeSandboxWorldStreamingPlan`, `plan_runtime_sandbox_world_streaming`, and `execute_runtime_sandbox_world_streaming_safe_point`; the selected package-budget plan only packages and validates these existing value-only rows.

| Status | Plan | Notes |
| --- | --- | --- |
| Active slice (`currentActivePlan`) | [Sandbox World Package Validation And Performance Budgets v1](2026-05-30-sandbox-world-package-validation-performance-budgets-v1.md) | Selected focused child for sample package smoke flags, installed validation, package-visible counters, and deterministic budget evidence after Phase 9 merged. It preserves the `plan_tile_chunk_renderer` / `--require-production-tile-renderer` package proof, `sandbox_world_tile_drop_rows` gameplay integration evidence, and native texture ownership non-claim while keeping SDL3 absent, broad online/modding/renderer-quality claims unsupported, and `unsupportedProductionGaps = []`. |
| Active milestone | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Parent milestone for the current 2D sandbox production lane. Phases 1-9 have landed as reviewable slices; Phase 10 is active through the selected child plan. |
| Parent roadmap | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | Governing production-completion roadmap while the selected child is implemented and reviewed. |
