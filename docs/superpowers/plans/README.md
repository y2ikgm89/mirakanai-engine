...
| Status | Plan | Notes |
| --- | --- | --- |
| Active slice (`currentActivePlan`) | [Sandbox World Network And Modding Gate v1](2026-05-30-sandbox-world-network-modding-gate-v1.md) | Selected focused child for sandbox-world-specific mutation replication and reviewed modding policy gates. It keeps ENet optional, SDL3 absent, broad online/modding claims unsupported, and `unsupportedProductionGaps = []`. |
| Parent milestone | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | Governing production-completion roadmap while the selected child is implemented and reviewed. |
| Completed slice | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Completed Phase 8 generic gameplay integration row evidence through `RuntimeSandboxTileDropRow`, `RuntimeSandboxConstructionCostConsumptionRow`, `RuntimeSandboxToolEffectivenessRow`, `RuntimeSandboxSpawnRegionRow`, `RuntimeSandboxDayNightEventRow`, and `RuntimeSandboxTriggerRow`, with selected 2D package `sandbox_world_cost_consumption_rows`, `sandbox_world_tile_drop_rows`, `sandbox_world_tool_effectiveness_rows`, `sandbox_world_spawn_region_rows`, `sandbox_world_day_night_event_rows`, and `sandbox_world_trigger_rows` counters, focused runtime/package tests, docs/manifest/static-check sync, and no game-rule execution, boss/NPC catalogs, hardcoded damage formulas, economy balance hooks, renderer/platform/editor APIs, native handles, legacy desktop middleware, SDL3, or broad sandbox content production. Retained Phase 7 evidence includes `SandboxWorldAuthoringPackageDesc`, `review_sandbox_world_authoring_package`, and `--require-sandbox-authoring-review`; retained Phase 6 evidence includes `TileChunkRendererPlan`, `plan_tile_chunk_renderer`, `--require-production-tile-renderer`, and zero native texture ownership invocation; retained Phase 5 evidence includes `RuntimeSandboxWorldStreamingPlan`, `plan_runtime_sandbox_world_streaming`, and `execute_runtime_sandbox_world_streaming_safe_point`; retained Phase 4 evidence includes `RuntimeSandboxWorldPersistenceDocumentPlan` and `plan_runtime_sandbox_world_atomic_save`; retained Phase 3 evidence includes `RuntimeSandboxTileSimulationPlan` and `plan_runtime_sandbox_tile_simulation`. |
...
| Proposed future milestone | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Planning milestone for a clean-break, first-party, large mutable 2D sandbox production lane. Its Phase 1 runtime foundation, Phase 2 mutation execution, Phase 3 tile simulation primitives, Phase 4 persistence planning, Phase 5 streaming/residency planning, Phase 6 production tile renderer evidence, Phase 7 sandbox authoring review evidence, and Phase 8 generic gameplay integration row evidence have landed as reviewable slices. The optional network/mod gate is selected through [Sandbox World Network And Modding Gate v1](2026-05-30-sandbox-world-network-modding-gate-v1.md). Package-budget closeout, final lane closeout, and persistence execution beyond value-only plans remain unimplemented until selected. It does not reopen `unsupportedProductionGaps` or mark planned future phases ready. |
...
| Parent milestone | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | Parent production-completion roadmap. It remains the governing milestone while the next selected production child is chosen explicitly. |
