...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [General Purpose Game Production v1](2026-05-25-general-purpose-game-production-v1.md) | Selected post-foundation production track; Phases 1-7 implement reusable gameplay runtime scheduling, world/entity ownership, addressable content streaming, production authoring workflow review, runtime UI workbench review, `genre-rpg-systems-pack-v1` through `plan_runtime_rpg_systems`, and `genre-sandbox-world-pack-v1` through `plan_runtime_sandbox_world_mutation`; the remaining candidates cover simulation management, network replication, and rendering/VFX/profiling surfaces. |
