...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | No child slice is currently selected. `recommendedNextPlan.id = next-production-gap-selection`, `unsupportedProductionGaps = []`, and the next developer-owned production gap must be selected explicitly before implementation. |
| Proposed milestone | [Native Win32 Editor Shell v1](2026-05-30-native-win32-editor-shell-v1.md) | Long-running clean-break plan to restore a launchable `MK_editor` as a Windows-native Win32 + Dear ImGui + Direct3D 12 shell after SDL3 removal. It keeps `MK_editor_core` GUI-independent, reintroduces Dear ImGui only as an audited optional `desktop-gui` dependency, forbids SDL3 compatibility shims, and phases launch smoke, core-backed panels, native services, viewport display, material preview, docs, manifest, and validation evidence. |
