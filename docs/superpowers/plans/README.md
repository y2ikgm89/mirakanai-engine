...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | No child plan is active after `Generic 2D Sandbox Production Tile Renderer v1` closeout. The composed manifest returns to `recommendedNextPlan.id = next-production-gap-selection` while keeping `unsupportedProductionGaps = []`. |
| Completed slice | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Completed Phase 6 production tile renderer evidence through `TileChunkCellRow`, `TileChunkDirtyRegion`, `TileChunkLightCellRow`, `TileChunkRendererDesc`, `TileChunkRendererPlan`, `TileChunkSpriteRow`, `TileChunkDrawRow`, `TileChunkDirtyRebuildRow`, `TileChunkLightmapRow`, and `plan_tile_chunk_renderer`, with deterministic visible-cell sprite rows, opaque/transparent draw grouping, dirty rebuild budgets, renderer-neutral lightmap rows, selected 2D package `--require-production-tile-renderer` counters, focused renderer/package tests, docs/manifest/static-check sync, and no backend-specific submission, native texture ownership, renderer/RHI residency, native handles, legacy desktop middleware, SDL3, or broad renderer quality claim. Retained Phase 5 evidence includes `RuntimeSandboxWorldStreamingPlan`, `plan_runtime_sandbox_world_streaming`, and `execute_runtime_sandbox_world_streaming_safe_point`; retained Phase 4 evidence includes `RuntimeSandboxWorldPersistenceDocumentPlan` and `plan_runtime_sandbox_world_atomic_save`; retained Phase 3 evidence includes `RuntimeSandboxTileSimulationPlan` and `plan_runtime_sandbox_tile_simulation`. |
...
| Proposed future milestone | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Planning milestone for a clean-break, first-party, large mutable 2D sandbox production lane. Its Phase 1 runtime foundation, Phase 2 mutation execution, Phase 3 tile simulation primitives, Phase 4 persistence planning, Phase 5 streaming/residency planning, and Phase 6 production tile renderer evidence have landed as reviewable slices. It depends on the completed Win32 desktop host stack (`MK_platform_win32`, `MK_runtime_host_win32`, `MK_runtime_host_win32_presentation`, `MK_audio_wasapi`) and keeps persistence execution beyond value-only plans, authoring/cook, generic gameplay row integration, optional network/mod gates, package-budget closeout, and final lane closeout as unimplemented until selected. It does not replace `currentActivePlan`, reopen `unsupportedProductionGaps`, or mark planned future phases ready. |
