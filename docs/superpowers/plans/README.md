...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Engine General Production Quality Expansion v1](2026-05-26-engine-general-production-quality-expansion-v1.md) | Selected post-1.0 production breadth plan for renderer quality, runtime UI text/font/IME/accessibility, broad reviewed import, physics/navigation breadth, networking execution/security, and audio production evidence. It keeps `unsupportedProductionGaps = []` because this is not a reopened Engine 1.0 blocker. Phase 1 records Renderer General Quality Matrix v1 through `plan_renderer_quality_matrix`, `renderer_quality_matrix_status=host_evidence_required`, and `renderer_quality_matrix_general_renderer_quality_ready=0`. |
