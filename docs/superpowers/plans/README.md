...
| Status | Plan | Notes |
| --- | --- | --- |
| Active slice (`currentActivePlan`) | [Renderer Production VFX Profiling Host Gate v1](2026-05-26-renderer-production-vfx-profiling-host-gate-v1.md) | Selected next developer-owned renderer production evidence slice for `production-rendering-vfx-profiling-v1`; it targets D3D12 and strict Vulkan host-evidence-backed VFX/profiling counters while keeping Metal Apple-host-gated and `unsupportedProductionGaps = []`. |
