...
| Status | Plan | Notes |
| --- | --- | --- |
| Active slice (`currentActivePlan`) | [Engine Contract Version Suffix Cleanup](2026-05-26-engine-contract-version-suffix-cleanup.md) | Clean-breaking pre-release cleanup now active on `codex/versionless-saved-formats`; Candidate 2 is canonicalizing repository-owned saved formats without compatibility parsers, aliases, dual writers, or fallback readers. |
| Broader active milestone | [Generated Game Studio v1](2026-05-26-generated-game-studio-v1.md) | Successor game-surface milestone remains the broader production track while the contract cleanup child is active. Candidate 5 returns `currentActivePlan` to this milestone or the selected next plan after cleanup validation. |
