...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [First-Party Desktop Platform And SDL3 Removal v1](2026-05-27-first-party-desktop-platform-sdl3-removal-v1.md) | Operator-selected long-running clean-break milestone to replace SDL3 desktop runtime/editor/audio surfaces with first-party Windows desktop backends before deleting SDL3 from build, package, generated-game, docs, legal, and agent contract surfaces. Phases 0-7 are merged, and the current Phase 9 deletion checkpoint removes SDL3 source adapters, SDL3 tests, root/package SDL3 CMake hooks, SDL3 vcpkg/features, and retired SDL3-only non-shell/3D package samples from active targets while preserving their deeper renderer/package evidence as historical until scoped Win32 advanced package lanes are selected. |
...
| Active master plan | [Clean Break Broad Production Readiness Master Plan v1](../master-plans/2026-05-27-clean-break-broad-production-readiness-master-plan-v1.md) | Long-running sequencing plan for broad production areas. It now treats the SDL3 removal milestone as the active platform cleanup track while keeping previous renderer/runtime UI/importer evidence fail-closed and `unsupportedProductionGaps = []`. |
