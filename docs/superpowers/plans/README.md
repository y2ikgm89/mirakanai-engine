...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | No child plan is active after `Generic 2D Sandbox Authoring Review v1` closeout. The composed manifest returns to `recommendedNextPlan.id = next-production-gap-selection` while keeping `unsupportedProductionGaps = []`. |
| Completed slice | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Completed Phase 7 sandbox authoring review evidence through `SandboxWorldAuthoringPackageDesc`, `SandboxWorldTileDefinitionRow`, `SandboxWorldPaletteBrushRow`, `SandboxWorldChunkTemplateRow`, `SandboxWorldProceduralSeedRow`, `sandbox_world_authoring_review_format_v1`, `review_sandbox_world_authoring_package`, and `apply_sandbox_world_authoring_package`, with deterministic changed files, tilemap package changed-file rows, dependency edges, selected 2D package `--require-sandbox-authoring-review` counters, focused tools/package tests, docs/manifest/static-check sync, and no external image decoding, downloads, importer plugin execution, package apply during review, renderer/RHI residency, native handles, legacy desktop middleware, SDL3, or broad editor/productized tilemap claim. Retained Phase 6 evidence includes `TileChunkRendererPlan`, `plan_tile_chunk_renderer`, `--require-production-tile-renderer`, and zero native texture ownership invocation; retained Phase 5 evidence includes `RuntimeSandboxWorldStreamingPlan`, `plan_runtime_sandbox_world_streaming`, and `execute_runtime_sandbox_world_streaming_safe_point`; retained Phase 4 evidence includes `RuntimeSandboxWorldPersistenceDocumentPlan` and `plan_runtime_sandbox_world_atomic_save`; retained Phase 3 evidence includes `RuntimeSandboxTileSimulationPlan` and `plan_runtime_sandbox_tile_simulation`. |
...
| Proposed future milestone | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Planning milestone for a clean-break, first-party, large mutable 2D sandbox production lane. Its Phase 1 runtime foundation, Phase 2 mutation execution, Phase 3 tile simulation primitives, Phase 4 persistence planning, Phase 5 streaming/residency planning, Phase 6 production tile renderer evidence, and Phase 7 sandbox authoring review evidence have landed as reviewable slices. It depends on the completed Win32 desktop host stack (`MK_platform_win32`, `MK_runtime_host_win32`, `MK_runtime_host_win32_presentation`, `MK_audio_wasapi`) and keeps generic gameplay row integration, optional network/mod gates, package-budget closeout, final lane closeout, and persistence execution beyond value-only plans as unimplemented until selected. It does not replace `currentActivePlan`, reopen `unsupportedProductionGaps`, or mark planned future phases ready. |
