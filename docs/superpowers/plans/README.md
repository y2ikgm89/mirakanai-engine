...
| Status | Plan | Notes |
| --- | --- | --- |
| Proposed focused slice (operator review) | [First-Party Editor Shell v1](2026-05-31-first-party-editor-shell-v1.md) | Clean-break plan to replace the optional Dear ImGui-backed `MK_editor` shell with a first-party retained UI/editor shell, rename the validation lane from `desktop-gui` to `desktop-editor`, and remove active UI-middleware dependency claims. It is not yet selected in `engine/agent/manifest.json.aiOperableProductionLoop.currentActivePlan`; Phase 0 performs that selection before implementation. |
| Active milestone (`currentActivePlan`) | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | No child slice is currently selected. `recommendedNextPlan.id = next-production-gap-selection`, `unsupportedProductionGaps = []`, and Native Win32 Editor Shell v1 closed through PR #322 before the next developer-owned production gap is selected explicitly. |
| Parent roadmap | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | Active production-completion index with `unsupportedProductionGaps = []` and `recommendedNextPlan.id = next-production-gap-selection` after the native editor shell milestone closed. |
...
| Parent milestone | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | Active production-completion index with `unsupportedProductionGaps = []` and `recommendedNextPlan.id = next-production-gap-selection` after Native Win32 Editor Shell v1 closed through PR #322. |
