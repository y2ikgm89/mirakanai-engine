...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [First-Party UI Editor Production Stack v1](2026-05-31-first-party-ui-editor-production-stack-v1.md) | Selected clean-break productization plan for first-party editor docking, rich text, Windows DirectWrite/TSF/UIA adapter execution, D3D12 viewport/material texture display, AI-operable editor UX expansion, Vulkan/Metal texture-display parity gates, and broader material-preview parity. Phase 8 adds exact `EditorAiOperationSnapshot.status_rows` ids for dock/rich-text/text-input/font/IME/UIA/viewport/material status plus reviewed rich-text copy commands without native handles, screen coordinates, shell execution, or validation-recipe execution. `recommendedNextPlan.id = first-party-ui-editor-production-stack-v1`; `unsupportedProductionGaps = []` remains unchanged. |
