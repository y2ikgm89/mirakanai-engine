...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Engine General Production Quality Expansion v1](2026-05-26-engine-general-production-quality-expansion-v1.md) | Selected post-1.0 production breadth plan for renderer quality, runtime UI text/font/IME/accessibility, broad reviewed import, physics/navigation breadth, networking execution/security, and audio production evidence. It keeps `unsupportedProductionGaps = []` because this is not a reopened Engine 1.0 blocker. Phase 1 records Renderer General Quality Matrix v1 through `plan_renderer_quality_matrix`, Phase 2 records Runtime UI Production Stack Evidence v1 through `plan_runtime_ui_production_stack`, and Phase 3 records Asset Import Production Review v1 through `review_asset_import_production_readiness`; none claims broad production readiness beyond the selected evidence. |
| Active phase checkpoint | [Engine General Production Quality Expansion v1 - Phase 2](2026-05-26-engine-general-production-quality-expansion-v1.md#phase-2---runtime-ui-text-font-ime-and-accessibility-production-stack) | Runtime UI Production Stack candidate adds side-effect-free selected package counters for text shaping, font rasterization, glyph atlas, renderer handoff, IME, and accessibility evidence. It keeps actual production text engines, font dependencies, native IME, OS accessibility, renderer upload execution, and broad platform UI parity host/dependency gated. |
| Active phase checkpoint | [Engine General Production Quality Expansion v1 - Phase 3](2026-05-26-engine-general-production-quality-expansion-v1.md#phase-3---broad-reviewed-asset-import-and-cook-pipeline) | Broad Reviewed Asset Import And Cook Pipeline candidate adds `MK_assets` `review_asset_import_production_readiness` as a value-only promotion gate plus descriptor-only sample manifest rows. It does not execute importers, download assets, run shader compilers, parse runtime sources, or claim broad codec/KTX2/Basis readiness. |
