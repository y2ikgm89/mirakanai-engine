...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Generated Game Studio v1](2026-05-26-generated-game-studio-v1.md) | Successor game-surface milestone that composes AI game design, generation, package review, validation preflight, evidence import, remediation, quality rubric, and editor AI command surfaces into one reviewable 2D/3D studio loop while keeping engine internals, arbitrary shell, native handles, renderer/RHI residency, Metal readiness, and broad editor productization outside the claim. |
| Planned docs/governance slice | [Engine Contract Version Suffix Cleanup](2026-05-26-engine-contract-version-suffix-cleanup.md) | Clean-breaking pre-release cleanup plan for removing removable current `vN` suffixes from engine-owned saved formats, public C++ APIs, editor ABI names, docs, schemas, manifests, and static checks. It is not selected as `currentActivePlan` until implementation starts on a branch/worktree that can be published. |
