...
| Status | Plan | Notes |
| --- | --- | --- |
| Active slice (`currentActivePlan`) | [Renderer Postprocess Tone Mapping Evidence v1](2026-05-29-renderer-postprocess-tone-mapping-evidence-v1.md) | Selected post-1.0 `renderer-postprocess-v1` follow-up. The composed manifest uses `recommendedNextPlan.id = renderer-postprocess-tone-mapping-evidence-v1` while keeping `unsupportedProductionGaps = []`, Metal host-gated, SDL3 absent, and native handles hidden. |
