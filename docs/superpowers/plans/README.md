...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Engine General Production Quality Expansion v1](2026-05-26-engine-general-production-quality-expansion-v1.md) | Selected post-1.0 production breadth plan for renderer quality, runtime UI text/font/IME/accessibility, broad reviewed import, physics/navigation breadth, networking execution/security, and audio production evidence. It keeps `unsupportedProductionGaps = []` because this is not a reopened Engine 1.0 blocker. Phases 1-4 are implemented through Renderer General Quality Matrix v1, Runtime UI Production Stack Evidence v1, Asset Import Production Review v1, and Physics/Navigation Production Breadth v1; none claims broad production readiness beyond selected evidence and host-gated rows. |
| Active phase checkpoint | [Engine General Production Quality Expansion v1 - Phase 5](2026-05-26-engine-general-production-quality-expansion-v1.md#phase-5---networking-transport-session-replication-and-security-gate) | Runtime Network Production Security Gate v1 adds `plan_runtime_network_production_security_gate` for threat-modeled value-only session, loopback, channel, delivery, replication, sequence/replay, input, snapshot, and rollback-window evidence, optional ENet loopback host evidence, and selected 2D package `network_production_security_*` counters. It does not claim encryption/authentication, matchmaking, NAT traversal, cloud backend services, internet-facing execution, rollback execution, native handles, or broad online readiness. |
