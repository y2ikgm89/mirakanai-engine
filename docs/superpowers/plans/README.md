...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [First-Party UI Editor Production Stack v1](2026-05-31-first-party-ui-editor-production-stack-v1.md) | Selected clean-break productization plan for first-party editor docking, rich text, Windows DirectWrite/TSF/UIA adapter execution, D3D12 viewport/material texture display, AI-operable editor UX expansion, cross-platform adapter boundary gates, Vulkan/Metal texture-display parity gates, and broader material-preview parity. Phase 8 adds exact `EditorAiOperationSnapshot.status_rows` ids for dock/rich-text/text-input/font/IME/UIA/viewport/material status plus reviewed rich-text copy commands without native handles, screen coordinates, shell execution, or validation-recipe execution. Phase 9 keeps macOS Core Text/InputMethodKit/NSAccessibility, Linux AT-SPI/IBus/Fcitx, Android InputMethodService/accessibility, iOS UITextInput/UIAccessibility, and HarfBuzz/FreeType/ICU-class adapters as `editor.cross_platform.adapter.*` future host/dependency gates. `recommendedNextPlan.id = first-party-ui-editor-production-stack-v1`; `unsupportedProductionGaps = []` remains unchanged. |
| Parent milestone | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | Parent production-completion index with `unsupportedProductionGaps = []`; it selected First-Party UI Editor Production Stack v1 after the First-Party Editor Shell v1 closeout. |
