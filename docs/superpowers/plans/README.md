...
| Status | Plan | Notes |
| --- | --- | --- |
| Active slice (`currentActivePlan`) | [Physics Navigation Commercial Coverage v1](2026-05-29-physics-navigation-commercial-coverage-v1.md) | Selected from the production-completion gate after `Reviewed Importers Codecs And Shader Generation v1` closeout. The slice expands physics/navigation production breadth review gates for optional Jolt/Recast/Detour-class adapter evidence while keeping `unsupportedProductionGaps = []`, native handles hidden, and broad middleware parity fail-closed. |
