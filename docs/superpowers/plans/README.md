...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [First-Party Editor Shell v1](2026-05-31-first-party-editor-shell-v1.md) | Clean-break plan selected by `recommendedNextPlan.id = first-party-editor-shell-v1` to replace the optional Dear ImGui-backed `MK_editor` shell with a first-party retained UI/editor shell, rename the validation lane from `desktop-gui` to `desktop-editor`, add AI-operable snapshot/catalog/dry-run/apply contracts, and remove active UI-middleware dependency claims without compatibility shims. |
| Parent roadmap | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | Active production-completion index with `unsupportedProductionGaps = []`; it now selects `recommendedNextPlan.id = first-party-editor-shell-v1` and `currentActivePlan = docs/superpowers/plans/2026-05-31-first-party-editor-shell-v1.md` until the first-party editor shell milestone closes. |
