...
| Status | Plan | Notes |
| --- | --- | --- |
| Active milestone (`currentActivePlan`) | [Production Completion Master Plan v1](../master-plans/2026-05-03-production-completion-master-plan-v1.md) | No child plan is active after `Generic 2D Sandbox Mutation Execution v1` closeout. The composed manifest returns to `recommendedNextPlan.id = next-production-gap-selection` while keeping `unsupportedProductionGaps = []`. |
...
| Proposed future milestone | [Generic 2D Sandbox Production Lane v1](2026-05-27-generic-2d-sandbox-production-lane-v1.md) | Planning-only future milestone for a clean-break, first-party, large mutable 2D sandbox production lane. It depends on the completed Win32 desktop host stack (`MK_platform_win32`, `MK_runtime_host_win32`, `MK_runtime_host_win32_presentation`, `MK_audio_wasapi`) and keeps planned chunk runtime, mutation execution, persistence, streaming, production tile renderer, authoring/cook, optional network/mod gates, package evidence, and closeout as unimplemented until selected. It does not replace `currentActivePlan`, reopen `unsupportedProductionGaps`, or mark planned capabilities ready. |
