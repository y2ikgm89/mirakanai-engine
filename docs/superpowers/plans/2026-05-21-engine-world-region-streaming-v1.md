...
- Phase 3 RED evidence: `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/ctest.ps1 --preset dev --output-on-failure -R sample_2d_desktop_runtime_package_smoke` first failed after adding `--require-world-region-streaming` because `sample_2d_desktop_runtime_package` did not yet accept the argument or emit `world_region_streaming_*` counters.
- Phase 3 implementation added selected package evidence to `sample_2d_desktop_runtime_package` by running reviewed `WorldRegionDesc` rows through `execute_runtime_world_region_streaming_safe_point`, loading one reviewed region package, keeping protected active regions, unloading one region, counting missing-region diagnostics, and emitting deterministic `world_region_streaming_*` status, plan/action, safe-point, budget, diagnostic, and reviewed package adoption fields.
- Phase 3 agent-surface closeout updated the selected 2D package game manifest and README, current capabilities, manifest fragments plus composed `engine/agent/manifest.json`, generated-game validation recipes/guidance, Codex/Claude/Cursor game-development skills, and static contract needles for the narrow package-visible world-region streaming claim while preserving broad/background package streaming, renderer/RHI residency, world persistence, and nav/physics partition execution as non-goals.
- Phase 3 focused evidence: `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/cmake.ps1 --build --preset dev --target sample_2d_desktop_runtime_package`, direct `out/build/dev/games/Debug/sample_2d_desktop_runtime_package/sample_2d_desktop_runtime_package.exe --package-root games/sample_2d_desktop_runtime_package/runtime --renderer null --headless --frames 3 --require-native-2d-sprites --require-procedural-generation --require-world-region-streaming`, `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/ctest.ps1 --preset dev --output-on-failure -R sample_2d_desktop_runtime_package_smoke`, `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-format.ps1`, `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-tidy.ps1 -Files games/sample_2d_desktop_runtime_package/main.cpp`, `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-json-contracts.ps1`, `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-agents.ps1`, `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-cpp-standard-policy.ps1`, `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-ai-integration.ps1`, and `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-production-readiness-audit.ps1` passed with `unsupported_gaps=0`.
- Phase 3 package evidence: `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/package-desktop-runtime.ps1 -GameTarget sample_2d_desktop_runtime_package` passed and installed validation reported `world_region_streaming_status=completed`, `world_region_streaming_ready=1`, `world_region_streaming_plan_rows=4`, `world_region_streaming_load_rows=1`, `world_region_streaming_keep_rows=2`, `world_region_streaming_unload_rows=1`, `world_region_streaming_safe_point_rows=4`, `world_region_streaming_committed=1`, `world_region_streaming_reviewed_package_adoptions=1`, `world_region_streaming_missing_region_diagnostics=1`, and `world_region_streaming_safe_point_diagnostics=0`.
- Phase 3 full gate evidence: `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1` passed; `production-readiness-audit` reported `unsupported_gaps=0`, all 69 CTest tests passed, and Apple/Metal diagnostics remained host-gated only.
