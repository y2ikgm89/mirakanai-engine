...
| Command | Result |
| --- | --- |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/cmake.ps1 --build --preset dev --target MK_runtime_ui_production_stack_tests` | RED failed as expected before implementation: missing `ime_session_begin_end_rows`, `ime_composition_update_rows`, `ime_text_area_cursor_rows`, `ime_clipboard_rows`, `ime_platform_adapter_proof_rows`, `ime_platform_host_gate_rows`, and matching diagnostics. |
