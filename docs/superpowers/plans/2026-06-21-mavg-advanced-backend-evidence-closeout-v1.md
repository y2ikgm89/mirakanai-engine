...
## 2026-06-22 Plan Quality Audit

This plan is acceptable as the current highest-rigor implementation plan for the unclaimed MAVG advanced backend cluster, but only as a fail-closed candidate plan. It is not a readiness claim. The audit resolves the user's open questions as follows:

- No overlap: the plan excludes completed conventional LOD, package graph/cook, byte-range streaming, safe-point adoption, streamed GPU upload, streamed backend draw, GPU culling, and D3D12/Vulkan compute-generated indirect rows; those remain retained evidence, not duplicated scope.
- No ambiguous unknowns: missing hardware, missing SDK/toolchain support, stale source gates, missing profiler artifacts, incomplete vendor/backend coverage, and unresolved Nanite legal/benchmark wording are explicit host-gated, blocked, or false rows.
- Official-recommended implementation path: every backend task starts from official feature/capability queries, enables required device features before command use, keeps native handles private, proves execution with deterministic readback/hash or reviewed host artifacts, and keeps fallback rows from promoting advanced readiness.
- Nanite wording is intentionally non-promotional: this plan can produce a public-doc comparison taxonomy, but `mavg_nanite_compatible`, `mavg_nanite_equivalent`, and `mavg_nanite_superior` stay false.
- Performance and broad optimization require measured evidence, not design intent: timing-window overlap, single-vendor traces, or synthetic-only package runs cannot satisfy the measured async-overlap or broad CPU/GPU/memory rows.
...
- `mavg_deformation_integration_ready=1` only after the later selected MAVG Deformation Backend Execution Evidence v1 bridge closeout; this remains a narrow runtime scene RHI compute morph/skinned bridge claim, not native D3D12 command execution, native Vulkan queue execution, Metal readiness, ray tracing, mesh shader execution, or broad MAVG backend readiness.
...
**2026-06-22 readback execution slice evidence:** `VulkanRuntimeMeshGraphicsPipeline` and `VulkanRuntimeMeshGraphicsPipelineDesc` are backend-private owners for Vulkan mesh/task/fragment graphics pipelines; `create_runtime_mesh_graphics_pipeline` builds a dynamic-rendering pipeline with `VK_SHADER_STAGE_TASK_BIT_EXT`, `VK_SHADER_STAGE_MESH_BIT_EXT`, and fragment stage SPIR-V, `pVertexInputState = nullptr`, `pInputAssemblyState = nullptr`, and no public native handles. `record_runtime_texture_rendering_mesh_tasks_draw` records direct `vkCmdDrawMeshTasksEXT` through the resolved device command, then `execute_vulkan_mavg_mesh_shader_lod` renders a 64x64 `rgba8_unorm` target, uses synchronization2 texture barriers for render-target and transfer-readback states, copies to a readback buffer, hashes bytes, and sets `mavg_mesh_shader_lod_vulkan_ready=true` only when the direct draw executes, readback is nonzero, no vertex/index fallback is used, and host/device feature gates pass. Test shader sources compile with DXC SPIR-V CodeGen using `-spirv -fspv-target-env=vulkan1.3 -fspv-extension=SPV_EXT_mesh_shader` for `as_6_5`, `ms_6_5`, and `ps_6_0`, and the generated artifacts pass `spirv-val --target-env vulkan1.3`. Focused evidence on this host: `MK_mavg_vulkan_mesh_shader_lod_tests` passes with `MK_VULKAN_TEST_MAVG_MESH_SHADER_LOD_TASK_SPV`, `MK_VULKAN_TEST_MAVG_MESH_SHADER_LOD_MESH_SPV`, and `MK_VULKAN_TEST_MAVG_MESH_SHADER_LOD_FRAGMENT_SPV`. This slice still keeps `vkCmdDrawMeshTasksIndirectEXT`, `vkCmdDrawMeshTasksIndirectCountEXT`, Metal readiness, Nanite compatibility/equivalence/superiority, package-visible broad `mavg_mesh_shader_lod_ready`, broad MAVG backend readiness, and broad optimization unclaimed.

- [x] Add backend-private API `probe_vulkan_mavg_mesh_shader_lod_capability()` and `execute_vulkan_mavg_mesh_shader_lod(...)`; do not expose native Vulkan handles through public RHI or gameplay APIs.
- [x] Capability probe must record `loader_available`, `instance_created`, `physical_device_selected`, `device_extension_supported`, `feature_query_executed`, `mesh_shader_supported`, `task_shader_supported`, `mesh_shader_queries_supported`, `mesh_shader_enabled`, `task_shader_enabled`, `draw_indirect_count_supported`, `draw_indirect_count_enabled`, `draw_mesh_tasks_direct_command_available`, `draw_mesh_tasks_indirect_command_available`, `draw_mesh_tasks_indirect_count_command_available`, `max_task_work_group_count_x`, `max_task_work_group_count_y`, `max_task_work_group_count_z`, `max_task_work_group_total_count`, `max_mesh_work_group_count_x`, `max_mesh_work_group_count_y`, `max_mesh_work_group_count_z`, `max_mesh_work_group_total_count`, `max_mesh_output_vertices`, `max_mesh_output_primitives`, adapter/vendor/device ids, and diagnostic text.
- [x] Readiness requires `VK_EXT_mesh_shader`, `VkPhysicalDeviceMeshShaderFeaturesEXT::meshShader == VK_TRUE`, a logical-device feature chain that enables `meshShader`, and queried `VkPhysicalDeviceMeshShaderPropertiesEXT` limits that cover the selected first-party meshlet workload. If `taskShader == VK_FALSE` or is not enabled, direct mesh-only execution can be tested, but task-shader LOD amplification remains `host_gated`.
- [x] Implement `vkCmdDrawMeshTasksEXT` execution for a selected first-party meshlet package row and require deterministic readback color/hash evidence before `mavg_mesh_shader_lod_vulkan_ready=true`.
- [x] Prove the execution row uses `VK_SHADER_STAGE_MESH_BIT_EXT` and, when task amplification is enabled, `VK_SHADER_STAGE_TASK_BIT_EXT`; prove no vertex input layout, no index buffer binding, and no conventional indexed indirect draw promoted the mesh-shader row.
**2026-06-23 follow-up completion note:** the three indirect/synchronization subclaims below were intentionally outside Task 4 direct-dispatch closeout and are now completed by [MAVG Vulkan Mesh Shader Indirect Dispatch v1](2026-06-23-mavg-vulkan-mesh-shader-indirect-dispatch-v1.md). That follow-up adds backend-private `vkCmdDrawMeshTasksIndirectEXT` and `vkCmdDrawMeshTasksIndirectCountEXT` execution, real `VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT` argument/count buffers, 4-byte offset and stride validation, exact `VkDrawMeshTasksIndirectCommandEXT` range checks, `drawIndirectCount` query/enablement, resolved `vkCmdDrawMeshTasksIndirectCountEXT`, actual count evidence, synchronization2 `VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT`, `VK_PIPELINE_STAGE_2_TASK_SHADER_BIT_EXT`, and `VK_PIPELINE_STAGE_2_MESH_SHADER_BIT_EXT` barriers, and descriptor-backed task/mesh payload consumption. The follow-up keeps package-visible broad `mavg_mesh_shader_lod_ready`, Metal readiness, Nanite compatibility/equivalence/superiority, deformation, ray tracing, autonomous scheduler breadth, broad MAVG backend readiness, and broad optimization unclaimed.
- [x] Add tests named `vulkan_mavg_mesh_shader_lod_rejects_empty_task_rows`, `vulkan_mavg_mesh_shader_lod_probe_records_mesh_shader_feature_state`, `vulkan_mavg_mesh_shader_lod_host_gates_without_mesh_shader_support`, `vulkan_mavg_mesh_shader_lod_rejects_invalid_workgroup_counts`, `vulkan_mavg_mesh_shader_lod_rejects_indirect_range_overflow`, `vulkan_mavg_mesh_shader_lod_does_not_promote_fallback_indexed_draw`, and `vulkan_mavg_mesh_shader_lod_executes_mesh_shader_when_host_supports_it`.
- [x] Test shader binaries are optional host evidence inputs: when the environment does not provide compiled SPIR-V paths, the execution test must assert a concrete skip/host-gated diagnostic rather than silently passing readiness. Shader source stays under `tests/shaders/` to match the repository's existing unit-test shader convention.
- [x] Run:
...
Final readback-slice validation evidence on 2026-06-22: DXC from Vulkan SDK 1.4.341.1 compiled `tests/shaders/vulkan_mavg_mesh_shader_lod.task.hlsl`, `.mesh.hlsl`, and `.frag.hlsl` to SPIR-V with `-spirv -fspv-target-env=vulkan1.3 -fspv-extension=SPV_EXT_mesh_shader`; `spirv-val --target-env vulkan1.3` passed for all three artifacts; `MK_mavg_vulkan_mesh_shader_lod_tests` passed with the three `MK_VULKAN_TEST_MAVG_MESH_SHADER_LOD_*_SPV` environment variables; focused `MK_backend_scaffold_tests|MK_mavg_vulkan_mesh_shader_lod_tests` passed; `tools/check-toolchain.ps1`, `tools/check-agents.ps1`, `tools/check-json-contracts.ps1`, `tools/check-public-api-boundaries.ps1`, `tools/check-ai-integration.ps1`, `tools/check-format.ps1`, and `git diff --check` passed; full `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1` passed with 142/142 CTest tests. `check-shader-toolchain.ps1` reports D3D12 DXIL and Vulkan SPIR-V ready while Metal toolchain remains missing/host-gated on this Windows host, so this evidence does not promote Metal readiness.
...
**2026-07-01 deformation backend execution bridge evidence:** `mavg-deformation-backend-execution-evidence-v1` supersedes the policy-only `mavg-deformation-integration-v1` state by adding `MavgDeformationBackendExecutionEvidenceDesc`, `MavgDeformationBackendExecutionEvidenceResult`, and `evaluate_mavg_deformation_backend_execution_evidence` beside `mavg_deformation_integration.hpp`, `MavgDeformationIntegrationDesc`, `MavgDeformationClusterBoundsRow`, `MavgDeformationBackendExecutionRow`, `MavgDeformationIntegrationResult`, and `plan_mavg_deformation_integrated_clusters`. The gate accepts selected rigid transform, linear blend skinning, and morph target policy rows only when stable cluster ids, conservative deformed AABBs, material partition roots, resident pages, and fallback draw ranges are preserved, and then accepts reviewed runtime scene RHI compute morph/skinned D3D12/Vulkan bridge rows only with upload execution, morph payload bytes, output vertex bytes, submitted upload fences, and renderer-consumption review. Retained validation through `tools/validate-mavg-deformation-integration.ps1 -RequireReady` now emits `mavg_deformation_policy_ready=1`, `mavg_deformation_integration_status=ready`, `mavg_deformation_integration_ready=1`, and `mavg_deformation_backend_execution_bridge_ready=1`. It also emits `mavg_deformation_native_d3d12_command_execution=0`, `mavg_deformation_native_vulkan_queue_execution=0`, `mavg_deformation_metal_readiness=0`, and `mavg_deformation_broad_backend_readiness=0`, so this slice does not promote ray tracing, mesh shader execution, Metal readiness, Nanite compatibility/equivalence/superiority, broad MAVG backend readiness, broad deformation readiness, or broad CPU/GPU/memory optimization.
...
### Task 9B: Apple-Host Object/Mesh Execution Artifact Contract

**2026-06-23 retained artifact contract:** [MAVG Metal Mesh LOD Apple Host Execution Evidence v1](2026-06-23-mavg-metal-mesh-lod-apple-host-execution-v1.md) adds `GameEngine.MavgMetalMeshLodHostExecutionEvidence.v1`, `schemas/mavg-metal-mesh-lod-host-execution.schema.json`, a ready retained fixture, and `tools/check-mavg-metal-host-evidence.ps1 -ArtifactRootRelative` parsing for future Apple-host retained object/mesh shader execution artifacts. The ready fixture proves local `mavg_metal_mesh_lod_status=ready` only when retained rows include macOS/full Xcode, `metal`/`metallib`, Apple Metal Feature Set Tables source id, Apple GPU family, object and mesh shader support, `drawMeshThreads`, object payload, mesh payload, deterministic readback hash, and package-visible output. Default repository validation remains `mavg_metal_mesh_lod_status=host_evidence_required` and `mavg_metal_mesh_lod_ready=0` because no real Apple-host artifact is committed under `artifacts/mavg/metal-mesh-lod-host-execution`. The contract still emits `mavg_mesh_shader_lod_ready=0`, `mavg_metal_ray_tracing_ready=0`, Nanite counters 0, native handles 0, broad MAVG backend readiness 0, and broad optimization 0.
