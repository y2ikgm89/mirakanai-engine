...
**2026-06-23 follow-up tracking note:** the three indirect/synchronization subclaims below are intentionally not completed by Task 4 direct-dispatch closeout. They are now tracked by [MAVG Vulkan Mesh Shader Indirect Dispatch v1](2026-06-23-mavg-vulkan-mesh-shader-indirect-dispatch-v1.md) and must remain unclaimed until that plan lands with validation evidence.

- Future `vkCmdDrawMeshTasksIndirectEXT` evidence requires a command buffer using a buffer created with `VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT`, offset 4-byte alignment, `stride >= sizeof(VkDrawMeshTasksIndirectCommandEXT)`, 4-byte stride alignment, `offset + stride * (drawCount - 1) + sizeof(VkDrawMeshTasksIndirectCommandEXT) <= buffer_size_bytes`, and draw counts within the queried per-axis and total mesh/task workgroup limits.
- Future `vkCmdDrawMeshTasksIndirectCountEXT` evidence requires Vulkan 1.2, `VK_KHR_draw_indirect_count`, or `VK_AMD_draw_indirect_count` support, enabled `drawIndirectCount`, `vkCmdDrawMeshTasksIndirectCountEXT` resolution from `vkGetDeviceProcAddr`, indirect-buffer usage for both parameter and count buffers, 4-byte aligned offsets, `maxDrawCount` bounded by the test workload, and actual count evidence. Otherwise only `mavg_mesh_shader_lod_vulkan_indirect_count_host_gated=true` may be emitted.
- Future synchronization2 evidence requires barriers for compute/page upload to mesh shader consumption and readback evidence. It must use `VK_PIPELINE_STAGE_2_TASK_SHADER_BIT_EXT` and `VK_PIPELINE_STAGE_2_MESH_SHADER_BIT_EXT` when those stages consume data; missing or fallback-only barriers are correctness failures.
