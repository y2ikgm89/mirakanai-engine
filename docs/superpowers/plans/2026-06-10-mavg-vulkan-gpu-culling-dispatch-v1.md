...
**Execution State:** Draft plan-only slice over merged `origin/main` after MAVG D3D12 Compute-Generated Indirect Consumption v1 activation PR #559; implementation PR #560 remains the active D3D12 compute-to-draw lane until it merges. This draft does not activate the manifest. Today only D3D12 exposes `dispatch_mavg_gpu_culling_indirect` in `engine/rhi/d3d12/src/d3d12_mavg_gpu_culling_dispatch.cpp`; no Vulkan dispatch module, no `MK_mavg_vulkan_gpu_culling_dispatch_tests` target, and Vulkan compute-generated indirect consumption remain unclaimed. This child is the symmetric Vulkan Phase 4 follow-up to completed D3D12 dispatch (`mavg-gpu-culling-dispatch-v1`, PR #556 / closeout PR #557) because completed `MavgGpuCullingSyncRequirement` Vulkan rows already record `VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT` to `VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT` synchronization without executing dispatch.
