...
**Execution State:** Active activation slice over merged `origin/main` after completed MAVG D3D12 Compute-Generated Indirect Consumption v1 through PR #560 and plan draft PR #561. Manifest activation points `currentActivePlan` and `recommendedNextPlan.id = mavg-vulkan-gpu-culling-dispatch-v1` without landing implementation in the activation PR. Today only D3D12 exposes `dispatch_mavg_gpu_culling_indirect` in `engine/rhi/d3d12/src/d3d12_mavg_gpu_culling_dispatch.cpp`; Vulkan `draw_indexed_indirect` still requires `BufferUsage::copy_source` upload usage in `engine/rhi/vulkan/src/vulkan_backend.cpp` v1, no Vulkan dispatch module, no `MK_mavg_vulkan_gpu_culling_dispatch_tests` target, and Vulkan compute-generated indirect consumption remain fail-closed until the independent implementation PR lands. This child is the symmetric Vulkan Phase 4 follow-up to completed D3D12 dispatch (`mavg-gpu-culling-dispatch-v1`, PR #556 / closeout PR #557) because completed `MavgGpuCullingSyncRequirement` Vulkan rows already record `VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT` to `VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT` synchronization without executing dispatch.
...
- [ ] Confirm Vulkan compute-to-indirect synchronization constraints through Khronos spec and retain completed GPU-culling planner plus D3D12 dispatch audit references.
- [ ] Run a read-only rendering subagent audit and split Vulkan compute dispatch from Vulkan compute-generated indirect consumption work.
- [ ] Add RED SPIR-V environment-gated dispatch/readback tests for visible and culled MAVG cluster cases.
- [ ] Implement backend-private Vulkan compute dispatch writing packed argument/count buffers with required `VK_KHR_synchronization2` barriers.
