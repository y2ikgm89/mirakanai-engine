...
**Execution State:** Active activation slice over merged `origin/main` after completed MAVG D3D12 Compute-Generated Indirect Consumption v1 through PR #560 and plan draft PR #561. Manifest activation points `currentActivePlan` and `recommendedNextPlan.id = mavg-vulkan-gpu-culling-dispatch-v1` without closeout until PR #564. Local implementation in this branch lands `engine/rhi/vulkan/src/vulkan_mavg_gpu_culling_dispatch.cpp` exposing `dispatch_mavg_gpu_culling_indirect` with `VK_KHR_synchronization2` buffer_memory_barrier2 rows matching completed `MavgGpuCullingSyncRequirement` Vulkan sync requirements, SPIR-V environment-gated `MK_mavg_vulkan_gpu_culling_dispatch_tests` (`MK_VULKAN_TEST_MAVG_GPU_CULLING_DISPATCH_SPV`), and deterministic packed argument/count readback for visible/culled clusters while preserving completed D3D12 dispatch and CPU-upload Vulkan indirect draw paths; Vulkan `draw_indexed_indirect` still requires `BufferUsage::copy_source` upload usage in `engine/rhi/vulkan/src/vulkan_backend.cpp` v1. Vulkan compute-generated indirect consumption, manifest closeout/selection gate, mesh shaders, Metal readiness, Nanite equivalence/superiority, and broad optimization remain unclaimed follow-up work. This child is the symmetric Vulkan Phase 4 follow-up to completed D3D12 dispatch (`mavg-gpu-culling-dispatch-v1`, PR #556 / closeout PR #557) because completed `MavgGpuCullingSyncRequirement` Vulkan rows already record `VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT` to `VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT` synchronization.
...
- [x] Synchronize docs, plan registry, architecture spec, `004-modules.json` evidence, and static checks (`check-ai-integration-113`); keep `010-aiOperableProductionLoop.json` activation/closeout for closeout PR #564.
