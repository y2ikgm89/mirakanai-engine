...
4. **`frame-graph-v1`** — active foundation gap; [Frame Graph Shadow Smoke Execution Migration v1](2026-05-16-frame-graph-shadow-smoke-execution-migration-v1.md) is complete after migrating the directional shadow smoke renderer to ordered Frame Graph RHI texture execution after the postprocess renderer executor slice landed. The next child should pick one coherent slice from production pass ownership, transient allocation/aliasing, multi-queue scheduling, package-visible graph evidence, or renderer-wide manual-transition removal.
