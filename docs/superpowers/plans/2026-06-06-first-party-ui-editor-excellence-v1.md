...
- Full TSF `ITextStoreACP` callback coverage, native IME candidate UI, reconversion, rich clipboard, platform shortcut localization, and grapheme-cluster editing.
...
- [ ] Add RED tests for first-party text edit state over grapheme-aware cursor movement, selection, composition range, committed text, candidate selection, reconversion request, surrounding text, and caret rectangle rows.
- [ ] Add Windows RED tests for TSF `ITextStoreACP` callback coverage, document locks, selection updates, text replacement, candidate UI host ownership, and reconversion diagnostics.
- [ ] Add macOS host-gated rows for `NSTextInputClient` and InputMethodKit.
- [ ] Add Linux host-gated rows for IBus and Fcitx integration.
- [ ] Add Android/iOS host-gated rows for selected platform text input and accessibility services only when mobile editor/runtime UI work is explicitly selected.
- [ ] Add counters:
