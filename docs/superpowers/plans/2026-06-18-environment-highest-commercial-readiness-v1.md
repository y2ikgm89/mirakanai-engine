...
- [ ] Add explicit validation-log capture through a Vulkan debug-utils messenger or equivalent first-party log sink, then allow `linux_vulkan_validation_log_clean=1` only when the sink records zero validation diagnostics.
...
Do not merge Task 25 until its focused local validation, publication preflight, and task-owned PR checks pass for the task-owned PR head SHA. The final commercial row remains unready until Linux Vulkan clean validation-log capture, Android Vulkan, strict Vulkan aggregate, platform readiness, and all-platform readiness close with exact host evidence.
