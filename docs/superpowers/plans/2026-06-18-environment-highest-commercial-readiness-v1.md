...
- [x] Add `-DeviceSerial`, `-StartEmulator`, `-AvdName`, `-EmulatorPort`, `-BootTimeoutSeconds`, `-ConfigureGpuDebugLayers`, and `-GpuDebugLayerPackage` options.
...
**Task 27 evidence (2026-06-20):** Static RED first failed `tools/check-json-contracts.ps1` because the Android validator lacked `StartEmulator`. The GREEN path adds explicit serial targeting, official emulator startup for `GameEngine_API36`, guarded emulator cleanup, optional official AGI GPU debug layer setting writes, and serial-aware package smoke routing. Local fail-closed validation reports SDK/NDK/profile/debuggable ready but no device as `environment_platform_android_vulkan_ready=0`; emulator validation reports `adb_device_or_emulator_ready=1` but `android_gpu_debug_layer_app_installed=0`, `android_gpu_debug_layer_settings_ready=0`, package/readback/log counters 0, and Android readiness still 0.
