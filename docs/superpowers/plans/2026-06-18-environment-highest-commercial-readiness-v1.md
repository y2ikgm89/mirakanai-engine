...
- [x] Update `validate.yml` `linux-vulkan` to bootstrap vcpkg to the pinned `builtin-baseline`, install Vulkan/Xvfb diagnostic packages, run the ready validator under `xvfb-run`, save the vcpkg tool cache, and upload evidence.
...
- [ ] Publish the task-owned branch and merge only after the PR head SHA proves hosted checks and PR Gate pass; current local blocker is missing GitHub `workflow` token scope for `.github/workflows/validate.yml`.

**Task 28 evidence (2026-06-20):** Static RED first failed `tools/check-ci-matrix.ps1` because `validate.yml linux-vulkan` still used a 20-minute report-only gate and did not pass `-RequireReady`. The GREEN path raises the timeout, restores/clones/updates vcpkg to the pinned baseline, bootstraps the Linux vcpkg executable without manifest auto-install, installs `xvfb`, `mesa-vulkan-drivers`, `spirv-tools`, `vulkan-tools`, and `vulkan-validationlayers`, runs the Linux validator under `xvfb-run -a` with expected package/readback/clean-log counters, and promotes only the Linux Vulkan platform row. Android Vulkan remains host-gated, so strict Vulkan aggregate, platform readiness, all-platform readiness, commercial readiness, and broad `environment_ready` remain unclaimed.
