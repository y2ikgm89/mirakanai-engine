...
**Status:** Active. Tasks 1-30 select this highest-level plan as `currentActivePlan`, keep the Context7/source gate ready, replace skeleton validators with exact package/host evidence, close backend parity v2, promote the AAA preset asset library, full OpenEXR/KTX2/Basis asset pipeline, physical weather simulation, production artist workflow, broad optimization, Windows D3D12, Windows Vulkan, macOS Metal, iOS Metal, and Linux Vulkan rows, and keep the final aggregate fail-closed. Task 28 promotes only the Linux Vulkan platform row through hosted `validate.yml linux-vulkan` evidence that bootstraps the pinned vcpkg executable, installs Vulkan/Xvfb diagnostics, runs `tools/validate-linux-vulkan-runtime-host.ps1 -RequireReady` under `xvfb-run`, and requires package smoke, readback, clean validation-log, `environment_platform_linux_vulkan_ready=1`, and `environment_platform_requires_linux_vulkan_host_evidence=0`. Task 29 tightens the C++ Android platform-readiness evaluator so it requires the same official AGI layer app install, APK install readiness, validation-layer enumeration, and clean validation-log rows already required by the Android host validator before Android Vulkan can be ready. Task 30 adds `android_gpu_debug_layer_ready=1` as the Android validator aggregate over official AGI GPU debug-layer settings, installed AGI layer app evidence, and host-supplied APK install readiness before package smoke can run. Current evidence has 12 ready rows, 4 host-gated rows, 0 unsupported rows, 21 retained optimization workload/backend rows, and 0 missing optimization artifacts. Android Vulkan, strict Vulkan aggregate, platform readiness, unconditional all-platform readiness, commercial readiness, and broad `environment_ready` remain unclaimed.
...
- [ ] Merge only after the task-owned PR head SHA proves the hosted `linux-vulkan` package smoke and PR Gate pass.
...
- [ ] Keep Android Vulkan, strict Vulkan aggregate, platform readiness, all-platform readiness, commercial readiness, and broad `environment_ready` unclaimed until approved Android package/device smoke emits every required counter.
...
Do not merge Task 28 until its focused local validation, publication preflight, and task-owned PR checks pass for the task-owned PR head SHA. Do not merge Tasks 29-30 until focused platform-evidence tests, JSON/static contracts, validation-recipe runner checks, AI integration checks, Android fail-closed validation, aggregate readiness validation, full validation, publication preflight, and task-owned PR checks pass for the task-owned PR head SHA. The final commercial row remains unready until approved Android Vulkan package/device smoke, strict Vulkan aggregate, platform readiness, and all-platform readiness close with exact host evidence.
