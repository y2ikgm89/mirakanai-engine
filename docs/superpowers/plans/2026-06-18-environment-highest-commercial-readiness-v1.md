...
- [x] Update `validate.yml` `linux-vulkan` to bootstrap vcpkg to the pinned `builtin-baseline`, install the official LunarG Ubuntu `vulkan-sdk` plus Xvfb/Mesa/XCB runtime prerequisites, run the ready validator under `xvfb-run`, save the vcpkg tool cache, and upload evidence.
