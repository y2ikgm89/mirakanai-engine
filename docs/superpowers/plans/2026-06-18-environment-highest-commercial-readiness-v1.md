...
## Task 14: Add Fail-Closed Linux Vulkan Hosted Diagnostics
...
**Goal:** Make Linux Vulkan SDK, validation-layer, DXC SPIR-V, SPIR-V validation, and Lavapipe ICD evidence visible in hosted CI while keeping `environment_platform_linux_vulkan_ready=0` until the first-party Linux runtime host, package script, installed validator, package smoke, and readback evidence exist.

**Steps:**
- [x] Add `linux-vulkan` to `.github/workflows/validate.yml` on `ubuntu-24.04`, required by `pr-gate` when Linux validation is selected.
- [x] Install the official LunarG Ubuntu SDK repository and `vulkan-sdk` package, plus `mesa-vulkan-drivers` and `libvulkan1` for hosted Lavapipe ICD diagnostics.
- [x] Require `vulkaninfo --summary`, `dxc`, and `spirv-val` before running `tools/validate-linux-vulkan-runtime-host.ps1`.
- [x] Make `tools/validate-linux-vulkan-runtime-host.ps1` fail if any expected counter is absent from the actual counter line.
- [x] Expand `tools/check-shader-toolchain.ps1` to search Linux/macOS vcpkg triplets and Linux `VULKAN_SDK/bin` paths.
- [x] Keep expected hosted counters fail-closed: Linux SDK/tool/ICD counters may be `1`, but `first_party_linux_runtime_host_ready=0`, `linux_package_script_ready=0`, `linux_installed_validator_ready=0`, and `environment_platform_linux_vulkan_ready=0` remain required until the missing implementation lands.
...
## Task 16: Add Android Vulkan Offscreen Readback And Official GPU Debug Layer Contract

**Goal:** Replace the old packaged-validation-layer assumption with the official Android AGI/GPU debug layer settings contract, add app-written offscreen Vulkan readback counters, and keep `environment_platform_android_vulkan_ready=0` until APK build, device/emulator launch, validation-layer settings, and logcat readback evidence can be executed on an approved Android host.

**Steps:**
- [x] Replace `android_validation_layer_packaged` with clean-break `android_gpu_debuggable_ready` and `android_gpu_debug_layer_settings_ready` in the platform evidence v2 row model.
- [x] Require `enable_gpu_debug_layers`, `gpu_debug_app`, `gpu_debug_layer_app`, `gpu_debug_layers=VK_LAYER_KHRONOS_validation`, and installed AGI layer APK evidence in `tools/validate-android-vulkan-runtime-host.ps1`.
- [x] Add a private Android GameActivity Vulkan smoke that uses `vkCmdFillBuffer`, a host-read pipeline barrier, fence wait, memory mapping, and logcat counters without using presentation surfaces or native handle exposure.
- [x] Make `tools/smoke-android-package.ps1` fail closed unless current-launch logcat contains `android_vulkan_readback_ready=1`, `android_vulkan_validation_layer_enumerated=1`, and no `VALIDATION:W` or higher output.
- [x] Sync validation recipes, manifest fragments, game manifest, static checks, docs, and plan registry while keeping commercial readiness fail-closed.

**Task 16 evidence (2026-06-19):** Context7 was queried for Android Developers (`/websites/developer_android`) and Khronos Vulkan (`/khronosgroup/vulkan-docs`). The Android docs require a debuggable app, AGI GPU debug layer settings (`enable_gpu_debug_layers`, `gpu_debug_app`, `gpu_debug_layer_app`, and `gpu_debug_layers=VK_LAYER_KHRONOS_validation`), validation layers enabled, and no validation warnings or errors before profiling. The Vulkan docs support host-visible readback after GPU writes, synchronization, fence wait, mapping, non-coherent invalidation, and `vkDeviceWaitIdle`/host waits before freeing memory that may still be used by submitted queue work. TDD RED: `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/cmake.ps1 --build --preset dev --target MK_env_platform_v2_tests` failed because `android_gpu_debug_layer_settings_ready` was not yet in `EnvironmentPlatformEvidenceV2Row`. The reviewed fix now requires installed AGI layer APK evidence via `pm path`, same-launch `vkEnumerateInstanceLayerProperties` evidence for `VK_LAYER_KHRONOS_validation`, clean `VALIDATION:W` or higher logcat output, current-launch logcat clearing, and `vkDeviceWaitIdle` cleanup for submitted work when fence wait fails. GREEN/focused evidence so far: `MK_runtime_rhi_environment_platform_evidence_v2_tests` passes; NDK `aarch64-linux-android26-clang++ -fsyntax-only` for `android_vulkan_readback_smoke.cpp` passes; `tools/validate-android-vulkan-runtime-host.ps1` emits fail-closed local counters with `android_gpu_debuggable_ready=1`, `android_gpu_debug_layer_settings_ready=0`, `android_gpu_debug_layer_app_installed=0`, `android_vulkan_readback_ready=0`, `android_vulkan_validation_layer_enumerated=0`, `android_vulkan_validation_log_clean=0`, and `environment_platform_android_vulkan_ready=0`; `tools/run-validation-recipe.ps1 -Mode DryRun -Recipe environment-platform-android-vulkan-package` now expects the clean-break GPU debug, installed-layer, validation-layer, clean-log, and readback counters; `tools/check-validation-recipe-runner.ps1`, `tools/check-json-contracts.ps1`, `tools/check-format.ps1`, `tools/check-text-format.ps1`, `tools/check-ai-integration.ps1`, `git diff --check`, and `tools/validate-environment-highest-commercial-readiness.ps1` pass. The highest-commercial closeout remains fail-closed with `environment_commercial_ready_rows=11`, `environment_host_gated_rows=4`, `environment_platform_android_vulkan_ready=0`, `environment_broad_optimization_ready=0`, and `environment_commercial_ready=0`. Android APK build/smoke was not executed in this session because `tools/build-mobile-android.ps1 -Game sample_headless -Configuration Debug` was blocked by the active Codex command policy requiring approval while approval policy is `never`; the Android ready row must remain host-gated until an approved Android package smoke produces `android_package_smoke_ready=1`, `android_vulkan_readback_ready=1`, `android_vulkan_validation_layer_enumerated=1`, `android_vulkan_validation_log_clean=1`, and `VK_LAYER_KHRONOS_validation_ready=1`.
...
16. Task 16 Android Vulkan offscreen readback and official GPU debug layer contract.
...
| 2026-06-19 | Task 15 Linux Vulkan CI repair | `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-cpp-standard-policy.ps1`; `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-format.ps1`; `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-ci-matrix.ps1`; `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1` | pass | `linux-vulkan-runtime-release` now disables CMake C++ module scanning/import-std for the hosted Linux Vulkan package compiler path; `validate: ok`; `100% tests passed, 0 tests failed out of 131`; hosted `Linux Vulkan Evidence` must rerun on PR head after this repair |
| 2026-06-19 | Task 15 Linux Vulkan package test build repair | `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-format.ps1`; `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-json-contracts.ps1`; `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-ci-matrix.ps1`; `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1` | pass | `tools/package-linux-runtime.ps1` now builds `MK_vulkan_environment_weather_solver_tests` before the filtered CTest lane; `validate: ok`; `100% tests passed, 0 tests failed out of 131`; hosted `Linux Vulkan Evidence` must rerun on PR head after this repair |
