...
**Official guidance consulted:** Context7 `/kitware/cmake` confirms preset-backed `cmake --build --preset`, `--parallel`, CTest parallel execution, Ninja/Visual Studio C++ module generator support, and target/source-level `CXX_SCAN_FOR_MODULES`; Context7 `/actions/cache` confirms split restore/save actions, primary-key reuse, restore keys, and explicit cache path selection; Microsoft Learn documents MSBuild `-maxcpucount` parallel project builds and MSVC `/Zf` faster PDB generation with `/MP` or parallel `cl.exe`.
...
- Use automatic CMake/CTest parallelism in all `tools/evaluate-cpp23.ps1` lanes, with `-Jobs <N>` available only as an explicit local throttle.
...
- `tools/check-ci-matrix.ps1` proves the workflow no longer caches/logs `out/build/cpp23-eval` in the Release job and that `evaluate-cpp23.ps1` exposes the explicit Debug lane plus automatic CMake/CTest parallelism.
- Agent-facing CMake guidance and manifest command text describe `[-Debug|-Release|-Gui] [-Jobs <N>]`.
...
| --- | --- | --- |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-ci-matrix.ps1` | PASS | Static CI contract guard, including Release-only behavior and automatic CMake/CTest parallelism. |
...
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1` | PASS | Final full local gate passed after automatic CMake/CTest parallelism sync; `production-readiness-audit: unsupported_gaps=0`, 66/66 tests passed. |
