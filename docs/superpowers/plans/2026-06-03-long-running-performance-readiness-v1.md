...
- [x] Add `host-cpu-profiling-matrix` as a manifest validation recipe descriptor for external official-profiler artifacts, without adding default validation or runner execution; follow-up evidence collection/import now uses `tools/collect-cpu-profiling-host-evidence.ps1 -Mode Import -EvidenceRoot <artifact-root> ...`, and evidence checking uses `tools/check-cpu-profiling-host-evidence.ps1 -EvidenceRoot <artifact-root> [-RequireReady]` so incomplete bundles stay host-gated.
