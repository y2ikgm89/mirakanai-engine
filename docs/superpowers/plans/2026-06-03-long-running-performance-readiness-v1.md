...
- [x] Add `host-cpu-profiling-matrix` as a manifest validation recipe descriptor for external official-profiler artifacts, without adding default validation or runner execution; follow-up evidence checking now uses `tools/check-cpu-profiling-host-evidence.ps1 -EvidenceRoot <artifact-root> [-RequireReady]` so incomplete bundles stay host-gated.
