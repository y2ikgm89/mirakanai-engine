...
**Execution State:** Parent milestone in the stacked MAVG implementation branch after explicit LoD supersession. `mavg-asset-graph-v1` remains the prerequisite foundation branch; `mavg-gpu-culling-indirect-v1`, `mavg-rhi-indirect-draw-v1`, `mavg-d3d12-indexed-indirect-draw-execution-v1`, `mavg-d3d12-indexed-indirect-count-buffer-execution-v1`, `mavg-d3d12-compute-generated-indirect-execution-v1`, `mavg-d3d12-gpu-culling-execution-v1`, `mavg-package-streaming-residency-dispatch-v1`, `mavg-page-addressable-payload-schema-v1`, `mavg-payload-byte-range-file-io-v1`, `mavg-directstorage-request-plan-v1`, and `mavg-native-directstorage-win32-async-io-dispatch-status-v1` are completed stacked children. `mavg-win32-async-file-io-adapter-v1` is the active child for a concrete caller-polled DirectStorage/Win32 boundary step: Win32 overlapped file-read adapter support through `RuntimeMavgPayloadDirectStorageRequestRow::source_file_path`, `destination_memory`, `Win32MavgPayloadAsyncFileIoDispatcherDesc`, `Win32MavgPayloadAsyncFileIoDispatcher`, `CreateFileW`, `ReadFile`, `OVERLAPPED`, `GetOverlappedResult`, and `CancelIoEx` over successful request-plan rows. This milestone remains the conventional static LoD evidence record and must not claim `dstorage.h` compilation, DirectStorage SDK/native execution, concrete `IDStorageFactory` / `IDStorageQueue` / `IDStorageStatusArray` / `ID3D12Fence`, IOCP worker execution, autonomous background workers, async-overlap/performance, broad GPU culling frameworks, Vulkan/Metal backend indirect execution, or Nanite equivalence/superiority.
...
- `mavg-win32-async-file-io-adapter-v1`: active child adding source-file paths, caller-owned destination memory, and a concrete private Win32 overlapped file-read adapter through `Win32MavgPayloadAsyncFileIoDispatcher`, `CreateFileW`, `ReadFile`, `OVERLAPPED`, `GetOverlappedResult`, and `CancelIoEx`. Remaining follow-up after this child is DirectStorage SDK/native queue execution, IOCP worker execution, autonomous worker execution, async overlap evidence, automatic eviction policy, and GPU memory pressure integration.
