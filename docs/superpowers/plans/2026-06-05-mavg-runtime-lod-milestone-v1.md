...
- This milestone's implementation checkpoints now add graph parent ids, geometric error, resident fallback ancestors, cluster draw ranges, draw-ready static cook payload rows, a deterministic value-only `MK_renderer` CPU selector, an `MK_runtime` resident-page evidence bridge, `MK_runtime` caller-reviewed page streaming planning, reviewed visible/fallback-ancestor eviction protection, a deterministic automatic eviction policy boundary limited to resident-page candidate ordering, `RuntimeMavgPageStreamingDispatchPlan` / `plan_runtime_mavg_page_streaming_dispatches` dispatch planning, the completed joined engine-owned page streaming worker boundary, range-aware conventional indexed draw execution, `MK_scene_renderer` conventional `MeshCommand` planning, and `MK_runtime_rhi` conventional package-visible MAVG mesh binding upload evidence. Sustained asynchronous package streaming, async-overlap/performance, LRU/recency/frequency eviction policy, DirectStorage native file IO execution, and GPU memory pressure integration remain future work.
...
- No sustained asynchronous package streaming service beyond the narrow joined engine-owned worker proof, no async overlap/performance claim, no LRU/recency/frequency eviction policy beyond deterministic resident-page automatic candidate ordering plus selected/fallback ancestor protection, no DirectStorage native `.mavgpayload` streaming execution, and no GPU memory pressure integration.
...
- `mavg-autonomous-page-streaming-worker-v1`: completed closeout child adding a narrow engine-owned joined worker boundary through `RuntimeMavgPageStreamingDispatchMode::engine_owned_background_worker`, `RuntimeMavgPageStreamingWorkerDesc`, `RuntimeMavgPageStreamingWorkerResult`, and `execute_runtime_mavg_page_streaming_worker`. Deterministic automatic eviction candidate ordering is now covered by the follow-up `mavg-automatic-eviction-policy-v1`; remaining follow-up after that child is DirectStorage SDK/native queue execution, DirectStorage native file IO execution, sustained async-overlap benchmark evidence, LRU/recency/frequency eviction policy evidence, and GPU memory pressure integration.
- `mavg-automatic-eviction-policy-v1`: completed follow-up child adding deterministic resident-page automatic eviction candidate ordering through `RuntimeMavgPageStreamingAutomaticEvictionPlanDesc`, `plan_runtime_mavg_page_streaming_automatic_evictions`, `planned_automatic_eviction_policy`, `automatic_eviction_candidate_count`, and `protected_eviction_candidate_skip_count`. Remaining follow-up is LRU/recency/frequency policy evidence, DirectStorage SDK/native queue execution, DirectStorage native file IO execution, sustained async-overlap benchmark evidence, and GPU memory pressure integration.
