...
**Execution State:** Parent milestone in the stacked MAVG implementation branch after explicit LoD supersession. `mavg-asset-graph-v1` remains the prerequisite foundation branch; `mavg-gpu-culling-indirect-v1`, `mavg-rhi-indirect-draw-v1`, `mavg-d3d12-indexed-indirect-draw-execution-v1`, `mavg-d3d12-indexed-indirect-count-buffer-execution-v1`, `mavg-d3d12-compute-generated-indirect-execution-v1`, `mavg-d3d12-gpu-culling-execution-v1`, `mavg-package-streaming-residency-dispatch-v1`, `mavg-page-addressable-payload-schema-v1`, and `mavg-payload-byte-range-file-io-v1` are completed stacked children. `mavg-directstorage-request-plan-v1` is the active child for value-only DirectStorage request planning through `RuntimeMavgPayloadDirectStorageRequestPlanDesc`, `RuntimeMavgPayloadDirectStorageRequestRow`, `RuntimeMavgPayloadDirectStorageRequestPlanResult`, `RuntimeMavgPayloadDirectStorageFenceWaitPoint`, and `plan_runtime_mavg_payload_directstorage_requests` over selected validated `GameEngine.MavgClusterPayload.v1` page rows with `requires_native_directstorage_sdk=true`, `used_native_directstorage=false`, and zero native enqueue/submit/fence/file-IO side effects. This milestone remains the conventional static LoD evidence record and must not claim native DirectStorage/Win32 async IO execution, autonomous background workers, async-overlap/performance, broad GPU culling frameworks, Vulkan/Metal backend indirect execution, or Nanite equivalence/superiority.
...
- `mavg-directstorage-request-plan-v1`: active child adding value-only DirectStorage request planning through `RuntimeMavgPayloadDirectStorageRequestPlanDesc`, `RuntimeMavgPayloadDirectStorageRequestRow`, `RuntimeMavgPayloadDirectStorageRequestPlanResult`, `RuntimeMavgPayloadDirectStorageFenceWaitPoint`, and `plan_runtime_mavg_payload_directstorage_requests` over selected validated payload page rows with `requires_native_directstorage_sdk=true`, `used_native_directstorage=false`, and no file IO, native enqueue, native submit, native fence signal, background worker, resident mount mutation, renderer/RHI handle access, or async-overlap/performance claim. Remaining follow-up after this child is SDK-gated native DirectStorage/Win32 async queue/fence/status execution, autonomous worker execution, async overlap evidence, automatic eviction policy, and GPU memory pressure integration.
