...
- 2026-07-01 re-audit result: the requested failure diagnosis/operator triage loop, `failed` / `legal-blocked` / `nondeterministic` classification, reimport-decision mapping, editor-core retained rows, `tools/check-asset-import-regression-operator-loop.ps1`, and synthetic retained-report verification are implemented. The remaining unimplemented work is only the approved real-corpus execution/evidence path and narrow commercial promotion.
- 2026-07-01 final re-check for this operator-loop slice: no additional visible-shell behavior is needed before the approved real corpus is available. `GameEngine.AssetImportRegressionTriage.v1` is the clean-break operator artifact; it classifies report rows into stable first-party actions, blocks legal and nondeterministic rows from apply/reimport, allows only dry-run reimport for inspectable source/axis/codec cases, serializes self-contained `failed_count`, `legal_blocked_count`, and `nondeterministic_count` header counters, rejects inconsistent aggregate values on deserialize, and exposes only sanitized retained `asset_browser.import_workflow.*` rows in editor-core. The real-corpus closeout remains host-gated by retained `report.gereport`, `triage.geoperator`, compact `*.gepreview`/preset-diff text artifacts, and legal/provenance evidence under `out/host-artifacts/asset-import-regression-corpus/`.
- 2026-07-01 handoff planner update: `tools/plan-asset-import-regression-corpus-handoff.ps1` and `tools/check-asset-import-regression-corpus-handoff.ps1` are implemented as host-independent operator handoff gates. They classify missing corpus, missing manifest, missing notices, source layout, expected hashes, selection-review metadata, dependency host, runner, retained success/failure evidence, operator-loop, and `-RequireReady` validation steps; emit exact next commands; fail closed under `-RequireReady`; and keep external-engine, Unity/Unreal/Godot compatibility, and legal-approval claims at zero. This does not run the large corpus, does not import assets, and does not change readiness counters.
...
- 2026-07-01 Task 15 retained report intake and handoff files: `tools/check-asset-import-regression-operator-loop.ps1`, `tools/plan-asset-import-regression-corpus-handoff.ps1`, `tools/check-asset-import-regression-corpus-handoff.ps1`, `tools/check-asset-import-regression-corpus.ps1`, `tools/check-ai-integration-147-asset-import-regression-corpus.ps1`, `engine/agent/manifest.fragments/002-commands.json`, `engine/agent/manifest.fragments/007-importerCapabilities.json`, and `engine/agent/manifest.fragments/009-validationRecipes.json`.
- 2026-07-01 re-audit for this update: Context7 `/spnda/fastgltf` confirms `Expected<Asset>` parse/validation errors must be checked before asset access and normalized into first-party stable diagnostic codes; Context7 `/microsoftdocs/powershell-docs` confirms PowerShell 7 scripts should fail closed with `$ErrorActionPreference = 'Stop'`, `ConvertFrom-Json` structured parsing, deterministic output rows, and no mutation outside explicit command intent. The command manifest now exposes the actual operator-loop `-CorpusRoot` / `-RequireReady` gate so agents can execute the real-corpus closeout without guessing parameters.
- 2026-07-01 final official-docs re-check: Context7 `/spnda/fastgltf` confirms `Parser::loadGltf` returns `Expected<Asset>`, `GltfDataBuffer` loading and `fastgltf::validate` errors must be checked before asset access, and glTF inspection covers scenes/nodes/meshes/primitives/materials/buffers/images without leaking `fastgltf` types into public reports. Context7 `/microsoftdocs/powershell-docs` confirms `ConvertFrom-Json -AsHashtable` / LF text parsing and explicit `exit` codes are the preferred script boundary. Khronos glTF registry still names glTF 2.0 as current; the plan keeps glTF 2.1 topics observation-only until official parser/importer support is explicit. SPDX official license identifiers remain the corpus license ID source; Creative Commons NC/ND terms remain rejected for commercial/import-modification evidence; Unity, Epic/Unreal, and Godot official legal/trademark/license pages remain category/legal references only, not implementation sources or compatibility targets.
- 2026-07-01 post-merge verification: clean `codex/asset-import-current-review` audit reconfirmed the operator-loop implementation on `origin/main`. No additional implementation delta is needed for synthetic report intake, `failed` / `legal_blocked` / `nondeterministic` triage classification, dry-run-only reimport decisions, editor-core retained rows, `tools/check-asset-import-regression-operator-loop.ps1`, or legal/external-engine zero-claim guards. The only remaining executable closeout is the approved host-owned real corpus plus optional `asset-importers` dependency-ready host.
...
- 2026-07-02 re-audit on clean `origin/main` worktree `codex/asset-import-operator-loop-reaudit-20260702`: no implementation delta is needed for the requested failure diagnosis/operator triage loop, `failed` / `legal_blocked` / `nondeterministic` classification, reimport decision gating, editor-core retained rows, `tools/check-asset-import-regression-operator-loop.ps1`, or synthetic fixture validation. `tools/cmake.ps1 --preset dev`, `tools/check-asset-import-regression-operator-loop.ps1 -SyntheticSmoke`, `tools/check-asset-import-regression-corpus.ps1`, `tools/check-json-contracts.ps1`, and `tools/check-ai-integration.ps1` passed. The only remaining work is the approved large real corpus, optional `asset-importers` dependency-ready host, real-corpus batch reimport/preset diff/axis-unit retained evidence, and Task 17 narrow promotion.
- 2026-07-02 official-source re-check: Context7 `/microsoftdocs/powershell-docs` confirms `SupportsShouldProcess`, `$ErrorActionPreference = 'Stop'`, and `Write-Error` fail-closed script behavior; Context7 `/spnda/fastgltf` confirms glTF 2.0 parser/options/accessor/material/image inspection boundaries; Context7 `/khronosgroup/ktx-software` confirms KTX2 dimensions/levels/layers/faces/metadata/transcoding diagnostics stay adapter-private. Khronos glTF registry still names glTF 2.0 as current while the June 11, 2026 glTF 2.1 article is a future/backward-compatible planning source; W3C PNG Third Edition, Khronos KTX 2.0, IETF RFC 9639 FLAC, Unity trademark, Epic Content License, and Godot license/compliance pages were rechecked as official source/legal references only. No Unity, Unreal, Godot, marketplace, runtime source parsing, native handle, or compatibility/equivalence claim is introduced.
...
- Create: `tools/plan-asset-import-regression-corpus-handoff.ps1`
- Create: `tools/check-asset-import-regression-corpus-handoff.ps1`
- Modify: `tools/check-asset-import-regression-corpus.ps1`
...
- [x] Synchronize the AI-operable command contract so `engine/agent/manifest.fragments/002-commands.json` exposes `tools/check-asset-import-regression-operator-loop.ps1 [-ReportPath <path>] [-CorpusRoot <path>] [-OutputRoot <path>] [-SyntheticSmoke] [-RequireReady]`, then compose `engine/agent/manifest.json` and guard it through `tools/check-ai-integration-147-asset-import-regression-corpus.ps1`.
- [x] Add `tools/plan-asset-import-regression-corpus-handoff.ps1` and `tools/check-asset-import-regression-corpus-handoff.ps1`. The planner is read-only unless a downstream command is explicitly run by the operator; it emits `asset_import_regression_handoff_*` counters for missing corpus, missing manifest, missing notices, source layout, expected hashes, selection review, dependency host, runner, retained success/failure reports, operator-loop blockers, and `-RequireReady` validation. It emits next commands for the exact repo scripts, fails closed under `-RequireReady`, and keeps `asset_import_regression_handoff_external_engine_claim=0`, `asset_import_regression_handoff_legal_approval_claim=0`, and `asset_import_regression_handoff_unity_unreal_godot_compatibility_claim=0`.
- [x] Integrate the handoff smoke into `tools/check-asset-import-regression-corpus.ps1`, command manifest, validation recipe manifest, importer capability manifest, and static guard so future agents can run `tools/plan-asset-import-regression-corpus-handoff.ps1 -CorpusRoot out/host-artifacts/asset-import-regression-corpus` before attempting real-corpus import or promotion.
...
```powershell
pwsh -NoProfile -ExecutionPolicy Bypass -File tools/plan-asset-import-regression-corpus-handoff.ps1 -CorpusRoot out/host-artifacts/asset-import-regression-corpus
pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-asset-import-regression-corpus-handoff.ps1
...
| Command | Required before completion | Observed 2026-07-01 / 2026-07-02 re-audit |
| --- | --- | --- |
...
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-json-contracts.ps1` | Schema/manifest/static-contract slice | Pass; rechecked 2026-07-02 on clean `origin/main` worktree with `agent-manifest-compose: ok` and `json-contract-check: ok` |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-ai-integration.ps1` | Manifest/docs/agent-surface slice | Pass; rechecked 2026-07-02 on clean `origin/main` worktree with `ai-integration-check: ok` |
...
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/cmake.ps1 --preset dev` | New linked-worktree configure before focused build/test smoke | Pass on 2026-07-02; created `out/build/dev` for the re-audit worktree without dependency install or source-tree mutation |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-asset-import-regression-operator-loop.ps1 -SyntheticSmoke` | Task 14 operator triage loop | Pass; rechecked 2026-07-02 after dev configure. It emitted `asset_import_regression_operator_loop_report_rows=6`, `failed_rows=5`, `legal_blocked_rows=1`, `nondeterministic_rows=1`, `reimport_candidates=2`, `blocked_rows=3`, `preset_diff_required=1`, `axis_unit_preview_required=1`, `editor_core_value_only=1`, and `external_engine_claim=0`; generated `triage.geoperator` also carries `failed_count=5`, `legal_blocked_count=1`, and `nondeterministic_count=1` |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-asset-import-regression-corpus-handoff.ps1` | Task 15 host-independent real-corpus handoff planner | Pass; synthetic fixtures prove `missing_corpus`, `corpus_manifest_required`, `operator_loop_required`, `failed_count=3`, `legal_blocked_count=1`, `nondeterministic_count=1`, retained success/failure report counting, exact next-command emission, `require_ready.missing_corpus` fail-closed behavior, `external_engine_claim=0`, `legal_approval_claim=0`, and `unity_unreal_godot_compatibility_claim=0` |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/plan-asset-import-regression-corpus-handoff.ps1 -CorpusRoot out/host-artifacts/asset-import-regression-corpus` | Task 15 real-corpus operator handoff | Pass as non-promoting planning evidence; current host path is absent and emits `asset_import_regression_handoff_status=missing_corpus`, `ready=0`, `next_action=create_or_mount_approved_host_corpus`, all expected evidence-present counters at `0`, and external-engine/legal-approval/Unity-Unreal-Godot compatibility claims at `0` |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-asset-import-regression-corpus.ps1` | Corpus workflow slice | Pass; rechecked 2026-07-02. Existing generator/runner/fresh-process checks passed, the integrated retained success/failure operator-loop corpus gate asserted `corpus_retained_reports=2`, `corpus_success_reports=1`, `corpus_failure_reports=1`, `require_ready=1`, `report_rows=5`, `failed_rows=4`, `legal_blocked_rows=1`, `nondeterministic_rows=1`, `reimport_candidates=1`, `blocked_rows=3`, `axis_unit_preview_required=1`, `editor_core_value_only=1`, `external_engine_claim=0`, and `corpus_ready=1`; the integrated handoff synthetic smoke passed; and the integrated operator-loop synthetic smoke passed. The default first-party fixture path still correctly reports `asset_import_regression_large_corpus_present=0`, so this is not commercial promotion evidence. |
...
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1` | Final implementation closeout | Pass; 167/167 tests passed in clean `codex/asset-import-current-review` post-merge audit |
