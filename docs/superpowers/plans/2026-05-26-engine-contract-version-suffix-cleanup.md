...
- Agent-surface recurrence guidance is aligned across `AGENTS.md`, Codex/Claude/Cursor agent-integration skills, Codex/Claude/Cursor project rules, and agent-surface-auditor subagents so unused suffix false positives are removed rather than treated as future contracts.
