...
- [x] For physics, add exact first-party 2D execution extensions only where missing: TOI/CCD where selected, joints/constraints, trigger/area evidence, kinematic contact resolution, deterministic replay, and package counters.
- [x] For input, prove actions, remapping data, device profiles, controller/mouse/keyboard/touch class handling, symbolic glyph keys, multiplayer device assignment where selected, and accessibility-friendly navigation.
...
- 2026-07-01 input closeout slice: added `Runtime2DCommercialInputOfficialSourceKind`, `Runtime2DCommercialInputOfficialSourceRow`, `Runtime2DCommercialInputNavigationRow`, `Runtime2DCommercialInputCloseoutDesc`, `Runtime2DCommercialInputCloseoutResult`, and `evaluate_runtime_2d_commercial_input_closeout` in `MK_runtime`; `sample_2d_desktop_runtime_package --require-2d-commercial-input-closeout`; `tools/validate-2d-commercial-input-closeout.ps1`; `installed-2d-commercial-input-closeout-smoke`; and manifest/static guidance for `desktop-runtime-2d-commercial-input-closeout`. The gate aggregates reviewed runtime input actions, rebinding overlays, device-production UX rows, accessibility-friendly navigation rows, and public official-source rows for Microsoft GameInput, Raw Input, Pointer Input, UI Automation, and repository legal policy. Refreshed official-source anchors: `https://learn.microsoft.com/en-us/gaming/gdk/docs/features/common/input/overviews/input-overview`, `https://learn.microsoft.com/en-us/windows/win32/inputdev/raw-input`, `https://learn.microsoft.com/en-us/windows/win32/inputmsg/messages`, and `https://learn.microsoft.com/en-us/windows/win32/winauto/entry-uiauto-win32`. It requires package-visible counters for ready action-map/rebinding/device/accessibility/official-source evidence, exact row counts, zero diagnostics, zero native handles, zero input middleware, zero UI rendering/widgets, zero external-engine compatibility, zero cross-platform parity, and zero legal-approval claims. Broad input middleware adoption, non-Windows input parity, visible rebinding UI, Unity/Unreal/Godot compatibility/equivalence/parity/replacement, and legal approval remain unclaimed.
- Official-source documentation checked on 2026-07-01 through Context7 `/websites/learn_microsoft_en-us` and Microsoft Learn: GameInput introduction (`https://learn.microsoft.com/en-us/gaming/gdk/docs/features/common/input/overviews/input-overview`), Raw Input (`https://learn.microsoft.com/en-us/windows/win32/inputdev/raw-input`), Pointer Input Messages and Notifications (`https://learn.microsoft.com/en-us/windows/win32/inputmsg/messages`), and UI Automation (`https://learn.microsoft.com/en-us/windows/win32/winauto/entry-uiauto-win32`). These references are engineering source rows only; they do not add Microsoft SDK redistribution, input middleware, external-engine compatibility, or legal approval.
- 2026-07-01 physics closeout slice: added `Runtime2DCommercialPhysicsFeatureKind`, `Runtime2DCommercialPhysicsSourceKind`, `Runtime2DCommercialPhysicsFeatureRow`, `Runtime2DCommercialPhysicsSourceRow`, `Runtime2DCommercialPhysicsCloseoutDesc`, `Runtime2DCommercialPhysicsCloseoutResult`, and `evaluate_runtime_2d_commercial_physics_closeout` in `MK_runtime`; `sample_2d_desktop_runtime_package --require-2d-commercial-physics-closeout`; `tools/validate-2d-commercial-physics-closeout.ps1`; `installed-2d-commercial-physics-closeout-smoke`; and manifest/static guidance for `desktop-runtime-2d-commercial-physics-closeout`. The gate aggregates existing first-party 2D physics runtime-extension package evidence for selected TOI/CCD rows, exact sweep shape pairs, joint/constraint rows, trigger/area events, kinematic contact resolution, deterministic replay, package-visible counters, and tracked repository source rows. It requires six feature rows, four source rows, exact package-visible simulation/TOI/sweep/kinematic/joint/trigger counters, a positive deterministic replay hash, zero diagnostics, zero native-handle rows, zero physics middleware rows, zero dynamic-vs-dynamic CCD broad claim rows, zero broad physics parity rows, zero external-engine compatibility rows, zero cross-platform parity rows, and zero legal-approval rows. Physics middleware adoption, dynamic-vs-dynamic CCD broad readiness, persistent joint assets, broad physics parity, cross-platform parity, Unity/Unreal/Godot compatibility/equivalence/parity/replacement, and legal approval remain unclaimed.
- Official-source documentation checked on 2026-07-01 through Context7 `/kitware/cmake`: CMake target-based source and link registration uses `target_sources` plus `target_link_libraries` with `PUBLIC` / `PRIVATE` / `INTERFACE` scope. This reference is build-system evidence only; the physics closeout remains first-party repository evidence and does not add external physics SDKs, middleware, external-engine compatibility, or legal approval.
...
- [x] Define fixed 2D workload scenes: dense sprites, large tilemaps, UI-heavy scene, animation-heavy scene, streaming stress, physics stress, audio/input stress, and long-running playtest.
- [x] Track CPU frame time, GPU frame time, input-to-present latency, present pacing, IO/decompression/upload overlap, memory high-water, GPU residency pressure, allocator churn, job queue depth, cache misses where available, and package miss/pop-in counters.
- [x] Add p50/p95/p99 thresholds and fail-closed over-budget diagnostics. Keep thresholds host-class-specific.
- [x] Add official profiler artifact collectors: Windows WPR/WPA/PIX where selected, Vulkan timestamp/debug-utils evidence, Apple `xctrace`/Metal capture evidence, and Linux perf where selected.
- [x] Keep PGO/LTO and release optimization lanes separate from debug/dev validation, with reproducible compiler flags and artifact metadata.
...
- 2026-07-01 2D Commercial Performance Regression Gates v1 slice: added `Runtime2DCommercialPerformanceWorkloadKind`, `Runtime2DCommercialPerformanceMetricKind`, `Runtime2DCommercialPerformanceOfficialSourceKind`, `Runtime2DCommercialPerformanceWorkloadRow`, `Runtime2DCommercialPerformanceMetricRow`, `Runtime2DCommercialPerformanceOfficialSourceRow`, `Runtime2DCommercialPerformanceRegressionGateDesc`, `Runtime2DCommercialPerformanceRegressionGateResult`, and `evaluate_runtime_2d_commercial_performance_regression_gate` in `MK_runtime`; `sample_2d_desktop_runtime_package --require-2d-commercial-performance-regression-gate`; `tools/validate-2d-commercial-performance-regression-gates.ps1`; `installed-2d-commercial-performance-regression-smoke`; and manifest/static guidance for `desktop-runtime-2d-commercial-performance-regression-gates`. The selected gate aggregates eight fixed 2D workload rows, eleven metric rows, host-class-specific p50/p95/p99 thresholds, selected D3D12 package performance/long-run counters, and public official-source rows for Microsoft Direct3D 12, Windows Performance Toolkit/WPA, PIX on Windows, Khronos Vulkan timestamp/debug-utils, Apple xctrace/Metal capture, Linux perf, and repository legal policy. It requires zero over-budget, broad optimization, cross-vendor/backend parity, native-handle, renderer/RHI residency, allocator/GPU enforcement, PGO/LTO default-lane, external-engine compatibility, and legal-approval counters. Real retained profiler capture artifacts, before/after optimization proof, cross-vendor/backend parity, broad optimized-game readiness, Unity/Unreal/Godot compatibility/equivalence/parity/replacement, and legal approval remain unclaimed.
- Official-source documentation checked on 2026-07-01 through Context7 and official documentation: Microsoft Direct3D 12 (`https://learn.microsoft.com/en-us/windows/win32/direct3d12/direct3d-12-graphics`), Microsoft PIX on Windows (`https://learn.microsoft.com/en-us/windows/win32/direct3dtools/pix/articles/general/pix-overview`), Windows Performance Toolkit (`https://learn.microsoft.com/en-us/windows-hardware/test/wpt/`), Khronos Vulkan specification (`https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html`), Apple Metal tools (`https://developer.apple.com/metal/tools/`), Linux perf wiki (`https://perf.wiki.kernel.org/index.php/Main_Page`), and repository legal policy. These references are engineering source rows only; they do not add profiler execution, external-engine compatibility, or legal approval.
- 2026-07-01 focused validation: `tools/validate-2d-commercial-performance-regression-gates.ps1`, `tools/cmake.ps1 --build --preset dev --target MK_runtime_2d_commercial_performance_regression_gate_tests`, and `tools/ctest.ps1 --preset dev --output-on-failure -R MK_runtime_2d_commercial_performance_regression_gate_tests` passed. `tools/validate-2d-commercial-performance-regression-gates.ps1 -RequireReady` also passed and verified installed package counters for `2d_commercial_performance_regression_ready=1`, eight workload rows, eleven metric rows, eleven host-class threshold rows, six package-visible metric rows, five host-gated profiler artifact rows, seven official-source rows, zero over-budget rows, zero unsafe claim rows, and `2d_commercial_performance_diagnostics=0`. Slice close validation `tools/validate.ps1` passed with 52 static checks and 165 CTest tests.
- 2026-07-01 2D Commercial Official Profiler Artifact Collectors v1 slice: added `tools/validate-2d-commercial-profiler-artifact-collectors.ps1` and `tools/check-ai-integration-151-2d-commercial-profiler-artifact-collectors.ps1` for the `GameEngine.2DCommercialProfilerArtifactCollector.v1` retained artifact manifest contract. The validator reads operator-supplied artifact rows for Windows WPR/WPA (`https://learn.microsoft.com/en-us/windows-hardware/test/wpt/`), PIX on Windows (`https://learn.microsoft.com/en-us/windows/win32/direct3dtools/pix/articles/general/pix-overview`), Khronos Vulkan timestamp/debug-utils (`https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html`), Apple xctrace/Metal capture (`https://developer.apple.com/documentation/xcode/recording-performance-data`), and Linux perf (`https://perf.wiki.kernel.org/index.php/Main_Page`). It validates safe repository-relative paths under the selected artifact root, SHA-256 hash matches, required tool ids, public official-source rows, reviewed command-line/capture policy rows, retained artifact rows, package-counter linkage, and fail-closed zero rows for broad optimization, cross-vendor/backend parity, public native handles, external-engine compatibility, Unity/Unreal/Godot compatibility/equivalence/parity/replacement, and legal approval. `-SyntheticSmoke` creates only first-party fixture text/JSON under `out/host-artifacts/2d-commercial-profiler-artifact-collectors/synthetic`; real retained profiler captures remain host-owned operator evidence and are not generated by local validation.
