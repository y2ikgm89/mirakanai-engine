...
- `sample_desktop_runtime_game` now ships selected `GameEngine.EnvironmentTextureGeassetPayload.v1` records for `environment_radiance_exr.texture.geasset` and `environment_skybox_basis.texture.geasset`. The package smoke requires two cooked payload records, two payload byte rows, two payload hash rows, one OpenEXR source-kind row, one KTX2/Basis source-kind row, ten backend policy rows, eight unsupported-host diagnostics, two profile dependency references, two package dependency edges, payload texture decode and Basis transcode evidence, and zero GPU upload or broad-ready counters.
