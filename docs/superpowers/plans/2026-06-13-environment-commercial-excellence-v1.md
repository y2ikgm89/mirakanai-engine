...
- Local `tools/build-asset-importers.ps1` still fails before optional dependency bootstrap at missing `SPNGConfig.cmake`, and `tools/bootstrap-deps.ps1` remains blocked by this session command policy. Hosted PR Windows MSVC `asset-importers` evidence is required for the real OpenEXR payload decode test. This slice still does not add runtime OpenEXR/KTX/Basis codec execution, KTX/Basis payload transcode, full source-to-package cooker promotion, backend API execution, GPU upload execution, strict Vulkan/Metal payload parity, broad source-image decode coverage, or `environment_asset_pipeline_openexr_ktx_basis_ready`.
