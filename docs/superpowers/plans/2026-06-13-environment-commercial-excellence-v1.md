...
**Status:** Active. Phase 0-2 selected the milestone, readiness taxonomy, and optional OpenEXR/KTX/Basis dependency/legal/bootstrap gates. Phase 3 slices 1-17 now cover descriptor metadata, optional-gated source review, hosted asset-importers validation wiring, real OpenEXR/KTX2/Basis metadata evidence, backend format policy rows, metadata-only and selected payload `.geasset` package evidence, dependency-free `MK_runtime` ingest of selected `GameEngine.EnvironmentTextureGeassetPayload.v1` records, selected RGBA8 runtime upload-plan-ready payload evidence, evidence-only hosted optional dependency bootstrap/build-asset-importers validation for the `asset-importers` manifest feature, optional-gated `MK_tools` OpenEXR scanline RGBA16F decode, optional-gated `MK_tools` KTX2/Basis RGBA8 transcode, a selected `cook_environment_texture_payload_v1` source-to-package orchestrator, backend-target decision rows in `TextureCookBackendDecisionV1` / `GameEngine.CookedTextureMetadata.v1` that serialize `payload_transcode_target`, `format_support_evidence_id`, and `official_format_support_api` for D3D12/Vulkan BC7 and Metal/Android Vulkan ASTC policy, and selected D3D12 WARP runtime upload/readback execution for the package RGBA8 environment payload through `execute_runtime_environment_texture_payload_upload` plus `--require-environment-texture-asset-pipeline-d3d12-upload` counters. Phase 4 promotes the selected first-party `GameEngine.EnvironmentPresetPack.v1` lane to `environment_aaa_preset_library_ready=1`. Phase 5-11 add strict Vulkan aggregate gates, Apple-host Metal aggregate recipe, backend parity matrix rows, exact platform readiness rows, seven D3D12 optimization workload measurements, selected physical-weather evidence, and editor-core artist workflow value rows. This still does not claim Windows-derived Metal host aggregate readiness, package-visible backend parity readiness, Vulkan/Metal weather solver parity, unconditional all-platform readiness, broad optimization readiness, full OpenEXR/KTX/Basis asset-pipeline readiness, backend-target compressed texture execution, strict Vulkan/Apple-host Metal texture upload execution, complete physical weather simulation readiness, production solver readiness, complete artist-workflow readiness, commercial aggregate readiness, or broad `environment_ready`.
...
The next implementation candidate from this plan remains Order 2 unless a hosted Vulkan/Metal/platform evidence window becomes available first. The selected D3D12 RGBA8 upload/readback row is implemented, but Order 2 still needs backend-target compressed payload execution plus strict Vulkan and Apple-host Metal upload/readback rows before asset-pipeline readiness can be promoted.
...
- [ ] Add strict Vulkan and Apple-host Metal runtime upload/readback rows, plus backend-target BC7/ASTC compressed payload execution where each backend's official format support evidence proves it.
