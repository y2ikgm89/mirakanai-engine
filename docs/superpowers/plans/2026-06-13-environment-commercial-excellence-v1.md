...
**Status:** Active. Phase 0-2 selected the milestone, readiness taxonomy, and optional OpenEXR/KTX/Basis dependency/legal/bootstrap gates. Phase 3 slices 1-18 now cover descriptor metadata, optional-gated source review, hosted asset-importers validation wiring, real OpenEXR/KTX2/Basis metadata evidence, backend format policy rows, metadata-only and selected payload `.geasset` package evidence, dependency-free `MK_runtime` ingest of selected `GameEngine.EnvironmentTextureGeassetPayload.v1` records, selected RGBA8 runtime upload-plan-ready payload evidence, evidence-only hosted optional dependency bootstrap/build-asset-importers validation for the `asset-importers` manifest feature, optional-gated `MK_tools` OpenEXR scanline RGBA16F decode, optional-gated `MK_tools` KTX2/Basis RGBA8 transcode, a selected `cook_environment_texture_payload_v1` source-to-package orchestrator, backend-target decision rows in `TextureCookBackendDecisionV1` / `GameEngine.CookedTextureMetadata.v1` that serialize `payload_transcode_target`, `format_support_evidence_id`, and `official_format_support_api` for D3D12/Vulkan BC7 and Metal/Android Vulkan ASTC policy, and selected D3D12 WARP plus strict Vulkan runtime upload/readback execution for the package RGBA8 environment payload through `execute_runtime_environment_texture_payload_upload`, `--require-environment-texture-asset-pipeline-d3d12-upload`, and `--require-environment-texture-asset-pipeline-vulkan-upload` counters. Phase 4 promotes the selected first-party `GameEngine.EnvironmentPresetPack.v1` lane to `environment_aaa_preset_library_ready=1`. Phase 5-11 add strict Vulkan aggregate gates, Apple-host Metal aggregate recipe, backend parity matrix rows, exact platform readiness rows, seven D3D12 optimization workload measurements, selected physical-weather evidence, and editor-core artist workflow value rows. This still does not claim Windows-derived Metal host aggregate readiness, package-visible backend parity readiness, Vulkan/Metal weather solver parity, unconditional all-platform readiness, broad optimization readiness, full OpenEXR/KTX/Basis asset-pipeline readiness, backend-target compressed texture execution, Apple-host Metal texture upload execution, complete physical weather simulation readiness, production solver readiness, complete artist-workflow readiness, commercial aggregate readiness, or broad `environment_ready`.
...
- Current evidence claims selected D3D12-primary readiness, selected strict Vulkan aggregate readiness, exact platform-readiness row mapping, seven D3D12 broad-optimization measurement rows, and selected first-party AAA preset-library readiness only through their package-visible lanes. It still does not claim Metal host aggregate readiness, backend parity readiness, unconditional all-platform readiness, broad optimization readiness, full OpenEXR/KTX/Basis production asset-pipeline readiness, physically complete weather simulation, or complete real-production artist workflow.
...
The next implementation candidate from this plan remains Order 2 unless an Apple-host Metal/platform evidence window becomes available first. The selected D3D12 and strict Vulkan RGBA8 upload/readback rows are implemented, but Order 2 still needs backend-target compressed payload execution plus Apple-host Metal upload/readback rows before asset-pipeline readiness can be promoted.
...
- [x] Add an optional-gated OpenEXR scanline pixel decode adapter that uses the official `InputFile` / `FrameBuffer` / `Slice(HALF)` / `readPixels` path to emit deterministic RGBA16F payload bytes while keeping backend upload, KTX/Basis runtime transcode, full payload-cooker promotion, and full asset-pipeline readiness unclaimed.
- [x] Add an optional-gated KTX2/Basis payload transcode adapter that uses official libktx load/transcode/data APIs to emit selected RGBA8 payload bytes while keeping runtime codec/transcode, backend-target compressed transcode, GPU upload, strict Vulkan/Metal payload parity, full payload-cooker promotion, and full asset-pipeline readiness unclaimed.
...
- This slice still does not execute BC7/ASTC compressed payload transcodes, run strict Vulkan upload/readback, run Apple-host Metal upload/readback, prove backend parity, execute runtime OpenEXR/KTX/Basis codecs, or promote full OpenEXR/KTX/Basis asset-pipeline readiness.
...
- This slice still does not execute BC7/ASTC compressed payload transcodes, run Apple-host Metal upload/readback, prove backend parity, execute runtime OpenEXR/KTX/Basis codecs, or promote full OpenEXR/KTX/Basis asset-pipeline readiness.
...
- This slice promotes only the selected first-party AAA preset-library claim. It still does not add external marketplace asset coverage, full OpenEXR/KTX/Basis asset-pipeline readiness, backend upload execution, renderer/RHI backend parity, all-platform readiness, complete artist workflow, commercial readiness, or broad `environment_ready`.
...
- [ ] Add strict Vulkan weather solver package-visible rows with synchronization2 barriers, descriptor rows, compute dispatch/readback proof, profiler budget rows, zero diagnostics, zero native-handle access, and no inference from D3D12.
...
- [ ] Add broad physical accuracy and visual quality review rows with named validation datasets/images, tolerance/error bounds, package counters, and reviewer-visible diagnostics.
