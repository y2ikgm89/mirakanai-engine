...
- Local `tools/build-asset-importers.ps1` still fails before optional dependency bootstrap at missing `SPNGConfig.cmake`; hosted PR Windows MSVC `Validate optional asset importers` remains the required evidence gate for compiling and running the real KTX2/Basis transcode fixture lane. This slice still does not add runtime OpenEXR/KTX/Basis codec execution, runtime Basis transcode, backend-target BC7/ASTC transcode execution, full source-to-package cooker promotion, backend API execution, GPU upload execution, strict Vulkan/Metal payload parity, broad source-image decode coverage, or `environment_asset_pipeline_openexr_ktx_basis_ready`.
