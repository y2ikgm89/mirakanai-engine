...
**Status:** Active. Phase 0-2 selected the milestone, readiness taxonomy, and optional OpenEXR/KTX/Basis dependency/legal/bootstrap gates. Phase 3 slices 1-14 now cover descriptor metadata, optional-gated source review, hosted asset-importers validation wiring, real OpenEXR/KTX2/Basis metadata evidence, backend format policy rows, metadata-only and selected payload `.geasset` package evidence, dependency-free `MK_runtime` ingest of selected `GameEngine.EnvironmentTextureGeassetPayload.v1` records, selected RGBA8 runtime upload-plan-ready payload evidence, evidence-only hosted optional dependency bootstrap/build-asset-importers validation for the `asset-importers` manifest feature, an optional-gated `MK_tools` OpenEXR scanline pixel decode adapter that emits deterministic RGBA16F payload bytes, and an optional-gated `MK_tools` KTX2/Basis transcode adapter that emits selected RGBA8 payload bytes before `.geasset` payload planning. Phase 4-11 add selected preset-library governance/browsing, strict Vulkan aggregate gates, Apple-host Metal aggregate recipe, backend parity matrix rows, exact platform readiness rows, seven D3D12 optimization workload measurements, selected physical-weather evidence, and editor-core artist workflow value rows. This still does not claim Windows-derived Metal host aggregate readiness, package-visible backend parity readiness, Vulkan/Metal weather solver parity, unconditional all-platform readiness, broad optimization readiness, full OpenEXR/KTX/Basis asset-pipeline readiness, AAA preset-library readiness, complete physical weather simulation readiness, production solver readiness, complete artist-workflow readiness, commercial aggregate readiness, or broad `environment_ready`.
