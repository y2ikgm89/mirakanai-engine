...
- Canonical automation uses **PowerShell 7** (`pwsh`) and scripts under `tools/` only. **Bun, Node.js, and `package.json` script aliases are not part of the supported workflow surface**, even when user-level preferences mention commands such as `bun run validate`.
...
- PR CI selection should use an always-running required gate plus conditional heavy lanes, not path-filtered required workflows that can stay pending. Run the full hosted matrix for default-branch, release, scheduled/nightly, and workflow-dispatch runs. On PRs, use the cheapest validating tier: docs/agent/rules/subagent-only changes should run formatting plus agent/static guards (`check-agents`, `check-ai-integration`, and matching JSON/CI guards), not Windows/MSVC, macOS, or full repository clang-tidy unless they also change CI, CMake/vcpkg, build/test/package tooling, `engine/`, `editor/`, `games/`, packaging, or public/runtime contracts.
...
- For parallel write work, prefer native Codex app Worktree/Handoff or Claude Code `--worktree` / subagent `isolation: worktree` before manual `git worktree`; keep `.worktrees/` and `.claude/worktrees/` ignored and clean them through `git worktree remove` / `git worktree prune`, not ad hoc directory deletion.
...
- Publish agent work through GitHub's official GitHub Flow: make a topic branch, commit a coherent validated slice, push the topic branch, open a PR, satisfy reviews/checks, merge through GitHub, and delete the branch after merge.
- Commit and push task-owned topic-branch changes at coherent, validated checkpoints without per-action confirmation. Push cadence is checkpoint-based, not commit-count-based: multiple local commits may be pushed together when they form one validated, reviewable slice. Before committing, inspect `git status --short --branch`, review staged diff, and stage only task-owned files.
...
- For GitHub publishing, push the topic branch, then create/update a PR through GitHub Web, `gh`, or GitHub Desktop with validation evidence; `gh pr create` may run after validation checkpoints. Before `gh pr merge --auto --merge --delete-branch`, inspect `gh pr view <pr> --json state,isDraft,baseRefName,headRefName,headRefOid,mergeable,mergeStateStatus,reviewDecision,statusCheckRollup,autoMergeRequest,url`; proceed only for an open, non-draft, task-owned PR with fresh validation, expected base, no failing/cancelled/timed-out/action-required checks, no requested changes/conflicts, and branch-protection-controlled requirements. Prefer `--match-head-commit <headRefOid>`. Treat `DIRTY` / `UNKNOWN` as blockers; `UNSTABLE` / `BLOCKED` is acceptable only for pending required checks or reviews with no failed checks, requested changes, conflicts, or action-required statuses.
