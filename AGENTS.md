...
- Treat Dear ImGui as optional developer/editor tooling only. Runtime game UI should target first-party `mirakana_ui` contracts and must not depend on editor, removed SDL3 adapters, Dear ImGui, or UI middleware APIs.
...
- Claude Code shared permissions live in `.claude/settings.json`. Deny secrets and default-branch pushes; allow guarded publication preflight via `tools/check-publication-preflight.ps1`, task-owned PR creation, guarded `tools/ready-task-pr.ps1`, and auto-merge registration after validation; keep destructive, network, dependency-bootstrap, mobile signing/smoke, force-push, immediate PR merge, raw `gh pr ready`, and other PR state changes behind approval. Keep `.claude/settings.local.json`, `.mcp.json`, and `AGENTS.override.md` uncommitted.
