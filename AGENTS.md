...
- GitHub PR publishing and state changes through `gh pr create`, `edit`, `merge`, `ready`, `close`, or `reopen` are prompt-gated. If approvals are unavailable in the current session, do not retry those commands; keep the branch pushed and use GitHub Web/Desktop or an approval-capable session.
...
- Claude Code shared project permissions live in `.claude/settings.json` with the official JSON schema. Keep secret-bearing files denied and keep destructive, network, dependency-bootstrap, and mobile signing/smoke commands behind approval. Keep `.claude/settings.local.json`, `.mcp.json`, and `AGENTS.override.md` uncommitted unless a future governance decision intentionally changes that.
