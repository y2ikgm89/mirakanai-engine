...
- After a PR merges into `main`, run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/remove-merged-worktree.ps1 -WorktreePath <path> [-BaseRef origin/main] [-BaseBranch main] [-Remote origin] [-LocalCheckoutPath <path>] [-DeleteLocalBranch] [-DeleteRemoteBranch]` from Local or another clean non-target base checkout; it fetches/prunes, fast-forwards, verifies local and remote branch ancestry, unlinks worktree-local vcpkg links, and keeps Windows fallback guarded/non-following. No ad hoc deletion.
