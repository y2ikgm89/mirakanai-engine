...
- `MK_apply_common_target_options` isolates MSVC PDBs (`COMPILE_PDB_OUTPUT_DIRECTORY`) and uses `/INCREMENTAL:NO`; wrappers serialize builds and clear stale MSVC `.tlog` roots.
...
- Static-analysis PR failures are root-cause work: keep `.clang-tidy` `HeaderFilterRegex` path aware; strict hosted lanes keep `--warnings-as-errors=*`; preserve diagnostics; suppress only `NN warnings generated.` summaries; use `tools/check-tidy.ps1 -Files` locally and sharded `-Jobs 0` in CI. `validate.ps1` reuses build File API, runs `test.ps1 -SkipBuild`; wrappers use automatic CMake/CTest parallelism.
...
- Keep reviewer, explorer, architect, planning, and auditor subagents read-only by default in both `.codex/agents/` and `.claude/agents/`; give write-capable tools only to builder/fixer roles expected to change files. Keep always-loaded instructions concise; move reusable procedures into skills and specialized review/build/debug behavior into subagents.
