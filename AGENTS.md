...
- PR CI selection should use an always-running required gate plus conditional heavy lanes, not path-filtered required workflows that can stay pending. Run the full hosted matrix for default-branch, release, scheduled/nightly, workflow-dispatch, and PR changes that touch CI, CMake, vcpkg, tools, validation, agent surfaces, `engine/`, `editor/`, `games/`, packaging, or public/runtime contracts; docs-only slices may use a narrower tier only with explicit PR justification.
