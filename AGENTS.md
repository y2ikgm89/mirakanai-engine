...
- For GitHub publishing, push the current topic branch, then create/update a PR through GitHub Web, `gh`, or GitHub Desktop with concise title/body and actual validation evidence. In unattended Codex sessions, `gh pr create` and `gh pr merge --auto --merge --delete-branch` may run after validation checkpoints so branch protection controls final `main` merge; report credential, protection, required-check, or auto-merge blockers instead of weakening policy.
