...
- After a task-owned PR merges into `main`, run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/remove-merged-worktree.ps1 -WorktreePath <path> [-BaseRef origin/main] [-BaseBranch main] [-Remote origin] [-LocalCheckoutPath <path>] [-DeleteLocalBranch]` for linked worktree cleanup; it fetches/prunes, requires a clean local checkout on the base branch, fast-forwards it, then removes only safe merged worktrees. If the task branch was merged from the base checkout and no target worktree/local branch remains, record that there is no cleanup target.
