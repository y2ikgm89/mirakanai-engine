...
- After a task-owned PR merges into `main`, run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/remove-merged-worktree.ps1`; it fetches/prunes, requires a clean local checkout on the base branch, fast-forwards it, then removes only safe merged worktrees.
