...
- Formatting: `tools/check-format.ps1` / `tools/format.ps1` cover C++ plus tracked text (LF, UTF-8 no BOM, one final newline, no EOF blanks), excluding hashed `games/*/runtime/**`; `tools/check-text-format.ps1` / `tools/format-text.ps1` are text-only. Raw `clang-format --dry-run ...` requires `direct-clang-format-status=ready`. Run other static lanes only when their files or slice gate require them.
