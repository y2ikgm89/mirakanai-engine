...
- Commit and push task-owned topic-branch changes at coherent, validated checkpoints without per-action confirmation. Before committing, inspect `git status --short --branch`, review staged diff, and stage only task-owned files.
