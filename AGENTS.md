...
- Use GitHub's official GitHub Flow: topic branch, coherent validated commit, non-forced push, `gh pr create`/GitHub Desktop PR update, review/checks, merge through GitHub, and delete the branch after merge. Commit/push task-owned changes at validated checkpoints without per-action confirmation; inspect `git status --short --branch`, staged diff, and `git diff --cached --check` first.
- A slice is **not publication-complete after local validation alone**. Unless the user asks for local-only/no-PR work, do not send the final completion report until GitHub Flow has reached task-owned branch, stage, commit, non-forced push, and PR create/update with validation evidence, or an exact blocker. If starting on default/protected branch, create a topic branch; if `codex/<topic>` conflicts, use `codex-<topic>`. Leave unrelated user changes unstaged.
...
- For `gh pr merge --auto --merge --delete-branch`, first inspect `gh pr view <pr> --json state,isDraft,baseRefName,headRefName,headRefOid,mergeable,mergeStateStatus,reviewDecision,statusCheckRollup,autoMergeRequest,url`; proceed only for an open, non-draft, task-owned PR with fresh validation, expected base, no failed/cancelled/timed-out/action-required checks, no requested changes/conflicts, and branch-protection requirements. Prefer `--match-head-commit <headRefOid>`. Treat `DIRTY` / `UNKNOWN` as blockers; `UNSTABLE` / `BLOCKED` is acceptable only for pending required checks or reviews.
