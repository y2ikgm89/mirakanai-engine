...
- Shared helpers live in `tools/common.ps1`; static-contract-only assertions live in `tools/static-contract-common.ps1`. `Get-RepoRoot` returns a **string**; use `$root`, never `$root.Path`. Keep dot-sourced pairs synchronized, including `tools/check-json-contracts.ps1` / `tools/check-ai-integration.ps1` with `tools/manifest-command-surface-legacy-guard.ps1`. `tools/static-contract-ledger.ps1` owns numeric-prefix discovery, line budgets, and core dispatcher data; do not hand-list chapter files.
...
- Current engine contracts stay versionless. `Assert-NoLiveVersionSuffixContractText` guards live roots; remove unused suffix false positives as dead code/stale guidance, not future contracts. Keep `vN` only for history, numeric versions, standards, or coordinates.
