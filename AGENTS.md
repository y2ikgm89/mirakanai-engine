...
- PR CI selection uses an always-running required gate plus job-level conditional lanes, not path-filtered required workflows.
  ...
  Checkout is pinned/read-only; caches use `actions/cache/restore` / `actions/cache/save`. Docs/agent/rules/subagent-only PRs
  use agent/static guards unless they touch CI/build/runtime/packaging/public contracts.
...
- Default branch protection uses GitHub ruleset `main-pr-gate`; audit with `tools/check-github-repository-ruleset.ps1`.
- Draft-to-ready automation must use `tools/ready-task-pr.ps1`, not raw `gh pr ready`.
- After merge to `main`, GitHub deletes merged PR head branches (`delete_branch_on_merge` on `origin`); prefer `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/post-merge-task-cleanup.ps1 -WorktreePath <path> [-HeadRefOid <headRefOid>]` from a clean base checkout, or `tools/remove-merged-worktree.ps1` with `-DeleteLocalBranch -DeleteRemoteBranch`, for guarded post-merge cleanup that fetches/prunes, fast-forwards the base checkout, verifies branch ancestry, unlinks worktree-local vcpkg links, and removes merged worktrees/branches; Windows fallback stays guarded/non-following. Expanded reachability and cleanup steps live in [docs/workflows.md](docs/workflows.md).
...
- Use `gameengine-codebase-memory` for local codebase-memory MCP.
