...
- After a task-owned PR merges into `main`, run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/remove-merged-worktree.ps1 -WorktreePath <path> [-BaseRef origin/main] [-BaseBranch main] [-Remote origin] [-LocalCheckoutPath <path>] [-DeleteLocalBranch]` for linked worktree cleanup; it fetches/prunes, requires a clean checkout, fast-forwards it, unlinks worktree-local vcpkg links before Git removal, removes only safe merged worktrees, and keeps Windows fallback guarded/non-following. No ad hoc deletion. If no target remains, record no cleanup target.
