...
- Static-analysis PR failures are root-cause work, not log cleanup: keep `.clang-tidy` `HeaderFilterRegex` path aware; strict hosted lanes keep `--warnings-as-errors=*`; preserve diagnostics; suppress only `NN warnings generated.` summaries; use `tools/check-tidy.ps1 -Files` locally and sharded `-Jobs 0` in full CI.
