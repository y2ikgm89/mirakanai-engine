...
- Treat plan files as capability/gap-cluster/milestone records, not PR/task-count units. Keep phase gates/checklists in the active plan; no dated plans for validation-only, docs/manifest/static-check, cleanup, or substeps. Production PRs default to one `unsupportedProductionGaps` row, or a named phase inside that row when too large.
...
- Use validated commit checkpoints: commit only task-owned, complete, validated work. Never commit broken work, scratch output, secrets, credentials, or unrelated changes. Runtime/C++/build/toolchain/public-contract commits need one fresh `tools/validate.ps1`; run `tools/build.ps1` only when standalone build evidence is requested. Docs/non-runtime slices may record narrower justified checks.
- Follow official GitHub Flow: topic branch, reviewable PR, review/checks, merge, branch deletion. Cadence is purpose/checkpoint-based, not commit-count-based: commit task-owned validated checkpoints, push validated checkpoints for backup/CI/review/handoff, and split unrelated work. Production PRs cover one `unsupportedProductionGaps` row; too-large rows use named phase PRs. Plan slices, checklist items, validation-only follow-up, and docs/manifest/static-check sync stay in that PR. Inspect status, staged diff, and remote state before pushing.
- A slice is **not publication-complete after local validation alone**. Unless local-only/no-PR, finish task-owned branch, stage, commit, push, and `gh pr create`/PR update with validation evidence or blocker before final report. For a large gap or phase, open one draft PR after the first coherent validated checkpoint and keep later checkpoint commits there until ready. Mark ready only after final validation/preflight. If starting on default/protected branch, create topic branch; if `codex/<topic>` conflicts, use `codex-<topic>`. Leave unrelated changes unstaged.
...
- Auto-merge registration must use `gh pr merge --auto --merge --delete-branch --match-head-commit <headRefOid>` after `gh pr view <pr> --json ...`. Require clean local worktree, task-owned open PR, fresh validation, expected base, current `headRefOid`, and no conflicts/requested changes/failed checks; `DIRTY`/`UNKNOWN` `mergeStateStatus` blocks. Runtime/build/public-contract PRs wait for selected hosted checks to complete and `PR Gate` `SUCCESS`; no auto-merge while `Full Repository Static Analysis` is pending. Fetch/prune after merge; commits pushed after a PR merged need a new PR.
- After a PR merges into `main`, use `tools/remove-merged-worktree.ps1 -WorktreePath <path> [-LocalCheckoutPath <path>] [-DeleteLocalBranch]` for linked worktree cleanup; it fetches/prunes, fast-forwards a clean base checkout, accepts standard roots, unlinks worktree-local vcpkg links, and keeps Windows fallback guarded/non-following. No ad hoc deletion.
...
- If Git prints credential helper warnings such as `credential-manager-core` missing, inspect `git config --show-origin --get-all credential.helper` and fix host/user Git config. On Git for Windows, prefer helper `manager`; do not add repo credential helper overrides or tokens.
