...
- Follow official GitHub Flow: branch, make changes, PR, review, merge, delete. Cadence is purpose/checkpoint-based, not commit-count-based: commit validated phases, push validated checkpoints, keep one PR per focused capability/gap-cluster/milestone, and split unrelated work. Ready PR at main-merge boundary; open draft PR only when CI/review/handoff outweighs overhead. Inspect status, staged diff, `git diff --cached --check`, and remote state.
- A slice is **not publication-complete after local validation alone**. Unless local-only/no-PR, finish branch, stage, commit, non-forced push, and create/update PR via GitHub Desktop or `gh pr create` with evidence/blocker before final report. If not yet reviewable, keep validated local commits/checkpoint pushes and record the next gate instead of extra ready PRs. Default/protected branch => topic branch; `codex/<topic>` conflict => `codex-<topic>`. Leave unrelated changes unstaged.
...
- Before unattended merges, run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-github-repository-policy.ps1`; changing shared repository settings needs explicit owner/admin approval.
- Auto-merge registration must use `gh pr merge --auto --merge --delete-branch --match-head-commit <headRefOid>` after the preceding `gh pr view <pr> --json ...`. Require clean local worktree, current `headRefOid`, no failed checks/conflicts/review blocks, and non-`DIRTY`/`UNKNOWN` `mergeStateStatus`. Runtime/build/public-contract PRs wait for selected hosted checks to complete, `PR Gate` `SUCCESS`, and no pending `Full Repository Static Analysis`. Fetch/prune after merge; commits pushed after a PR merged need a new PR.
- After a PR merges into `main`, run `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/remove-merged-worktree.ps1 -WorktreePath <path> [-BaseRef origin/main] [-BaseBranch main] [-Remote origin] [-LocalCheckoutPath <path>] [-DeleteLocalBranch]`; it fast-forwards it, accepts standard roots, unlinks worktree-local vcpkg links, and keeps Windows fallback guarded/non-following.
