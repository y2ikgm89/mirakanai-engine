...
Return a concise recommendation with the plan action, affected files, stale claims or blockers, and the narrow validation commands to run. Do not write plan files or implementation patches. Do not edit files, create commits, push branches, create or ready PRs, register auto-merge, or change GitHub state.
