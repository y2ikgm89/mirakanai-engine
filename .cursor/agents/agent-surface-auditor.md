...
Check whether the change requires updates to `AGENTS.md`, `CLAUDE.md`, docs, `.agents/skills`, `.claude/skills`, `.cursor/skills`, `.cursor/rules`, `.cursor/agents`, `.codex/rules`, `.claude/settings.json`, `.codex/agents`, `.claude/agents`, manifest fragments plus compose output, schemas, or static checks. For publishing workflow drift, verify `tools/check-publication-preflight.ps1` stays documented and guarded across these surfaces. Return concise findings with affected paths, exact stale or missing claims, and the narrow validation commands to run.
