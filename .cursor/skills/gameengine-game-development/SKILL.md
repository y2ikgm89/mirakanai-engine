...
Generated item/inventory/crafting systems should validate reusable item catalogs with `mirakana::runtime::RuntimeItemCatalogDocument`, `RuntimeItemCatalogValidationContext`, `RuntimeItemCatalogValidationResult`, and `validate_runtime_item_catalog_document` before game-owned inventory, placement, shop, loot, reward, equipment, or crafting code executes. Use `RuntimeInventoryState`, `validate_runtime_inventory_state`, `RuntimeCraftingRecipeDocument`, `RuntimeInventoryTransitionRequest`, `RuntimeInventoryTransitionStatus`, and `advance_runtime_inventory_state` for value-only add/remove/craft transitions that normalize stacks, preserve state on blocked/invalid rows, and classify rows as accepted/ignored/blocked/completed/invalid. Treat rows and diagnostics as reviewed input; keep item art, balance, reward execution, shop logic, loot tables, equipment semantics, persistence mutation, and game-specific progression rules game-owned until later engine phases explicitly promote them.
