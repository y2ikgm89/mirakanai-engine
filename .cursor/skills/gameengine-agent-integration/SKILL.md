...
Plan files are coherent behavior/API/validation slices, not tiny task lists. Keep small checkboxes inside the active plan, and prefer a gap-level burn-down or phase-gated milestone when one architecture decision and validation surface spans several related phases.
