...
MSVC targets use bounded `/MP2` plus `/Zf`, target-named `COMPILE_PDB_OUTPUT_DIRECTORY` / `COMPILE_PDB_NAME`, and `/INCREMENTAL:NO` for linkable targets through `MK_apply_common_target_options`; the PDB directory stays short under the build tree for linked worktrees, and long single-source test suites use compact CMake target names plus descriptive CTest names so Visual Studio object paths stay below MSVC limits. `tools/cmake.ps1` clears stale `.tlog` directories with older aliased `.lastbuildstate` roots before Visual Studio builds to avoid MSB8028. See the canonical skills and `docs/building.md`.
