...
**Static analysis:** use `tools/check-tidy.ps1`, keep `.clang-tidy` `HeaderFilterRegex` absolute-path and Windows/Linux separator aware, keep strict CI on `--warnings-as-errors=*`, prefer `-Files` for focused local TUs and `-ShardCount` / `-ShardIndex` plus `-Jobs 0` for full hosted matrix lanes, and suppress only summary lines like `NN warnings generated.` after actionable diagnostics and exit codes are preserved. `tools/validate.ps1` now prepares the File API query before its single `build.ps1` configure/build, reuses that reply for the tidy smoke, then runs `test.ps1 -SkipBuild`.
