...
**Static analysis:** use `tools/check-tidy.ps1`, keep `.clang-tidy` `HeaderFilterRegex` absolute-path and Windows/Linux separator aware, keep strict CI on `--warnings-as-errors=*`, prefer `-Files` for focused local TUs and `-ShardCount` / `-ShardIndex` plus `-Jobs 0` for full hosted matrix lanes, and suppress only summary lines like `NN warnings generated.` after actionable diagnostics and exit codes are preserved. `tools/validate.ps1` prepares the File API query before its single `build.ps1` configure/build, runs that build with automatic CMake parallelism, reuses that reply for the tidy smoke, then runs `test.ps1 -SkipBuild` with automatic CMake/CTest parallelism. Pass `-Jobs <N>` to `tools/build.ps1` or `tools/test.ps1` only when a local host needs an explicit lower throttle.
