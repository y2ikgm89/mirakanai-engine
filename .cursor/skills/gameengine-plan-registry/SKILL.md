...
4. Keep plan width broader than PR width: one plan may hold multiple phase checkpoints, while each ready PR stays one main-merge review boundary with validation evidence. Do not split or merge plans to satisfy a commit, push, or PR count; use validated GitHub Flow checkpoints and draft PRs only for work in progress or when a broader plan needs staged CI, review, or handoff feedback. When subagent or parallel work is user-authorized, plan phases should expose material independent sidecar tasks that can be delegated without blocking the parent path.
