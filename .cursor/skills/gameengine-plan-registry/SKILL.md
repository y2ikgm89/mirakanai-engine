...
4. Keep plan width broader than PR width: one plan may hold multiple phase checkpoints, while production-completion PRs default to one `unsupportedProductionGaps` row or one explicitly named phase inside that row. Do not split or merge plans to satisfy a commit, push, or PR count; use validated GitHub Flow checkpoints and one draft PR for early feedback when a broader plan needs staged review.
