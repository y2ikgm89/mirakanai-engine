...
- **GitHub publishing:** Follow official GitHub Flow. Commit task-owned complete phases and non-forced push topic-branch checkpoints without asking for per-action confirmation. Cadence is purpose/checkpoint-based, not commit-count-based: push validated checkpoints for backup/CI/review/handoff, keep one PR per focused capability/gap-cluster/milestone at the main-merge review boundary, avoid PRs per commit/checklist item, and inspect or refresh remote state before pushing. Follow **`AGENTS.md` -> Git Workflow** and **`docs/workflows.md` -> Commit, Push, And Pull Request Workflow**: stage only task-owned files, run required validation or record blockers, push topic branches, prefer pull requests, use draft PRs only when early CI/review/handoff value outweighs overhead, allow task-owned `gh pr create` and `gh pr merge --auto --merge --delete-branch --match-head-commit <headRefOid>` only after validation checkpoints plus PR preflight (`state`, `isDraft`, `baseRefName`, `headRefName`, `headRefOid`, `mergeable`, `mergeStateStatus`, `reviewDecision`, `statusCheckRollup`, `autoMergeRequest`, and `url`; runtime/C++/build/toolchain/public-contract PRs must wait for selected hosted checks to complete and `PR Gate` to report `SUCCESS`; rerun preflight after every commit or push because a stale `headRefOid` invalidates the merge decision), run `tools/check-github-repository-policy.ps1` before unattended merge decisions and treat failures as GitHub settings blockers requiring explicit owner/admin approval for remote setting changes, prune stale remote-tracking refs after deleted task branches when useful, run `tools/remove-merged-worktree.ps1` for guarded post-merge cleanup that fetches/prunes, fast-forwards a clean base local checkout, removes only clean merged task worktrees, unlinks worktree-local dependency reparse points before Git removal, and keeps non-reparse-following Windows long-path fallback inside the guarded script, and never push directly to the default branch or bypass protected-branch/required-check policy. Commits pushed after a PR merged need a new PR. Required PR checks must be always-running or aggregate-gated, not path-filtered workflows that can remain pending; `tools/classify-pr-validation-tier.ps1` owns PR touched-surface classification and `tools/check-ci-matrix.ps1` verifies docs-only/static-policy/runtime/workflow/non-PR cases. docs/agent/rules/subagent-only PRs should use lightweight static validation instead of unrelated Windows/MSVC, macOS, or full repository clang-tidy lanes. When `.codex/rules` changes in the same task, wait for policy reload or a new session before relying on newly allowed commands. Prompt-gated PR state changes such as `gh pr edit`, `gh pr ready`, or immediate `gh pr merge` still need GitHub Web/Desktop or an approval-capable session if approvals are unavailable. If Git credential helper warnings mention missing helpers such as `credential-manager-core`, inspect host/user `credential.helper` settings and fix them outside the repository.
...
|-----------|------------|
| C++/runtime/build/packaging/public-contract slice-closing claim | `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/validate.ps1` once near the gate, or record toolchain blocker |
| Docs/agent/rules/subagent-only claim | `git diff --check`, `tools/check-format.ps1`, `tools/check-agents.ps1`, and matching static guard such as `tools/check-ai-integration.ps1` |
...
| Large `MK_editor` shell (`editor/src/main.cpp` and focused shell `.cpp` under `editor/src/`) | Same as row above; use `-Files editor/src/main.cpp` or include `editor/src/material_preview_gpu_cache.cpp`, `editor/src/sdl_viewport_texture.cpp` when those TUs change, after `tools/cmake.ps1 --preset dev`; see **Dear ImGui shell** / **MK_editor Windows and material-preview host cache** in `.claude/skills/gameengine-editor/SKILL.md` / `.agents/skills/editor-change/SKILL.md` and `AGENTS.md` Testing |
...
| clangd / IDE includes | Tracked `.clangd` → `CompilationDatabase: out/build/dev` after `tools/cmake.ps1 --preset dev`; fallback `editor/src/compile_flags.txt` and `editor/include/compile_flags.txt` when TUs or `editor/include/**/*.hpp` are missing from the database—extend them when new include roots are required (`AGENTS.md` AI Development Workflow) |
