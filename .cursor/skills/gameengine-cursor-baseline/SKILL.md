...
- **GitHub publishing:** Commit and non-forced push task-owned changes at coherent, validated checkpoints without asking for per-action confirmation. Follow **`AGENTS.md` → Git Workflow** and **`docs/workflows.md` → Commit, Push, And Pull Request Workflow**: stage only task-owned files, run required validation or record blockers, push topic branches, prefer pull requests, and never bypass protected-branch or required-check policy.
