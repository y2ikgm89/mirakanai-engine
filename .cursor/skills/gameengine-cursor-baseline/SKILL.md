...
- **GitHub publishing:** Commit and non-forced push task-owned changes at coherent, validated checkpoints without asking for per-action confirmation. Follow **`AGENTS.md` → Git Workflow** and **`docs/workflows.md` → Commit, Push, And Pull Request Workflow**: stage only task-owned files, run required validation or record blockers, push topic branches, prefer pull requests, merge only after required checks/reviews, prune stale remote-tracking refs after deleted task branches when useful, and never bypass protected-branch or required-check policy. When `.codex/rules` changes in the same task, wait for policy reload or a new session before relying on newly allowed commands. If prompt-gated PR commands such as `gh pr create` are blocked because approvals are unavailable, keep the branch pushed and use GitHub Web/Desktop or an approval-capable session. If Git credential helper warnings mention missing helpers such as `credential-manager-core`, inspect host/user `credential.helper` settings and fix them outside the repository.
