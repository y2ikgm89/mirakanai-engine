...
Environment backend parity: use `EnvironmentBackendParityRequest` / `plan_environment_backend_parity` from the shared rendering skills; `desktop-runtime-sample-game-environment-backend-parity` is a host-gated package review recipe with `environment_backend_parity_ready=0`, not a readiness promotion. Do not promote package-visible `environment_backend_parity_ready=1` from feature-count-only, stale, inferred, host-gated, fallback, native-handle, or diagnostic-bearing rows.
