...
- Noisy warnings in **`tests/unit/*.cpp`** and similar churn in **`editor/core`** production TUs: fix patterns first (complete designated
  initializers—including declaration order when the toolchain flags skipped members—C++23 **`std::string::contains` / `starts_with`** and
  the same on **`std::string_view`**, one anonymous namespace through test bodies with `int main()` outside for large TUs, `class` test
  doubles with private state and `const` getters instead of public counters, **Rule of Five** on file-local C API adapters when
  special-member clang-tidy checks fire). See **Unit tests** and production notes in `docs/cpp-style.md`. Use narrow NOLINT only after the
  pattern fix or for documented false positives.
