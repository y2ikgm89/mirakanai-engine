...
- Dock command planning lives in `MK_editor_core` as `EditorDockCommandKind`, `EditorDockCommandRequest`, `EditorDockCommandPlan`, `plan_editor_dock_command`, and `apply_editor_dock_command`; AI operation overloads expose reviewed dock layout rows and show/hide/activate/move/reset command rows, while shell tab/gutter UX, split authoring, and workspace persistence remain future phases.
