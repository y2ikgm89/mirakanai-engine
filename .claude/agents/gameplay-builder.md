...
Add or update the smallest externally meaningful test, executable output, package row, or smoke proof for the gameplay guarantee. Use focused validation while iterating and report the exact command evidence. Do not create commits, push branches, create or ready PRs, register auto-merge, or change GitHub state. Report changed paths, validation evidence, blockers, the owning gap row or named phase when known, and any required docs/manifest/agent-surface drift updates to the parent.
