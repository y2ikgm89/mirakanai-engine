...
- Use `pwsh -NoProfile -ExecutionPolicy Bypass -File tools/check-tidy.ps1` for clang-tidy. It verifies `.clang-tidy`, configures the active CMake preset when needed, accepts native Makefile/Ninja `compile_commands.json`, or **synthesizes** `compile_commands.json` under the preset `binaryDir` from CMake File API codemodel data for the default Windows Visual Studio `dev` preset when needed; missing CMake or clang-tidy remains a local tool blocker. `tools/validate.ps1` runs independent static checks through bounded parallel jobs with per-check timeouts, writes complete per-script logs under ignored `out/validation-logs/`, prepares the File API query before its single `build.ps1` configure/build, runs that build with automatic CMake parallelism, runs the tidy smoke with explicit reply reuse unless `-SkipTidySmoke` is explicitly passed, then runs `test.ps1 -SkipBuild` with automatic CMake/CTest parallelism. Pass `-StaticJobs <N>` to `tools/validate.ps1` only when a local host needs a static-check throttle, and pass `-StaticCheckTimeoutSeconds <N>` only for host-specific timeout diagnostics. Hosted CI uses `-StaticOnly -StaticJobs 1 -StaticCheckTimeoutSeconds 120` for the dedicated static lane and `-SkipStaticChecks -SkipTidySmoke` for the Windows MSVC lane; raw Linux, sanitizer, and macOS hosted lanes pass explicit official `--parallel` job counts to CMake build and CTest. Windows vcpkg tool/package/install/build caches restore with `actions/cache/restore`, fall back to clone/bootstrap/build on restore transport failures, and attempt fresh non-blocking `actions/cache/save` writes only after success. Keep local slice gates on the default full path unless a narrower check is justified. Pass `-Jobs <N>` to `tools/build.ps1` or `tools/test.ps1` only when a local host needs an explicit lower throttle.
...
- Keep MSVC compile throughput on bounded `/MP2` plus `/Zf` through `MK_apply_common_target_options` so compiler-level parallelism helps multi-TU targets without overwhelming MSBuild target-level parallelism. Keep MSVC compile PDBs target-named through `COMPILE_PDB_OUTPUT_DIRECTORY` / `COMPILE_PDB_NAME`, with the output directory kept short under the build tree so long linked-worktree paths do not trip C1041; do not add global `/FS` unless a measured follow-up proves it is needed. Keep long single-source test suites on compact CMake target names plus descriptive CTest names so Visual Studio `$(IntDir)` and `.obj` paths stay below MSVC path limits in linked worktrees. Keep MSVC linkable targets on `/INCREMENTAL:NO` through `MK_apply_common_target_options` so validation builds do not depend on `.ilk` state or lock-prone Debug relinks. Keep Visual Studio builds on `tools/cmake.ps1`; the wrapper serializes overlapping builds and clears stale `.tlog` directories when `.lastbuildstate` points at an older aliased build root, avoiding MSB8028 without reducing MSBuild parallelism.
