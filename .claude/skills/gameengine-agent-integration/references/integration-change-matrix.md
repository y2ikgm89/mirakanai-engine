...
- Keep branch-protection-required checks always-running or behind an always-running aggregate gate; do not make path-filtered required checks. Docs/agent/rules/subagent-only changes run formatting plus agent/static guards; docs/agent-only PRs use lightweight static validation.
