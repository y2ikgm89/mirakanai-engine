...
- When a completed plan changes a capability from deferred/unsupported to active/ready, replace stale deferred wording in always-loaded guidance instead of adding parallel status text. Keep exact readiness claims in manifest fragments and current capability docs, not in skills.
