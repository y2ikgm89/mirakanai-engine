...
- Generated item/inventory/crafting systems should validate reusable item catalogs with `mirakana::runtime::RuntimeItemCatalogDocument`, `RuntimeItemCatalogValidationContext`, `RuntimeItemCatalogValidationResult`, and `validate_runtime_item_catalog_document` before game-owned inventory, placement, shop, loot, reward, equipment, or crafting code executes. Treat valid rows and diagnostics as value-only reviewed input; keep item art, balance, reward execution, shop logic, loot tables, equipment semantics, persistence mutation, and recipe transitions game-owned until later engine phases explicitly promote them.
