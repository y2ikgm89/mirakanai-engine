...
- Generated 3D renderer-quality package smokes keep exact frame graph evidence: base postprocess expects `renderer_quality_expected_framegraph_passes=2` and `renderer_quality_expected_framegraph_barrier_steps=4`; depth/shadow variants add `framegraph_barrier_steps_executed` counts in `references/full-guidance.md`.
