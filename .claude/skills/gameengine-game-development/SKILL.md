...
- Missing generated-game art/audio should use reviewed engine tooling such as `mirakana::PlaceholderAssetBundleRequest` / `mirakana::plan_placeholder_asset_bundle` to return a `PlaceholderAssetBundlePlan` with deterministic first-party placeholder source documents, changed-file hashes, provenance, and fail-closed diagnostics, then flow through source registry/cook/package lanes; do not download external assets or parse source assets at runtime.
