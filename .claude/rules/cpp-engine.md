...
- The visible `MK_editor` shell is deferred after SDL3 removal. Do not recreate the removed SDL3/Dear ImGui `editor/src/main.cpp`, material preview GPU cache, SDL viewport bridge, or editor-local `compile_flags.txt` fallbacks without a focused native shell plan.
- Future visible editor shell work must use first-party Win32/D3D12 adapters, keep backend handles private, and add its own ImGui/native-adapter coding rules and validation lanes before claiming editor readiness.
