...
struct JobExecutionPool::Impl {
    ...
    explicit Impl(JobExecutionPoolDesc pool_desc) : desc(std::move(pool_desc)) {
        ...
        for (std::uint32_t worker_id = 0; worker_id < desc.worker_count; ++worker_id) {
            ...
            worker->thread =
                std::jthread([this, worker_ptr](std::stop_token token) { run_worker(*worker_ptr, token); });
    ...
    void request_stop() noexcept {
        ...
        for (auto& worker : workers) {
            (void)worker->thread.request_stop();
            {
    ...
    [[nodiscard]] bool enqueue_task(const JobExecutionTaskDesc& task, std::shared_ptr<ExecutionGroupState> group) {
        ...
        {
            std::scoped_lock lock(worker.mutex);
            if (worker.stopping || worker.thread.get_stop_token().stop_requested() ||
    ...
    void run_worker(WorkerState& worker, std::stop_token token) noexcept {
        while (true) {
            auto work = WorkItem{};
            {
                std::unique_lock lock(worker.mutex);
                worker.cv.wait(lock, token, [&worker] { return worker.stopping || !worker.queue.empty(); });
                if ((token.stop_requested() || worker.stopping) && worker.queue.empty()) {
                    return;
                }
            ...
            try {
                auto context =
                    JobExecutionContext{.worker_id = worker.worker_id, .stop_token = token, .scratch = worker.scratch};
                work.body(context);
