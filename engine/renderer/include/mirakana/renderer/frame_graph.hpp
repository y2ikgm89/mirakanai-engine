...
[[nodiscard]] FrameGraphBuildResult compile_frame_graph(const FrameGraphDesc& desc);

/// Single step in a backend-neutral execution schedule derived from a successful frame graph plan.
struct FrameGraphExecutionStep {
    ...
    /// Valid when `kind == pass_invoke`; matches an entry in `FrameGraphBuildResult::ordered_passes`.
...
[[nodiscard]] std::vector<FrameGraphExecutionStep> schedule_frame_graph_execution(const FrameGraphBuildResult& built);

/// Dispatches a linear frame graph schedule to caller-owned callbacks. Barrier steps require `barrier_callback`;
...
[[nodiscard]] FrameGraphExecutionResult execute_frame_graph_schedule(std::span<const FrameGraphExecutionStep> schedule,
