...
struct RuntimeMavgLodResidencyDesc {
    AssetId graph_asset;
    // Call-bound non-owning observers. The bridge does not retain either pointer and reports diagnostics when
    // required observers are null.
...
struct RuntimeMavgLodResidencyResult {
    ...
    // Accepted request rows whose graph asset and page index match the supplied graph.
