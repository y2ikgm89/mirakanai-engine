...
void validate_ime_row(std::vector<RuntimeUiProductionDiagnostic>& diagnostics,
    ...
    require_flag(diagnostics, row.ime_win32_adapter_proof_rows,
                 RuntimeUiProductionDiagnosticCode::missing_ime_win32_adapter_proof_rows, row.id,
                 "runtime UI IME evidence requires selected Win32 platform adapter proof rows");
...
[[nodiscard]] std::uint64_t compute_replay_hash(const RuntimeUiProductionStackRequest& request) noexcept {
    ...
    for (const auto& row : request.rows) {
        ...
        hash_bool(hash, row.ime_win32_adapter_proof_rows);
